...
static char ArchEvtGroupe,ArchMemeLdB,ArchEvtLoupe;
...
static EvtCouleur ArchEvtCouleur;
...
/* ========================================================================== */
void ArchEvtAligneLdB(char actif) { ArchMemeLdB = actif; }
/* ========================================================================== */
void ArchEvtEffetLoupe(char actif) { ArchEvtLoupe = actif; }
#ifdef CHANGE_UNITE
/* ========================================================================== */
		...
		if(cap) {
			...
			for(vt=0; vt<event->nbvoies; vt++) /* if(event->voie[vt].dim > 0) */ {
				...
				if(AfficheVoie[vm]) {
					...
					i = PanelFloat(pVoieEvt,cap? 0: "Base",&(ajoutee->base));
					i = PanelFloat(pVoieEvt,cap? 0: "Bruit",&(ajoutee->bruit));
					i = PanelFloat(pVoieEvt,cap? 0: "Amplitude",&(ajoutee->amplitude));
					cap++;
				}
			}
...
/* ========================================================================== */
static void ArchEvtFenetres(int stocke, char trace_unique, Graph gevt, int x0, int y0, int dx, int dy) {
	TypeEvt *event; TypeVoieDonnees *signaux; Graph gv; OpiumColorState *couleur;
	char titre[64]; // char nom[MAXARCHNOM];
	int bolo,voie,vm,cap,bolo_vu[MAXDETAFFICHES],nb_vus,num_vu,nb_graphs;
	int i,dim,dep,arr; int vt; int kb,kd;
	int niveau; WndColorLevel r,g,b;
	int x,y,h,v; // xp,yp; pour positionner le n-tuple
	int temps,brut,fit,deconv; char sans_fit,sans_deconv;
	float *evt_unite; int num,max,decalage,lngr;
	char un_seul_graphique,regroupe,legende,verifie;
	...
	couleur = stocke? (OpiumColorState *)&(EvtStock[stocke-1]->couleur): (OpiumColorState *)&ArchEvtCouleur;
	regroupe = !trace_unique && ArchEvtGroupe;
	un_seul_graphique = trace_unique || ArchEvtGroupe;
	...
	if(regroupe) {
		for(vt=0; vt<event->nbvoies; vt++) if(event->voie[vt].dim > 0) {
			...
			if(signaux->avant_evt > decalage) decalage = signaux->avant_evt; // ou on laisse a 0?
		}
	}
	if(verifie) printf("      Decalage maximum=%d\n",decalage);

	GraphErase(gevt);
	x = y = h = v = 0; gv = 0; // GCC
	nb_vus = 0; nb_graphs = 0;
	if(regroupe) { OpiumColorInit(couleur); kb = kd = OpiumColorNext(couleur); }
			...
			if(AfficheBolo[bolo]) {
				...
				sprintf(titre,"Trace evenement #%d",event->num);
 				if(un_seul_graphique) { gv = gevt; legende = (Bolo[bolo].nbcapt > 1); }
					...
					decalage = signaux->avant_evt; // ou on laisse a 0?
					if(verifie) printf("      Decalage unique=%d\n",decalage);
				}
				if(verifie) printf("      %s: decalage utilise=%d moins %d avant le trigger @%g ms\n",
					VoieManip[voie].prenom,decalage,signaux->avant_evt,signaux->horloge);
				ArchAb6[voie][0] = (float)(decalage - signaux->avant_evt) * signaux->horloge; // 0.0;
...
//+				printf("  | ajoute %s @%08X\n",VoieManip[voie].nom,(hexa)(signaux->brutes.t.sval));
				...
				if(regroupe) {
					kb = OpiumColorNext(couleur);
					if(GraphQualite == WND_Q_PAPIER) {
						niveau = (3 * OpiumColorVal[kb].r) / 4; r = (WndColorLevel)niveau;
						niveau = (3 * OpiumColorVal[kb].g) / 4; g = (WndColorLevel)niveau;
						niveau = (3 * OpiumColorVal[kb].b) / 4; b = (WndColorLevel)niveau;
					} else {
						r = OpiumColorVal[kb].r; g = OpiumColorVal[kb].g; b = OpiumColorVal[kb].b;
					}
					GraphDataRGB(gv,brut,r,g,b);
				...
				} else if((bolo == BoloTrigge) && ArchEvtLoupe && (VoiesNb > 1)) {
				...
				sans_deconv = 1;
				if(AfficheEvtDeconv && ((dim = signaux->brutes.max)) && VoieDeconv[voie] && (VoieDeconvDim[voie] >= dim)) {
					float perte;
					if(verifie) printf("      Superposition de l'evt deconvolue prevue\n");
					perte = expf(-signaux->horloge / VoieManip[voie].RC);
					SambaDeconvolue(VoieDeconv[voie],(TypeSignee *)signaux->brutes.t.sval,dim,
									Base[voie][event->num-1],ProdPreLissage,ProdPostLissage,perte,0);
					...
					if(regroupe) {
						kd = OpiumColorNext(couleur);
						if(GraphQualite == WND_Q_PAPIER) {
							niveau = (3 * OpiumColorVal[kd].r) / 4; r = (WndColorLevel)niveau;
							niveau = (3 * OpiumColorVal[kd].g) / 4; g = (WndColorLevel)niveau;
							niveau = (3 * OpiumColorVal[kd].b) / 4; b = (WndColorLevel)niveau;
						} else {
							r = OpiumColorVal[kd].r; g = OpiumColorVal[kd].g; b = OpiumColorVal[kd].b;
						}
						GraphDataRGB(gv,deconv,r,g,b);
					} else {
				...
				if(un_seul_graphique || (bolo != BoloTrigge) || !ArchEvtLoupe) OpiumDisplay(gv->cdr);
			}
		}
	}
	if(!nb_graphs) {
	...
	EvtDemandeMontre();
	if(!regroupe && ArchEvtLoupe && (VoiesNb > 1)) {
		for(vt=0; vt<event->nbvoies; vt++) if(event->voie[vt].dim > 0) {
			voie = event->voie[vt].num;
			vm = VoieManip[voie].type;
			if(AfficheVoie[vm]) {
