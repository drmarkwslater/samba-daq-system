...
static float ProdAmplMin,ProdNiveauMin;
...
static char  ProdMemeLdB,ProdLouppe;
...
static TypeFigZone zProdChoixMontee,zProdChoixMulti,zProdChoixBase;
...
void ArchEvtEffetLoupe(char actif);
...
int ProdPostTemps() {
	...
	for(voie=0; voie<VoiesNb; voie++) {
		...
		VoieTemps[msr].avant = (float)VoieEvent[voie].avant_evt * VoieEvent[voie].horloge;
		TempsMontee[VoieManip[voie].type] = (int)((VoieTemps[msr].avant - VoieTemps[msr].stop) / VoieEvent[voie].horloge);
		TempsDescente[VoieManip[voie].type] = (int)(VoieTemps[msr].apres / VoieEvent[voie].horloge);
	}
	EvtUniteMont = VoieTemps[0].avant - VoieTemps[0].stop;
	EvtUniteDesc = VoieTemps[0].apres;
	return(0);
}
/* ========================================================================== */
void ProdTempsComplete() {
	int voie,msr,n;

	printf("* Verification des temps pour les calculs\n");
	for(msr=0; msr<VOIES_MAX; msr++) VoieTemps[msr].mesure = -1;
	for(msr=0; msr<VoieTempsNb; msr++) VoieTemps[VoieTemps[msr].num].mesure = msr;
//	for(msr=0; msr<VoieTempsNb; msr++) printf("  | | Definition #%d/%d: concerne la voie #%d\n",msr+1,VoieTempsNb,VoieTemps[msr].num+1);
	n = VoieTempsNb;
	for(voie=0; voie<VoiesNb; voie++) {
		printf("  | Definition des temps pour la voie #%d ('%s')\n",voie+1,VoieManip[voie].nom);
//+		for(msr=0; msr<VoieTempsNb; msr++) if(VoieTemps[msr].num == voie) break; if(msr < VoieTempsNb) continue;
		if((msr = VoieTemps[voie].mesure) >= 0) {
			printf("  | | Temps deja definis\n");
			VoieTemps[msr].avant = (float)VoieEvent[voie].avant_evt * VoieEvent[voie].horloge;
			continue;
		}
		printf("  | | Ajout de la definition #%d\n",VoieTempsNb+1);
		...
		/* printf("(%s) Relu: duree=%g ms, dont pretrigger %g%%, dont base [%g%%..%g%%]\n",
		 __func__,VoieManip[voie].evt.duree,VoieManip[voie].evt.pretrigger,
		 VoieManip[voie].evt.base_dep,VoieManip[voie].evt.base_arr); */
		msr = VoieTempsNb;
		VoieTemps[msr].num = voie;
		VoieTemps[voie].mesure = msr;
		VoieTemps[msr].avant = VoieManip[voie].def.evt.duree * VoieManip[voie].def.evt.pretrigger / 100.0; // ou bien (float)VoieEvent[voie].avant_evt * VoieEvent[voie].horloge;
		VoieTemps[msr].start = VoieTemps[msr].avant * VoieManip[voie].def.evt.base_dep / 100.0;
		VoieTemps[msr].stop  = VoieTemps[msr].avant * VoieManip[voie].def.evt.base_arr / 100.0;
		VoieTemps[msr].apres = VoieManip[voie].def.evt.duree - VoieTemps[msr].avant;
		VoieTemps[msr].RC    = VoieManip[voie].RC;
		printf("  | | Temps par defaut: base=[%g - %g[, pretrigger=%g, posttrigger=%g, RC=%g (ms)\n",
			   VoieTemps[msr].start,VoieTemps[msr].stop,
			   VoieTemps[msr].avant,VoieTemps[msr].apres,VoieTemps[msr].RC);
	...
	for(msr=0; msr<VoieTempsNb; msr++) {
		voie = VoieTemps[msr].num;
		if(VoieTemps[msr].apres < VoieEvent[voie].horloge) {
			printf("  ! Definition #%d (voie '%s'): 'apres'=%g < horloge=%g\n",msr+1,VoieManip[voie].nom,VoieTemps[msr].apres,VoieEvent[voie].horloge);
			VoieTemps[msr].apres = VoieManip[voie].def.evt.duree - VoieTemps[msr].avant; n = 0;
			if(VoieTemps[msr].apres < VoieEvent[voie].horloge) {
				VoieTemps[msr].avant = VoieManip[voie].def.evt.duree * VoieManip[voie].def.evt.pretrigger / 100.0;
				VoieTemps[msr].apres = VoieManip[voie].def.evt.duree - VoieTemps[msr].avant;
			}
		}
		if(VoieTemps[msr].start < VoieEvent[voie].horloge) {
			printf("  ! Definition #%d (voie '%s'): 'start'=%g < horloge=%g\n",msr+1,VoieManip[voie].nom,VoieTemps[msr].start,VoieEvent[voie].horloge);
			VoieTemps[msr].start = VoieTemps[msr].avant * VoieManip[voie].def.evt.base_dep / 100.0; n = 0;
		}
		if(VoieTemps[msr].stop < VoieEvent[voie].horloge) {
			printf("  ! Definition #%d (voie '%s'):  'stop'=%g < horloge=%g\n",msr+1,VoieManip[voie].nom,VoieTemps[msr].stop,VoieEvent[voie].horloge);
			VoieTemps[msr].stop  = VoieTemps[msr].avant * VoieManip[voie].def.evt.base_arr / 100.0; n = 0;
			if(VoieTemps[msr].stop < (VoieTemps[msr].start + VoieEvent[voie].horloge))
				VoieTemps[msr].stop  = VoieTemps[msr].avant * VoieManip[voie].def.evt.base_arr / 100.0;
		}
		if(VoieTemps[msr].RC < VoieEvent[voie].horloge) {
			printf("  ! Definition #%d (voie '%s'):  'RC'=%g < horloge=%g\n",msr+1,VoieManip[voie].nom,VoieTemps[msr].RC,VoieEvent[voie].horloge);
			VoieTemps[msr].RC = TangoRCdefaut; n = 0;
		}
	}
	printf("  | Definition des temps en service (lues: %d, utilisees %d):\n",n,VoieTempsNb);
	ArgPrint("*",ProdTempsDesc);
	if(VoieTempsNb != n) ProdTempsSauve();
}
/* ========================================================================== */
int ProdTempsSauve() {
#ifdef FICHIER_TEMPS_MANU
	char ok; FILE *f; int voie;
	
	ok = 1;
	if(OpiumReadBool("Sauver ces valeurs",&ok) == PNL_CANCEL) return(0);
	if((f = fopen(ProdTempsFile,"w"))) {
		fprintf(f,"# temps en ms, relatifs au maximum de l'evemement\n");
		fprintf(f,"# %14s : %12s %12s %12s\n","voie","debut_base","fin_base","duree_descente");
		for(voie=0; voie<VoiesNb; voie++) {
			fprintf(f,"%16s : %12g %12g %12g\n",VoieManip[voie].nom,
					VoieTemps[voie].start-VoieTemps[voie].avant,
					VoieTemps[voie].stop-VoieTemps[voie].avant,
					VoieTemps[voie].apres);
		}
		fclose(f);
		for(voie=0; voie<VoiesNb; voie++) {
			printf("* Evenement voie %s: base=[%g - %g[, pretrigger=%g, posttrigger=%g (ms) [sauve dans %s]\n",
				   VoieManip[voie].nom,VoieTemps[voie].start,VoieTemps[voie].stop,VoieTemps[voie].avant,VoieTemps[voie].apres,
				   ProdTempsFile);
		}
	} else OpiumFail("Fichier %s inutilisable (%s)",ProdTempsFile,strerror(errno));
#else
	if(!ArgPrint(ProdTempsFile,ProdTempsDesc))
		OpiumFail("Fichier %s inutilisable (%s)",ProdTempsFile,strerror(errno));
	else printf("* Temps pour mesures sauves dans le fichier %s\n",ProdTempsFile);
#endif
	return(0);
}
/* ========================================================================== */
...
void Prod1EvtSamba(int voie, int evt, int vt) {
	Base[voie][evt] = ArchEvt.voie[vt].base;
	Bruit[voie][evt] = ArchEvt.voie[vt].bruit;
	Amplitude[voie][evt] = ArchEvt.voie[vt].amplitude;
//	AmplAdu[voie][evt] = Amplitude[voie][evt];
	Montee[voie][evt] = ArchEvt.voie[vt].montee;
...
void ProdChannelBasics(float *adrs, int dim, float *base) {
	...
	debut_examen = (int)(VoieTemps[msr].start / VoieEvent[voie].horloge);
	if(debut_examen < 0) debut_examen = 0;
	duree_base = (int)((VoieTemps[msr].stop - VoieTemps[msr].start) / VoieEvent[voie].horloge);
...
void Prod1Amplitude(int voie, int evt, int vt, TypeSignee *adrs_evt, int duree_evt) {
	...
	msr = VoieTemps[voie].mesure; if(msr < 0) return;
	debut_examen = (int)(VoieTemps[msr].start / VoieEvent[voie].horloge); if(debut_examen < 0) debut_examen = 0;
	fin_base = (int)(VoieTemps[msr].stop / VoieEvent[voie].horloge); if(fin_base <= debut_examen) fin_base = debut_examen + 1;
	duree_base = fin_base - debut_examen;
	duree_montee = (int)(VoieTemps[msr].avant / VoieEvent[voie].horloge) - fin_base;
	duree_descente = (int)(VoieTemps[msr].apres / VoieEvent[voie].horloge);
...
//					if(evt < 10) {
//						printf("================== Evt #%d: seuil a %g, horloge=%g\n",evt+1,threshold,VoieEvent[voie].horloge);
//						printf("precedente: %g @ %d, actuelle: %g @ %d => pente_inverse=%g\n",vprec,tprec,val,t,pente_inverse);
//					}
					pente_trouvee = 1;
				}
			}
	...
	if(VoieDeconv[voie] && (VoieDeconvDim[voie] < duree_evt)) {
		free(VoieDeconv[voie]); VoieDeconv[voie] = 0; VoieDeconvDim[voie] = 0;
	}
	if(!VoieDeconv[voie]) {
		VoieDeconv[voie] = (float *)malloc(duree_evt * sizeof(float));
		if(VoieDeconv[voie]) VoieDeconvDim[voie] = duree_evt;
		// printf("      malloc(%d x 4) retourne VoieDeconv[voie #%d] = %08X\n",dim,voie,(hexa)(VoieDeconv[voie]));
	}
	if(VoieDeconv[voie]) {
		float perte;
		if(VoieManip[voie].RC < VoieEvent[voie].horloge) VoieManip[voie].RC = TangoRCdefaut;
		perte = expf(-VoieEvent[voie].horloge / VoieManip[voie].RC);
		Depot[voie][evt] = SambaDeconvolue(VoieDeconv[voie],adrs_evt,duree_evt,base,ProdPreLissage,ProdPostLissage,perte,1);
	}
...
#ifdef ETUDIE_CALAGE
	for(vt=0; vt<ArchEvt.nbvoies; vt++) if(voie == ArchEvt.voie[vt].num) break;
	if(vt < ArchEvt.nbvoies) {
...
int ProdAjusteDelais(Menu menu, MenuItem *item) {
	...
	for(evt=0; evt<EvtNb; evt++) if(!PlotNtupleExclu(EvtEspace,evt)) ProdCalculeDelai(evt);
...
//	int delta_t; 
	...
	if(!EvtNb) { TangoPrint(OPIUM_ERROR,"Pas d'evenement lu, calcul abandonne"); return(1); }
...
//				if(evt < 20) printf("   soit date=%d,%06d\n",EvtDateExacte[evt].t.s,EvtDateExacte[evt].t.us);
		...
		if(VoiesNb > 1) {
			EcartT0[evt] = (float)(ArchEvt.voie[0].sec - ArchEvt.voie[1].sec) + ((float)(ArchEvt.voie[0].msec - ArchEvt.voie[1].msec) / 1000000.0);
		#ifdef BOLOMETRES
...
/*	Coupure (non memorisee) sur le numero de bolo touche */
	...
	/*	EvtCoupure(OPER_ET,num,mini,maxi,MARQUE_ABSENT); memorisation de la coupure a chaque appel... */
	EvtCoupeOpere(OPER_ET,num,mini,maxi,MARQUE_ABSENT);  /* coupure non memorisee */
	printf(". %d evenement%s valide%s par %g < bolo < %g\n",Accord2s(EvtValides),mini,maxi);
...
/* Coupure sur le numero de bolo touche */
	mini = (float)(BoloNum + 1) - 0.2;
	maxi = (float)(BoloNum + 1) + 0.2;
	if(!EvtCoupeSur(OPER_NOUVEAU,"bolo",mini,maxi,1)) return(0);

/* Coupure sur le numero de voie ionisation touchee (obligatoirement centre si voie chaleur) */
	sprintf(nom,"voie-ionis");
	if(!EvtCoupeSur(OPER_ET,nom,1.8,2.2,1)) return(0); /* evts centre uniquement */
...
/* Coupure des evts avec un bon fit */
	for(voie=0; voie<VoiesNb; voie++) if(VoieManip[voie].det == BoloNum) {
		...
		if(voie == 0) {
			if(!EvtCoupeSur(OPER_ET,nom,0.0,ProdKhi2ChalMax,1)) return(0);
		} else {
			if(!EvtCoupeSur(OPER_ET,nom,0.0,ProdKhi2IonisMax,1)) return(0);
		}
	}
...
/* Calcul de la fonction de correction */
	...
	rc = FitSecondDegre(bins,histo,erreur,valide,ProdChaleurFitNb,&a,&b,&c);
...
int ProdCalibApplique() {
	...
	char nom_complet[MAXFILENAME];
	char ligne[1024],*item,*c;
	int bolo,voie,evt,bolo_avant,voie_avant; int i,k; float coef;
	int vcent,vgard,capt,vt;
...
int ProdCalibCalcule1Voie(char affiche) {
	...
	/* Coupure sur le numero de bolo touche */
	if(BoloNb > 1) {
		mini = (float)(bolo + 1) - 0.2;
		maxi = (float)(bolo + 1) + 0.2;
		if(!EvtCoupeSur(OPER_NOUVEAU,"bolo",mini,maxi,1)) return(0);
	} else EvtCoupeSur(OPER_TOUT,"\0",0.0,0.0,1);
	...
	if(VoiesNb > 1) {
		if((VoieManip[voie].type == VoieChaleur) || (VoieManip[voie].type == VoieCentre) || (VoieManip[voie].type == VoieGarde)) {
			...
			if(!EvtCoupeSur(OPER_ET,nomvar,mini,maxi,1)) return(0);
		}
	}
	...
	if(MesurePourCalib == EDW_EVT) {
		...
		if(VoieManip[voie].type == 0) {
			if(!EvtCoupeSur(OPER_ET,nomvar,0.0,ProdKhi2ChalMax,1)) return(0);
		} else {
			if(!EvtCoupeSur(OPER_ET,nomvar,0.0,ProdKhi2IonisMax,1)) return(0);
		} 
		if(!EvtValides) {
	...
	if(ProdCalibFaite[MesurePourCalib][voie]) {
		mini = Etalon[MesurePourCalib][voie] / 3.0;
		maxi = Etalon[MesurePourCalib][voie] * 1.5;
	} else {
#ifdef A_AMELIORER
		mini = 0.0; maxi = 0.0;
		for(evt=0; evt<EvtNb; evt++) {
		...
		if(ref > 0.0) { mini = ref / 3.0; maxi = ref * 1.5; }
		else { mini = ref * 1.5; maxi = ref / 3.0; }
#endif
	}
	mini=-10000;
	maxi=10000;
	...
	printf(". %s dans [%g .. %g]\n",nomvar,mini,maxi);
	bins = 100;
	PlotRempliHisto(p,PLOT_HISTO,var,mini,maxi,bins);
	...
	switch(MesurePourCalib) {
		...
		case EDW_SURF:
			...
			break;
		case EDW_EVT:
			EvtCoupure(OPER_ET,var,mini,maxi,MARQUE_TAGGE);
			printf(". %d evenements %sselectionnes par %g < %s < %g\n",EvtValides,ProdEvtManu?"pre-":"",mini,nomvar,maxi);
			...
			printf(". %s dans [%g .. %g]\n",nomvar,mini,maxi);
			bins = 50;
			PlotRempliHisto(p2,PLOT_HISTO,var,mini,maxi,bins);
	...
	EvtCoupure(OPER_TOUT,var,mini,maxi,MARQUE_NORMAL);
	printf(". %s normalisee\n",nomvar);
	PlotAnnule(p);
...
/* ========================================================================== */
int ProdUniteAffiche() {
	AfficheEvtCalib = 1 - AfficheEvtCalib;
	OpiumNote("L'evenement unite sera desormais %s",AfficheEvtCalib? "trace": "masque");
	return(0);
}
/* ========================================================================== */
int ProdDeconvAffiche() {
	AfficheEvtDeconv = 1 - AfficheEvtDeconv;
	OpiumNote("L'evenement deconvolue sera desormais %s",AfficheEvtDeconv? "trace": "masque");
	return(0);
}
/* ========================================================================== */
int ProdDiagAffiche() {
	...
	do {
		if(EvtLitASuivre(++evt,1) > 0) {
			if(ArchEvt.bolo_hdr == bolo) {
				if(fabsf(ArchEvt.voie[ArchEvt.voie_evt].amplitude) > ProdAmplMin) {
					vu = 1; break; 
				}
			}
		} else {
...
/* ========================================================================== */
int ProdAligneLdB() {
	ProdMemeLdB = 1 - ProdMemeLdB;
	ArchEvtAligneLdB(ProdMemeLdB);
	OpiumNote("Les lignes de base des voies seront desormais %s",ProdMemeLdB? "alignees": "aux valeurs reelles");
	return(0);
}
/* ========================================================================== */
int ProdEffetLoupe() {
	ProdLouppe = 1 - ProdLouppe;
	ArchEvtEffetLoupe(ProdLouppe);
	OpiumNote("L'affichage du detecteur touche sera desormais %s",ProdLouppe? "agrandi": "de taille normale");
	return(0);
}
/* ========================================================================== */
...
MenuItem iProdAffGeneral[] = {
	"Entete generale",           MNU_FONCTION   ProdHdrAffiche,
...
MenuItem iProdAffEvts[] = {
	...
	"Tracer evt unite?",         MNU_FONCTION   ProdUniteAffiche,
	"Tracer evt deconvolue?",    MNU_FONCTION   ProdDeconvAffiche,
	"Alignement lignes de base", MNU_FONCTION   ProdAligneLdB,
	"Zoom bolo touche",          MNU_FONCTION   ProdEffetLoupe,
	"Dumper evenement",          MNU_FONCTION   ProdEvtDump,
	MNU_END
};

MenuItem iProdSupplement[] = {
...
#define TESTE_DECLA
...
#else /*  TESTE_DECLA */
	//	defini_voie(&i,&j,L_("adu"),"ADU",AmplAdu);
	for(voie=0; voie<VoiesNb; voie++) {
		if(!defini_1_voie(&i,L_("montee"),"ms",voie,&(Montee[voie]))) break;
		j++;
	}
	if(dbg) printf("- %d %s%s cree%s, total %d/%d Ntuple%s\n",voie,"montee",AccordAvec(voie),AccordAvec(voie),i,Accord1s(j));
	for(voie=0; voie<VoiesNb; voie++) {
		if(!defini_1_voie(&i,L_("amplitude"),"ADU",voie,&(Amplitude[voie]))) break;
		j++;
	}
	if(dbg) printf("- %d %s%s cree%s, total %d/%d Ntuple%s\n",voie,"amplitude",AccordAvec(voie),AccordAvec(voie),i,Accord1s(j));
	for(voie=0; voie<VoiesNb; voie++) {
		if(!defini_1_voie(&i,L_("integrale"),"",voie,&(Integrale[voie]))) break;
		j++;
	}
	if(dbg) printf("- %d %s%s cree%s, total %d/%d Ntuple%s\n",voie,"integrale",AccordAvec(voie),AccordAvec(voie),i,Accord1s(j));
	for(voie=0; voie<VoiesNb; voie++) {
		if(!defini_1_voie(&i,L_("rc"),"ms",voie,&(RCmesure[voie]))) break;
		j++;
	}
	if(dbg) printf("- %d %s%s cree%s, total %d/%d Ntuple%s\n",voie,"rc",AccordAvec(voie),AccordAvec(voie),i,Accord1s(j));
	for(voie=0; voie<VoiesNb; voie++) {
		if(!defini_1_voie(&i,L_("charges"),"ADU",voie,&(Depot[voie]))) break;
		j++;
	}
	for(voie=0; voie<VoiesNb; voie++) {
		if(!defini_1_voie(&i,L_("longueur"),"ms",voie,&(Longueur[voie]))) break;
		j++;
	}
	if(dbg) printf("- %d %s%s cree%s, total %d/%d Ntuple%s\n",voie,"longueur",AccordAvec(voie),AccordAvec(voie),i,Accord1s(j));
	for(voie=0; voie<VoiesNb; voie++) {
		if(!defini_1_voie(&i,L_("descente"),"ms",voie,&(Descente[voie]))) break;
		j++;
	}
	if(dbg) printf("- %d %s%s cree%s, total %d/%d Ntuple%s\n",voie,"descente",AccordAvec(voie),AccordAvec(voie),i,Accord1s(j));
	for(voie=0; voie<VoiesNb; voie++) {
		if(!defini_1_voie(&i,L_("duree"),"ms",voie,&(Duree[voie]))) break;
		j++;
	}
	if(dbg) printf("- %d %s%s cree%s, total %d/%d Ntuple%s\n",voie,"duree",AccordAvec(voie),AccordAvec(voie),i,Accord1s(j));
	for(voie=0; voie<VoiesNb; voie++) {
		if(!defini_1_voie(&i,L_("pente"),"",voie,&(Pente[voie]))) break;
		j++;
	}
	if(dbg) printf("- %d %s%s cree%s, total %d/%d Ntuple%s\n",voie,"pente",AccordAvec(voie),AccordAvec(voie),i,Accord1s(j));
	for(voie=0; voie<VoiesNb; voie++) {
		if(!defini_1_voie(&i,"base","ADU",voie,&(Base[voie]))) break;
		j++;
	}
	if(dbg) printf("- %d %s%s cree%s, total %d/%d Ntuple%s\n",voie,"base",AccordAvec(voie),AccordAvec(voie),i,Accord1s(j));
	for(voie=0; voie<VoiesNb; voie++) {
		if(!defini_1_voie(&i,L_("bruit"),"ADU",voie,&(Bruit[voie]))) break;
		j++;
	}
	if(dbg) printf("- %d %s%s cree%s, total %d/%d Ntuple%s\n",voie,"bruit",AccordAvec(voie),AccordAvec(voie),i,Accord1s(j));
#ifdef COMPARE_NTUPLES
	defini_glob(&i,L_("samba_delai"),"s",&DelaiSamba); j++;
	for(voie=0; voie<VoiesNb; voie++) {
		if(!defini_1_voie(&i,L_("samba_base"),"ADU",voie,&(BaseSamba[voie]))) break;
		j++;
	}
	for(voie=0; voie<VoiesNb; voie++) {
		if(!defini_1_voie(&i,L_("samba_bruit"),"ADU",voie,&(BruitSamba[voie]))) break;
		j++;
	}
	for(voie=0; voie<VoiesNb; voie++) {
		if(!defini_1_voie(&i,L_("samba_ampl"),"ADU",voie,&(AmplitudeSamba[voie]))) break;
		j++;
	}
	for(voie=0; voie<VoiesNb; voie++) {
		if(!defini_1_voie(&i,L_("samba_montee"),"ms",voie,&(MonteeSamba[voie]))) break;
		j++;
	}
#endif
	for(voie=0; voie<VoiesNb; voie++) {
		if(!defini_1_voie(&i,"ener","",voie,&(Energie[voie]))) break;
		j++;
	}
	if(dbg) printf("- %d %s%s cree%s, total %d/%d Ntuple%s\n",voie,"ener",AccordAvec(voie),AccordAvec(voie),i,Accord1s(j));
	for(voie=0; voie<VoiesNb; voie++) {
		if(!defini_1_voie(&i,"khi2","",voie,&(Khi2[voie]))) break;
		j++;
	}
	if(dbg) printf("- %d %s%s cree%s, total %d/%d Ntuple%s\n",voie,"khi2",AccordAvec(voie),AccordAvec(voie),i,Accord1s(j));
#ifdef CALCULE_SURFACE
	for(voie=0; voie<VoiesNb; voie++) {
		if(!defini_1_voie(&i,"surf","",voie,&(Surface[voie]))) break;
		j++;
	}
#endif
	for(voie=0; voie<VoiesNb; voie++) {
		if(!defini_1_voie(&i,"tevt","",voie,&(Tevt[voie]))) break;
		j++;
	}
	if(dbg) printf("- %d %s%s cree%s, total %d/%d Ntuple%s\n",voie,"tevt",AccordAvec(voie),AccordAvec(voie),i,Accord1s(j));
#ifdef ETUDIE_CALAGE
	for(voie=0; voie<VoiesNb; voie++) {
		if(!defini_1_voie(&i,"dtmax","",voie,&(DTmax[voie]))) break;
		j++;
	}
	for(voie=0; voie<VoiesNb; voie++) {
		if(!defini_1_voie(&i,"dtthe0","",voie,&(DTtheo[voie]))) break;
		j++;
	}
#endif
#endif /*  TESTE_DECLA */
	...
	if(i < j) {
		TangoPrint(OPIUM_ERROR,"Toutes les variables n'ont pas ete creees (%d/%d)",i,j);
		return(0);
	} else if(i >= MAX_VARS_NTUPLE) {
		TangoPrint(OPIUM_ERROR,"Toutes les variables n'ont pas ete creees (maxi: %d)",MAX_VARS_NTUPLE-1);
		return(0);
	} else TangoPrint(OPIUM_NOTE,"%d variable%s creee%s",Accord2s(i));
	Ntuple[i].nom = "\0";
	NtTotNb = NtStdNb = i;
...
//	int voie;
    ...
    if(ProdVar[var].niveau == PRODVAR_VOIE) /* niveau voies */ {
        if(ProdVarVoieNb >= AJOUTSVOIE_MAX)
            TangoPrint(OPIUM_ERROR,"Seulement %d variables de niveau 'voie' permises",AJOUTSVOIE_MAX);
        else {
//            for(voie=0; voie<VoiesNb; voie++) {
...
//            }
        ...
        if(ProdVarDetecNb >= AJOUTSDETEC_MAX)
            TangoPrint(OPIUM_ERROR,"Seulement %d variables de niveau 'detecteur' permises",AJOUTSDETEC_MAX);
        else {
            for(bolo=0; bolo<BoloNb; bolo++) /* if(Bolo[bolo].lu != DETEC_ARRETE) */ {
...
static void ProdVarRestoreNtuple() {
    ...
    if(!PlotVarsDeclare(Ntuple)) {
        TangoPrint(OPIUM_ERROR,"Manque de memoire pour la declaration des variables");
        return;
    }
...
int ProdVarCree(Menu menu, MenuItem *item) {
    if(ProdVarNb >= PRODVAR_MAX) {
        TangoPrint(OPIUM_ERROR,"Pas plus de %d variables supplementaires",PRODVAR_MAX);
        return(0);
    }
...
int ProdVarLit(Menu menu, MenuItem *item) {
    ...
    if((f = fopen(ProdVarsFile,"r"))) {
        ...
        ProdVarNb = var;
        ProdVar[ProdVarNb].nom[0] = '\0';
        if(!ok) TangoPrint(OPIUM_ERROR,"Definition des variables en defaut (voir log)");
    }
    return(0);
}
/* ========================================================================== */
...
/* ========================================================================== */
void ProdCreeUI() {
	Panel p; int i,k,n; int bolo,voie,vm,msr;
	int x,y,xm,ym; int cols;
	...
	PanelItemSouligne(pTrigger,PanelItemSelect(pTrigger,PanelText(pTrigger,0,"Amin",6),0),1);
	PanelItemSouligne(pTrigger,PanelItemSelect(pTrigger,PanelText(pTrigger,0,"Amax",6),0),1);
	PanelItemSouligne(pTrigger,PanelItemSelect(pTrigger,PanelText(pTrigger,0,"Tmin",6),0),1);
	PanelItemSouligne(pTrigger,PanelItemSelect(pTrigger,PanelText(pTrigger,0,"Tmax",6),0),1);
	for(voie=0; voie<VoiesNb; voie++) {
		...
		i = PanelFloat(pTrigger,0,&(VoieManip[voie].def.trgr.underflow)); PanelLngr(pTrigger,i,6);
		i = PanelFloat(pTrigger,0,&(VoieManip[voie].def.trgr.overflow)); PanelLngr(pTrigger,i,6);
		i = PanelFloat(pTrigger,0,&(VoieManip[voie].def.trgr.montmin)); PanelLngr(pTrigger,i,6);
		i = PanelFloat(pTrigger,0,&(VoieManip[voie].def.trgr.montmax)); PanelLngr(pTrigger,i,6);
	}
	...
	BoardAreaOpen("Montee",WND_PLAQUETTE); // donne des rainures
	...
	BoardAreaClose(bProdChoixTemps,&zProdChoixMontee,&xm,&ym);

	BoardAreaOpen("Multiplicite",WND_PLAQUETTE); // donne des rainures
	...
	BoardAreaMargesVert(0,Dy);
	BoardAreaClose(bProdChoixTemps,&zProdChoixMulti,&x,&y);

	BoardAreaOpen("Evaluation ligne de base (ms)",WND_PLAQUETTE); // donne des rainures
	p = PanelCreate(4 * (1 + VoiesNb)); PanelSupport(p,WND_CREUX);
	PanelColumns(p,4); PanelMode(p,PNL_BYLINES);
	PanelItemSouligne(p,PanelItemSelect(p,PanelText(p,0,"debut base",10),0),1);
	PanelItemSouligne(p,PanelItemSelect(p,PanelText(p,0,"fin base",10),0),1);
	PanelItemSouligne(p,PanelItemSelect(p,PanelText(p,0,"descente",10),0),1);
	PanelItemSouligne(p,PanelItemSelect(p,PanelText(p,0,"RC (ms)",10),0),1);
	k = 0;
	for(voie=0; voie<VoiesNb; voie++) {
		char fin_de_serie;
		fin_de_serie = !((k++ + 1) % 3);
		msr = VoieTemps[voie].mesure; if(msr < 0) continue;
		i = PanelFloat(p,VoieManip[voie].nom,&(VoieTemps[msr].start));
		PanelItemSouligne(p,i,fin_de_serie);
		i = PanelFloat(p,0,&(VoieTemps[msr].stop));
		PanelItemSouligne(p,i,fin_de_serie);
		i = PanelFloat(p,0,&(VoieTemps[msr].apres));
		PanelItemSouligne(p,i,fin_de_serie);
		i = PanelFloat(p,0,&(VoieTemps[msr].RC));
		PanelItemSouligne(p,i,fin_de_serie);
	}
	PanelOnApply(p,ProdPostTemps,0);
	PanelOnOK(p,ProdPostTemps,0);
	BoardAddPanel(bProdChoixTemps,p,2 * Dx,ym + (2 * Dy),0);
	BoardAreaMargesVert(0,Dy);
	BoardAreaClose(bProdChoixTemps,&zProdChoixBase,&x,&y);

	BoardAddBouton(bProdChoixTemps,"Sauver",ProdTempsSauve,(x / 2) - (3 * Dx),y + Dy,0);
...
int ProdInit(char a_nouveau) {
	int i,n; int bolo,voie,vm,msr,mode;
#ifdef FICHIER_TEMPS_MANU
	FILE *f; char ligne[MAXLIGNE],*r,*item;
#endif
	//- char *nom;
	ArgDesc *desc;
	char dbg;

//	printf("ProdInit, a_nouveau = %d", a_nouveau);
	...
	if((f = fopen(ProdTempsFile,"r"))) {
		...
		while(LigneSuivante(ligne,MAXLIGNE,f)) {
			...
			for(voie=0; voie<VoiesNb; voie++) if(!strcmp(item,VoieManip[voie].nom)) break;
			if(voie < VoiesNb) {
				item = ItemSuivant(&r,' '); if(*item) {
					sscanf(item,"%g",&val);
					VoieTemps[voie].start = VoieTemps[voie].avant + val;
				}
				item = ItemSuivant(&r,' '); if(*item) {
					sscanf(item,"%g",&val);
					VoieTemps[voie].stop = VoieTemps[voie].avant + val;
				}
				item = ItemSuivant(&r,' '); if(*item) sscanf(item,"%g",&(VoieTemps[voie].apres));
				printf("* Evenement voie %s: base=[%g - %g[, pretrigger=%g, posttrigger=%g (ms) [d'apres %s]\n",
					   VoieManip[voie].nom,VoieTemps[voie].start,VoieTemps[voie].stop,VoieTemps[voie].avant,VoieTemps[voie].apres,ProdTempsFile);
			}
		};
		fclose(f);
	}
#else
	desc = &(ProdTempsDesc[0]);
	ArgAdd(desc++,"montee_10",     DESC_FLOAT &ProdMontee10,    "Pourcentage de l'amplitude pour le debut de la montee");
	ArgAdd(desc++,"montee_90",     DESC_FLOAT &ProdMontee90,    "Pourcentage de l'amplitude pour la fin de la montee");
	ArgAdd(desc++,"montee_50",     DESC_FLOAT &ProdQuotaMontee, "Quota de montee a retirer de la duree");
	ArgAdd(desc++,"pre-lissage",   DESC_INT   &ProdPreLissage,  "Largeur de lissage avant deconvolution");
	ArgAdd(desc++,"post-lissage",  DESC_INT   &ProdPostLissage, "Largeur de lissage apres deconvolution");
	ArgAdd(desc++,"multiple_si",   DESC_INT   &ProdPaquetAmpl,   "Amplitude minimum pour compter un evenement multiple");
	ArgAdd(desc++,"pendant_maxi",  DESC_FLOAT &ProdPaquetDuree,  "Duree maximum d'un evenement multiple (s)");
	ArgAdd(desc++,"voies",         DESC_STRUCT_SIZE(VoieTempsNb,VOIES_MAX) &ProdTempsDescAS, 0);

	desc = &(ProdTempsVoieDesc[0]);
	ArgAdd(desc++,0,    DESC_LISTE VoieNom,   &VoieTempsLue.num,   0);
	ArgAdd(desc++,"base_start",    DESC_FLOAT &VoieTempsLue.start, 0); // "Debut relatif du calcul de la ligne de base");
	ArgAdd(desc++,"base_stop",     DESC_FLOAT &VoieTempsLue.stop,  0); // "Fin relative du calcul de la ligne de base");
	ArgAdd(desc++,"fit_descente",  DESC_FLOAT &VoieTempsLue.apres, 0); // "Temps a utiliser apres le maximum, pour l'evt unite");
	ArgAdd(desc++,"RC",            DESC_FLOAT &VoieTempsLue.RC,    0); // "Temps de descente de l'ampli a utiliser (ms)");

	printf("* Definition des temps lue:\n");
	ArgPrint("*",ProdTempsDesc);
	//	pProdTempsArg = PanelDesc(ProdTempsDesc,0); PanelDesc pas encore au point si DESC_STRUCT_SIZE incluse
	...
	VoieNum = 0;
	for(voie=0; voie<VoiesNb; voie++) {
		// for(i=0; i<VoieTempsNb; i++) if(VoieTemps[i].num == voie) break; if(i >= VoieTempsNb) continue;
		msr = VoieTemps[voie].mesure; if(msr < 0) continue;
		TempsMontee[VoieManip[voie].type] = (int)((VoieTemps[msr].avant - VoieTemps[msr].stop) / VoieEvent[voie].horloge);
		TempsDescente[VoieManip[voie].type] = (int)(VoieTemps[msr].apres / VoieEvent[voie].horloge);
		if(voie == VoieNum) {
			EvtUniteMont = VoieTemps[msr].avant - VoieTemps[msr].stop;
			EvtUniteDesc = VoieTemps[msr].apres;
		}
	}
