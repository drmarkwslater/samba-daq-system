...
static void SvrAlign(char *ptr, char *buffet, int ldata);
...
int SvrCheck(SvrBox link) {
	...
	if(!rc) {
		if(!link->lngr) {
		  ...
		  link->lngr = (int)strlen(SvrTxtRecv);
		}
...
int SvrRecv(SvrBox link) {
	IntAdrs bord,size,alire,l;
	register int lngr,lu,tmp;
	register char *next,*ptr;

	ptr = link->cmde;
	size = link->max;
	bord = (long)ptr + size;
	if((link->dejalu > 0) && (long)link->next > (long)ptr) {
	...
	if(SvrDebug) {
	  printf("[%d]   recu: %08X %08X %08X %08X  [",getpid(),
		*(int *)ptr,*(int *)(ptr+4),*(int *)(ptr+8),*(int *)(ptr+12));
	  for(l=0; l<SVR_LEN_CMDE; l++) 
	    if(isprint(ptr[l])) putchar(ptr[l]); else putchar(' ');
	  printf("] + %d/%d octet(s)\n",lu-SVR_HEADER,lngr);
	}
	if(lngr < 0) {
		sprintf(SvrTxtRecv,"%02d: %s (%d)",SVR_LENGTH,SvrTxtLENGTH,lngr);
		goto boum;
	}
	...
	if(alire > size) {
		...
		if(!next) {
			sprintf(SvrTxtRecv,
				"%02d: %s (requetes de %lld octets)",SVR_OVER,SvrTxtOVER,size);
			goto boum;
		}
	...
	while(lu < alire) {
		tmp = (int)read(link->skt,ptr+lu,size-lu);
		if(!tmp) {
		...
		lu += tmp;
		if(SvrDebug) printf("[%d]   recu: %d octet(s) (total: %d/%lld)\n",
			getpid(),tmp,lu-SVR_HEADER,alire-SVR_HEADER);
	}
	link->lngr = lngr;
	link->next = ptr + alire;
	link->dejalu = (int)(lu - alire);
	return(1);
boum:
	lu = link->dejalu;
	link->lngr = lu;
	...
	if(link->dejalu >= SVR_HEADER)
		printf("Deja recu: %08X %08X %08X %08X + %d octet(s)\n",
		*(int *)ptr,*(int *)(ptr+4),*(int *)(ptr+8),*(int *)(ptr+12),lngr);
	*/
	link->dejalu = 0;
	tmp = (int)strlen(SvrTxtRecv);
	sprintf(SvrTxtRecv+tmp,". %d octet(s) perdu(s)",lu);
	return(0);
}
/* ========================================================================== */
		...
		do {
			...
			if(!strncmp(cmde,SVR_FERMER,7)) pas_fini = 0;
				...
				if(SvrUser()) {
					...
					sprintf(SvrTxtRecv,"%02d: %s (\'%s\' + %d octets)",
...
int SvrHeader(SvrBox link, int serial, char *cmde, int ltot) {
	...
	SvrPutInt(adrs,ltot);
	if(SvrDebug) {
	  printf("[%d]  envoi: %08X %08X %08X %08X  [",getpid(),
		*(int *)(header),*(int *)(header+4),*(int *)(header+8),
		*(int *)(header+12));
	  for(l=0; l<SVR_LEN_CMDE; l++) 
