...
int FiltreCalculeStd(float omega1, float omega2, char modele, char type, short degre, TypeFiltre *coef) {
	...
	if((modele >= FILTRE_BUT) && (modele <= FILTRE_ELL)) {
		...
		if(log > 1) {
			printf("ellfcalc(modele=%d, type=%d, degre=%d, dbr=%g, (int)MAXFILTRECOEF=%d,\n omega1=%g, omega2=%g, omega3=%g, &ncoef=%08X, dir=%08X, inv=%08X)\n",
				modele, type, degre, dbr, (int)MAXFILTRECOEF, omega1, omega2, omega3, (hexa)&ncoef, (hexa)dir, (hexa)inv);
		}
		...
		coef->calcule = 1;
		coef->nbetg = 1;
		coef->etage[0].nbdir = ncoef + 1;
		coef->etage[0].nbinv = ncoef;
		coef->nbmax = ncoef;
...
int FiltresLit(char *nom, char log) {
	...
	int nbcoef,nbmax; int num,etage;
	int i; size_t l;

	num = etage = 0; /* gcc */
	strncpy(FiltreFichier,nom,MAXFILENAME);
	printf("= Lecture des filtres                   dans '%s'\n",nom);
	if(!(fichier = fopen(nom,"r"))) {
		printf("  ! Fichier des filtres illisible: '%s' (%s)\n",nom,strerror(errno));
		return(0);
	}
	FiltreNb = 0;
...
//				printf("(FiltresLit) Le filtre %s est du modele #%d\n",FiltreGeneral[num].nom,i);
				...
				else FiltreGeneral[num].omega1 = 0.1;
				if(unite[0]) {
				...
				else FiltreGeneral[num].omega2 = 0.0001;
				if(unite[0]) {
					for(i=0; i<FILTRE_NBUNITES; i++) if(!strncmp(unite,FiltreUnites[i],3)) break;
	...
	FiltreNum = 0;
	// for(i=0; i<FiltreNb; i++) if(log) printf("%d/ %s @%08X\n",i,FiltreGeneral[i].nom,&(FiltreGeneral[i].coef));
	return(1);
}
#define MAXFREQNOM 10
/* ========================================================================== */
