...
/* ===================================================================== */
int	FitLineaire ( float *a,float *erra,float *b,float *errb,float *chisq,float *x,float *y,float *sigma,char  *valid,int   npt   )
...
/* ========================================================================= */
int FitSecondDegre(	float *x, float *y, float *sigma, char *valid ,int longueur, float *a, float *b, float *c) {
/*
Codes de retour:
 1: tout va bien
...
/* ========================================================================= */
int FitParabole(float *x, float *y, float *sigma, char *valid, int longueur, float *C, float *k, float *x0)
{
/*
Codes de retour:
 1: tout va bien
...
/* soit y = k(x-x0)**2 + C */
	*k = a;                           /* "amplitude"          */
	*x0 = -b / (2.0 * a);             /* position x du sommet */
	*C = c - ((b * b) / (4.0 * a));   /* position y du sommet */
	return(1);
}
/* ========================================================================= */
int FitGaussienne(float *x, float *y, char *valid, int longueur,  float fond, 
...
//	printf("(FitGaussienne) %d valeurs au dessus de %g\n",j,fond);
	if(j < 3) return(-1);
	det = DET(sx4,sx3,sx2,sx3,sx2,sx1,sx2,sx1,sx0);
//	printf("(FitGaussienne) Determinant = %g\n",det);
	/* if ( fabs(det)<0.00001 ) return (-2); */
	if(det == 0.0) return (-2);
...
/* Log(y-fond) = ax2 + bx + c */
	...
	c   = DET(sx4,sx3,sx2,sx3,sx2,sx1,syx2,syx,sy)/det;
	/* if(a > -0.001) return(-3); */
//	printf("(FitGaussienne) Coefficient 2nd degre = %g\n",a);
	if(a >= 0.0) return(-3);

//	printf("(FitGaussienne) Polynome: %g X2   %+g X   %+g\n",a,b,c);
