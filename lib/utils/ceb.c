...
/* Deroule le code (listing et/ou execution).
   Si execution, renvoie le dernier resultat calcule present sur la pile */
	...
	unsigned char instr; unsigned mode;
	float *adrs;
	int num,i,nb_nop;
	...
	while((instr = pc->instr) != CEB_EXIT) {
		...
		switch(instr) {
		  ...
		  case CEB_AND:
			switch(mode) {
			  case CEB_IMME: *reg = *reg && pc->val.imme; break;
			  case CEB_ADRS: *reg = *reg && *(pc->val.adrs); break;
			  case CEB_PILE: --reg; *reg = *reg && *(reg + 1); break;
			}
			break;
