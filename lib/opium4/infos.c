...
Info InfoCreate(int lignes, int colonnes) {
	...
	if(info) {
		if(DEBUG_INFO(1)) WndPrint("Info cree @%08X\n",(hexa)info);
		...
		if(DEBUG_INFO(3)) WndPrint("Utilise cdr @%08X\n",(hexa)info->cdr);
	}
	return(info);
}
/* ========================================================================== */
...
int OpiumSizeInfo(Cadre cdr, char from_wm) {
	...
	if(DEBUG_OPIUM(1) && (cdr->type != OPIUM_INFO)) {
		WndPrint("+++ OpiumSizeInfo sur le %s @%08X\n",cdr->nom,(hexa)cdr->adrs);
		return(0);
	}
	if(from_wm) {
...
int OpiumRefreshInfo(Cadre cdr) {
	...
	if(DEBUG_OPIUM(1) && (cdr->type != OPIUM_INFO)) {
		WndPrint("+++ OpiumRefreshInfo sur le %s @%08X\n",cdr->nom,(hexa)cdr->adrs);
		return(0);
	}
...
int OpiumRunInfo(Cadre cdr, WndUserRequest *e) {
	...
	if(DEBUG_OPIUM(1) && (cdr->type != OPIUM_INFO)) {
		WndPrint("+++ OpiumRunInfo sur le %s @%08X\n",cdr->nom,(hexa)cdr->adrs);
		return(0);
	}
	if(cdr->planche) return(-1);
	info = (Info)cdr->adrs;
	lig = WndPixToLig(e->y);
	if(DEBUG_INFO(1)) WndPrint("Info @%08X, ligne %d\n",(hexa)info,lig);
	code_rendu = -1;
	if(e->type == WND_KEY) {
		l = strlen(e->texte);
		if(DEBUG_INFO(2)) {
