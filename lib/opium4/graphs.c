...
#endif
#define GraphDim(graph) (graph->dim)
...
static void GraphMarkDraw(WndFrame f, WndContextPtr gc, int type, int nb);
...
/* ========================================================================== */
Graph GraphCreate(int larg, int haut, int max) {
	Cadre cdr; Graph graph; int i;

	graph = (Graph)malloc(sizeof(TypeGraph) + (max * sizeof(TypeGraphData)));
	if(graph) {
		if(DEBUG_GRAPH(2)) WndPrint("Graph %d x %d cree @%08X\n",larg,haut,(hexa)graph);
		...
		graph->onclic = 0;
		if(DEBUG_GRAPH(1)) {
			WndPrint("(%s) GraphData (%d preallouees @%08X)",__func__,max,(hexa)graph->data);
			for(i=0; i<GRF_MAX_DATA; i++) {
				if(!(i%8)) WndPrint("\n%3d:",i);
				WndPrint(" %08X",(hexa)graph->data_ptr[i]);
			}
			WndPrint("\n");
		}
		...
		GraphErase(graph);
		OpiumSetOptions(cdr);
		if(DEBUG_GRAPH(3)) WndPrint("Utilise cdr @%08X\n",(hexa)graph->cdr);
	}
...
/* Mode d'un graphe:
	...
	if(mode == GRF_DIRECT) {
		...
		graph->axe[0].marge    = graph->axe[1].marge      = 0;
		graph->axe[0].pixel    = graph->axe[1].pixel      = 1.0;
		...
		graph->axe[0].marge = 0; /* on ne sait jamais.. */
		graph->axe[1].marge = graph->grad + graph->of7 + WndLigToPix(1);
	}
}
/* ========================================================================== */
void GraphErase(Graph graph) {
	...
	for(num=graph->max; num<graph->dim; num++, data_ptr++) {
		if(DEBUG_GRAPH(1)) printf("(GraphErase) data_ptr[%d] = %08X\n",num,(hexa)data_ptr);
		data = *data_ptr; if(!data) continue;
...
//	GraphMode(graph,GRF_DIRECT);
	...
	graph->axe[0].pixel      = graph->axe[1].pixel      = 1.0;
	graph->axe[0].ech        = graph->axe[1].ech        = 0.0;
	graph->axe[0].u.i.min = 0; graph->axe[1].u.i.min = 0;
	graph->axe[0].u.i.max = graph->axe[0].lngr; graph->axe[1].u.i.max = graph->axe[1].lngr;
	graph->axe[0].ech = (float)(graph->axe[0].u.i.max - graph->axe[0].u.i.min);
	graph->axe[1].ech = (float)(graph->axe[1].u.i.max - graph->axe[1].u.i.min);
...
int GraphAdd(Graph graph, int type, void *adrs, int dim) {
	...
	if((type & GRF_ICONE)) {
		...
		if((type_axe == GRF_IMAGE) || (type_axe == GRF_ZAXIS)) {
			if((data->parm & GRF_TYPE) == GRF_INT) {
				...
				data->gamma.r.xmin = 0.0;
				data->gamma.r.xmax = 255.0;
				data->gamma.r.vmin = 0.0;
				data->gamma.r.vmax = 255.0;
			}
	...
	if((data->parm & GRF_TYPE) == GRF_FLOAT) data->of7.r.v = 0.0; else data->of7.i.v = 0;
	// if((type_axe != GRF_IMAGE) && (graph->mode == GRF_DIRECT)) GraphMode(graph,GRF_2AXES);
	if(index == graph->dim) graph->dim = index + 1;
	graph->pret = 0;
	if(DEBUG_GRAPH(3)) WndPrint("graph->mode=%x, axeX=%x, axeY=%x (GRF_DIRECT=%x)\n",
...
int GraphInclude(Graph graph, int type, void *adrs, int dim) {
	int index; int type_axe; int qual;
	GraphData data;

	if(!graph) return(-1);
	index = graph->dim;
	if(index >= GRF_MAX_DATA) {
		OpiumError("(GraphInclude) Pas plus de %d tableaux",GRF_MAX_DATA);
		return(-1);
	} else if(index >= graph->max) {
		data = (GraphData)malloc(sizeof(TypeGraphData));
		if(data) graph->data_ptr[index] = data;
		else {
			OpiumError("(GraphInclude) Manque de place memoire");
			return(-1);
		}
	} else data = graph->data_ptr[index];
	/* Parametres par defaut */
...
int GraphDimGet(Graph graph) {
	if(graph) return(graph->dim); else return(0);
}
/* ========================================================================== */
...
void *GraphDataGet(Graph graph, int num, int *dim) {
	...
	if((num < 0) || (num >= graph->dim) ) {
		OpiumError("(GraphDataGet) Numero de tableau invalide (%d/%d)",num,graph->dim);
		return(0);
	}
	data = graph->data_ptr[num];
	if(!data) {
		OpiumError("(GraphDataGet) Tableau #%d pas alloue en memoire",num); return(0);
	}
	*dim = data->dim;
	return(data->fctn);
}
...
int GraphDataReplace(Graph graph, int num, int type, void *adrs, int dim) {
	...
	if(!graph) return(-1);
	if((num < 0) || (num >= graph->dim) ) {
		OpiumError("(GraphDataReplace %s,%d) Numero de tableau invalide (dim=%d)",
			(graph->cdr)->titre,num,graph->dim);
		return(-1);
	}
	data = graph->data_ptr[num];
	if(!data) {
		OpiumError("(GraphDataReplace) Tableau #%d pas alloue en memoire",num); return(-1);
	}
	data->parm = (unsigned char)type; /* (char)(type & 0xFF); */
	data->fctn = adrs;
	data->min = 0;
	data->max = dim;
	data->dim = dim;
...
char GraphDataDisconnect(Graph graph, int num) {
	...
	if(!graph) return(-1);
	if((num < 0) || (num >= graph->dim) ) {
		OpiumError("(GraphDataDisconnect %s,%d) Numero de tableau invalide (dim=%d)",
			(graph->cdr)->titre,num,graph->dim);
		return(-1);
	}
	data = graph->data_ptr[num];
	if(!data) {
		OpiumError("(GraphDataDisconnect) Tableau #%d pas alloue en memoire",num); return(-1);
	}
	data->fctn = (void *)-1;
	return(1);
}
...
char GraphDataRemove(Graph graph, int num) {
	...
	if(!graph) return(-1);
	if((num < 0) || (num >= graph->dim) ) {
		OpiumError("(GraphDataRemove %s,%d) Numero de tableau invalide (dim=%d)",
			(graph->cdr)->titre,num,graph->dim);
		return(-1);
	}
	data = graph->data_ptr[num];
...
int GraphDataTranslate(Graph graph, int num, char transfo) {
	...
	if(!graph) return(-1);
	if((num < 0) || (num >= graph->dim) ) {
		OpiumError("(GraphDataTranslate %s,%d) Numero de tableau invalide (dim=%d)",
			(graph->cdr)->titre,num,graph->dim);
		return(0);
	}
	data = graph->data_ptr[num];
	if(!data) {
		OpiumError("(GraphDataTranslate) Tableau #%d pas alloue en memoire",num); return(0);
	}
...
int GraphDataIntGamma(Graph graph, int num, int xmin, int xmax, int vmin, int vmax) {
	...
	if(!graph) return(-1);
	if((num < 0) || (num >= graph->dim) ) {
		OpiumError("(GraphDataIntGamma %s,%d) Numero de tableau invalide (dim=%d)",
			(graph->cdr)->titre,num,graph->dim);
		return(0);
	}
	data = graph->data_ptr[num];
	if(!data) {
		OpiumError("(GraphDataIntGamma) Tableau #%d pas alloue en memoire",num); return(0);
	}
	if((data->parm & GRF_TYPE) == GRF_FLOAT) {
		OpiumError("(GraphDataIntGamma) Tableau #%d de type float",num); return(0);
	}
	...
	data->parm |=  GRF_GAMMA;
	if(DEBUG_GRAPH(2))
		WndPrint("(GraphDataIntGamma) gamma: [%d .. %d] -> [%d .. %d]\n",
...
int GraphDataFloatGamma(Graph graph, int num, float xmin, float xmax, float vmin, float vmax) {
	...
	if(!graph) return(-1);
	if((num < 0) || (num >= graph->dim) ) {
		OpiumError("(GraphDataFloatGamma %s,%d) Numero de tableau invalide (dim=%d)",
			(graph->cdr)->titre,num,graph->dim);
		return(0);
	}
	data = graph->data_ptr[num];
	if(!data) {
		OpiumError("(GraphDataFloatGamma) Tableau #%d pas alloue en memoire",num); return(0);
	}
	if((data->parm & GRF_TYPE) != GRF_FLOAT) {
		OpiumError("(GraphDataFloatGamma) Tableau #%d de type entier",num); return(0);
	}
	...
	data->parm &= (GRF_AXIS | GRF_ADRS | GRF_TYPE);
	data->parm |=  GRF_GAMMA;
	if(DEBUG_GRAPH(1)) WndPrint("(GraphDataFloatGamma) gamma: [%g .. %g] -> [%g .. %g]\n",
...
int GraphDataBmp(Graph graph, int num, WndColor *lut, int dim) {
	...
	//- printf("(%s) LUT[%d] @%08X\n",__func__,data->couleur[f->qualite].def.lut.dim,(hexa)(data->couleur[f->qualite].def.lut.val));
	graph->pret = 0;
	return(1);
}
/* ========================================================================== */
...
int GraphDataSupportTrace(Graph graph, int num, int qual, int mode, int type, int width) {
	...
	if(width < 1) width = 1; else if(width > 7) width = 7;
	data->trace[qual] = ((mode & 3) << GRF_SHIFT_MODE) | ((type  & 7) << GRF_SHIFT_LINE) | ((width & 7) << GRF_SHIFT_WIDZ);
	graph->pret = 0;
	return(1);
}
/* ========================================================================== */
...
void GraphDump(Graph graph) {
	...
	for(num = 0, data_ptr = graph->data_ptr; num < graph->dim ; num++,data_ptr++) {
		data = *data_ptr; if(!data) continue;
		WndPrint("D[%d] (%d - %d): parm=%02X, fctn@%08X",
			data->dim,data->min,data->max,data->parm,(hexa)data->fctn);
...
/* Choisi le tableau definissant l'axe (x ou y selon data->parm) */
	...
	if((num < 0) || (num >= graph->dim)) {
		OpiumError("(GraphAxisDefine %s,%d) Numero de tableau invalide (dim=%d)",
			(graph->cdr)->titre,num,graph->dim);
		return(0);
	}
	...
	data = graph->data_ptr[num];
	if(!data) {
		OpiumError("(GraphAxisDefine) Tableau supprime");
		return(0);
	}
	...
	if(scale->pret) {
		...
		if(!scale->minauto) { };
		if(!scale->maxauto) { };
	} else printf("(GraphAxisDefine) Axe pas pret (%c)\n",reel? 'R': 'I');
	*/
	if(reel && !scale->reel) {
//		printf("(GraphAxisDefine) On avait   [%d .. %d] (I)\n",scale->u.i.min,scale->u.i.max);
		rmin = scale->u.i.min; scale->u.r.min = rmin;
		rmax = scale->u.i.max; scale->u.r.max = rmax;
...
//		printf("(GraphAxisDefine) On avait   [%g .. %g] (R)\n",scale->u.r.min,scale->u.r.max);
		rmin = scale->u.r.min; scale->u.i.min = rmin;
		rmax = scale->u.r.max; scale->u.i.max = rmax;
...
int GraphAxisIntRange(Graph graph, unsigned char axe, int imin, int imax) {
	...
	if(scale->reel) {
		scale->u.r.min = imin; scale->u.r.max = imax;
	} else {
...
/* Impose l'intervalle des graduations sur l'axe <axe> si ce sont des reels */
	...
	if(scale->reel) {
		...
		scale->u.i.min = rmin; scale->u.i.max = rmax;
	}
	scale->minauto = scale->maxauto = 0;
	scale->pret = 0;
	return(1);
...
/* Impose le minimum des graduations sur l'axe <axe> si ce sont des entiers */
	...
	if(axe == GRF_YAXIS) scale = graph->axe + 1; else scale = graph->axe;
	if((scale->num < 0) || (scale->num >= graph->dim)) scale->reel = 0;
	if(scale->reel) scale->u.r.min = imin; else scale->u.i.min = imin;
	scale->minauto = 0;
	scale->pret = 0;
	return(1);
}
...
/* Impose le minimum des graduations sur l'axe <axe> si ce sont des reels */
	...
	if(axe == GRF_YAXIS) scale = graph->axe + 1; else scale = graph->axe;
	if((scale->num < 0) || (scale->num >= graph->dim)) scale->reel = 1;
	if(scale->reel) scale->u.r.min = rmin; else scale->u.i.min = rmin;
	scale->minauto = 0;
	scale->pret = 0;
	return(1);
}
...
/* Impose le maximum des graduations sur l'axe <axe> si ce sont des entiers */
	...
	if(axe == GRF_YAXIS) scale = graph->axe + 1; else scale = graph->axe;
	if((scale->num < 0) || (scale->num >= graph->dim)) scale->reel = 0;
	if(scale->reel) scale->u.r.max = imax; else scale->u.i.max = imax;
	scale->maxauto = 0;
	scale->pret = 0;
	return(1);
}
...
/* Impose le maximum des graduations sur l'axe <axe> si ce sont des reels */
	...
	if(axe == GRF_YAXIS) scale = graph->axe + 1; else scale = graph->axe;
	if((scale->num < 0) || (scale->num >= graph->dim)) scale->reel = 1;
	if(scale->reel) scale->u.r.max = rmax; else scale->u.i.max = rmax;
	scale->maxauto = 0;
	scale->pret = 0;
	return(1);
}
...
int GraphAxisIntGet(Graph graph, unsigned char axe, int *imin, int *imax, int *igrad) {
	...
	if(axe == GRF_YAXIS) scale = graph->axe + 1; else scale = graph->axe;
	if((scale->num < 0) || (scale->num >= graph->dim)) scale->reel = 0;
	if(scale->reel) { *imin = scale->u.r.min; *imax = scale->u.r.max; }
...
/* Recupere les limites et graduation de l'axe <axe> si ce sont des reels */
	...
	else { *rmin = scale->u.i.min; *rmax = scale->u.i.max; }
	*rgrad = scale->grad;
	return(1);
}
/* ========================================================================== */
int GraphAxisMarge(Graph graph, unsigned char axe) {
    // printf("(%s) marge[%d]=%d\n",__func__,axe,graph->axe[axe].marge);
    if(graph) return(graph->axe[axe].marge); else return(0);
}
/* ========================================================================== */
int GraphUserScrollX(Graph graph, int ix, float rx) {
	...
	cdr = graph->cdr;
	if(xscale->reel)
		cdr->dx = (rx - xscale->u.r.min) * xscale->pixel;
	else
		cdr->dx = (ix - xscale->u.i.min) * xscale->total / (int)xscale->ech;
...
int GraphUserScrollY(Graph graph, int iy, float ry) {
	...
	cdr = graph->cdr;
	if(yscale->reel)
		cdr->dy = (ry - yscale->u.r.min) * yscale->pixel;
	else
		cdr->dy = (iy - yscale->u.i.min) * yscale->total / (int)yscale->ech;
...
void GraphAjusteParms(Graph graph, unsigned char axe, GraphAxeParm *parms) {
	...
	if(parms->autom) {
		scale->minauto = scale->maxauto = 1;
		if(scale->reel) {
			if(parms->log) scale->u.r.min = 0.001; else scale->u.r.min = 0.0;
		} else {
...
int GraphBlank(Graph graph) {
	...
	WndPaint(f,0,0,cdr->dh,cdr->dv,WndColorBgnd[GraphQualite],3);
	return(1);
}
/* ========================================================================== */
inline static void GraphPlacePoint(WndFrame f, WndContextPtr gc, unsigned char trace,
	...
	switch (trace) {
	  case GRF_CODE_POINT:  /* un point */
		...
		break;
	  case GRF_CODE_CROSS:  /* une croix */
		GraphMarkOrigin(GRF_CROSS,x,y - (GRF_DIM_CROSS / 2));
		GraphMarkDraw(f,gc,GRF_CROSS,4);
		break;
...
/* ========================================================================== */
void OpiumGraphColorNb(Cadre cdr, Cadre fait, int *nc) {
	Graph graph,modele; int ic;

	if(!(graph = (Graph)cdr->adrs)) return;
	if(fait) modele = (Graph)fait->adrs; else modele = 0;
	if(modele) {
		graph->pomme = modele->pomme; graph->grille = modele->grille;
	} else {
		ic = *nc;
...
int OpiumSizeGraph(Cadre cdr, char from_wm) {
	...
	if(DEBUG_OPIUM(1) && (cdr->type != OPIUM_GRAPH)) {
		WndPrint("+++ OpiumSizeGraph sur le cadre '%s' @%08X\n",cdr->nom,(hexa)cdr->adrs);
		return(0);
	}
...
/*		marche si figure toujours complete -> si tj meme rapport affiche/total (=zoom) */
		...
		graph->axe[0].pret = graph->axe[1].pret = 0;
		if(DEBUG_OPIUM(1)) GraphDump(graph);
	} else {
		...
		if(DEBUG_OPIUM(1)) WndPrint("larg=%d, zoomh=%d soit dh=%d\n",cdr->larg,graph->axe[0].zoom,cdr->dh);
	}
	return(1);
}
#ifdef A_COMPLETER
...
static void GraphTraceAxes(Graph graph, GraphScale *xscale, GraphScale *yscale, Cadre cdr, WndFrame f) {
	...
	if(((graph->mode & GRF_NBAXES) != GRF_0AXES) && (graph->mode != GRF_DIRECT)) {
		...
		if(graph->mode & GRF_GRID) {
			...
			xlim = xscale->total + xscale->marge - cdr->dx;
			if(xlim > cdr->dh) xlim = cdr->dh;
			timeout = 0;
			xcp = xaxe1;
			do {
				if((xc >= xscale->marge) && (xc != xaxe1) && (xc != xaxe2) && (abs(xc - xcp) > 0)) {
					GraphAxisYDraw(graph,xc,gc,GRF_GRADNONE);
					xcp = xc;
				}
				if(xscale->logd) {
					if(xscale->grad < 0.0) rx *= 10.0;
					else {
						rx += xscale->grad;
						if(--nbgrad == 0) { xscale->grad *= 10.0; nbgrad = 9; xcp = xaxe1; }
					}
				} else rx += xscale->grad;
				xc = GraphPosFloat(1,rx,xscale) - cdr->dx;
			...
			if(yscale->logd) {
				...
				if(yscale->grad > 0.0) yscale->grad = ry;
				nbgrad = 9;
			} else ry = yscale->g1 + (yscale->gmin * yscale->grad);
			yc = GraphPosFloat(1,ry,yscale) + cdr->dy;
			ylim = cdr->haut - yscale->marge + cdr->dy;
			if(ylim > cdr->haut) ylim = cdr->haut;
			timeout = 0;
			ycp = yaxe1;
			do {
				if((yc >= yscale->marge) && (yc != yaxe1) && (yc != yaxe2) && (abs(yc - ycp) > 0)) {
					GraphAxisXDraw(graph,yc,gc,GRF_GRADNONE);
					ycp = yc;
				}
				if(yscale->logd) {
					if(yscale->grad < 0.0) ry *= 10.0;
					else {
						ry += yscale->grad;
						if(--nbgrad == 0) { yscale->grad *= 10.0; nbgrad = 9; ycp = yaxe1; }
					}
				} else ry += yscale->grad;
				yc = GraphPosFloat(1,ry,yscale) + cdr->dy;
				if(++timeout > 100) break;
...
int GraphTrace(Cadre cdr, int ajoutes) {
	...
	int usr_i,ytot_i; float usr_r,ytot_r;
	int xcur,yvus,type; char montre_tout,premier;
	unsigned char mode,trace,trace_general,trace_point,typegrad;
	char doit_terminer,complet,reel,court,fctn,histo_libre;

	if(cdr->type != OPIUM_GRAPH) {
		if(DEBUG_OPIUM(1)) WndPrint("+++ OpiumRefreshGraph sur le cadre '%s' @%08X\n",cdr->nom,(hexa)cdr->adrs);
		return(0);
	}
//	WndPrint("(OpiumRefreshGraph) Cadre '%s' @%08X (graph @%08X)\n",cdr->nom,(hexa)cdr,(hexa)cdr->adrs);
...
//	WndErase(f,0,0,cdr->dh,cdr->dv); laisse en blanc le fond dans les planches
	...
	if(complet) {
		if(cdr->planche) WndPaint(f,0,0,cdr->dh,cdr->dv,WndColorBgnd[f->qualite],3);
		else WndPaint(f,0,0,cdr->dh+WND_ASCINT_WIDZ,cdr->dv+WND_ASCINT_WIDZ,WndColorBgnd[f->qualite],3);
	}
	...
	xscale = graph->axe;
	yscale = xscale + 1;
	premier = 0; nb_ab6 = xcur = yvus = ytot_i = x = y = 0; ytot_r = 0.0; data_ab6 = 0; // compilo

/*
 * Trace des axes
 */
	gc_ab6 = graph->data_ptr[xscale->num]->gc[f->qualite];
	// printf("(%s) gc_ab6 = data[%d](@%08X).gc[%d] @%08X\n",__func__,xscale->num,(hexa)&(graph->data_ptr[xscale->num]),f->qualite,(hexa)gc_ab6);
	...
	if(complet) /* GraphTraceAxes(graph,xscale,yscale,cdr,f); */ {
		...
		#ifdef QUICKDRAW
			// WndString(f,gc,xp,yp,"ð");
			icone[0] = 0xF0; icone[1] = 0; WndString(f,gc,xp,yp,(char *)icone);
		#else
			WndString(f,gc,xp,yp,"?");
		#endif
		}
...
//	axeX = x = 0; /*pour gcc */
	if((xscale->num & 0xFFFF) != GRF_DIRECT) {
		...
		if(!data_ab6) {
			WndWrite(f,0,0,0,"(OpiumRefreshGraph) Abcisse definie sur un tableau absent");
	...
	for (num = 0; num < graph->dim ; num++,data_ptr++) {
		if(DEBUG_GRAPH(1)) {
			WndPrint("(OpiumRefreshGraph) Tableau #%d/%d @%08X",num,graph->dim,(hexa)*data_ptr);
			if(*data_ptr) WndPrint("->%08X\n",(hexa)((*data_ptr)->fctn)); else WndPrint(" inutilisable\n");
		}
		data = *data_ptr; if(!data) continue;
		if(data->fctn == (void *)-1) continue; // 0 permis a ce niveau
		//- printf("(%s.1[%d]) @%08X.%08X.line_width = %d\n",__func__,num,(hexa)data,(hexa)(data->gc),(data->gc)->line_width);
		...
		if(DEBUG_GRAPH(1)) WndPrint("(OpiumRefreshGraph) Donnees #%d: %d valeurs a partir de %d en %c\n",
			num,nb,data->min,
		...
		if(((data->parm & GRF_AXIS) == GRF_ZAXIS) || ((data->parm & GRF_AXIS) == GRF_IMAGE)) {
			...
			imin = imax = iof7 = itop = 0; rmin = rmax = rof7 = rtop = rcor = 0.0;
			...
			if(data->couleur.type == GRF_ICN) {
				...
				ymax = GraphPosData(1,data_ord,nb_ord-1,yscale);
			...
			if(DEBUG_GRAPH(2)) {
				...
				WndPrint("(%s) Marge %3d x %3d\n",__func__,xscale->marge,yscale->marge);
				...
				WndPrint("(%s) ----- %3s   %3s\n",__func__,"---","---");
				xmax = cdr->larg - cdr->dx - xscale->marge;
				ymax = cdr->haut - cdr->dy - yscale->marge;
...
//					 - ((GraphPosData(1,data_ord,1,yscale) + GraphPosData(1,data_ord,0,yscale)) / 2);
				ymax = GraphPosData(1,data_ord,nb_ord-1,yscale) - yscale->marge;
		...
		//- printf("(%s.2) @%08X.%08X.line_width = %d\n",__func__,(hexa)data,(hexa)gc,gc->line_width);
...
//			si nbx pas calcule: xp = cdr->dh - WndColToPix(strlen(data->titre));
		...
		trace_general = data->trace[f->qualite] & GRF_LINE;
		montre_tout = ((trace_general == GRF_CODE_TOUT) || (mode == GRF_CODE_ERR) || (mode == GRF_CODE_PNT));
		if(nbpts > nbmem) {
			if(nbmem) { free(p); nbmem = 0; }
			p = (WndPoint *)malloc(nbpts * sizeof(WndPoint));
			if(!p) continue;
			nbmem = nbpts;
		}
//		if(DEBUG_GRAPH(2))
//			WndPrint("%d points stockables @%08X\n",nbmem,(hexa)p);
//		if(DEBUG_GRAPH(2)) printf("(%s) trace = %02X (mode %02X, ligne #%02X)\n",__func__,
//			data->trace[f->qualite],data->trace[f->qualite] & GRF_MODE,data->trace[f->qualite] & GRF_LINE);
		if(complet && ((data_ab6->parm & GRF_ADRS) == GRF_INDEX) && (cdr->dx > 0)) {
			...
			if((data_ab6->parm & GRF_TYPE) == GRF_FLOAT) {
				rval = (float)cdr->dx * xscale->ech; rval /= (float)xscale->total; rval += xscale->u.r.min;
...
//					xscale->u.r.min,rval,*rtab,*(rtab+1),jxmin);
				...
				ival = (int64)cdr->dx * (int64)xscale->ech; ival /= xscale->total; ival += xscale->u.i.min;
...
//					cdr->dx,cdr->larg,xscale->ech,xscale->total,xscale->u.i.min,
//					(int)ival,*itab,*(itab+1),jxmin);
		...
		if(!montre_tout) { yvus = 0; ytot_i = 0; ytot_r = 0.0; xcur = -99999; premier = 1;}
		for(i=0, ii=0; i<nb; i++, jx++,jy++) {
			...
			if(indiv) {
				...
				if(trace_point == GRF_NODISPLAY) continue;
				trace = trace_point & GRF_LINE;
			} else trace = trace_general;
			x = GraphPosData(1,data_ab6,jx,xscale);
			x -= cdr->dx;
			if((x >= cdr->dh) && ((data_ab6->parm & GRF_ADRS) == GRF_INDEX)) break;
			if(montre_tout) y = cdr->haut - GraphPosData(1,data,jy,yscale);
			else {
				GraphUserVal(data,jy,type,&usr_i,&usr_r);
				if(!premier) {
					if(x == xcur) {
						yvus++; if(reel) ytot_r += usr_r; else ytot_i += usr_i;
						continue;
					}
					if(reel) {
						...
						if(yvus > 1) ytot_i /= (float)yvus;
						y = cdr->haut - GraphPosInt(1,ytot_i + data->of7.i.v,yscale);
					}
				}
				yvus = 1; xcur = x; if(reel) ytot_r = usr_r; else ytot_i = usr_i; /* memorise le pt courant */
				if(premier) { premier = 0; continue; }
			}
			y -= cdr->dy;
			if(y < 0) y = 0; else if(y >= cdr->dv) y = cdr->dv - 1;
			if(((i == 0) || (x != xp) || (y != yp)) && (x >= 0) && (x < cdr->dh)
			/* && (y >= 0) && (y < cdr->dv) */ ) switch (mode) {
				  case GRF_CODE_HST:
					xm = 0.5 * (xp + x);
		...
		if(ii > nbmem) {
			WndPrint("(OpiumRefreshGraph) ERREUR SYSTEME, PLACE MEMOIRE MAL CALCULEE\n");
			WndPrint(" reservee: %d points, utilisee: %d points\n",nbmem,ii);
		}
		if(DEBUG_GRAPH(2)) WndPrint("%d points a tracer @%08X\n",ii,(hexa)p);
		//- printf("(%s.2[%d]) @%08X.%08X.line_width = %d\n",__func__,num,(hexa)data,(hexa)(data->gc),(data->gc)->line_width);
		switch (mode) {
...
/* ========================================================================== */
int OpiumRunGraph(Cadre cdr, WndUserRequest *e) {
	...
	char trois_d,doit_terminer; float z;
	...
	if(DEBUG_OPIUM(1) && (cdr->type != OPIUM_GRAPH)) {
		WndPrint("+++ OpiumRunGraph sur le cadre '%s' @%08X\n",cdr->nom,(hexa)cdr->adrs);
		return(0);
	}
	if(!OpiumCoordFenetre(cdr,&f)) return(0);
	graph = (Graph)cdr->adrs;
	...
	if(e->type == WND_KEY) {
		WndPrint("(%s) KeyBoard en (%d, %d) + (%d, %d)\n",__func__,
		...
		if(l <= 0) {
			...
			if(l == 1) switch(e->texte[0]) {
			  case '?':
				WndPaint(f,0,0,WndColToPix(60),WndLigToPix(19),WndColorSub[GraphQualite],3);
				i = 0;
...
//			if(!yscale->reel) GraphUser[1].r1 = (float)GraphUser[1].i1; // pour avoir la meme valeur entiere
			...
			if((ab6 && ord && (ab6->parm & GRF_ADRS) == GRF_INDEX) && ((ord->parm & GRF_ADRS) == GRF_INDEX)) {
				...
				if(reel) {        rtab = (float *)ab6->fctn; if(rtab[1]) i = (GraphUser[0].r1 - rtab[0] + (rtab[1] / 2.0)) / rtab[1]; xok = 1; }
				...
				court = (type == GRF_SHORT); reel = (type == GRF_FLOAT);
				if(reel) {        rtab = (float *)ord->fctn; if(rtab[1]) j = (GraphUser[1].r1 - rtab[0] + (rtab[1] / 2.0)) / rtab[1]; yok = 1; }
	  ...
	  case WND_MSELEFT:
		// if(DEBUG_GRAPH(0))
		WndPrint("(%s) MseRelease en (%d, %d) + (%d, %d)\n",__func__,
			e->x,e->y,cdr->dx,cdr->dy);
		if(OpiumZoomEnCours) {
...
void GraphPosUser(GraphScale *scale, int x, int *ival, float *rval) {
	...
	if(DEBUG_GRAPH(1)) printf("(%s) p=(%d - %d)/%g + %d\n",__func__,x,scale->marge,scale->pixel,scale->u.i.min);
	pval = (float)(x - scale->marge) / scale->pixel;
	if(scale->logd) *rval = (float)pow(10.0,(double)pval + scale->logmin);
	else if(scale->reel) *rval = pval + scale->u.r.min;
	else *rval = pval + (float)scale->u.i.min;
	*ival = (int)(*rval + 0.5);
}
/* ========================================================================== */
void GraphEventUserInt(Graph graph, WndUserRequest *e, int *ix, int *iy) {
...
/* ========================================================================== */
void GraphEventUserFloat(Graph graph, WndUserRequest *e, float *rx, float *ry) {
...
static size_t GraphXcols(Graph graph, GraphScale *yscale) {
	...
	if(!yscale->logd && (yscale->grad < 0.0)) { sprintf(GraphErrorText,"(%s) GradY=%g",__func__,yscale->grad); return(0); }
	fctn = (float (*)(Graph, int, float))yscale->chgt_unite;
	cols = 0;
	if(yscale->reel) rlim = yscale->u.r.max; else rlim = (float)yscale->u.i.max;
	ry = yscale->g1;
	while(ry <= rlim) {
		if(!yscale->logd) { if(fabs((double)ry) < (yscale->grad / 100.0)) ry = 0.0; }
		if(yscale->reel) {
			if(fctn) {
				// float (*fctnTmp)(Graph, int, float) =  (float (*)(Graph, int, float))fctn;
				sprintf(s,yscale->format,fctn(graph,0,ry));
			} else sprintf(s,yscale->format,ry);
		} else {
			if(fctn) {
				// float (*fctnTmp)(Graph, int, float) =  (float (*)(Graph, int, float))fctn;
				sprintf(s,yscale->format,fctn(graph,0,ry));
			} else sprintf(s,"%d",inintf(ry));
		}
		lngr = strlen(s);
		if(lngr > cols) cols = lngr;
		if(yscale->logd) ry *= 10.0; else ry += yscale->grad;
	};
	return(cols);
}
/* .......................................................................... */
...
/* Prepare les echelles */
	...
	unsigned int type,epaisseur; int num,cols,qual; char *nom;
...
/* Choix de l'axe X */
	...
	if(xscale->num == GRF_UNDEFINED) {
		sprintf(GraphErrorText,"(GraphScaleDefine) Abcisse non definie pour le graphe '%s'",cdr->nom);
		return(0);
	}
//	if(DEBUG_GRAPH(1)) WndPrint("(GraphScaleDefine) Abcisse graphe %s: tableau #%d\n",cdr->nom,xscale->num);
...
/* Choix de l'axe Y */
	...
	if(yscale->num == GRF_UNDEFINED) {
		sprintf(GraphErrorText,"(GraphScaleDefine) Aucune courbe definie pour le graphe '%s'",cdr->nom);
		return(0);
	}
//	if(DEBUG_GRAPH(1)) WndPrint("(GraphScaleDefine) Ordonnee graphe '%s': tableau #%d\n",cdr->nom,yscale->num);
	...
	if(!graph->pret) {
		...
		for(num=0; num<graph->dim; num++, data_ptr++) {
			...
			if((data->couleur.type == GRF_NOM) || (data->couleur.type == GRF_RGB)) for(qual=0; qual<WND_NBQUAL; qual++) {
				// printf("(%s:%d) ---------- @%08X: DATA[%d].gc[%d], couleur %s\n",__func__,__LINE__,(hexa)data,num,qual,(data->couleur.type == GRF_RGB)? "rgb": "nommee");
				...
				// printf("(%s.3[%d]) @%08X.%08X.line_width = %d\n",__func__,num,(hexa)data,(hexa)(data->gc[qual]),(data->gc[qual])->line_width);
...
/* Calcul des graduations utilisateur puis des marges */
	if(!yscale->pret) {
		if(!GraphGradsDefine(graph,GRF_YAXIS,graph->axe[1].zoom)) {
			// sprintf(GraphErrorText,"(GraphScaleDefine) Graduations Y non definies pour le graphe '%s'",cdr->nom);
			return(0);
		}
		...
		if(!graph->avec_axes) {
			...
			yscale->marge_max = yscale->marge_min = graph->grad + graph->of7 + WndLigToPix(1);
		} else if(yscale->position == GRF_GRADINF) {
			yscale->marge_min = graph->grad + graph->of7 + WndLigToPix(1);
			yscale->marge_max = 0;
		} else if(yscale->position == GRF_GRADSUP) {
			yscale->marge_min = 0;
			yscale->marge_max = graph->grad + graph->of7 + WndLigToPix(1);
		} else {
			yscale->marge_min = 0;
			yscale->marge_max = 0;
		}
		yscale->total = cdr->haut - yscale->marge_min - yscale->marge_max;
#endif
		if((graph->mode & GRF_NBAXES) == GRF_0AXES) yscale->marge = 0;
		else {
			yscale->marge = graph->grad + graph->of7 + WndLigToPix(1);
			if(xscale->titre[0] || yscale->titre[0]) yscale->marge += (1 * WndLigToPix(1));
		}
		if(cdr->titre[0]) yscale->marge += (1 * WndLigToPix(1));
		yscale->pret = 1;
		if(xscale->pret) {
			if((cols = (int)GraphXcols(graph,yscale)) != xscale->cols) {
				xscale->cols = WndColToPix(cols);
				if((graph->mode & GRF_NBAXES) == GRF_0AXES) xscale->marge = 0;
				else xscale->marge = graph->grad + graph->of7 + xscale->cols;
				xscale->total = cdr->larg - (2 * xscale->marge);
				if(xscale->ech != 0.0) xscale->pixel = (float)xscale->total / xscale->ech;
				else xscale->pixel = 1.0;
			}
		}
//		printf("(GraphScaleDefine) GradY=%g\n",yscale->grad);
	} // else printf("(GraphScaleDefine) Axe Y deja pret\n");
	if(DEBUG_GRAPH(1)) printf("(GraphScaleDefine) Echelle Y prete\n");

	if(!xscale->pret) {
		if(!GraphGradsDefine(graph,GRF_XAXIS,graph->axe[0].zoom)) {
			// sprintf(GraphErrorText,"(GraphScaleDefine) Graduations X non definies pour le graphe '%s'",cdr->nom);
			return(0);
		}
		if((graph->mode & GRF_NBAXES) == GRF_0AXES) {
			xscale->cols = 0;
			xscale->marge = 0;
//			if(DEBUG_GRAPH(1)) printf("(GraphScaleDefine) Axe X absent pour le graphe '%s'\n",cdr->nom);
		} else {
			...
			xscale->pret = 1;
			//			printf("(GraphScaleDefine) GradX=%g\n",xscale->grad);
		} // else printf("(GraphScaleDefine) Axe X deja pret\n");
//		if(DEBUG_GRAPH(1)) printf("(GraphScaleDefine) Echelle X prete\n");
	}

	yscale->total = cdr->haut - (2 * yscale->marge);
	xscale->total = cdr->larg - (2 * xscale->marge);
	if(DEBUG_GRAPH(1)) WndPrint("(GraphScaleDefine) Axes prepares: Lx=%d/%g,Ly=%d/%g\n",xscale->total,xscale->ech,yscale->total,yscale->ech);
	if(xscale->ech != 0.0) xscale->pixel = (float)xscale->total / xscale->ech;
	else xscale->pixel = 1.0;
	if(yscale->ech != 0.0) yscale->pixel = (float)yscale->total / yscale->ech;
	else yscale->pixel = 1.0;
	return(1);
}
/* .......................................................................... */
...
/* 	Valeurs minimum et/ou maximum dans le tableau */
	...
	if(DEBUG_GRAPH(2)) WndPrint("(%s) Parms de l'axe @%08X (data #%d)=%02x\n",__func__,(hexa)scale,scale->num,data->parm);
#else
	data_ptr = graph->data_ptr;
	for(num=0; num<graph->dim; num++, data_ptr++) {
		data = *data_ptr; if(!data) continue;
		...
		stab = (short *)0;             /* pour gcc */
		rtab = (float *)0;             /* pour gcc */
		imin = imax = 0; rmin = rmax = 0.0; /* pour gcc */
		fc = 0;                             /* pour gcc */
		nb = data->max - data->min;
		if(((data->min != 0) || (data->max != 0)) && (nb <= 0)) nb += data->dim;
		if(!nb) {
...
//			return(0);
		...
		if(DEBUG_GRAPH(1)) WndPrint("(%s) Data #%d: %d valeurs %s @%08X (pmin=%d)\n",__func__,
			num,nb,reel?"reelles":"entieres",reel?(hexa)rtab:(hexa)itab,pmin);
		...
		if(scale->minauto && scale->maxauto) {
			...
			if(reel) {
				...
				if(prem) {
					...
					if(scale->reel) {
						if(rmin < scale->u.r.min) scale->u.r.min = rmin;
						if(rmax > scale->u.r.max) scale->u.r.max = rmax;
					} else {
			...
			if(reel) {
				rmin += data->of7.r.v;
				if(scale->reel) {
					if(prem) scale->u.r.min = rmin;
					else if(rmin < scale->u.r.min) scale->u.r.min = rmin;
				} else {
			...
			if(reel) {
				rmax += data->of7.r.v;
				if(scale->reel) {
					if(prem) scale->u.r.max = rmax;
					else if(rmax > scale->u.r.max) scale->u.r.max = rmax;
				} else {
...
//					(axe == GRF_YAXIS)? 'Y': 'X',scale->num,cdr->nom);
//			return(0);
			if((hexa)(scale->u.i.min) != 0x80000000) scale->u.i.min -= 1;
			if((hexa)(scale->u.i.max) != 0x7FFFFFFF) scale->u.i.max += 1;
			return(1);
		}
	}
	return(1);
...
/* Valeur utilisateur automatique d'une graduation */
	if(scale->gradauto) {
		...
		} else scale->grad = OpiumGradsLineaires(echelle);
	}
...
//	modf(scale->u.r.min/scale->grad,&nbgrad);
	...
	nbgrad = (double)igrad;
	if(scale->u.r.min >= 0.0) {
		scale->u.r.axe = scale->u.r.min; nbgrad += 1.0;
		scale->typegrad = GRF_GRADINF;
	} else if(scale->u.r.max >= 0.0) {
		scale->u.r.axe = 0.0;
...
static int GraphGradsFloatLog(GraphScale *scale) {
	int logmin,logmax; double echelle;
/* Variables auxiliaires pour manipuler les graduations */
	if((scale->u.r.min <= 0.0) || (scale->u.r.max <= 0.0)) {
//		sprintf(GraphErrorText,"(GraphGradsFloatLog) Limite negative pour un axe logarithmique");
		if(scale->u.r.min < 0.0) { scale->u.r.min = 0.001; scale->minauto = 0; }
		if(scale->u.r.max < 0.0) { scale->u.r.max = scale->u.r.min * 10.0; scale->maxauto = 0; }
//		return(0);
	}
	scale->logmin = log10((double)scale->u.r.min);
	logmin = scale->logmin; if((double)logmin > scale->logmin) logmin--;
	echelle = log10((double)scale->u.r.max);
	logmax = echelle; if((double)logmax < echelle) logmax++;
	echelle = echelle - scale->logmin;
	scale->ech = (float)echelle;

/* Valeur utilisateur automatique d'une graduation */
	if(scale->gradauto) scale->grad = ((logmax - logmin) <= 5)? 1.0: -1.0;

/* position de l'autre axe (si 2 axes), type de graduation, valeur de la premiere graduation */
	scale->u.r.axe = scale->u.r.min;
	scale->typegrad = GRF_GRADINF;
	scale->g1 = pow(10.0,(double)logmin);
	return(1);
}
/* .......................................................................... */
...
/* Valeur utilisateur automatique d'une graduation */
	if(scale->gradauto) scale->grad = ((logmax - logmin) <= 5)? 1: -1;

/* position de l'autre axe (si 2 axes), type de graduation, valeur de la premiere graduation */
	scale->u.i.axe = scale->u.i.min;
	scale->typegrad = GRF_GRADINF;
	scale->g1 = (int)pow(10.0,(double)logmin);
	return(1);
}
/* .......................................................................... */
static int GraphGradsDefine(Graph graph, unsigned char axe, unsigned int zoom)  {
...
static void GraphGradMin(GraphScale *scale, int decalage) {
	...
	if(DEBUG_GRAPH(3)) {
		...
		WndPrint("min: %d, total: %d, echelle: %g\n",scale->u.i.min,scale->total,scale->ech);
...
static void GraphAxisXDraw(Graph graph, int y, WndContextPtr gc, unsigned char typegrad ) {
	...
	float (*fctn)(Graph, int, float);
	int ygrad,lgrad,sgrad,xtext,ytext,nbgrad; int i;
	int xc,xp,xd,xs,ys,xlim,lngr,total;
	float rx;
	char s[80];
/*--	int i2; float r2; int delta; */
	...
	total = cdr->dh - (2 * xscale->marge);
	WndLine(f,gc,xscale->marge,y,total,0);
...
*/
	...
	do {
		...
		if(xc >= xscale->marge) {
			if(!xscale->logd && (f->qualite == WND_Q_PAPIER)) {
				if(xp >= 0) {
					xd = xc - xp;
					for(i = 1; i < 5; i++) {
						xs = xp + ((i * xd) / 5);
			...
			if(xc > xlim) break;
			WndLine(f,gc,xc,ygrad,0,lgrad);
			if(!xscale->logd || (nbgrad == 9)) {
				if(xscale->reel) {
					if(fabs((double)rx) < (xscale->grad / 100.0)) rx = 0.0;
					if(fctn) sprintf(s,xscale->format,fctn(graph,0,rx));
					else sprintf(s,xscale->format,rx);
				} else {
					if(fctn) sprintf(s,xscale->format,fctn(graph,0,rx));
					else sprintf(s,"%d",inintf(rx));
				}
				if(DEBUG_GRAPH(1)) WndPrint("Axe X: graduation=%s\n",s);
				lngr = WndColToPix((int)strlen(s));
				if((xc - (lngr/2)) > (xtext + WndColToPix(1))) {
					WndString(f,gc,xc - (lngr/2),ytext,s);
					xtext = xc + (lngr / 2) + 1;
		...
		if(xscale->logd) {
			if(xscale->grad < 0.0) rx *= 10.0;
				...
				rx += xscale->grad;
				if(--nbgrad == 0) { xscale->grad *= 10.0; nbgrad = 9; }
			}
		} else rx += xscale->grad;
		xc = GraphPosFloat(1,rx,xscale) - cdr->dx;
		if(++timeout > 100) break;
...
*/
	...
	} else if (x == xscale->marge+xscale->total) {
...
/* static */ void GraphAxisYDraw(Graph graph, int x, WndContextPtr gc, unsigned char typegrad) {
	...
	float (*fctn)(Graph, int, float);
	int xgrad,lgrad,sgrad,xtext,ytext,lngr,total; int i;
...
/*--	int i2; float r2; int delta; */
	...
	total = cdr->dv - (2 * yscale->marge);
	WndLine(f,gc,x,yscale->marge,0,total);
	if((graph->mode & GRF_NOGRAD) || (typegrad == GRF_GRADNONE)) return;
...
*/
	...
	do {
		...
		if(yc <= (cdr->haut - yscale->marge)) {
			if(!yscale->logd && (f->qualite == WND_Q_PAPIER)) {
				if(yp >= 0) {
					yd = yc - yp;
					for(i = 1; i < 5; i++) {
						ys = yp + ((i * yd) / 5);
			...
			if(yc < ylim) break;
			WndLine(f,gc,xgrad,yc,lgrad,0);
			dy = (yc > ytext)? yc - ytext: ytext - yc;
			if((!yscale->logd || (nbgrad == 9)) && (dy > WndLigToPix(1))) {
					// float (*fctnTmp)(Graph, int, float) =  (float (*)(Graph, int, float))fctn;
				if(yscale->reel) {
					if(fabs((double)ry) < (yscale->grad / 100.0)) ry = 0.0;
...
/*--		yc -= delta; */
		if(yscale->logd) {
			if(yscale->grad < 0.0) ry *= 10.0;
				...
				ry += yscale->grad;
				if(--nbgrad == 0) { yscale->grad *= 10.0; nbgrad = 9; }
			}
		} else ry += yscale->grad;
		yc = GraphPosFloat(1,ry,yscale);
		yc = cdr->haut - yc - cdr->dy;
...
*/
	...
	} else if (y == yscale->marge+yscale->total) {
...
static inline void GraphMarkDraw(WndFrame f, WndContextPtr gc, int type, int nb) {
	...
	for(i=1; i<nb; i++) {
		...
		if(DEBUG_GRAPH(2)) WndPrint("(GraphMarkDraw) Trace de (%d, %d) +(%d, %d)\n",x,y,dx,dy);
...
/* Converti la valeur d'un point float quelconque en un nombre de pixels
   mode = 0: taille ecran / 1: position sur l'ecran */
	if(mode == 0) {
		if(scale->logd) {
			if(r > 0.0) return((int)((float)log10((double)r) * scale->pixel)); else return(-1);
		} else return((int)(r * scale->pixel));
	} else {
		if(scale->logd) {
			if(r > 0.0) return((int)((float)(log10((double)r) - scale->logmin) * scale->pixel) + scale->marge);
			else return(-1 /*scale->marge*/);
		} else {
			if(scale->reel)
				return((int)((r - scale->u.r.min) * scale->pixel) + scale->marge);
			else return((int)((r - (float)scale->u.i.min) * scale->pixel) + scale->marge);
		}
	}
}
/* .......................................................................... */
...
/* Converti la valeur d'un point short quelconque en un nombre de pixels
   mode = 0: taille ecran / 1: position sur l'ecran */
	...
	if(mode == 0) {
		if(scale->logd) {
			if(i > 0) {
				prod = (int64)(log10((double)i) * (double)scale->total);
		...
		if(scale->logd) {
			if(i > 0) {
				prod = (int64)((log10((double)i) - scale->logmin) * (double)scale->total);
			} else prod = -(0x7FFFFFFF);
		} else {
	...
	if((data->parm & GRF_ADRS) == GRF_FCTN) {
		...
		if((data->parm & GRF_ADRS) == GRF_INDEX) {
			if(stab == 0) {
				...
				if(k >= data->min) s = *stab + (k * *(stab+1));
				else s = *stab + ((k + data->dim) * *(stab+1));
			}
		} else if(data->fctn == 0) return(0);
		else s = stab[k];
		*i = (int)s;
	...
	switch(num) {
	  ...
	  case GRF_ZONE_XGRAD:
		if(g->axe[0].logd) switch(GraphZoneLog) {
		  case 0: g->axe[0].grad = -1.0; break;
		  case 1: g->axe[0].grad =  0.0; break;
		  case 2: g->axe[0].grad =  1.0; break;
		}
		g->axe[0].gradauto = 0;
		g->axe[0].pret = 0;
		break;
	  case GRF_ZONE_YGRAD:
		if(g->axe[1].logd) switch(GraphZoneLog) {
		  case 0: g->axe[1].grad = -1.0; break;
		  case 1: g->axe[1].grad =  0.0; break;
		  case 2: g->axe[1].grad =  1.0; break;
		}
		g->axe[1].gradauto = 0;
		g->axe[1].pret = 0;
		break;
...
void GraphZoneAcces(Graph g, int num) {
	...
	switch(num) {
	  ...
	  case GRF_ZONE_XGRAD:
	  	if(g->axe[0].logd) {
	  		GraphZoneLog = (g->axe[0].grad > 0.0)? 2: ((g->axe[0].grad < 0.0)? 0: 1);
	  		PanelKeyB(p,0,"simple/neant/dixiemes",&GraphZoneLog,10);
		} else PanelFloat(p,0,&(g->axe[0].grad));
		break;
	  case GRF_ZONE_YGRAD:
		if(g->axe[1].logd) {
	  		GraphZoneLog = (g->axe[1].grad > 0.0)? 2: ((g->axe[1].grad < 0.0)? 0: 1);
	  		PanelKeyB(p,0,"simple/neant/dixiemes",&GraphZoneLog,10);
...
static char  GraphPanelforme;
static unsigned int GraphPaneltype,GraphPanelepais;
...
static void GraphDialogPlancheZ(GraphData data) {
	...
	itab = (int *)0;            /* pour gcc */
	fc = 0;                     /* pour gcc */
	rmin = rmax = 0.0; imin = imax = 0;   /* pour gcc */
	nb = data->max - data->min;
	court = (data->parm & GRF_TYPE) == GRF_SHORT;
	reel = (data->parm & GRF_TYPE) == GRF_FLOAT;
	fctn = (data->parm & GRF_ADRS) == GRF_FCTN;
...
//	}
	...
	rc = OpiumReadList("Donnee a retracer",nom,&GraphPaneltableau,OPIUM_MAXNOM);
	for(index=0; index<graph->dim; index++) {
		data = graph->data_ptr[index];
	...
	if(((data->parm & GRF_AXIS) == GRF_ZAXIS) || ((data->parm & GRF_AXIS) == GRF_IMAGE)) GraphDialogPlancheZ(data);
		...
		PanelItemSelect(GraphPtableau,4,1);
		switch(data->couleur.type) {
		  case GRF_NOM: strcpy(GraphPanelcouleur,&(data->couleur.def.nom[qual][0])); break;
		  case GRF_RGB:
			sprintf(GraphPanelcouleur,"0x%04X%04X%04X",
				data->couleur.def.rgb[qual].r,data->couleur.def.rgb[qual].g,data->couleur.def.rgb[qual].b);
			break;
		  case GRF_LUT:
			PanelItemSelect(GraphPtableau,4,0);
		...
		//- printf("(%s) @%08X.%08X.line_width = %d\n",__func__,(hexa)data,(hexa)(data->gc),(data->gc)->line_width);
	}
...
/* ========================================================================== */
static int GraphDialogContraste() {
...
/* ========================================================================== */
static int GraphDialogZoomPlus() { OpiumZoomAutorise = 1; return(1); }
/* ========================================================================== */
static int GraphDialogZoomMoins() {
	...
	for(i=0,scale = GraphDialogGraph->axe; i<2; i++,scale++) {
		if(scale->reel) {
			rmilieu = (scale->u.r.max + scale->u.r.min) / 2.0;
			ramplitude = scale->u.r.max - scale->u.r.min;
			scale->u.r.min = rmilieu - ramplitude;
			scale->u.r.max = rmilieu + ramplitude;
			if(scale->logd && (scale->u.r.min <= 0.0)) scale->u.r.min = 0.001;
		} else {
			imilieu = (scale->u.i.max + scale->u.i.min) / 2.0;
...
static void GraphDialogTablesVal(GraphData data, int k, char demi, char *val) {
	...
	if(k < nb) {
		...
		if((data->parm & GRF_ADRS) == GRF_INDEX) {
			if(reel) {
				rtab = (float *)data->fctn;
				rval = *rtab + ((data->min + k) * *(rtab+1)); if(demi) rval /= 2.0;
				// fprintf(OpiumTxtfile," %g",rval);
				sprintf(val,"%g",rval);
			} else if(court) {
				stab = (short *)data->fctn;
				sval = *stab + ((data->min + k) * *(stab+1)); if(demi) sval /= 2;
				sprintf(val,"%d",sval);
			} else {
				itab = (int *)data->fctn;
			...
			if(reel) {
				...
				(*fctnTmp)(i,&rval); if(demi) rval /= 2.0;
				sprintf(val,"%g",rval);
			} else if(court) {
		...
		while(y < ymax) {
			...
			for(num=0; num<graph->dim; num++, data_ptr++) {
				...
				if((data->parm & GRF_ADRS) == GRF_INDIV) {
					...
					indiv_point = indiv + x;
					if(indiv_point->trace == GRF_NODISPLAY) { ligne[0] = '\0'; break; }
					else continue;
				}
				if(trois_d) {
...
/* ========================================================================== */
static int GraphDialogEtatSauve() {
...
/* ========================================================================== */
int GraphDialogExec(Cadre cdr /*, WndUserRequest *u */) {
	int x,dx;
