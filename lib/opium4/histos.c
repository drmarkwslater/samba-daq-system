...
Histo HistoCreateFloat(float min, float max, int nb) {
	...
	for(qual=0; qual<WND_NBQUAL; qual++)
		h->axe[qual].r = h->axe[qual].g = h->axe[qual].b = WndLevelText[qual];
	h->last = h->first = 0;
//	printf("(HistoCreateFloat) Create Histo @%08X\n",(hexa)h);
...
void HistoErase(Histo histo) {
	...
	if(!histo) return;
//	printf("(HistoErase) Erase Histo @%08X (first data @%08X)\n",(hexa)histo,(hexa)(histo->first));
	data = histo->first;
	while(data) {
//		printf("(HistoErase) data to free @%08X\n",(hexa)data);
		next = data->next;
//		printf("(HistoErase) next data @%08X\n",(hexa)next);
		free(data);
		data = next;
	}
...
/* ===================================================================== */
void HistoDelete(Histo histo) { if(histo) free(histo); /* printf("(HistoDelete) freed Histo @%08X\n",(hexa)histo); */ }
/* ===================================================================== */
void HistoDataDelete(HistoData data) { if(data) free(data); }
/* ===================================================================== */
...
/* ===================================================================== */
HistoData HistoAdd(Histo histo, int type) {
	int taille,qual;
	HistoData data,adrs;

	if(!histo) return(0);
//	printf("(HistoAdd) Add to Histo @%08X (last data @%08X)\n",(hexa)histo,(hexa)(histo->last));
	switch(type) {
...
//	printf("(HistoAdd) data added @%08X\n",(hexa)adrs);
	data = histo->last;
	if(data) {
		data->next = adrs;
//		printf("(HistoAdd) %08X->next = @%08X\n",(hexa)data,(hexa)adrs);
		data = data->next;
	} else histo->first = data = adrs;
//	printf("(HistoAdd) %08X->first = @%08X\n",(hexa)histo,(hexa)(histo->first));
	data->type = type;
	data->titre[0]= '\0';
	...
	data->next = 0;
//	printf("(HistoAdd) %08X->next = @%08X\n",(hexa)data,(hexa)(data->next));
	histo->last = data;
//	printf("(HistoAdd) => %08X->last = @%08X\n",(hexa)histo,(hexa)(histo->last));
	return(data);
}
/* ===================================================================== */
...
/* <val> est du type <histo->x.type>, <data> et <poids> du type <data->type> */
	...
	bin = (val - histo->x.v.r[0]) / histo->x.v.r[1];
	if(bin < 0) data->under.i += poids;
	else if(bin >= histo->x.nb) data->over.i += poids;
	else { adrs = (int *)data->adrs; adrs[bin] += poids; }
}
...
int HistoRebin(Histo histo, int nb) {
	...
	switch(histo->x.type) {
	  case HST_INT:
		...
		break;
	  case HST_SHORT:
		smax = histo->x.v.s[0] + (histo->x.nb * histo->x.v.s[1]);
...
void HistoDump(Histo histo) {
	...
	while(data) {
		switch(data->type) {
		  case HST_INT:
			WndPrint("          donnees INT  @%08X",(hexa)data->adrs);
			for(qual=0; qual<WND_NBQUAL; qual++)
			...
			break;
		  case HST_SHORT:
			WndPrint("          donnees SHORT @%08X",(hexa)data->adrs);
			for(qual=0; qual<WND_NBQUAL; qual++)
				WndPrint(", couleur %s=rgb:%04X.%04X.%04X",WndSupportNom[qual],data->coul[qual].r,data->coul[qual].g,data->coul[qual].b);
			WndPrint("\n");
			s = (short *)data->adrs;
			...
			break;
		  case HST_FLOAT:
			WndPrint("          donnees FLOAT @%08X",(hexa)data->adrs);
			for(qual=0; qual<WND_NBQUAL; qual++)
				WndPrint(", couleur %s=rgb:%04X.%04X.%04X",WndSupportNom[qual],data->coul[qual].r,data->coul[qual].g,data->coul[qual].b);
			WndPrint("\n");
			r = (float *)data->adrs;
			...
			break;
		  default:
			WndPrint("          donnees non definies @%08X\n",(hexa)data->adrs);
		}
		data = data->next;
	}
	WndPrint("(HistoDump) Fin histo @%08X:\n",(hexa)histo);
}
/* ===================================================================== */
...
/* <*val> sont du type <histo->*.type>, <poids> du type <data->type> */
	...
	if(data->type != HST_INT) return;
	xbin = (xval - histo->x.v.i[0]) / histo->x.v.i[1];
	ybin = (yval - histo->y.v.r[0]) / histo->y.v.r[1];
	zbin = (ybin * histo->x.nb) + xbin;
	if((xbin < 0) || (ybin < 0) ||  (zbin < 0)) data->under.i += poids;
	else if((xbin >= histo->x.nb) || (ybin >= histo->y.nb) || (zbin >= (histo->x.nb * histo->y.nb)))
		data->over.i += poids;
...
/* <*val> sont du type <histo->*.type>, <poids> du type <data->type> */
	...
	data = &(histo->data);
	if(data->type != HST_INT) return;
	xbin = (xval - histo->x.v.r[0]) / histo->x.v.r[1];
	ybin = (yval - histo->y.v.i[0]) / histo->y.v.i[1];
	zbin = (ybin * histo->x.nb) + xbin;
	if((xbin < 0) || (ybin < 0) ||  (zbin < 0)) data->under.i += poids;
	else if((xbin >= histo->x.nb) || (ybin >= histo->y.nb) || (zbin >= (histo->x.nb * histo->y.nb)))
...
void H2DDump(H2D histo) {
	...
	WndPrint("(H2DDump) Fin histo @%08X:\n",(hexa)histo);
}
/* ===================================================================== */
void H2DGraphAdd(H2D histo, Graph graph) {
	...
	GraphDataTranslate(graph,i,GRF_NODSP);
	nb = (histo->x).nb * (histo->y).nb;
	data = &(histo->data);
	switch(data->type) {
	  case HST_INT:
