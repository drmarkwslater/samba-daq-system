...
int HelpRead(),HelpChange(),HelpWrite();

MenuItem HelpItemPpal[] = {
...
int HelpMenu(Cadre cdr, int cle) {
	...
	if(DEBUG_OPIUM(1)) WndPrint("titre(%08X)=[%s]\n",(hexa)cdr,cdr->nom);
	HelpCdrCalled = cdr;
	switch(cdr->type) {
	...
	HelpInitItm = 1;
	HelpItemTemp = 0;
	if(HelpModify) OpiumExec(HelpMenuPpal->cdr); else HelpRead();
	return(0);
}
/* ======================================================================= */
static void HelpSetName(char * nom)  {
...
/* ....................................................................... */
static void HelpCreateMenuTempo() {
	int nbopt,i;
	...
	if(HelpInitItm) {
		switch(HelpCdrCalled->type) {
		  case OPIUM_MENU:
			...
			HelpItemTemp = (MenuItem *)malloc((nbopt + 1) * sizeof(MenuItem));
...
/* ....................................................................... */
static int HelpFindItem(FILE *f, char *texte){
	size_t l;
		...
		*soulignes = (int)strlen(texte) - 4 - *blancs;
		...
		switch(menucalled[i].type) {
		  ...
		  case MNU_CODE_RETOUR:
			sprintf(complement,"rend la valeur %d au menu appelant",i+1);
			strcat(texte,complement);
			break;
		  case MNU_CODE_CONSTANTE:
//			v = (int64)adrs;
			v = (int)adrs;
			sprintf(complement,"rend la valeur %d au menu appelant",v);
			strcat(texte,complement);
			break;
		  case MNU_CODE_BOOLEEN:
			sprintf(complement,"inverse un booleen");
			strcat(texte,complement);
			break;
		  ...
		  case MNU_CODE_MENU:
			...
			if((nextmenu->cdr)->titre[0])
				sprintf(complement,"appelle le menu \"%s\"",(nextmenu->cdr)->titre);
			else sprintf(complement,"appelle un menu (qui devrait etre nomme \"%s\")",menucalled[i].texte);
			strcat(texte,complement);
			break;
		  case MNU_CODE_PANEL:
			nextpanel = *(Panel *)adrs;
			if((nextpanel->cdr)->titre[0])
				sprintf(complement,"demande le panel \"%s\"",(nextpanel->cdr)->titre);
			else sprintf(complement,"demande un panel (qui devrait etre nomme \"%s\")",menucalled[i].texte);
			strcat(texte,complement);
			break;
		}
		break;
		...
		*soulignes = (int)strlen(texte) - 4 - *blancs;
		switch(panelcalled[i].type) {
		  case PNL_SEPARE:
			break;
		  case PNL_TEXTE:
			sprintf(complement,"introduit une chaine (d'au plus %d octets)",panelcalled[i].maxuser);
			strcat(texte,complement);
			break;
		  case PNL_PSWD:
			sprintf(complement,"introduit un mot de passe (d'au plus %d octets)",panelcalled[i].maxuser);
			strcat(texte,complement);
			break;
		  case PNL_INT:
		  ...
		  case PNL_LISTE: case PNL_LISTL: case PNL_LISTS: case PNL_LISTB:
			sprintf(complement,"introduit un mot-cle (d'au plus %d octets)",panelcalled[i].maxuser);
			strcat(texte,complement);
			break;
		  case PNL_KEY: case PNL_KEYL: case PNL_KEYS: case PNL_KEYB:
			sprintf(complement,"introduit l'un mot-cle de la liste %s",(char *)panelcalled[i].fctn);
			strcat(texte,complement);
			break;
		  case PNL_USER:
...
static int HelpScan(int change) {
	...
	genre = OpiumCdrType[(int)HelpCdrCalled->type];
...
/* 	TermPrint(HelpTerm,"			\x1b[4mOption\x1b[0m \"\x1b[1m%s\x1b[0m\": ",HelpItemTemp[i].texte); */
	...
	switch(HelpCdrCalled->type) {
	  ...
	  default:
		OpiumError("Aide sur les items de %s pas implementee",
			OpiumCdrType[(int)HelpCdrCalled->type]);
	}
...
/* ======================================================================= */
int HelpChange() { HelpLevel = 0; return(HelpScan(1)); }
/* ======================================================================= */
int HelpWrite() {
