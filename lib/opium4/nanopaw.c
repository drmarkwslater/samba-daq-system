...
#define FWHM_POURCENT (100.0 * FWHM)
...
char PlotNtupleAjoute(int i, int max, float init) {
    ...
    if(!espace->sel) {
        ...
        if(espace->sel) for(j=0; j<dim; j++) {
            espace->sel[j].trace = CodeMarquage[MARQUE_NORMAL];
...
int PlotNtupleEcrit(char *nom, char sel_var, char sel_evt, char ajoute) {
	...
	do {
		...
		for(i=0,c='\0'; i<PlotVarNb; i++) if(!sel_var || PlotVar[i].sel) {
			...
			espace = PlotVar[i].espace; dim = *(espace->dim);
			if(num < dim) {
				if(sel_evt) { if(PlotNtupleExclu(espace,num)) continue; }
...
void PlotNtupleGetSize(int *h, int *v) { OpiumGetSize((espace->pNtuple)->cdr,h,v); }
   =====================================================================
void PlotNtupleRefresh() { OpiumClear((espace->pNtuple)->cdr); OpiumDisplay((espace->pNtuple)->cdr); }
   ===================================================================== */
...
/* ===================================================================== */
static int PlotNtuplePointeUneFois() {
...
int PlotNtupleRect() {
	...
	for(num=0; num<dim; num++) {
		if(sel[num].trace != CodeMarquage[MARQUE_ABSENT]) {
			if((*xadrs >= rxpnt0) && (*xadrs <= rxpnt1) && (*yadrs >= rypnt0) && (*yadrs <= rypnt1)) {
				sel[num].trace = CodeMarquage[MARQUE_NORMAL]; nb++;
			} else sel[num].trace = CodeMarquage[MARQUE_ABSENT];
		}
		xadrs++; yadrs++;
	}
	return(nb);
...
void PlotNtupleMarque(PlotEspace espace, int num, unsigned char marque) {
	espace->sel[num].trace = CodeMarquage[marque];
}
/* ===================================================================== */
char PlotNtupleInclu(PlotEspace espace, int num) {
	return(espace->sel[num].trace == CodeMarquage[MARQUE_NORMAL]);
}
/* ===================================================================== */
...
char PlotNtuplePresent(PlotEspace espace, int num) {
	return(espace->sel[num].trace != CodeMarquage[MARQUE_ABSENT]);
}
/* ===================================================================== */
...
int PlotChoix() {
	...
	p = PlotCourant;
	if(PlotsNb > 1) {
		if(OpiumReadList("Quel plot",PlotName,&p,16) == PNL_CANCEL)
			return(-1);
		if((p < 0) || (p >= PlotsNb)) {
...
int PlotNouveau(int type) {
	...
	if(p >= MAXPLOTS) {
		OpiumError("Deja %d traces! Il faut purger...",p);
		return(-1);
	}
	Plot[p].type = type;
	Plot[p].h = 0;
...
/* ===================================================================== */
static int PlotBuild(unsigned char mode) {
	int p;

	if((p = PlotNouveau(PLOT_FONCTION)) < 0) return(0);
	PlotMode = mode;
	return(PlotTrace(p,-1));
}
/* ===================================================================== */
int PlotBuildFonction() { return(PlotBuild(PLOT_FONCTION)); }
/* ===================================================================== */
int PlotBuildHisto() { return(PlotBuild(PLOT_HISTO)); }
/* ===================================================================== */
int PlotBuild2D() { return(PlotBuild(PLOT_H2D)); }
/* ===================================================================== */
int PlotBuildBiplot() { return(PlotBuild(PLOT_BIPLOT)); }
/* ===================================================================== */
int PlotRedraw() { return(PlotTrace(PlotChoix(),0)); }
/* ===================================================================== */
int PlotRefresh() {
	int p;
...
void PlotRefreshSiUniqueBiplot() {
	...
	if(n == 1) {
		...
		if(Plot[p].type == PLOT_H2D) {
			...
			GraphDataIntGamma(Plot[p].g,2,0,zmax,0,LUT_DIM-1);
		}
		OpiumDisplay(Plot[p].g->cdr);
	}
}
...
void PlotRempliHisto(int p, int mode, int numvar, float mini, float maxi, int bins) {
	...
	for(num=0; num<dim; num++, adrs++) {
		if(PlotNtupleInclu(espace,num))
			HistoFillFloatToFloat(Plot[p].hd,*adrs,1.0);
	}
#ifdef LABEL_AXES
	...
	snprintf(nom,OPIUM_MAXTITRE,"%d: %s %s",p+1,PlotTypeTexte[Plot[p].type],Plot[p].titre);
...
/* ===================================================================== */
char PlotConstruitFonction(char nouveau, int p) {
...
/* ===================================================================== */
char PlotConstruitHisto(char nouveau, int p) {
	...
	for(num=0; num<dim; num++, adrs++) {
		if(PlotNtupleInclu(espace,num)) HistoFillFloatToFloat(Plot[p].hd,*adrs,1.0);
	}

	return(1);
}
/* ===================================================================== */
char PlotConstruit2D(char nouveau, int p) {
	PlotEspace espace; int dim;
	...
	if(nouveau) {
		...
		Plot[p].hd = 0;
		H2DLUT((H2D)Plot[p].h,PlotLut,LUT_DIM);
		Plot[p].g = GraphCreate(400,400,3);
		H2DGraphAdd((H2D)Plot[p].h,Plot[p].g);
		snprintf(nom_donnee,80,"histo 2D:%d",p+1);
		PlotAxeNom(Plot[p].ab6,nom_ab6,80);
		GraphDataName(Plot[p].g,0,nom_ab6);
		PlotAxeNom(Plot[p].ord,nom_ord,80);
		GraphDataName(Plot[p].g,1,nom_ord);
		GraphDataName(Plot[p].g,2,nom_donnee);
	#ifdef LABEL_AXES
		GraphAxisTitle(Plot[p].g,GRF_XAXIS,nom_ab6);
		GraphAxisTitle(Plot[p].g,GRF_YAXIS,nom_ord);
	#endif
...
/* ===================================================================== */
char PlotConstruitBiplot(char nouveau, int p) {
	...
	GraphDataName(Plot[p].g,0,nom_ab6);
	PlotAxeNom(Plot[p].ord,nom_ord,80);
	GraphDataName(Plot[p].g,y,nom_ord);
#ifdef LABEL_AXES
	GraphAxisTitle(Plot[p].g,GRF_XAXIS,nom_ab6);
	GraphAxisTitle(Plot[p].g,GRF_YAXIS,nom_ord);
#endif
...
/* ===================================================================== */
char PlotConstruit(char nouveau, int p) {
	char ok; char nom[OPIUM_MAXTITRE];
...
int PlotTrace(int p, char nouveau) {
	...
	if(nouveau) {
		mode = PlotMode;
		if(nouveau > 0) {
			if(OpiumReadListB("Type de trace",PlotTypeTraduit,(char *)&mode,10) == PNL_CANCEL) return(0);
		}
		Plot[p].type = mode;
	} else mode = Plot[p].type;

/*
 *	Graphiques de type 'FONCTION'
 */
	if(mode == PLOT_FONCTION) {
		numvar = NumVar;
 ...
 */
	...
	if(PlotConstruit(nouveau,p)) {
		if(p == PlotsNb) {
			PlotsNb++;
			printf("* %d%s fenetre definie: %s\n",PlotsNb,(PlotsNb>1)?"eme":"ere",Plot[p].titre);
		}
	}
...
static void PlotPointDesigne(int p, int *x, int *y) {
	...
	*x = (rxpnt - Plot[p].xmin + (dx / 2.0)) / dx;
	dy = (Plot[p].ymax - Plot[p].ymin) / (float)Plot[p].biny;
	*y = (rypnt - Plot[p].ymin + (dy / 2.0)) / dy;
...
int PlotExtraitProfilX() {
	...
	sprintf(Plot[p2].titre,"%d: %s @%s=%g",p2,PlotVar[Plot[p2].ab6].nom,PlotVar[Plot[p].ord].nom,vy);
	...
	snprintf(nom_donnee,80,"profilX:%d",p);
	...
	Plot[p2].vide = 0;
	OpiumTitle(Plot[p2].g->cdr,"\0");
	OpiumName(Plot[p2].g->cdr,Plot[p2].titre);
	PlotCourant = p2;
	if(p2 == PlotsNb) {
		PlotsNb++;
		printf("* %d%s fenetre definie: %s\n",PlotsNb,(PlotsNb>1)?"eme":"ere",Plot[p2].titre);
	}
...
int PlotExtraitProfilY() {
	...
	sprintf(Plot[p2].titre,"%d: %s @%s=%g",p2,PlotVar[Plot[p2].ab6].nom,PlotVar[Plot[p].ab6].nom,vx);
	Plot[p2].h = HistoCreateFloat(Plot[p2].xmin,Plot[p2].xmax,Plot[p2].binx);
	Plot[p2].hd = HistoAdd(Plot[p2].h,HST_FLOAT);
	for(qual=0; qual<WND_NBQUAL; qual++)
		HistoDataSupportRGB(Plot[p2].hd,qual,PLOT_COLOR_FIG(qual));
	Plot[p2].g = GraphCreate(400,300,4);
	HistoGraphAdd(Plot[p2].h,Plot[p2].g);
	snprintf(nom_donnee,80,"profilY:%d",p);
	GraphDataName(Plot[p2].g,0,PlotVar[Plot[p2].ab6].nom);
	GraphDataName(Plot[p2].g,1,nom_donnee);
	GraphAxisTitle(Plot[p2].g,GRF_XAXIS,PlotVar[Plot[p2].ab6].nom);
	Plot[p2].vide = 0;
...
/* ===================================================================== */
static int PlotAfficheFit(Graph graph, WndUserRequest *e) {
	char texte[2][32];
...
int PlotLineaire() {
	...
	if(Plot[p].type == PLOT_HISTO) {
		...
		if(PlotFitMethode == PLOT_FIT_VALEURS) {
			...
			if(!GraphGetPoint(Plot[p].g,&ix,&FitMini,&iy,&rypnt)) {
				FitMaxi = FitMini+ (3.0 * (float)hbin);
				OpiumWarn("Echec du pointage du minimum. On garde [%g, %g]",FitMini,FitMaxi);
			}
			FitMaxi = FitMini+ (3.0 * hbin); OpiumPosition(pFitLimites->cdr,px,posy); OpiumDisplay(pFitLimites->cdr);
		...
		imin = (FitMini - hmin) / hbin;
		dim = ((FitMaxi - hmin) / hbin) - imin + 1;
		if(dim < 2) {
		...
		for(i=0; i<dim; i++)
			valide[i] = PlotNtupleInclu(espace,i)? 1: 0;
	} else {
		OpiumFail("Le fit lineaire ne gere pas (encore) ce type de trace");
		return(0);
	}

	rc = FitLineaire(&b,&errb,&a,&erra,&khi2,xadrs,yadrs,eadrs,valide,dim);
	free(valide); if(Plot[p].type == PLOT_HISTO) { free(xadrs); free(eadrs); }
	if(rc != 1) {
		OpiumWarn("Fit impossible (code %d)",rc);
		return(1);
	}
	...
	*/
	...
	if(Plot[p].avec_fit != FIT_LINEAIRE) {
		...
		OpiumTitle(Plot[p].g->cdr,"\0");
		OpiumName(Plot[p].g->cdr,Plot[p].titre);
#ifdef LABEL_AXES
		if(Plot[p].type == PLOT_HISTO) {
...
int PlotFitNonlin(int p, char mode) {
	...
	if(mode == FIT_PARABOLIQUE) {
		rc = FitSecondDegre(xadrs,yadrs,erreur,valide,dim,&a,&b,&c);
		fond = 0.0;  /* apres tout... */
	} else /* if(mode == FIT_GAUSSIEN) */ {
/*		fond = (yadrs[0] + yadrs[dim - 1]) / 2.0; */
		fond = (yadrs[0] < yadrs[dim - 1])? yadrs[0]: yadrs[dim - 1]; /* en attendant mieux */
		fond = 0.0;  /* apres tout... */
		rc = FitGaussienne(xadrs,yadrs,valide,dim,fond,&amplitude,&sigma,&x0);
	}
	...
	if(mode == FIT_PARABOLIQUE) {
		...
		if(a != 0.0) {
			ResultatFit.par.x0 = -b / (2.0 * a);
			ResultatFit.par.base = c - ((b * b) / (4.0 * a));
		...
		if(ResultatFit.gau.x0 != 0.0) {
			PlotResolSigma = 100.0 * ResultatFit.gau.sigma / ResultatFit.gau.x0;
		...
		GraphAjoute(Plot[p].g,PlotAfficheFit);
		OpiumPosition(pFitGauss->cdr,px,posy);
		if(OpiumDisplayed(pFitDegre2->cdr)) OpiumClear(pFitDegre2->cdr);
		if(OpiumDisplayed(pFitParab->cdr)) OpiumClear(pFitParab->cdr);
	...
	if(mode == FIT_PARABOLIQUE) for(i=0; i<dim; i++,xadrs++,yadrs++)
		*yadrs = (a * *xadrs * *xadrs) + (b * *xadrs) + c;
	else /* if(mode == FIT_GAUSSIEN) */ for(i=0; i<dim; i++,xadrs++,yadrs++)
		*yadrs = fond + (amplitude * expf(-(*xadrs - x0) * (*xadrs - x0)/(2.0 * sigma * sigma)));

	if(OpiumDisplayed(Plot[p].g->cdr)) OpiumClear(Plot[p].g->cdr);
	if(Plot[p].avec_fit != mode) {
		GraphErase(Plot[p].g);
		if(Plot[p].type == PLOT_HISTO) HistoGraphAdd(Plot[p].h,Plot[p].g);
		else {
			GraphAdd(Plot[p].g,GRF_FLOAT|GRF_XAXIS,*PlotVar[Plot[p].ab6].adrs,dim);
...
#endif
		Plot[p].avec_fit = mode;
	}
	if(mode == FIT_GAUSSIEN) {
		...
		OpiumPosition(pPlotStats->cdr,px,posy + WndLigToPix(9));
...
/* ===================================================================== */
int PlotParabolique() { return(PlotFitNonlin(-1,FIT_PARABOLIQUE)); }
/* ===================================================================== */
int PlotGaussien() { return(PlotFitNonlin(-1,FIT_GAUSSIEN)); }
/* ===================================================================== */
int PlotFitGaussImprime() {
	Cadre *cdr,liste[8];
...
int PlotLimite() {
	...
	if(mode == PLOT_H2D) {
		if(Plot[p].z.automatique) {
			...
			GraphDataIntGamma(Plot[p].g,2,zmin,zmax,0,LUT_DIM-1);
		}
	}
...
void PlotBuildUI() {
	...
	PlotLut = GraphLUTCreate(LUT_DIM,GRF_LUT_ALL);
	...
	mPlotTypes = MenuTitled("Nouveau traces",0);
	if((ok = MenuItemArray(mPlotTypes,PLOT_NBTYPES))) {
	...
	// printf("(PlotInit) Creation des panels\n");
	...
	i = PanelFloat(pFitGauss,"Resolution Sigma",&PlotResolSigma);
	PanelFormat(pFitGauss,i,"%.1f");
	PanelItemRScale(pFitGauss,i,0.0,10.0); PanelItemColors(pFitGauss,i,PlotVertJauneOrangeRouge,5);
	i = PanelFloat(pFitGauss,"Resolution FWHM",&PlotResolFWHM);
	PanelFormat(pFitGauss,i,"%.1f");
	...
	pCourbe = PanelCreate(2);
	pHistoNeuf = PanelCreate(4);
	pHistoCree = PanelCreate(4);
	pH2DNeuf = PanelCreate(10);
	pH2DCree = PanelCreate(8);
	pAxes = PanelCreate(2);
...
/* ===================================================================== */
int PlotExit() { PlotVarsDeclare(0); if(PlotMarqueTraduit) free(PlotMarqueTraduit); return(1); }
