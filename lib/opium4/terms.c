...
Term TermCreate(int ligs, int cols, int taille) {
	...
	if(term) {
		if(DEBUG_TERM(1)) WndPrint("Term cree @%08X\n",(hexa)term);
		...
		TermEmpty(term);
		OpiumSetOptions(cdr);
		if(DEBUG_TERM(3)) WndPrint("Utilise cdr @%08X\n",(hexa)term->cdr);
	}
	return(term);
}
/* ========================================================================== */
...
void TermRelease(Term term) {
	...
	if(term->printed) {
		cdr = term->cdr;
		if(cdr->f) OpiumRefresh(cdr);
	}
}
/* ========================================================================== */
void TermBlank(Term term) {
	WndFrame f; if((f = (term->cdr)->f)) WndBlank(f); 
}
/* ========================================================================== */
	...
	while(*ecrit) {
		...
		if((car == term->debut) && (term->nbcars > 0)) {
			...
			if(suivant == term->ligne[suivante]) {
				...
				term->nblignes -= 1;
				term->col1 = 0;
				if(DEBUG_TERM(2)) WndPrint("(TermAdd) premiere=%d @%08X\n",term->premiere,
				...
				if(term->ligne[term->premiere] == car) {
					...
					term->col1 += 1;
					if(DEBUG_TERM(2)) WndPrint("(TermAdd) colonne1=%d @%08X\n",term->col1,
						term->ligne[term->premiere]);
				}
...
void TermDump(Term term, FILE *f) {
	...
	fprintf(f,"Buffer[%d] rempli avec %04X caracteres, debut @%08X\n",
	term->taille,term->nbcars,term->debut);
	fprintf(f,"%d/%d lignes enregistrees\n",term->nblignes,term->maxlignes);
	fprintf(f,"Premiere ligne=%d, demarre en colonne %d, pointe @%08X\n",
	term->premiere,term->col1,term->ligne[term->premiere]);
	fprintf(f,"Derniere ligne=%d, pointe @%08X\n",
...
int OpiumSizeTerm(Cadre cdr, char from_wm) {
	...
	if(DEBUG_OPIUM(1) && (cdr->type != OPIUM_TERM)) {
		WndPrint("+++ OpiumSizeTerm sur le %s @%08X\n",cdr->nom,(hexa)cdr->adrs);
		return(0);
	}
	term = (Term)cdr->adrs;
	if(from_wm) {
...
/*	OpiumDebug(OPIUM_DEBUG_TERM,1); */
	if(DEBUG_OPIUM(1) && (cdr->type != OPIUM_TERM)) {
		WndPrint("+++ OpiumDisplayTerm sur le %s @%08X\n",cdr->nom,(hexa)cdr->adrs);
		return(0);
	}
	term = (Term)cdr->adrs;
	...
	if(DEBUG_TERM(2)) {
		WndPrint("Buffer[%d] rempli avec %04X caracteres, debut @%08X\n",
			term->taille,term->nbcars,term->debut);
		WndPrint("%d/%d lignes enregistrees\n",term->nblignes,term->maxlignes);
		WndPrint("Premiere ligne=%d, demarre en colonne %d, pointe @%08X\n",
			term->premiere,term->col1,term->ligne[term->premiere]);
		WndPrint("Derniere ligne=%d, pointe @%08X\n",
			term->derniere,term->ligne[term->derniere]);
...
//+	WndBlank(f); /* de toutes facons, on commence a lig = 0 */
	WndPaint(f,0,0,cdr->dh,cdr->dv,WndColorBgnd[WndQual],3);
	...
	while((nbcars < term->nbcars) && (lig < cdr->lignb)) {
		...
		*ecrit = '\0';
		...
		if(col1 >= cdr->colmin)
			WndWrite(f,WND_TEXT,lig,col1-cdr->colmin,ligne);
		else WndWrite(f,WND_TEXT,lig,0,ligne+cdr->colmin-col1);
...
int OpiumRunTerm(Cadre cdr, WndUserRequest *e) {
	...
	if(DEBUG_OPIUM(1) && (cdr->type != OPIUM_TERM)) {
		WndPrint("+++ OpiumRunTerm sur le %s @%08X\n",cdr->nom,(hexa)cdr->adrs);
		return(0);
	}
	if(!OpiumCoordFenetre(cdr,&f)) return(0);
	term = (Term)cdr->adrs;
	...
	if(e->type == WND_KEY) {
		...
		if(l <= 0) {
			...
			WndWrite(f,WND_TEXT,term->premiere,term->col1,e->texte);
		}
		if(fin_de_ligne) {
