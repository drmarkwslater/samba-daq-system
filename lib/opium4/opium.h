...
//	TypeOpiumFctnArg enter,exit;
	...
	int (*a_vide)(struct StructCadre *, WndUserRequest *, void *);
...
#define OpiumCadre(boite) (boite? boite->cdr: 0)
#define OpiumDisplayed(cdr) (cdr? cdr->displayed: 0)
#define OpiumAlEcran(boite) (boite? ((boite->cdr)? (boite->cdr)->displayed: 0): 0)
...
void OpiumPromptBrut(char brut);
...
int InstrumGradOscillo(float v, char sup, int *d);
void InstrumOnChange(Instrum i, TypeFctn fctn, void *arg);
void *InstrumChangeArg(Instrum i);
void OpiumInstrumColorNb(Cadre cdr, Cadre fait, int *nc);
...
int OpiumRunInstrum(Cadre cdr, WndUserRequest *e);
...
int  OpiumRunInfo(Cadre cdr, WndUserRequest *e);
...
typedef struct StructFigure {
	...
	int (*fctn)(struct StructFigure *fig, WndUserRequest *e);
} TypeFigure,*Figure;

typedef int (*TypeFigFctn)(Figure fig, WndUserRequest *e);
...
#define GRF_TRAN  0x03      /* 0: aucune / 1: log10 / 2: gamma                 */
...
#define GRF_NODISPLAY GRF_CODE_ERR
...
/* Si data->trace indique un mode 'points' (#3=GRF_PNT),
 ...
 ATTENTION: la valeur 7 est reservee! */
typedef enum {
	GRF_POINT = 0,
	GRF_CROSS,
	GRF_STAR,
...
#define GRF_CODE_CROSS (GRF_CROSS << GRF_SHIFT_LINE)
...
#define GRF_CODE_TOUT  (GRF_TOUT  << GRF_SHIFT_LINE)
...
//	unsigned int haut,larg;
	...
	int (*ajouts)(struct StructGraph *graph, WndUserRequest *e);
	int (*onclic)(struct StructGraph *graph, WndUserRequest *e);
} TypeGraph,*Graph;

typedef int (*TypeGraphFctn)(Graph graph, WndUserRequest *e);
...
void GraphAxisYDraw(Graph graph, int x, WndContextPtr gc, unsigned char typegrad);
...
typedef struct {
	...
	char hold,printed;
	int taille,debut,nbcars; /* taille du tampon des caracteres, debut logique, 
	                            nombre de caracteres dispo */
} TypeTerm,*Term;
...
int OpiumRunTerm(Cadre cdr, WndUserRequest *e);
...
typedef enum {
	...
	PNL_LISTE, PNL_LISTL, PNL_LISTS, PNL_LISTB,
...
#define MNU_RETOUR     0,0,1,-1,1,MNU_CODE_RETOUR,    0
#define MNU_CONSTANTE  0,0,1,-1,1,MNU_CODE_CONSTANTE, (void *)
#define MNU_FONCTION   0,0,1,-1,1,MNU_CODE_FONCTION,  (void *)
#define MNU_MENU       0,0,1,-1,1,MNU_CODE_MENU,      (void *)
#define MNU_PANEL      0,0,1,-1,1,MNU_CODE_PANEL,     (void *)
#define MNU_EXEC       0,0,1,-1,1,MNU_CODE_EXEC,      (void *)
#define MNU_FORK       0,0,1,-1,1,MNU_CODE_FORK,      (void *)
#define MNU_DISPLAY    0,0,1,-1,1,MNU_CODE_DISPLAY,   (void *)
#define MNU_COMMANDE   0,0,1,-1,1,MNU_CODE_COMMANDE,  (void *)
#define MNU_BOOLEEN    0,0,1,-1,1,MNU_CODE_BOOLEEN,   (void *)
#define MNU_SEPARATION 0,0,0,-1,0,MNU_CODE_SEPARE,    0
...
typedef struct {
	...
	int lig;         /* nombre de lignes dans une colonne     */
	int col;         /* nombre de caracteres dans une colonne */
	int nbcols,loc[MNU_MAXCOLS],larg[MNU_MAXCOLS];  /* nb, emplacement, larg. des colonnes */
	int defaut;      /* item par defaut                       */
	int item_pointe; /* item pointe par la souris (WND_PRESS) */
	int curs;        /* curseur (choix par clavier)           */
	short hilite,reverse;
	char taille_calculee;
} TypeMenu,*Menu;
...
#ifdef MENU_BARRE
int MenuBarreManage(int menu_num, unsigned int item_num); // , char *texte
...
OPIUM_VAR int (*OpiumEntryPre)();                 /* pre-traitement entree    */
...
OPIUM_VAR char OpiumPromptFmtBrut;
...
OPIUM_VAR WndUserRequest *OpiumEditDernierEvt;
...
OPIUM_VAR char OpiumEventType[WND_NBEVENTS][16];
OPIUM_VAR char OpiumMseButton[WND_NBBUTTONS][12];
OPIUM_VAR char *OpiumCdrType[OPIUM_NBTYPES+1]
#ifdef OPIUM_C
= {
	"menu", "panel", "term", "graph", "info", "prompt", "board", "instrum", "figure", "onglet", "\0"
}
#endif
;
...
= {
	...
	"float", "double", "adu",
	"liste", "listL", "listS", "listB", "key", "keyL", "keyS", "keyB", 
	"user", "bouton",
	"\0"
}
#endif
