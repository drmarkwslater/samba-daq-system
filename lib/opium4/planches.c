...
/* ========================================================================== */
Cadre BoardCreate() { return(OpiumCreate(OPIUM_BOARD,0)); }
/* ========================================================================== */
void BoardOnClic(Cadre board, int (*reponse)(Cadre, WndUserRequest *, void *), void *arg) {
	if(!board) return;
...
static void BoardPlaceComposant(Cadre boitier, Cadre precedent) {
	...
	if(DEBUG_BOARD(1)) WndPrint("(%s) Ajout prevu %-12s@%08X (%d, %d) + [%d x %d]\n",
		__func__,boitier->nom,(hexa)boitier,boitier->defaultx,boitier->defaulty,boitier->larg,boitier->haut);
...
Cadre BoardAdd(Cadre board, Cadre boitier, int x, int y, Cadre ref) {
	...
	if(DEBUG_BOARD(2)) printf("(%s) Ajoute cadre @%08X a la planche @%08X\n",__func__,(hexa)boitier,(hexa)board);
	if(!board || !boitier) return(0);
	if(board->type != OPIUM_BOARD) return(0);
	if(DEBUG_BOARD(1)) WndPrint("(%s/1) %s '%s' @%08X ajoute a la %s @%08X\n",__func__,
			OpiumCdrType[(int)boitier->type],boitier->nom,(hexa)boitier,OpiumCdrType[(int)board->type],(hexa)board);
	...
	if(DEBUG_BOARD(1)) WndPrint("(%s/2) Premier boitier @%08X\n",__func__,(hexa)suivant);
	while(suivant) { dernier = suivant; suivant = dernier->suivant; if(DEBUG_BOARD(1)) WndPrint("(%s/3) Boitier suivant @%08X\n",__func__,(hexa)suivant); }
	if(dernier) dernier->suivant = boitier; else board->adrs = (void *)boitier;
	/* boitier->suivant = 0 au moment de sa creation */
	if(DEBUG_BOARD(1)) WndPrint("(%s/4) %s '%s' @%08X [%08X] ajoute a @%08X, suivant: %08X\n",__func__,
			 OpiumCdrType[(int)boitier->type],boitier->nom,(hexa)boitier,(hexa)(boitier->adrs),dernier?(hexa)dernier:(hexa)board,(hexa)boitier->suivant);
	...
	if(boitier->type == OPIUM_PANEL) {
		...
		else WndPrint("(%s) !! Adresse du panel du cadre @%08X (inclus dans la planche '%s' @%08X) perdue\n",__func__,(hexa)boitier,board->nom,(hexa)board);
	}
	// if(BoardZoneNb) printf("(%s)  %s %s %dx%d ajoute en (%d, %d)\n",__func__,OpiumCdrType[(int)boitier->type],boitier->nom,boitier->larg,boitier->haut,x,y);
...
Cadre BoardAddMenu(Cadre board, Menu menu, int x, int y, Cadre ref) {
	return(BoardAdd(board,menu->cdr,x,y,ref));
}
/* ========================================================================== */
...
void BoardAreaClose(Cadre board, TypeFigZone *zone, int *x, int *y) {
	...
	zone->larg = BoardZone[n]->xmax - BoardZone[n]->xmin + (2 * WndColToPix(1));
	zone->haut = BoardZone[n]->ymax - BoardZone[n]->ymin + (2 * WndLigToPix(1));
	zone->support = BoardZone[n]->support;
...
void BoardTrash(Cadre board) {
	...
	if(!board) return;
	if(board->type != OPIUM_BOARD) return;
	// printf("(%s) --- poubellisation de la planche '%s' @%08X\n",__func__,board->nom,(hexa)board);
	suivant = dernier = (Cadre)board->adrs;
	while(suivant) {
		dernier = suivant;
		suivant = dernier->suivant;
		if(DEBUG_BOARD(1) && dernier->f) {
			printf("(%s) Suppression du %s %s @%08X ->cdr@%08X ->f@%08X dans la planche %s\n",__func__,OpiumCdrType[dernier->type],
			   dernier->nom,(hexa)(dernier->adrs),(hexa)dernier,(hexa)(dernier->f),board->nom);
			printf("(%s) Element actif: %s %s ->cdr@%08X ->f@%08X\n",__func__,OpiumCdrType[(int)(board->actif)->type],(board->actif)->nom,(hexa)(board->actif),(hexa)((board->actif)->f));
		}
		dernier->f = 0; /* evite que OpiumDelete(cdr) appelle OpiumClear(cdr) */
		switch(dernier->type) {
...
Onglet OngletDefini(char **liste, void (*construit)(int)) {
	...
	if(onglet) {
		...
		onglet->numero = 0;
		onglet->nblig = 1; onglet->nbcol = OngletNb(onglet);
		onglet->selec = 0;
		OpiumSetOptions(cdr);
	}
	return(onglet);
...
void OngletNbLigs(Onglet onglet, short nblig) {
	...
	onglet->nblig = nblig;
	onglet->nbcol = ((OngletNb(onglet) - 1) / onglet->nblig) + 1;
}
/* ========================================================================== */
...
/* ========================================================================== */
void OpiumOngletColorNb(Cadre cdr, Cadre fait, int *nc) {
	Onglet onglet,modele; int ic;
...
/* ========================================================================== */
int OpiumRunOnglet(Cadre cdr, WndUserRequest *e) {
...
/* ========================================================================== */
static int BoardRepertoireFichierOuvre(Figure fig, WndUserRequest *e) {
...
/* ========================================================================== */
static int BoardRepertoireFichierNomme(Figure fig, WndUserRequest *e) {
	// printf("  | Choix du fichier '%s'\n",(char *)(fig->adrs));
...
/* ========================================================================== */
static int BoardRepertoireChange(Figure fig, WndUserRequest *e) {
	char *repert;
...
/* ========================================================================== */
void OpiumBoardColorNb(Cadre cdr, Cadre fait, int *nc) {
	Cadre board,boitier,modele,vu[OPIUM_NBTYPES]; int ic; int i;
...
void OpiumBoardColorSet(Cadre cdr, WndFrame f) {
	...
	while(boitier) {
		if(!(vu[boitier->type])) {
			switch(boitier->type) {
				case OPIUM_MENU: OpiumMenuColorSet(boitier,f); break;
	...
	WndFenColorSet(f,board->fond,WND_GC_BOARD);
}
/* ========================================================================== */
...
/* Remarque: la taille d'une planche depend de la POSITION de ses composants,
	...
	if(DEBUG_OPIUM(1) && (cdr->type != OPIUM_BOARD)) {
		WndPrint("+++ OpiumSizeBoard sur le %s @%08X\n",cdr->nom,(hexa)cdr->adrs);
		return(0);
	}
	...
	while(boitier) {
		...
		if(boitier->type == OPIUM_ONGLET) { cdr_onglets = boitier; OpiumSizeChanged(cdr_onglets); }
			...
			if(DEBUG_BOARD(1)) WndPrint("(%s) Ajout final %-12s@%08X (%d, %d) + [%d x %d] -> dim = %d x %d\n",
				__func__,boitier->nom,(hexa)boitier,boitier->x,boitier->y,boitier->larg,boitier->haut,larg,haut);
		}
		precedent = boitier;
		boitier = boitier->suivant;
	}
	...
	// if(premier) printf("(%s) @%08X: %s.type=%d -> %08X\n",__func__,(hexa)premier->items,premier->items[0].texte,premier->items[0].type,(hexa)(premier->items[0].adrs));
	return(1);
}
/* ========================================================================== */
int OpiumRefreshBoard(Cadre cdr) {
	...
	if(DEBUG_OPIUM(1)) printf("(%s) ----- Affichage de la planche %s %d x %d @(%d, %d) -----\n",__func__,cdr->nom,cdr->dh,cdr->dv,cdr->x,cdr->y);
	//- if(!OpiumCoordFenetre(cdr,&f)) return(0);
	f = cdr->f;
	board = cdr;
	if(board->type != OPIUM_BOARD) return(0);
	...
	doit_terminer = WndRefreshBegin(f);
//+	WndFillBgnd(f,WND_GC(f,board->fond),0,0,cdr->dh,cdr->dv); // BRD_FOND == WND_GC(f,board->fond)
	WndFillBgnd(f,WND_GC(f,board->fond),0,0,cdr->dh+WND_ASCINT_WIDZ,cdr->dv+WND_ASCINT_WIDZ); // BRD_FOND == WND_GC(f,board->fond)
	boitier = (Cadre)(board->adrs);
		if(DEBUG_BOARD(1)) {
			WndPrint("(%s/1) %s @%08X, contient %s @%08X\n",__func__,
				OpiumCdrType[(int)board->type],(hexa)board,OpiumCdrType[(int)boitier->type],(hexa)boitier);
		}
	if(!(board->actif)) board->actif = boitier;
	while(boitier) {
		if(DEBUG_BOARD(1)) {
			WndPrint("(%s/2) %s @%08X (suivant: %08X)\n",__func__,
				OpiumCdrType[(int)boitier->type],(hexa)boitier,(hexa)boitier->suivant);
		}
		if(DEBUG_OPIUM(1)) printf("(%s) Insertion de %s (%d + %d ; %d + %d) ",__func__,boitier->nom,boitier->x,boitier->dh,boitier->y,boitier->dv);
#define EXCLUSION_STRICTE
// #define EXCLUSION_LARGE
#ifdef EXCLUSION_STRICTE
		if((boitier->x < cdr->dx) || (boitier->y < cdr->dy)
//		|| ((boitier->x + boitier->dh) < cdr->dx) || ((boitier->y + boitier->dv) < cdr->dy)
//		|| (boitier->x > (cdr->dx + cdr->dh)) || (boitier->y > (cdr->dy + cdr->dv))
		|| ((boitier->x + boitier->dh) > (cdr->dx + cdr->dh)) || ((boitier->y + boitier->dv) > (cdr->dy + cdr->dv))
			) {
			if(DEBUG_OPIUM(1)) printf("abandonnee!\n");
...
#endif
		if(DEBUG_OPIUM(1)) printf("entreprise\n");
		OpiumRefreshIf(OPIUM_TOUJOURS,boitier);
		/* if(boitier->titre[0]) {
		...
		if((support = boitier->support) != WND_RIEN) {
			if(boitier->type == OPIUM_MENU) {
				...
				if((menu->items[0].type == MNU_CODE_BOOLEEN) && (menu->items[0].adrs)) {
					support = (*(char *)(menu->items[0].adrs))? WND_RAINURES: WND_PLAQUETTE;
					if(*(char *)(menu->items[0].adrs)) MenuItemAllume(menu,1,0,GRF_RGB_YELLOW); else MenuItemEteint(menu,1,0);
				}
			}
			// WndRepere(f,-cdr->dx,-cdr->dy,cdr->dh,cdr->dv);
			// WndEntoure(f,support,boitier->x,boitier->y,boitier->dh,boitier->dv);
		...
		if((boitier->type == OPIUM_MENU) && boitier->titre[0]) {
			...
			// WndRepere(f,-cdr->dx,-cdr->dy,cdr->dh,cdr->dv);
			// WndString(f,f->text[WND_GC_CLEAR],boitier->x + x,boitier->y + boitier->dv + WndLigToPix(1) + 1,boitier->titre);
			WndRepere(f,0,0,cdr->dh,cdr->dv);
	...
	if(DEBUG_OPIUM(2)) WndPrint("(%s) Planche %s %d x %d, affichage %d x %d\n",__func__,cdr->nom,cdr->larg,cdr->haut,cdr->dh,cdr->dv);
	if(doit_terminer) WndRefreshEnd(f);

	return(1);
}
/* ========================================================================== */
int OpiumRunBoard(Cadre cdr, WndUserRequest *e) {
	int lig,larg;
	int code_rendu;
	char return_done;
	int m;
	...
	if(DEBUG_OPIUM(1) && (cdr->type != OPIUM_BOARD)) {
		WndPrint("+++ OpiumRunPrompt sur le %s @%08X\n",cdr->nom,(hexa)cdr->adrs); return(0);
	}
	lig = WndPixToLig(e->y);
	if(DEBUG_BOARD(1)) WndPrint("Board @%08X, ligne %d, bouton %d\n",(hexa)cdr,lig,e->x/(cdr->larg/2));
	code_rendu = 0;
	if(e->type == WND_KEY) {
		size_t l;
		l = strlen(e->texte);
...
Cadre BoardComposant(Cadre board, int x, int y) {
	...
	boitier = (Cadre)(board->adrs);
	while(boitier) {
		if(DEBUG_BOARD(1)) WndPrint("(BoardComposant) Evt (%d,%d): Boitier examine, %08X/%12s (%d, %d) + [%d x %d]\n",
			x,y,(hexa)boitier,boitier->nom,boitier->x,boitier->y,boitier->dh,boitier->dv);
