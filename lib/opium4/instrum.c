...
/* ========================================================================== */
int InstrumCircRayon(TypeInstrumCirc *cercle) { return(cercle->rayon); }
/* ========================================================================== */
...
//	printf("(%s) %08X/%2d, relief gc@%08X <- couleur #%d\n",__func__,(hexa)f,instrum->relief,(hexa)f->gc[WND_Q_ECRAN].coul[instrum->relief],WND_GC_GREY);
//	printf("(%s) %08X/%2d, cadran gc@%08X <- couleur #%d\n",__func__,(hexa)f,instrum->cadran,(hexa)f->gc[WND_Q_ECRAN].coul[instrum->cadran],WND_GC_MODS);
//	printf("(%s) %08X/%2d, bouton gc@%08X <- couleur #%d\n",__func__,(hexa)f,instrum->bouton,(hexa)f->gc[WND_Q_ECRAN].coul[instrum->bouton],WND_GC_BUTN);
//	printf("(%s) %08X/%2d, interr gc@%08X <- couleur #%d\n",__func__,(hexa)f,instrum->interr,(hexa)f->gc[WND_Q_ECRAN].coul[instrum->interr],WND_GC_HILITE);
}
/* ========================================================================== */
int OpiumSizeInstrum(Cadre cdr, char from_wm) {
	...
	if(DEBUG_OPIUM(1) && (cdr->type != OPIUM_INSTRUM)) {
		WndPrint("+++ OpiumSizeInstrum sur le %s @%08X\n",cdr->nom,(hexa)cdr->adrs);
		return(0);
	}
	  ...
	  case INS_CADRAN: case INS_POTAR:
		...
		haut = (int)((y * (cercle->rayon + (1.0 * cercle->graduation))) + (by * cercle->rayon));
			if(instrum->forme == INS_POTAR) {
				if((inf < -(3.0 * PI / 4.0)) || (sup > (0.75 * PI))) haut += cercle->graduation;
			} else haut += cercle->graduation;
...
int OpiumRefreshInstrum(Cadre cdr) {
	...
	TypeInstrumCirc *cercle; TypeInstrumRect *rect; TypeInstrumLevier *levier;
	TypeInstrumBascule *bascule;
	int i,j,nb,v,d; char doit_terminer;
	double logmin,logmax;
	double dev,angle,sina,cosa;
	double grad,g1,nbgrad; char legende[1024];
	char **liste; char *modeles;
	...
	if(instrum->cdr != cdr_demande) {
		OpiumError("(OpiumRefreshInstrum) cadre demande: %08X, gere: %08X\n",cdr_demande,instrum->cdr);
		return(0);
	}
		...
		if(instrum->log) {
			if(instrum->lim.s.min > 0) {
				if(instrum->log > 0) logmin = log10((double)instrum->lim.s.min); 
					...
					v = InstrumGradOscillo((float)instrum->lim.s.min,0,&d);
			...
			if(instrum->lim.s.max > 0) {
				if(instrum->log > 0) logmax = log10((double)instrum->lim.s.max); 
					...
					v = InstrumGradOscillo((float)instrum->lim.s.max,1,&d);
		...
		if(instrum->log) {
			if(instrum->lim.i.min > 0) {
				if(instrum->log > 0) logmin = log10((double)instrum->lim.i.min); 
					...
					v = InstrumGradOscillo((float)instrum->lim.i.min,0,&d);
					logmin = log10((double)(v * d));
				}
			} else logmin = 0.0;
			if(instrum->lim.i.max > 0) {
				if(instrum->log > 0) logmax = log10((double)instrum->lim.i.max); 
					...
					v = InstrumGradOscillo((float)instrum->lim.i.max,1,&d);
					logmax = log10((double)(v * d));
				}
			} else logmax = logmin + 1.0;
		} else {
		...
		if(instrum->log) {
			if(instrum->lim.f.min > 0.0) {
				if(instrum->log > 0) logmin = log10((double)instrum->lim.f.min); 
					...
					v = InstrumGradOscillo(instrum->lim.f.min,0,&d);
					logmin = log10((double)(v * d));
				}
			} else logmin = 0.0;
			if(instrum->lim.f.max > 0.0) {
				if(instrum->log > 0) logmax = log10((double)instrum->lim.f.max); 
					...
					v = InstrumGradOscillo(instrum->lim.f.max,1,&d);
					logmax = log10((double)(v * d));
				}
			} else logmax = logmin + 1.0;
		} else {
		...
		if(instrum->log) {
			if(instrum->lim.d.min > 0) {
				if(instrum->log > 0.0) logmin = log10((double)instrum->lim.d.min); 
					...
					v = InstrumGradOscillo((float)instrum->lim.d.min,0,&d);
					logmin = log10((double)(v * d));
				}
			} else logmin = 0.0;
			if(instrum->lim.d.max > 0) {
				if(instrum->log > 0.0) logmax = log10((double)instrum->lim.d.max); 
					...
					v = InstrumGradOscillo((float)instrum->lim.d.max,1,&d);
					logmax = log10((double)(v * d));
				}
			} else logmax = logmin + 1.0;
		} else {
	...
	switch(instrum->forme) {
	  ...
	  case INS_POTAR:
		...
		if(cercle->graduation > 0.0) do {
			...
			WndLine(f,WND_GC(f,instrum->gc.fgnd),x,y,l,h);
			InstrumLegende(instrum,g1,nb,&i,&j,legende); c = (int)strlen(legende);
			x = x + l + (0.5 * (sina - 1.0) * (double)WndColToPix(1));
			y = y + h - (0.5 * (cosa - 1.0) * (double)WndLigToPix(1));
		...
		if(rect->graduation > 0.0) do {
			...
			if(rect->position <= 0) {
				if(rect->horiz) {
					...
					WndLine(f,WND_GC(f,instrum->gc.fgnd),x,y,0,rect->graduation);
					x -= (WndColToPix(c) / 2);
				} else {
					...
					WndLine(f,WND_GC(f,instrum->gc.fgnd),x,y,rect->graduation,0);
					x -= WndColToPix(rect->nbchars);
					y += (WndLigToPix(1) / 2);
				}
				x1 = x + WndColToPix(c);
				y1 = y + WndLigToPix(1);
...
/* Transfert dans planches.c (== valable pour tous les cadres inclus) deja tente... */
	...
	if(instrum->cdr != cdr_demande) {
		OpiumError("(OpiumRefreshInstrum) cadre demande: %08X, rafraichi: %08X\n",cdr_demande,instrum->cdr);
		return(0);
	}
...
int InstrumRefreshVar(Instrum instrum) {
	char c; short s; int i; int64 i64; float r; double val; int nb;
	double dev;
	...
	if(instrum->echelle == 0.0) dev = 0.0;
		...
		switch(instrum->type) {
		  ...
		  case PNL_LISTE: case PNL_KEY:
			i = *(int *)(instrum->adrs);
			nb = (int)instrum->echelle;
			if(i < 0) i = 0; else if(i > nb) i = nb;
			val = (double)i;
			break;
		  case PNL_LISTL: case PNL_KEYL:
			i64 = *(int64 *)(instrum->adrs);
			nb = (int)instrum->echelle;
			if(i64 < 0) i64 = 0; else if(i64 > nb) i64 = nb;
			val = (double)i64;
			break;
...
static int InstrumUpdate(Instrum instrum, double dev) {
	...
	TypeInstrumCirc *cercle; TypeInstrumRect *rect; TypeInstrumLevier *levier;
	TypeInstrumBascule *bascule;
	int x,y,l,h,v,da; char doit_terminer;
	...
	switch(instrum->forme) {
	  ...
	  case INS_GLISSIERE:
		...
		if(instrum->deja_trace) {
			...
			if(instrum->forme == INS_COLONNE) {
				WndFillBgnd(f,WND_GC(f,instrum->gc.bgnd),x,y,l,h);
				// printf("2eme dessin: "); PRINT_FGC(f,instrum->gc.bgnd);
			} else {
				if(rect->horiz) {
					l *= 3;
					x -= (l / 2);
					/* WndErase(f,x,y,l,h); */
					// mais pourquoi pas? WndFillFgnd(f,WND_GC(f,instrum->relief),x,y,l,h);
					WndFillFgnd(f,WND_GC(f,instrum->relief),x,y-2,l,h+3);
					y = instrum->geom.rect.decal + (rect->largeur / 2) - 1;
					h = 3;
					...
					y -= (h / 2);
					/* WndErase(f,x,y,l,h); */
					WndFillFgnd(f,WND_GC(f,instrum->relief),x,y-1,l,h+2);
					x = instrum->geom.rect.decal + (rect->largeur / 2) - 1;
					l = 3;
		...
		if(rect->horiz) {
			...
			if(instrum->forme == INS_COLONNE) {
				...
				x = da + (rect->largeur / 2); l = rect->largeur / 3;
				if(rect->longueur < 0)  x -= rect->longueur;
				...
				WndRelief(f,1,WND_BORD_HAUT,x-(rect->largeur/2)+1,y,rect->largeur-3);
			...
			if(instrum->forme == INS_COLONNE) {
				...
				WndRelief(f,2,WND_BORD_DRTE,x+rect->largeur-2,y-(rect->largeur/2)+1,rect->largeur-3);
		  ...
		  default:
			switch(instrum->type) {
			  case PNL_LISTE: case PNL_KEY:  i = *(int *)instrum->adrs; break;
                case PNL_LISTL: case PNL_KEYL: l = *(int64 *)instrum->adrs; i = (int)l; break;
...
//			if((lig < 0) || (lig >= ???)) lig = 0;
			...
			if(l) {
				InstrumCode(instrum,PNL_WRITE,avant/*,&bidon*/);
				if(col) {
					strncpy(valeur,avant,col); valeur[col] = '\0';
					strcat(valeur,e->texte);
					strcat(valeur,avant+col);
				} else strcpy(valeur,e->texte);
				if(InstrumCode(instrum,PNL_VERIFY,valeur/*,&col*/)) {
					InstrumCode(instrum,PNL_READ,valeur/*,&bidon*/);
					cdr->var_modifiees = 1;
					col += l;
				}
			}
...
//	} else if((e->type == WND_RELEASE) || ((e->type == WND_PRESS) && (e->code == WND_MSERIGHT))) {
		...
		if(dev != instrum->dev) {
			...
			switch(instrum->type) {
			  ...
			  case PNL_LISTL: case PNL_KEYL: *(int64 *)(instrum->adrs) = (int64)(val + 0.4); break;
