...
/* ========================================================================== */
// int ImpressionLargeurCourante() { return(ImpressionLargeur); }
/* ========================================================================== */
int ImpressionHauteurCourante() { return(ImpressionMaxLignes); }
/* ========================================================================== */
int ImpressionLigneCourante() { return(ImpressionNumLigne); }
/* ========================================================================== */
void ImpressionSurSupport(IMPRESSION_SUPPORT support, void *adrs) {
	...
	if(ImpressionSupport == IMPRESSION_TERMINAL) {
		if((ImpressionFormat != IMPRESSION_PS) && (ImpressionFormat != IMPRESSION_PDF)) {
			strcpy(ImpressionNomComplet,"*");
		#ifdef OPIUM_H
...
void ImprimeTraitVertPlace(int i, int col) {
	...
	ImprTraitVert[i].col = pos;
	if(i) {
		ImprTraitVert[i-1].largr = ImprTraitVert[i].col - ImprTraitVert[i-1].col;
...
void ImprimeTraitVertEcarte(int i, int larg) {
	int espace;
	if(i < 0) return;
	espace = (larg > 0)? larg: -larg;
	if(espace <= 0) return;
	if(i) {
		ImprTraitVert[i].col = ImprTraitVert[i - 1].col + espace;
		ImprTraitVert[i-1].largr = espace;
...
*/
	...
	//- printf("(%s) Impressions dans %08X\n",__func__,(hexa)PrintPath);
#endif
	ImpressionEnCours = 1;
...
void ImprimeCadreTexte(char *texte) {
	if(ImprCadresFmt == IMPRESSION_HTTP) HttpEcrit(texte);
	else fputs(texte,ImprCadreDest);
}
/* ========================================================================== */
void ImprimeCadreEcrit(char *fmt, ...) {
	...
	va_start(argptr, fmt);
	vsnprintf(texte, 1024, fmt, argptr);
	va_end(argptr);
	ImprimeCadreTexte(texte);
}
/* ========================================================================== */
void ImprimeCadreOuvre(char *style, char *fmt, ...) {
	char texte_brut[1024],texte_html[1024],*legende; int l,max;
...
//	if(ImprCadresFmt == IMPRESSION_TEXTE) ImprCadreDest = stdout; else ImprCadreDest = PrintPath;
	if(fmt) {
		...
		va_end(argptr);
		if(ImprCadresHtml) {
			ImprimeToHtml(texte_brut+l,legende);
			strcat(texte_brut,"</legend>\n");
			// printf("(%s) Ouvre le cadre #%d dans %08X\n",__func__,ImprCadresOuverts+1,(hexa)ImprCadreDest);
...
void ImprimeCadreLigneDebut(int n) {
	...
	// printf("(%s) Demarre %d nouvelle%s ligne%s dans %08X\n",__func__,Accord2s(n),(hexa)ImprCadreDest);
	for(i=0; i<(n-1); i++) {
...
/* ========================================================================== */
void ImprimeCadreLigneFin() { ImprimeCadreTexte("\n"); }
/* ========================================================================== */
void ImprimeCadreFerme() {
	// printf("(%s) Ferme le cadre #%d dans %08X\n",__func__,ImprCadresOuverts,(hexa)ImprCadreDest);
...
static inline void ImprimeWeb(char *fmt, ...) {
	...
	va_start(argptr, fmt);
	nb = vsnprintf(texte, 1024, fmt, argptr);
	va_end(argptr);

	if(ImpressionFormat == IMPRESSION_HTTP) HttpEcrit(texte);
	else fputs(texte,PrintPath);
}
/* ========================================================================== */
static void ImprimeWebTableDebut() {
...
/* ========================================================================== */
static void ImprimeWebTableFin() {
...
/* ========================================================================== */
void ImprimeNouvellePage() { ImprimeFinLigne(); ImprimeSautPage(); }
/* ========================================================================== */
...
void ImprimeColonneFormat(int k, IMPRESSION_ALIGNEMENT aligne, IMPRESSION_TYPELARGEUR mode, int val) {
	...
	if((k >= 0) && (k < i)) {
		ImprTraitVert[k].aligne = aligne;
		switch(mode) {
			case IMPRESSION_INCHANGEE: break;
			case IMPRESSION_LIBRE:     ImprTraitVert[k].largr = 0;    break;
			case IMPRESSION_NBCARS:    ImprTraitVert[k].largr = val;  break;
			case IMPRESSION_POURCENT:  ImprTraitVert[k].largr = -val; break;
		}
	}
}
/* ========================================================================== */
...
/* ========================================================================== */
int ImprimeToHtml(char *dest, char *srce) {
	unsigned char *c,*d; char *t; int i;
	...
	while(*c) {
		if(*c > 0x7E) {
			...
			switch(*c) {
				case 0x80: strcpy(t+i,"&euro;"); i += 7; break;
				case 0x82: strcpy(t+i,"&eacute;"); i += 8; break;
				case 0x85: strcpy(t+i,"&agrave;"); i += 8; break;
				case 0x88: strcpy(t+i,"&agrave;"); i += 8; break;
				case 0x8a: strcpy(t+i,"&egrave;"); i += 8; break;
				case 0x8d: strcpy(t+i,"&ccedil;"); i += 8; break;
				case 0x8e: strcpy(t+i,"&eacute;"); i += 8; break;
				case 0x8f: strcpy(t+i,"&egrave;"); i += 8; break;
				case 0x90: strcpy(t+i,"&ecirc;");  i += 7; break;
				case 0x94: strcpy(t+i,"&icirc;");  i += 7; break;
				case 0x99: strcpy(t+i,"&ocirc;");  i += 7; break;
				case 0xb0: strcpy(t+i,"&ordm;");   i += 6; break;
				case 0xe0: strcpy(t+i,"&agrave;"); i += 8; break;
				case 0xe2: strcpy(t+i,"&acirc;");  i += 7; break;
				case 0xe7: strcpy(t+i,"&ccedil;"); i += 8; break;
				case 0xe8: strcpy(t+i,"&egrave;"); i += 8; break;
				case 0xe9: strcpy(t+i,"&eacute;"); i += 8; break;
				case 0xea: strcpy(t+i,"&ecirc;");  i += 7; break;
				case 0xeb: strcpy(t+i,"&euml;");   i += 6; break;
				case 0xee: strcpy(t+i,"&icirc;");  i += 7; break;
				case 0xef: strcpy(t+i,"&iuml;");   i += 6; break;
				case 0xf4: strcpy(t+i,"&ocirc;");  i += 7; break;
				case 0xf9: strcpy(t+i,"&ugrave;"); i += 8; break;
				case 0xfb: strcpy(t+i,"&ucirc;");  i += 7; break;
				default: strcpy(t+i,"_"); i += 1; break;
			}
			d = c + 1;
		}
		c++;
	}
	strncpy(t+i,(char *)d,c-d); i += (c - d);
	*(t+i) = '\0';
...
static void ImprimeInsere(int col, char *texte) {
	...
	if(ImprWeb.active) {
		...
		#define ACCENTS_HTML_SUPPRIMES
			...
			i += ImprimeToHtml(ImpressionLigne+i,texte);
...
void ImprimeTableauChapeau(char *titre) {
	...
	int trait[MAX_TRAITSVERT+1],largr[MAX_TRAITSVERT+1],aligne[MAX_TRAITSVERT+1];
...
/* ========================================================================== */
void ImprimeTableauPret() { ImprimeColonnesHaut(ImprimeTraitVertNb()); }
/* ========================================================================== */
...
/* ========================================================================== */
void ImpressionTerminePage() {
	if(ImprPs.active) fprintf(PrintPath,"stroke\nshowpage\n");
}
/* ========================================================================== */
void ImpressionFerme() { fclose(PrintPath); }
/* ========================================================================== */
...
void ImpressionFin(char avec_opium) {
	...
	} else if(ImprPs.active && (ImpressionFormat == IMPRESSION_PDF)) {
		strcpy(nom_initial,ImpressionNomComplet);
		if((rc = ImpressionPStoPDF(ImpressionNomComplet)) == 0) {
...
/* ========================================================================== */
char *ImpressionFichierCree() { return(ImpressionNomComplet); }
/* ========================================================================== */
char *ImpressionLog() { return(ImpressionCR); }
