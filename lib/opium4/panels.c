...
Panel PanelCreate(int maxitems) {
	...
	if(panel) {
		if(DEBUG_PANEL(1)) WndPrint("Panel cree @%08X\n",(hexa)panel);
		cdr = OpiumCreate(OPIUM_PANEL,panel);
		if(!cdr) {
			free(panel); return(0); 
		}
		panel->items = (PanelItem *)malloc(maxitems * sizeof(PanelItem));
		if(!panel->items) {
			OpiumDelete(cdr); free(panel); return(0); 
		}
		panel->cdr = cdr;
		panel->max = maxitems;
		...
		for(i=0; i<PNL_NBREPLIES; i++) panel->bouton[i] = OpiumReplyStandard[i];
		panel->trace = panel->mods = panel->hilite = 0;
		PanelErase(panel);
		OpiumSetOptions(cdr);
		if(DEBUG_PANEL(3)) WndPrint("Utilise cdr @%08X\n",(hexa)panel->cdr);
		if(DEBUG_PANEL(2)) printf("(%s) Tableau %d x %d\n",__func__,panel->nbcols,panel->maxligs);
	}
	return(panel);
}
/* ========================================================================== */
...
/* recalcule a la construction via PanelAdd */
	...
	for(n=0, item = panel->items; n<panel->max; n++,item++) {
		memset(item,0,sizeof(PanelItem));
		item->type = -1;
...
int PanelHex(Panel panel, char *texte, int *adrs) {
	return(PanelAdd(panel,texte,PNL_HEX,"%08X",(void *)adrs,8));
}
/* ========================================================================== */
...
/* ========================================================================== */
int PanelListL(Panel panel, char *texte, char **liste, int64 *adrs, int lngr) {
	int nb;
...
int OpiumReadHex(char *texte, int *adrs) {
	PanelErase(OpiumPanelQuick);
	PanelAdd(OpiumPanelQuick,texte,PNL_HEX,"%08X",(void *)adrs,8);
	return(OpiumExec(OpiumPanelQuick->cdr));
}
/* ========================================================================== */
...
/* ========================================================================== */
int OpiumReadListL(char *texte, char **liste, int64 *adrs, int lngr) {
	int nb;
...
void OpiumReadLngr(int lngr) {
	if((lngr > 0) && (lngr <= PanelVarLngrMaxi)) {
		(OpiumPanelQuick->items[0]).maxuser = lngr + 1;
		(OpiumPanelQuick->items[0]).maxdisp = lngr;
	}
}
/* ========================================================================== */
...
void PanelPrepare(Panel panel, int col, int lig) {
	...
	&& (lig > 0) && (lig <= panel->maxligs)) {
		panel->prochaine.col = col;
		panel->prochaine.lig = lig;
...
/*	int lig,col;
	...
	if(DEBUG_PANEL(1)) {
		if(panel->prochaine.col > 0) {
			...
			printf("(%s) Item #%d: %s @%08X dans panel de %d lignes x %d cols (%d total): position %d\n",
				   __func__,numitem,texte?texte:"anonyme",(hexa)adrs,panel->maxligs,panel->nbcols,panel->max,panel->cur);
		}
	}
	...
	if(panel->prochaine.col > 0) {
		if(numitem > panel->cur) panel->cur = numitem;
...
int PanelItemLngr(Panel panel, int num, int lngr) {
	...
	if((num < 1) || (num > panel->max)) return(0);
	if((lngr > 0) && (lngr <= PanelVarLngrMaxi)) {
		(panel->items[num - 1]).maxuser = lngr + 1;
		(panel->items[num - 1]).maxdisp = lngr;
	}
	return(num);
}
/* ========================================================================== */
...
int PanelItemIScale(Panel panel, int num, int min, int total) {
	...
	switch(item->type) {
		...
		case PNL_LISTE: case PNL_LISTL: case PNL_LISTS: case PNL_LISTB:
	...
	if(!adrs) {
	  ...
	  case PNL_TEXTE:
		...
		if(item->nbfonds && item->echelle.i.total) {
			k = (int)ptrecr[0];
			item->numfond = (item->nbfonds * (k - item->echelle.i.min)) / item->echelle.i.total;
	  ...
	  case PNL_ADU:
		...
		if(item->limites) {
			if(*(short *)adrs < item->lim.i.min) *(short *)adrs = item->lim.i.min;
			else if(*(short *)adrs > item->lim.i.max) *(short *)adrs = item->lim.i.max;
		}
		if(item->nbfonds && item->echelle.i.total) {
			k = (int)*(short *)adrs;
			item->numfond = (item->nbfonds * (k - item->echelle.i.min)) / item->echelle.i.total;
	  ...
	  case PNL_LETTRE: 
		...
		if(item->limites) {
			if(*(char *)adrs < item->lim.i.min) *(char *)adrs = item->lim.i.min;
			else if(*(char *)adrs > item->lim.i.max) *(char *)adrs = item->lim.i.max;
		}
		if(item->nbfonds && item->echelle.i.total) {
			k = (int)*(char *)adrs;
			item->numfond = (item->nbfonds * (k - item->echelle.i.min)) / item->echelle.i.total;
	  ...
	  case PNL_BYTE:
		...
		if(item->limites) {
			if(*(unsigned char *)adrs < item->lim.i.min) *(unsigned char *)adrs = item->lim.i.min;
			else if(*(unsigned char *)adrs > item->lim.i.max) *(unsigned char *)adrs = item->lim.i.max;
		}
		if(item->nbfonds && item->echelle.i.total) {
	  ...
	  case PNL_BOOL:
		...
		if(item->nbfonds >= 2) item->numfond = (int)*(char *)adrs;
	  ...
	  case PNL_FLOAT:
		...
		if(item->nbfonds && (item->echelle.r.total != 0.0)) {
			x = *(float *)adrs;
			item->numfond = ((float)item->nbfonds * (x - item->echelle.r.min)) / item->echelle.r.total;
			if(item->numfond < 0) item->numfond = 0;
			else if(item->numfond >= item->nbfonds) item->numfond = item->nbfonds - 1;
		}
		if(ecrit) {
		...
		if(lit) {
			...
			if(ptrlec[k - 1] == '?') {
				...
				*(short *)adrs = entier;
				...
				if(trouve < 0) trouve = bon_debut;
				if(trouve >= 0) *(short *)adrs = trouve; else if(col && *col) *col -= 1;
			}
		}
		if(item->nbfonds) {
			item->numfond = (int)*(short *)adrs;
			if(item->numfond < 0) item->numfond = 0;
			else if(item->numfond >= item->nbfonds) item->numfond = item->nbfonds - 1;
		}
		...
		if(lit) {
			...
			if(ptrlec[k - 1] == '?') {
				...
				*(char *)adrs = entier;
				...
				if(trouve < 0) trouve = bon_debut;
				if(trouve >= 0) *(char *)adrs = trouve; else if(col && *col) *col -= 1;
			}
		}
		if(item->nbfonds) {
			item->numfond = (int)*(char *)adrs;
			if(item->numfond < 0) item->numfond = 0;
			else if(item->numfond >= item->nbfonds) item->numfond = item->nbfonds - 1;
		}
		...
		if(lit) {
			...
			if(ptrlec[k - 1] == '?') {
				...
				*(short *)adrs = entier;
				...
				if(l < 0) *(short *)adrs = entier; else if(col && *col) *col -= 1;
			}
		}
		if(item->nbfonds) {
			item->numfond = (int)*(short *)adrs;
		...
		if(lit) {
			...
			if(ptrlec[k - 1] == '?') {
				...
				*(char *)adrs = entier;
				...
				if(l < 0) *(char *)adrs = entier; else if(col && *col) *col -= 1;
				if(DEBUG_PANEL(5)) printf("(%s) @%08X=%d\n",__func__,(hexa)adrs,*(char *)adrs);
			}
		}
		if(item->nbfonds) {
			item->numfond = (int)*(char *)adrs;
...
static short PanelItemCouleur(Panel panel, PanelItem *item) {
	switch(item->etat) {
		case PNL_PROTEGE: return(WND_GC_TEXT);
		case PNL_MODIFIABLE: return(panel->mods);
		case PNL_MODIFIE: return(panel->hilite);
...
static void PanelPrintVar(WndFrame f, Panel panel, PanelItem *item, char selectr, int limp, int cimp, int max) {
	...
	if(!f) return;
//	printf("(%s) panel P%08X, item I%08X\n",__func__,(hexa)panel,(hexa)item);
//	printf("(%s) item '%s'\n",item->texte);
...
static void PanelCursorSet(Panel panel, WndFrame f, int lig, int col) {
	...
	if(f) WndCursorSet(f,limp,col-(panel->cdr)->colmin);
}
/* ========================================================================== */
void OpiumPanelColorNb(Cadre cdr, Cadre fait, int *nc) {
	Panel panel,modele; int ic;
...
//	printf("(%s) %08X/%2d, trace  gc@%08X <- couleur #%d\n",__func__,(hexa)f,panel->trace,(hexa)f->gc[WND_Q_ECRAN].coul[panel->trace],WND_GC_DARK);
//	printf("(%s) %08X/%2d, mods   gc@%08X <- couleur #%d\n",__func__,(hexa)f,panel->mods,(hexa)f->gc[WND_Q_ECRAN].coul[panel->mods],WND_GC_MODS);
//	printf("(%s) %08X/%2d, hilite gc@%08X <- couleur #%d\n",__func__,(hexa)f,panel->hilite,(hexa)f->gc[WND_Q_ECRAN].coul[panel->hilite],WND_GC_HILITE);
}
/* ========================================================================== */
int OpiumSizePanel(Cadre cdr, char from_wm) {
	...
	if(DEBUG_OPIUM(1) && (cdr->type != OPIUM_PANEL)) {
		WndPrint("+++ OpiumSizePanel sur le %s @%08X\n",cdr->nom,(hexa)cdr->adrs);
		return(0);
	}
	if(DEBUG_OPIUM(3)) WndPrint("OpiumSizePanel (panel @%08X)\n",(hexa)panel);
...
//					tete_colonne,col,panel->nbcols,suppl,larg_txt);
				...
				if(col < (panel->nbcols - 1)) {
					...
					panel->col[col + 1] = panel->col[col] + panel->var[col] + larg_txt;
		...
		if(DEBUG_PANEL(1)) WndPrint("(%s) Panel>%08X/%s [%d x %d] (%d colonnes x %d lignes) -> [%d x %d]\n",__func__,
			(hexa)cdr,cdr->nom,cdr->larg,cdr->haut,WndPixToCol(cdr->larg),WndPixToLig(cdr->haut),cdr->dh,cdr->dv);
		return(1);
	}
	...
	for(tete_colonne=0,col=0; tete_colonne<panel->cur; tete_colonne += panel->nbligs,col++) {
		...
		panel->col[col] = larg_tot + larg_txt;
		panel->var[col] = larg_var;
	...
	if(DEBUG_PANEL(2)) printf("(%s) panel %08X/%-12s: %d lignes de donnees\n",__func__,(hexa)cdr,cdr->nom,panel->nbligs);
	...
	if(DEBUG_PANEL(2)) printf("(%s) panel %08X/%-12s [%d x %d] (%d colonnes x %d lignes)\n",__func__,
		(hexa)cdr,cdr->nom,cdr->larg,cdr->haut,WndPixToCol(cdr->larg),WndPixToLig(cdr->haut));
...
void PanelUpdate(Panel panel, int maj) {
	...
	for(tete_colonne=0,col=0; tete_colonne<panel->cur; tete_colonne += panel->nbligs,col++) {
		...
		for(lig=cdr->ligmin; lig<fin; lig++) {
			...
			// printf("(%s)   . Valeur #%d: @%08X\n",__func__,numitem,(hexa)(item->ptrval));
			zone = panel->col[col] - cdr->colmin;
			limp = PanelLigneSurEcran(panel,lig);
...
int OpiumRefreshPanel(Cadre cdr) {
	...
	if(DEBUG_OPIUM(1) && (cdr->type != OPIUM_PANEL)) {
		WndPrint("+++ OpiumDisplayPanel sur le %s @%08X\n",cdr->nom,(hexa)cdr->adrs);
		return(0);
	}
	if(DEBUG_PANEL(1)) WndPrint("(%s) Cadre @%08X/%s\n",__func__,(hexa)cdr,cdr->nom);
	if(!OpiumCoordFenetre(cdr,&f)) return(0);
	if(DEBUG_PANEL(1)) WndPrint("(%s) Decalage fenetre=(%d, %d)\n",__func__,cdr->colmin,cdr->ligmin);
	panel = (Panel)cdr->adrs;
/*  if(panel->not_to_refresh) return(0); */

	if(DEBUG_PANEL(2)) printf("(%s) Panel de %d item%s sur %d colonne%s x %d ligne%s/%d\n",__func__,
	...
	for(tete_colonne=0,col=0; tete_colonne<panel->cur; tete_colonne += panel->nbligs,col++) {
		...
		if(DEBUG_PANEL(4)) WndPrint("x=%d, d'ou coltext[%d]=%d, col[%d]=%d, var[%d]=%d, soit h=%d\n",
		...
		for(lig=debut; lig<fin; lig++) {
			...
			limp = PanelLigneSurEcran(panel,lig);
			cimp = coltext;
			if(DEBUG_PANEL(1)) { if(!tete_colonne) printf("(OpiumRefreshPanel)    Affichage ligne %d en %d\n",lig,limp); }
			if(item->coultexte) {
			...
			if(item->type == PNL_REMARK) {
				if(l) {
					if(!item->format[0]) {
						m = (h - l) / 2;
						if(DEBUG_PANEL(4)) WndPrint("'%s' place en %d+(%d-%d)/2, soit au final %d\n",
				...
				if(panel->support) {
					...
					if(DEBUG_PANEL(1)) printf("(%s) Ecrit '%s' en (%d, %d)\n",__func__,item->traduit,WndColToPix(limp),WndLigToPix(zone-sep-l));
				} else {
					if(DEBUG_PANEL(4)) WndPrint("'%s' place en %d-%d-%d, soit au final %d\n",
			...
			if(/* !cdr->displayed && */ item->adrs) {
				...
				if(item->type == PNL_FILE) {
					...
					if(l > panel->var[col]) item->first = l - panel->var[col] + 1;
				}
				...
				PanelPrintVar(f,panel,item,0,limp,zone,panel->var[col]);
				if(DEBUG_PANEL(2)) WndPrint("(%s)    Variable ecrite: '%s'\n",__func__,item->ptrval);
				if(DEBUG_PANEL(1)) printf("(%s) Ecrit '%s' en (%d, %d)\n",__func__,item->ptrval + item->first,WndColToPix(limp),WndLigToPix(zone));
			...
			if(DEBUG_PANEL(2)) WndPrint("(%s)    => Item #%d ecrit\n",__func__,numitem);
		}
		if(panel->support) {
...
/* ========================================================================== */
void PanelDeselecte(WndFrame f) {
	//+	printf("(%s) suppression selection depuis 0x%08X\n",__func__,(hexa)f);
	if(OpiumEditSurligne[0]) {
		...
		/* printf("(%s) texte '%s' reaffiche dans 0x%08X(%d,%d) en mode %d\n",__func__,OpiumEditSurligne,
		   (hexa)OpiumEditFen,OpiumEditLig,OpiumEditCol,OpiumEditType); */
		OpiumUserAction();
	}
...
/* ========================================================================== */
int OpiumRunPanel(Cadre cdr, WndUserRequest *e) {
	PanelItem *item; Panel panel;
	...
	if(DEBUG_OPIUM(1) && (cdr->type != OPIUM_PANEL)) {
		WndPrint("+++ OpiumRunPanel sur le %s @%08X\n",cdr->nom,(hexa)cdr->adrs);
		return(0);
	}
	OpiumEditDernierEvt = e;
	if(cdr->modexec == OPIUM_DISPLAY) return(0);
	panel = (Panel)cdr->adrs;
	if(DEBUG_PANEL(1)) WndPrint("(OpiumRunPanel) Acces au panel %s\n",cdr->nom);
	if(!OpiumCoordFenetre(cdr,&f)) {
		WndPrint("(OpiumRunPanel) Panel %s: pas de fenetre associee\n",cdr->nom);
		return(0);
	}
	souris_glissante = 0;
// #ifdef GLISSE
	souris_glissante = ((e->type == WND_PRESS) && (e->w == WndEventPrecedent.w));
...
// #endif
	if(e->type == WND_KEY) {
		lig = f->lig - f->lig0;
		col = f->col - f->col0;
		if(DEBUG_PANEL(1)) WndPrint("(OpiumRunPanel) Touche en (%d, %d), soit lig=%d, col=%d\n",e->x,e->y,lig,col);
	} else {
		lig = WndPixToLig(e->y);
		col = WndPixToCol(e->x);
		if(DEBUG_PANEL(1)) WndPrint("(OpiumRunPanel) Evt en (%d, %d), soit lig=%d, col=%d\n",e->x,e->y,lig,col);
	}
...
//			if(DEBUG_PANEL(1)) WndPrint("(OpiumRunPanel) lig=%d/%d, col=%d+%d\n",lig,nbligs,col,cdr->colmin);
		/* recuperer la bonne valeur associee au bouton! */
			...
			if(DEBUG_PANEL(1)) WndPrint("(OpiumRunPanel) Bouton '%s'\n",OpiumPanelReply[code_rendu]);
		}
	} else {
		...
		if(e->type == WND_KEY) {
			...
			if(l <= 0) {
				...
				if(!traite) {
					...
					if(DEBUG_PANEL(2)) WndPrint("item: %s, entree: '%s' en %s\n",item->ptrval,e->texte,
					...
					if(DEBUG_PANEL(2)) WndPrint("valeur obtenue: \"%s\"\n",valeur);
					car += l;
					PanelEntryVerif(cdr,panel,f,item,direct,valeur,limp,zone-cdr->colmin,&car);
					panel->precedent = num;
					curs = car - item->first;
...
/*					if(direct) { poub = 999; PanelCode(item,PNL_READ,0,&poub); }
					...
					if((fctn_item = (TypePanelFctnItem)item->rtn)) {
						if(DEBUG_PANEL(2)) {
							WndPrint("(%s) rtn sur C%08X='%s' -> P%08X[%d->%d] -> I%08X\n",__func__,
								(hexa)cdr,cdr->nom,(hexa)panel,num_item+1,num+1,(hexa)item);
							WndPrint("(%s) item '%s'\n",__func__,item->texte);
						}
			...
			if(souris_glissante) {
				...
				if(WndEventPrecedent.type != WND_PRESS) {
					...
					if(nb > 0) {
						...
						/* printf("(%s) texte '%s' surligne dans 0x%08X(%d,%d) en mode %d\n",__func__,OpiumEditSurligne,
							   (hexa)OpiumEditFen,OpiumEditLig,OpiumEditCol,OpiumEditType); */
					}
				}
			}
...
//									WndPrint("soit dir(%d)='%s'\n",i_dir,items_dir[i_dir]);
							...
							if(!encore && pointe) {
								l = (int)strlen(valeur);
								if(l > panel->var[k]) item->first = l - panel->var[k] + 1;
								curs = l - 1;
	...
	if(DEBUG_PANEL(1)) WndPrint("(OpiumRunPanel) Code rendu: %d\n",code_rendu);
...
/*  	else if(code_rendu == PNL_NOMODIF) PanelUpdate(panel,0); */
	...
	if(DEBUG_PANEL(3)) 
		WndPrint("(OpiumRunPanel) Curseur laisse en %d x %d\n",panel->ycurseur,panel->xcurseur);
