...
Figure FigureCreate(char type, void *forme, void *adrs, TypeFigFctn fctn) {
	...
	if(fig) {
		if(DEBUG_FIGURE(1)) WndPrint("Figure creee @%08X\n",(hexa)fig);
		cdr = OpiumCreate(OPIUM_FIGURE,fig);
		if(!cdr) { free(fig); return(0); }
		if(DEBUG_FIGURE(1)) WndPrint("Figure creee @%08X: %s\n",(hexa)fig,cdr->nom);
		fig->cdr = cdr;
...
void FigureChangeForme(Figure fig, char type, void *forme) {
	...
	// printf("(%s:%d) %s @%08X: couleur ecriture #%d\n",__func__,__LINE__,FigTypeNom[fig->type],(hexa)fig,fig->ecrit);
...
/* ========================================================================== */
void FigureDelete(Figure fig) { if(fig) { OpiumDelete(fig->cdr); free(fig); } }
/* ========================================================================== */
...
/* ========================================================================== */
void FigureEcrit(Figure fig, WND_GC_COUL ecrit) { if(fig) fig->ecrit = ecrit; }
/* ========================================================================== */
...
/* ========================================================================== */
static void FigureTraceDroite(Figure fig, TypeFigDroite *droite, WndFrame f, int x, int y) {
	char fait;
	...
	if(!fait) WndLine(f,fig->p.gc,x,y,droite->dx,droite->dy);
}
/* ========================================================================== */
char FigureRedessine(Figure fig, char plus_fond) {
	...
	TypeFigDroite segment; int x,y,xd,yd,k; int l; // size_t l;
	...
	switch(fig->type) {
	  case FIG_DROITE:
		droite = fig->forme.droite;
		FigureTraceDroite(fig,droite,f,xd,yd);
		break;
	  case FIG_LIEN:
		lien = fig->forme.lien;
		...
		if(!(lien->coude)) {
			...
			FigureTraceDroite(fig,&segment,f,xd,yd);
		} else if(lien->horiz) {
			x = xd; y = yd;
			segment.dx = lien->coude; segment.dy = 0; 
			FigureTraceDroite(fig,&segment,f,x,y); x += segment.dx;
			segment.dx = 0; segment.dy = lien->dy; 
			FigureTraceDroite(fig,&segment,f,x,y); y += segment.dy;
			segment.dx = lien->dx - lien->coude; segment.dy = 0; 
			FigureTraceDroite(fig,&segment,f,x,y);
		} else {
			x = xd; y = yd;
	  ...
	  case FIG_ZONE:
		...
		WndPaint(f,xd,yd,zone->larg,zone->haut,fig->p.c,0);
		break;
	  case FIG_ICONE:
		icone = fig->forme.icone;
		//+ if(plus_fond) WndFillFgnd(f,WND_GC(f,fig->icone),xd,yd,icone->larg,icone->haut);
		WndImageOffset(f,xd,yd); // avant WndImageInit
	  ...
	  case FIG_CADRE:
		...
		WndRelief(f,2,zone->support | WND_HORIZONTAL,xd,yd,zone->larg);
		WndRelief(f,2,zone->support | WND_VERTICAL,xd,yd,zone->haut);
		WndRelief(f,2,zone->support | WND_VERTICAL,xd+zone->larg,yd,zone->haut);
		WndRelief(f,2,zone->support | WND_HORIZONTAL,xd,yd+zone->haut,zone->larg);
		break;
	}
	if(fig->texte[0] && (fig->type != FIG_LIEN)) {
		...
		// printf("(%s:%d) %s @%08X: couleur ecriture #%d\n",__func__,__LINE__,FigTypeNom[fig->type],(hexa)fig,fig->ecrit);
//		printf("(%s:%d) gc@%08X\n",__func__,__LINE__,(hexa)(WND_GC(f,fig->ecrit)));
		s = f->s;
	...
	if(cdr->titre[0]) {
		...
		WndString(f,WND_GC(f,WND_GC_BOARD),x,cdr->haut - 1,cdr->titre); // WND_GC_CLEAR?
	}
	return(1);
}
/* ========================================================================== */
