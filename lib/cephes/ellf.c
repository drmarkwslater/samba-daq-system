...
//	if(dbg) printf("(%s) x = (%g + i x %g) [zord=%d]\n",__func__,x.r,x.i,zord);
	if(dbg) {
		for( j=0; j<zord; j++ ) printf("(%s) z[%d] = (%g + i x %g)\n",__func__,j,z[j].r,z[j].i);
		for( j=0; j<zord; j++ ) printf("(%s) z[%d] = (%g + i x %g)\n",__func__,j,z[j+zord].r,z[j].i);
	}
