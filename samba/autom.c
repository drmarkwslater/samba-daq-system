...
#define AUTOM_FMT_CLES "edw/json"
...
#define AUTOM_AFF_CLES "non/voyant/cadrant"
...
static ArgDesc AutomDesc[] = {
	...
	{ "fichier",   DESC_STR(MAXFILENAME)      AutomSrceFic,   "nom du fichier source" },
...
static ArgDesc AutomVarDesc[] = {
	...
	{ "type",   DESC_KEY AUTOM_TYPE_CLES, &AutomVarLue.type, 0 },
...
static ArgDesc AutomUserVarDesc[] = {
	...
	{ "format",  DESC_KEY AUTOM_TYPE_CLES,   &AutomUserVarLue.type,    0 },
...
//- static AUTOM_TRAME AutomTypeAttendu;
...
/* ========================================================================== */
char AutomType(int num) {
	if((num >= 0) && (num < AutomVarNb)) return(AutomVar[num].type);
	else return(0);
}
/* ========================================================================== */
...
void AutomUserFromVar(int j) {
	TypeAutomUserVar *user; TypeAutomVar *var; int i;
	...
	switch(user->type) {
		case AUTOM_BOOL:
			switch(var->type) {
				case AUTOM_BOOL:
				case AUTOM_CLE:   user->val.b = var->val.b; break;
				case AUTOM_INT:   user->val.b = (var->val.i > 0); break;
				case AUTOM_FLOAT: user->val.b = (var->val.r > 0.5); break;
			}
			break;
		case AUTOM_CLE:
			switch(var->type) {
				case AUTOM_BOOL:
				case AUTOM_CLE:   user->val.b = var->val.b; break;
				case AUTOM_INT:   user->val.b = (char)var->val.i; break;
				case AUTOM_FLOAT: user->val.b = (char)(var->val.r + 0.5); break;
			}
			break;
		case AUTOM_INT:
			switch(var->type) {
				case AUTOM_BOOL:
				case AUTOM_CLE:   user->val.i = (int)var->val.b; break;
				case AUTOM_INT:   user->val.i = var->val.i; break;
				case AUTOM_FLOAT: user->val.i = (int)var->val.r; break;
			}
			break;
		case AUTOM_FLOAT:
			switch(var->type) {
				case AUTOM_BOOL:
				case AUTOM_CLE:   user->val.r = (float)var->val.b; break;
				case AUTOM_INT:   user->val.r = (float)var->val.i; break;
				case AUTOM_FLOAT: user->val.r = var->val.r; break;
			}
			break;
	}
}
...
void AutomSrceFromVar(int j) {
	TypeAutomVar *var; TypeAutomSrceVar *srce; int i;
	...
	switch(var->type) {
		case AUTOM_BOOL:
		case AUTOM_CLE:   srce->active = var->val.b; break;
		case AUTOM_INT:   srce->active = (var->val.i > 0); break;
		case AUTOM_FLOAT: srce->active = (var->val.r > 0.5); break;
	}
}
/* ========================================================================== */
static void AutomExemple() {
	int i = 0; int source;
	strcpy(AutomVar[i].nom,"Pmain"); AutomVar[i].type = AUTOM_FLOAT; i++;
	strcpy(AutomVar[i].nom,"Pauxi"); AutomVar[i].type = AUTOM_FLOAT; i++;
	strcpy(AutomVar[i].nom,"HVnorth"); AutomVar[i].type = AUTOM_INT; i++;
	strcpy(AutomVar[i].nom,"HVsouth"); AutomVar[i].type = AUTOM_INT; i++;
	strcpy(AutomVar[i].nom,"SourceType"); AutomVar[i].type = AUTOM_CLE; strcpy(AutomVar[i].cles,"Fe/AmBe"); i++;
	strcpy(AutomVar[i].nom,"SourceInUse"); AutomVar[i].type = AUTOM_BOOL; source = i; i++;
	AutomVarNb = i;
	for(i=0; i<AutomVarNb; i++) AutomVarRaz(&(AutomVar[i]),AUTOM_READONLY,AUTOM_DO);

	AutomUserNb = AutomVarNb;
	for(i=0; i<AutomUserNb; i++) {
		AutomUserVar[i].num = i;
		AutomUserVar[i].type = AutomVar[i].type;
		strcpy(AutomUserVar[i].libelle,AutomVar[i].nom);
		strcpy(AutomUserVar[i].cles,AutomVar[i].cles);
	}
	AutomSrceNb = 1;
	for(i=0; i<AutomSrceNb; i++) {
		AutomSrceVar[i].num = source;
	...
	if(AutomVarNb) {
		...
		for(num=0; num<AutomVarNb; num++) {
			...
			switch(var->type) {
			  ...
			  case AUTOM_INT:   ArgAdd(elt++,var->nom,DESC_INT   &(var->val.b),0); break;
...
#ifndef CODE_WARRIOR_VSN
	else { if((AutomLink >= 0) && !AutomPermanent) { SocketFermee(AutomLink); AutomLink = -1; }; return(-1); }
#endif
...
static int Autom1Lecture(AUTOM_TRAME attendu, int indx, char *raison) {
	char lue[MAXTRAME],*c,*d,*s,*e; int j,k,t,rc,n,v;
								...
								switch(type) /* decodage d'une ligne */ {
								  ...
								  case AUTOM_TRAME_CONFIG:
									if(AutomVarNb < AUTOM_VAR_MAX) {
										...
										sscanf(d,"%d",&contenu);
										AutomVar[AutomVarNb].type = AUTOM_FLOAT;
...
#ifndef CODE_WARRIOR_VSN
    if((AutomLink >= 0) && (AutomLink != AUTOM_NOLINK) && !AutomPermanent) { SocketFermee(AutomLink); AutomLink = -1; }
#endif
...
char AutomEcritUser(int j, char *raison) {
	TypeAutomUserVar *user; TypeAutomVar *var;
	...
	switch(user->type) {
	  ...
	  case AUTOM_CLE:
		switch(var->type) {
		  case AUTOM_BOOL:
		  case AUTOM_CLE:   var->val.b = user->val.b; break;
		  case AUTOM_INT:   var->val.i = (int)user->val.b; break;
		  case AUTOM_FLOAT: var->val.r = (float)user->val.b; break;
		}
		break;
	  case AUTOM_INT:
		switch(var->type) {
		  case AUTOM_BOOL:  var->val.b = (user->val.i > 0); break;
		  case AUTOM_CLE:   var->val.b = (char)(user->val.i & 0x7F); break;
		  case AUTOM_INT:   var->val.i = user->val.i; break;
		  case AUTOM_FLOAT: var->val.r = (float)user->val.i; break;
		}
		break;
	  case AUTOM_FLOAT:
		switch(var->type) {
		  case AUTOM_BOOL:  var->val.b = (user->val.r > 0.0); break;
		  case AUTOM_CLE:   var->val.b = (char)((int)(user->val.r) & 0x7F); break;
		  case AUTOM_INT:   var->val.i = (int)user->val.r; break;
		  case AUTOM_FLOAT: var->val.r = user->val.r; break;
		}
		break;
	}
...
char AutomEcritSrce(int j, char *raison) {
	TypeAutomSrceVar *srce; TypeAutomVar *var;

	srce = &(AutomSrceVar[j]); var = &(AutomVar[srce->num]);
	switch(var->type) {
	  case AUTOM_BOOL:
	  case AUTOM_CLE:   var->val.b = srce->active; break;
	  case AUTOM_INT:   var->val.i = (int)srce->active; break;
	  case AUTOM_FLOAT: var->val.r = (float)srce->active; break;
	}
...
int AutomCreeTrame(Menu menu, MenuItem *item) {
	char raison[80],nom_complet[MAXFILENAME];

	if(AutomLink >= 0) {
		if(OpiumReadFile("Sur quel fichier",AutomSrceFic,32) == PNL_CANCEL) return(0);
		RepertoireIdentique(FichierPrefAutom,AutomSrceFic,nom_complet);
		if((AutomTrameFile = fopen(nom_complet,"w"))) {
...
/* ========================================================================== */
Menu mAutomRefresh;
MenuItem iAutomRefresh[] = { { "Rafraichir", MNU_FONCTION AutomRefresh }, MNU_END };
/* ========================================================================== */
static void AutomConstruit() {
	int j,k,l,xc,yc,larg_max,ymin,ylim;
	int xp,yp,xi,yi,yq,dx,lngr,col,blancs,naff;
	TypeAutomUserVar *user; TypeAutomVar *var; char *nom;
    Instrum instrum; Panel panel;
	...
	xc = Dx; ymin = 3 * Dy;
	lngr = 15; /* lngr noms variables */
	xp = (lngr + 6) * Dx;
	yp = 2 * Dy;
	xi = (int)(1.5 * Cadrant2.rayon);
	yi = (int)(Cadrant2.rayon + Cadrant2.graduation) + Dy; yq = yi + (2 * Dy);
	col = xi + (4 * Dx); if(col < xp) col = xp;
	larg_max = 1000; ylim = ymin + yq;
	...
	for(j=0; j<AutomUserNb; j++) if(AutomUserVar[j].affiche) {
		...
		switch(user->type) {
		  case AUTOM_BOOL:
			k = PanelBool(panel,nom,&(user->val.b)); PanelItemColors(panel,k,AutomFonds,2);
			break;
		  case AUTOM_INT:
			k = PanelInt(panel,nom,&(user->val.i));
			...
			break;
		  case AUTOM_FLOAT:
			k = PanelFloat(panel,nom,&(user->val.r));
		  ...
		  case AUTOM_CLE:
			...
			k = PanelKeyB(panel,nom,user->cles,&(user->val.b),l);
		...
		switch(user->affiche) {
		  case AUTOM_VOYANT:
			if(panel) {
				...
				blancs = lngr - strlen(nom); if(blancs < 0) blancs = 0;
				BoardAddPanel(bAutomate,panel,xc + (blancs * Dx),yc,0);
				naff++;
				yc += yp;
			}
			break;
		  case AUTOM_CADRANT:
			...
			instrum = 0;
			switch(user->type) {
			  case AUTOM_BOOL: instrum = InstrumBool(INS_CADRAN,(void *)&Cadrant2,&(user->val.b)); break;
			  case AUTOM_INT: instrum = InstrumInt(INS_CADRAN,(void *)&Cadrant2,&(user->val.i),user->min.i,user->max.i); break;
			  case AUTOM_FLOAT: instrum = InstrumFloat(INS_CADRAN,(void *)&Cadrant2,&(user->val.r),user->min.r,user->max.r); break;
			  case AUTOM_CLE: instrum = InstrumKeyB(INS_CADRAN,(void *)&Cadrant2,&(user->val.b),user->cles); break;
			}
			if(instrum) { InstrumSupport(instrum,WND_CREUX); BoardAddInstrum(bAutomate,instrum,xc,yc,0); }
			user->instrum = instrum;
			if(panel) {
				yc += yi;
				dx = (xi - ((strlen(nom) + 7) * Dx)) / 2; if(dx < 0) dx = 0;
...
int AutomChoisi(Menu menu, MenuItem *item) {
	...
	for(i=0; i<AutomVarNb; i++) if(AutomVar[i].utilisee && (AutomVar[i].usr == -1)) {
		...
		AutomUserVar[j].type = AutomVar[i].type;
	...
	for(j=0; j<AutomUserNb; j++) {
		...
		PanelKeyB(p,0,AUTOM_TYPE_CLES,&(AutomUserVar[j].type),6);
...
void AutomVerifieConditions(char *prefixe, int k) {
	...
	if(AutomSrceType == AUTOM_FICHIER) {
		sprintf(texte,"L'information sur l'environnement est simulee");
...
int AutomInit() {
	...
	strcpy(AutomSrceFic,"SupportDefs");
	strcpy(AutomUserFic,"SupportVars");
	strcpy(AutomUserVal,"SupportVals");
	strcpy(AutomSrceSvr,"192.168.3.13"); // 134.158.176.112
	AutomPort = 6342;
	AutomPermanent = 1;
	AutomValeursDesc = 0;
	AutomVarNb = 0;
	AutomUserNb = 0; // AutomAffgNb = 0;
	AutomSrceNb = 0;
	AutomSrceAccede = 0;
	for(j=0; j<AUTOM_ARCH_MAX; j++) {
		AutomUserVar[j].num = -1;
	...
	AutomNum = 0; AutomChoix = 0; //- AutomTypeAttendu = 0;
	AutomValeursLues = 0;
		...
		while(LigneSuivante(ligne,256,fichier)) {
			...
			if(*r == '@') {
				...
				if(AutomSrceType == AUTOM_FICHIER) /* fichier */ {
					//- SambaAjoutePrefPath(nom_complet,AutomSrceFic);
					RepertoireIdentique(FichierPrefAutom,AutomSrceFic,nom_complet);
					SambaLogDef(". Noms des variables a trouver","dans",nom_complet);
					AutomLink = open(nom_complet,O_RDONLY,0);
					if(AutomLink == -1) {
					...
					close(-AutomLink);
					AutomLink = AUTOM_NOLINK; // plus la peine de relire le fichier (??)
				}
	...
	#ifdef EDW_ORIGINE
		for(i=0; i<AUTOM_REGEN_MAX; i++) AutomSrceRegenIndex[i] = AutomIndex(AutomRegenNom[i]);
		for(i=0; i<AUTOM_CALIB_MAX; i++) AutomSrceCalibIndex[i] = AutomIndex(AutomCalibNom[i]);
		if(CompteRendu.vars_manip) {
			int num;
			...
			switch(AutomInterne[i].type) {
			  case AUTOM_BOOL: case AUTOM_CLE:
				if(AutomInterne[i].cle) {
					*(AutomInterne[i].cle) = AutomAdrsKey(AutomIndex(AutomInterne[i].nom));
			  ...
			  case AUTOM_FLOAT:
				if(AutomInterne[i].reel) {
					*(AutomInterne[i].reel) = AutomAdrsFloat(AutomIndex(AutomInterne[i].nom));
