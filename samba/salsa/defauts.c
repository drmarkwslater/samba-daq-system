...
#undef printf
/* ========================================================================== */
void SambaTopDirAt(char *top_dir, char *args) {
	FILE *f; char start_dir[MAXFILENAME];

	if(getcwd(start_dir,MAXFILENAME)) RepertoireTermine(start_dir);
	else strcat(strcpy(start_dir,HomeDir),"acq/");
	printf("  Repertoire local: '%s'\n",start_dir);
	strcpy(top_dir,start_dir);
	if(!(f = fopen(args,"r"))) {
		char topfile[MAXFILENAME];
		printf("  > Fichier %s absent du repertoire de travail\n",args);
		strcat(strcpy(topfile,HomeDir),".samba_top");
		printf("  > Recherche du fichier de localisation '%s'\n",topfile);
		f = fopen(topfile,"r");
		if(f) {
			size_t l;
			printf("  > Localisation de Samba trouvee\n");
			fgets(top_dir,MAXFILENAME,f);
			fclose(f);
			l = strlen(top_dir); while(l) {
				--l;
				if((top_dir[l] == '\n') || (top_dir[l] == 0x0a)) top_dir[l] = '\0';
				else if(top_dir[l] == ' ') top_dir[l] = '\0';
				else break;
			}
			chdir(top_dir);
			RepertoireTermine(top_dir);
		} else printf("  ! Ce fichier est inutilisable (%s)\n",strerror(errno));
	} else fclose(f);
...
//	printf("  Valeurs par defaut pour Samba\n");
	...
	Echantillonnage = (HorlogeSysteme * 1000.0) / (Diviseur0 * Diviseur1); /* kHz */
...
void SettingsDefaults() {
	printf("  Valeurs par defaut des settings\n");
...
/* ========================================================================== */
void SambaLangueDefini(char *dico_compilo, char *dico_user) {
	char nom_langue[64];

	printf("  Langue demandee: %s\n",!strcmp(LangueDemandee,".")? "par defaut": LangueDemandee);
	DicoSrce[0] = '\0';
