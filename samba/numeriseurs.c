...
/* ========================================================================== */
static int NumerModlRessInt(TypeBNModele *modele_bn, short col, char *nom, unsigned short ssadrs, int bits, 
...
/* ========================================================================== */
static int NumerModlRessFloat(TypeBNModele *modele_bn, short col, char *nom, unsigned short ssadrs, int bits, 
...
/* ========================================================================== */
static int NumerModlRessCles(TypeBNModele *modele_bn, short col, char *nom, unsigned short ssadrs, int bits, 
...
/* ========================================================================== */
static int NumerModlRessPuiss(TypeBNModele *modele_bn, short col, char *nom, unsigned short ssadrs, int bits, 
...
/* ========================================================================== */
static void NumerModlRessInit(TypeBNModele *modele_bn, int i, unsigned short hval) {
	TypeBNModlRessource *ress;
...
char NumeriseursModlLit(char *fichier) {
	...
	if((f = fopen(nom_complet,"r"))) {
		printf("  . Lecture des modeles de numeriseurs  dans '%s'\n",nom_complet);
		//- ArgDebug(2);
...
void NumeriseurModlAccesInit(void *bloc) {
	...
	acces->nb = 3;
	acces->var[0] = NUMER_VAR_IDENT;
	acces->var[1] = NUMER_VAR_ADRS;
	acces->var[2] = NUMER_VAR_VAL; 
}
/* ========================================================================== */
void NumeriseurModlInit(void *bloc) {
	...
	modele_bn->acces.var[0] = NUMER_VAR_IDENT;
	modele_bn->acces.var[1] = NUMER_VAR_ADRS;
	modele_bn->acces.var[2] = NUMER_VAR_VAL; 
}
/* ========================================================================== */
...
static void NumeriseursDecodeDef(char *modl, char *ligne, TypeNumeriseurDef *def, char *fin) {
	...
	int type_bn,cap;
	unsigned short serial,adrs; /* 0xFFFF: pas de num. de serie */
	char *item; char *r,d;
...
/* ========================================================================== */
static void NumeriseurRaz(TypeNumeriseur *numeriseur, TypeBNModele *modele_bn, unsigned char adrs) {
	int i,k;
...
/* ========================================================================== */
void NumeriseurNeuf(TypeNumeriseur *numeriseur, int type, unsigned short ident, TypeRepartiteur *repart) {
	TypeBNModele *modele_bn; int k; unsigned char adrs;
...
int NumeriseursInitTous() {
	...
	for(bb=0; bb<NumeriseurNb; bb++) if(Numeriseur[bb].sel) {
		...
		if(nb) NumeriseurChargeEntierement(&(Numeriseur[bb]),0,log);
	}
...
unsigned short NumeriseurRessUserDecode(TypeNumeriseur *numeriseur, int num, char *cval, int *i, float *r) {
	...
	TypeBNModele *modele_bn; TypeBNModlRessource *ress;
	unsigned short hval; int ival; float rval,fval;
	...
	if(ress->type == RESS_INT) {
		...
		if(ress->lim_i.max != ress->lim_i.min)
			hval = (unsigned short)((ival - ress->lim_i.min) * ress->masque / (ress->lim_i.max - ress->lim_i.min));
		else hval = (unsigned short)(*(unsigned int *)&ival & 0xFFFF);
		if(i) *i = ival;
	} else if(ress->type == RESS_FLOAT) {
		...
		hval = (unsigned short)(((fval - ress->lim_r.min) * numeriseur->ress[num].rval_to_hex) + 0.5);
		if(r) *r = rval;
...
/* ========================================================================== */
void NumeriseurRessFloatDecode(TypeNumeriseur *numeriseur, short num, float rval, unsigned short *hval) {
	...
	if(ress->type == RESS_INT) {
		...
		*hval = (unsigned short)(*(unsigned int *)&ival & 0xFFFF);
	} else if(ress->type == RESS_FLOAT) {
		fval = rval * numeriseur->ress[num].correctif;																				
		if(ress->lim_r.min < ress->lim_r.max) {
...
/* ========================================================================== */
unsigned short NumeriseurRessValEncode(TypeNumeriseur *numeriseur, int num, char *cval, int ival, float rval) {
...
/* ========================================================================== */
unsigned short NumeriseurRessHexaEncode(TypeNumeriseur *numeriseur, int num, unsigned short hval, char *cval, int *i, float *r) {
...
/* ========================================================================== */
unsigned short NumeriseurRessCurrentVal(TypeNumeriseur *numeriseur, int num, int *i, float *r) {
...
unsigned short NumeriseurRessCurrentVal(TypeNumeriseur *numeriseur, int num, int *i, float *r) {
	/* Prend la valeur reellement ecrite, .hval, et la transforme en valeur utilisateur i ou r.
	 Pour les cles, i est l'index de la cle */
	TypeBNModele *modele_bn; TypeBNModlRessource *ress;
	unsigned short hval; int ival; float rval;
	int dval,min,mval;
...
#endif /* OBSOLETE */
/* ========================================================================== */
INLINE char NumeriseurChargeValeurBB(TypeNumeriseur *numeriseur, char ss_adrs, TypeADU valeur) {
	if(SambaMaitre) {
		int n;
		n = SambaEcritSatCourt((numeriseur->repart)->sat,SMB_NUMER,SMB_NUMER_MANU_ADRS,0,ss_adrs,2,valeur,(unsigned short)(numeriseur->bb));
...
/* ========================================================================== */
INLINE char NumeriseurCommandeObsolete(TypeNumeriseur *numeriseur, int k, TypeADU valeur) {
	unsigned char cmde[80]; int j,l; void *adrs[NUMER_ACCES_MAXVAR];
	TypeBNModele *modele_bn; TypeBNModlRessource *ress;
	...
	if(SambaMaitre) {
		int n;
		n = SambaEcritSatCourt((numeriseur->repart)->sat,SMB_NUMER,SMB_NUMER_MANU_ADRS,0,ress->ssadrs,2,valeur,(unsigned short)(numeriseur->bb));
		strcpy(RepartiteurValeurEnvoyee,AcquisMsgEnvoye);
		return(n);
		
	} else if(modele_bn->acces.mode == NUMER_ACCES_BB) {
		// return(RepartiteurEcritRessourceBB(numeriseur->repart,numeriseur->entree,numeriseur->def.adrs,ress->ss_adrs,valeur));
		l = 0;
		cmde[l++] = numeriseur->def.adrs;
		cmde[l++] = ress->ssadrs;
		cmde[l++] = (byte)((valeur >> 8) & 0xFF);
		cmde[l++] = (byte)(valeur & 0xFF);
		
		/* CAEN SY127: format = "1BAA%s\rC%d\r1" */
	} else if(modele_bn->acces.mode == NUMER_ACCES_TXT) {
		for(j=0; j<modele_bn->acces.nb; j++) switch(modele_bn->acces.var[j]) {
			case NUMER_VAR_IDENT: adrs[j] = (void *)((int)numeriseur->def.adrs); break;
			case NUMER_VAR_ADRS:  adrs[j] = (void *)((int)ress->ssadrs); break;
			case NUMER_VAR_NOM:   adrs[j] = (void *)ress->nom; break;
			case NUMER_VAR_VAL:   adrs[j] = (void *)((int)valeur); break;
		}
...
/* ========================================================================== */
static char NumeriseurCommande(TypeNumeriseur *numeriseur, int k, void *ref) {
	unsigned char cmde[80]; int j,l; void *adrs[NUMER_ACCES_MAXVAR];
	TypeBNModele *modele_bn; TypeBNModlRessource *ress;
	TypeADU hval; int ival; // float rval;
	...
	if(SambaMaitre) {
		int n;
		hval = *(TypeADU *)ref;
		n = SambaEcritSatCourt((numeriseur->repart)->sat,SMB_NUMER,SMB_NUMER_MANU_ADRS,0,ress->ssadrs,2,hval,(unsigned short)(numeriseur->bb));
		strcpy(RepartiteurValeurEnvoyee,AcquisMsgEnvoye);
		return(n);

	} else if(modele_bn->acces.mode == NUMER_ACCES_BB) {
		// return(RepartiteurEcritRessourceBB(numeriseur->repart,numeriseur->entree,numeriseur->def.adrs,ress->ss_adrs,hval));
		hval = *(TypeADU *)ref;
		l = 0;
		cmde[l++] = numeriseur->def.adrs;
		cmde[l++] = ress->ssadrs;
		...
		for(j=0; j<modele_bn->acces.nb; j++) switch(modele_bn->acces.var[j]) {
			...
			case NUMER_VAR_NOM:   adrs[j] = (void *)ress->nom; break;
			case NUMER_VAR_VAL:
				if(ress->type == RESS_INT) { ival = *(int *)ref; adrs[j] = (void *)ival; }
// float et void* incompatibles				else if(ress->type == RESS_FLOAT) { rval = *(float *)ref; adrs[j] = (void *)rval; }
				else adrs[j] = 0;
				break;
		}
		switch(modele_bn->acces.nb) {
...
INLINE void NumeriseurAutoriseBB2(TypeNumeriseur *numeriseur, char autorise, char log) {
	...
	if((numeriseur->repart)->ecr.path >= 0) NumeriseurChargeValeurBB(numeriseur,ss_adrs_alim,alim);
	if(log) {
...
INLINE static char NumeriseurAccesAuto(TypeNumeriseur *numeriseur, char autorise, char *suite, char *marge) {
	...
	if(autorise) {
		...
		if(NumeriseurDejaAutorise >= 0) {
			...
			if(ancien->vsnid >= 2) {
				if(rep_avant->ecr.path >= 0) NumeriseurChargeValeurBB(ancien,ss_adrs_alim,_bloque_commandes);
				//? unless(ecrit) { printf("\n"); ecrit = 1; }
		...
		if(numeriseur->vsnid >= 2) {
			if(repart->ecr.path >= 0) NumeriseurChargeValeurBB(numeriseur,ss_adrs_alim,_autorise_commandes);
			//? unless(ecrit) { printf("\n"); ecrit = 1; }
			if(prefixe) printf("%s ",prefixe); else printf("%s/   . ",DateHeure());
...
/* ========================================================================== */
unsigned short NumeriseurChargeRessourceAuto(TypeNumeriseur *numeriseur, int k, char *suite, char *marge) {
	TypeBNModele *modele_bn; TypeBNModlRessource *ress; TypeRepartiteur *repart;
	unsigned char ssadrs; unsigned short hval;
	int i; char reconstruit,ok,ecrit;
	char explics[80],*msg,*prefixe;
	
	modele_bn = &(ModeleBN[numeriseur->def.type]); ress = modele_bn->ress + k;
    hval = 0; msg = explics; ok = 0; prefixe = suite; ecrit = 0;
	if(!(repart = numeriseur->repart)) sprintf(explics,"! Numeriseur %s inaccessible, %s inchange",numeriseur->nom,ress->nom);
		...
		if((ssadrs = ress->ssadrs) == 0xFF) sprintf(explics,"! Transmis: neant (la ressource %s est en lecture seule)\n",ress->nom);
		else if(ssadrs != 0) {
			if(SambaMaitre) /* pas bon: la ressource n'a pas ete mise a jour */ {
				SambaEcritSatCourt(repart->sat,SMB_NUMER,SMB_NUMER_AUTO_RESS,0,(unsigned char)k,1,(unsigned short)(numeriseur->bb));
				msg = AcquisMsgEnvoye;
			} else {
				...
				if(modele_bn->acces.mode == NUMER_ACCES_BB) {
					...
					unless(reconstruit) hval = numeriseur->ress[k].hval;
					//- ok = NumeriseurChargeValeurBB(numeriseur,ssadrs,hval);
					ok = NumeriseurCommande(numeriseur,k,&hval);
				} else if(ress->type == RESS_INT) ok = NumeriseurCommande(numeriseur,k,&(numeriseur->ress[k].val.i));
				else if(ress->type == RESS_FLOAT) ok = NumeriseurCommande(numeriseur,k,&(numeriseur->ress[k].val.r));
	...
	if(prefixe) {
		...
		if(ok) printf("%s %s | %8s: %13s = %s %s\n",prefixe,msg,numeriseur->nom,ress->nom,numeriseur->ress[k].cval,ress->unite);
		else printf("%s %s\n",prefixe,msg);
	}
	return(hval);
}
/* ========================================================================== */
unsigned short NumeriseurChargeAdrsAuto(TypeNumeriseur *numeriseur, unsigned char ssadrs, unsigned short hval, char *suite, char *marge) {
	TypeRepartiteur *repart;
	char ok,ecrit;
	char explics[80],*msg,*prefixe;
	...
	if(!(repart = numeriseur->repart)) sprintf(explics,"! Numeriseur %s inaccessible, sous-adresse %02X inchangee",numeriseur->nom,ssadrs);
		...
		if(SambaMaitre) {
			SambaEcritSatCourt(repart->sat,SMB_NUMER,SMB_NUMER_AUTO_ADRS,0,ssadrs,2,hval,(unsigned short)(numeriseur->bb));
			msg = AcquisMsgEnvoye;
		} else {
			ecrit = NumeriseurAccesAuto(numeriseur,1,prefixe,marge);
			ok = NumeriseurChargeValeurBB(numeriseur,ssadrs,hval);
...
/* ========================================================================== */
void NumeriseurChargeRessourceFin(char *prefixe) {
	if(NumeriseurDejaAutorise >= 0) {
		NumeriseurAccesAuto(&(Numeriseur[NumeriseurDejaAutorise]),0,prefixe,prefixe);
	}
}
/* ========================================================================== */
unsigned short NumeriseurChargeRessource(TypeNumeriseur *numeriseur, int k, char *prefixe) {
	TypeBNModele *modele_bn; TypeBNModlRessource *ress;
	unsigned char ssadrs; unsigned short hval;
	...
	hval = 0;
	modele_bn = &(ModeleBN[numeriseur->def.type]); ress = modele_bn->ress + k;
	if((ssadrs = ress->ssadrs) == 0xFF) {
		if(prefixe) {
			if(*prefixe == '\0') printf("%s/",DateHeure());
			printf("%s ! Transmis: neant (la ressource %s est en lecture seule)\n",prefixe,ress->nom);
		}
	} else if(ssadrs != 0) {
		if(modele_bn->acces.mode == NUMER_ACCES_BB) {
			reconstruit = 0;
...
/* ========================================================================== */
int NumeriseurChargeEntierement(TypeNumeriseur *numeriseur, char tout, char log) {
	int k;
	unsigned char ssadrs; unsigned short hval; int i; char reconstruit;
	char deja_envoye[NUMER_RESS_MAX],ok;
	TypeBNModele *modele_bn; TypeBNModlRessource *ress;
	...
	for(k=0; k<modele_bn->nbress; k++) {
		...
		ress = &(modele_bn->ress[k]);
		if((ssadrs = ress->ssadrs) == 0xFF) continue;
		if(modele_bn->acces.mode == NUMER_ACCES_BB) {
			reconstruit = 0;
			if(ress->bit0 >= 0) {
				if(deja_envoye[k]) continue;
				hval = 0;
				for(i=0; i<modele_bn->nbress; i++) if((modele_bn->ress[i].bit0 >= 0) && (modele_bn->ress[i].ssadrs == ssadrs)) {
					hval = hval | (numeriseur->ress[i].hval << modele_bn->ress[i].bit0); reconstruit = 1;
					deja_envoye[i] = 1;
			...
			if(reconstruit) deja_envoye[k] = 1;
		...
		if(log) {
			if(ok) printf("            . envoye: %s / %s = %s\n",RepartiteurValeurEnvoyee,ress->nom,numeriseur->ress[k].cval);
...
/* ========================================================================== */
INLINE void NumeriseurRecopieStatus(TypeNumeriseur *numeriseur, short index, unsigned int lu) {
...
void NumeriseurRapportEtabli(TypeNumeriseur *numeriseur) {
	int i,k,l,item,prems,der; int bb; unsigned short hval; /* float rval; */ char *texte;
	TypeBNModele *modele_bn;
	...
	for(j=0; j<NumeriseurNb; j++) {
		...
		if(repart->famille == FAMILLE_IPE) {
			switch(ClassementCourant[CLASSE_NUMER]) {
			  ...
			  case CLASSE_POSITION: case CLASSE_TOUR: groupe_courant = (int)(OrdreNumer[j].ref) >> 4; break;
			}
...
char NumeriseurRafraichi(TypeNumeriseur *numeriseur) {
	...
	Cadre cdr; Graph g; int y; unsigned int zoom;
...
static int NumeriseurInit(Menu menu, MenuItem *item) {
	...
	if((numeriseur = NumeriseurOrigineDuPave(menu))) {
		...
		NumeriseurAutoriseAcces(numeriseur,1,1);
		for(k=0; k<modele_bn->nbress; k++) if(numeriseur->ress[k].init) NumeriseurChargeRessource(numeriseur,k,"*");
		NumeriseurAutoriseAcces(numeriseur,0,1);
		NumeriseurMontreModifs(numeriseur);
	}
...
/* ========================================================================== */
static MenuItem iNumeriseurBoutons[] = {
...
static MenuItem iNumeriseurBlocage[] = {
	...
	for(k=0; k<modele_bn->nbress; k++) {
		...
		if(optn_brutes) {
			...
			if(NumeriseurChoix.optn.hexa) {
				...
				if(ress->type == RESS_FLOAT) i = PanelAdu(panel,nom,(unsigned short *)&(numeriseur->ress[k].hval));
			...
			if(ress->ssadrs == 0xFF) PanelItemSelect(panel,i,0);
				...
				PanelItemModif(panel,i,NumeriseurRessEnCours,(void *)j);
				PanelItemExit(panel,i,NumeriseurRessFromADU,(void *)j);
			}
			n++;
		}
		...
		if(NumeriseurChoix.optn.codees) {
			...
			if(ress->type == RESS_CLES) {
				...
				if(ress->ssadrs == 0xFF) PanelItemSelect(panel,i,0);
					...
					PanelItemModif(panel,i,NumeriseurRessEnCours,(void *)j);
					PanelItemExit(panel,i,NumeriseurRessFromKey,(void *)j);
				}
			} else {
				...
				if(ress->ssadrs == 0xFF) PanelItemSelect(panel,i,0);
					...
					PanelItemModif(panel,i,NumeriseurRessEnCours,(void *)j);
					PanelItemExit(panel,i,NumeriseurRessFromUser,(void *)j);
				}
			}
			PanelLngr(panel,i,NUMER_DIGITS_DAC);
			n++;
...
/* ========================================================================== */
int NumeriseurActionSouris(TypeNumeriseur *numeriseur, WndUserRequest *e) {
...
/* ========================================================================== */
int NumeriseurLectClique(Figure fig, WndUserRequest *e) {
	NumeriseurActionSouris((TypeNumeriseur *)(fig->adrs),e);
	return(0);
}
/* ========================================================================== */
int NumeriseurOperationSouris(TypeNumeriseur *numeriseur, WndUserRequest *e) {
...
/* ========================================================================== */
int NumeriseurOrgaClique(Figure fig, WndUserRequest *e) {
