...
static struct { unsigned short xinf,yinf,xsup,ysup; } OrgaPositionSlot[ORGA_GALETTES_MAX][ORGA_TOURS_MAX];
static struct { unsigned short xinf,yinf,xsup,ysup; } OrgaPositionFischer[ORGA_QUADRANT_MAX][ORGA_RAIL_MAX];
...
static int OrgaCliqueSurSlot(Figure cliquee, WndUserRequest *e);
static int OrgaCliqueSurFlt(Figure fig, WndUserRequest *e);
static int OrgaCablageChoixAdc(Figure fig, WndUserRequest *e);

#ifdef A_CONTINUER
static Menu mCliqueSurMac; 
static MenuItem iCliqueSurMac[] = {
...
static Menu OrgaSelecteur(char *choix, char *var) {
   ...
   .......................................................................... 
static int OrgaAnnule(Figure cliquee, WndUserRequest *e) {
	OrgaReponse = -1; return(1);
}
   .......................................................................... 
static int OrgaContinue(Figure cliquee, WndUserRequest *e) {
	OrgaReponse = 1; return(1);
}
   .......................................................................... 
static Menu mOrgaStopStart;
static MenuItem iOrgaStopStart[] = {
...
/* ========================================================================== */
static int OrgaCliquePlanche(Cadre planche, WndUserRequest *e, void *arg) {
...
/* ========================================================================== */
static int OrgaSelectionNumerClique(Figure cliquee, WndUserRequest *e) {
...
/* ========================================================================== */
static int OrgaSelectionDetecClique(Figure cliquee, WndUserRequest *e) {
...
int OrgaSelectionDetecteurConstruit(char exclusive) {
	...
	for(galette=0; galette<GALETTES_NB; galette++) {
		...
		for(tour=0; tour<TOURS_NB; tour++) {
			...
			if(!ChassisDetecUnique) {
				strcpy(slot,CablageEncodePosition(CABLAGE_POSITION(galette,tour))); FigureTexte(fig,slot);
			}
	...
	for(bolo=0; bolo<BoloNb; bolo++) {
		if(Bolo[bolo].pos != CABLAGE_INDEFINI) {
			...
			strcpy(slot,CablageEncodePosition(CABLAGE_INDEFINI));
			fig = FigureCreate(FIG_ZONE,(void *)(&OrgaZoneSlot),0,0);
			if(!ChassisDetecUnique) {
				strcpy(slot,CablageEncodePosition(CABLAGE_INDEFINI)); FigureTexte(fig,slot);
			}
...
/* ========================================================================== */
static void OrgaRepartRetire(Figure fig, WndUserRequest *e, void *adrs) {
	int rep,l,bb;
...
#endif
/* ========================================================================== */
static void OrgaRepartAjoute(Figure fig, WndUserRequest *e, void *adrs, int x, int y) {
...
/* ========================================================================== */
static int OrgaFibreSurRepartiteur(Figure fig, WndUserRequest *e) {
...
/* ========================================================================== */
static int OrgaFibreSurNumeriseur(Figure fig, WndUserRequest *e) {
...
/* ========================================================================== */
static void OrgaNumerAjoute(Figure fig, WndUserRequest *e, void *adrs) {
...
/* ========================================================================== */
static void OrgaNumerRetire(Figure fig, WndUserRequest *e, void *adrs) {
...
/* ========================================================================== */
static void OrgaNumerBranche(Figure fig, WndUserRequest *e, void *adrs) {
	...
	i = (int)adrs; if((bb = (OrgaNumerNum[i] - 1)) < 0) return;
...
/* ========================================================================== */
static void OrgaNumerDeplace(Figure fig, WndUserRequest *e, void *adrs, int x, int y) {
...
static TypeCablePosition OrgaCablageSlotVise(Figure fig) {
	Cadre cdr; short galette,tour; int xc,yc,xinf;
	...
	for(galette=OrgaGalMin; galette<OrgaGalMax; galette++) {
		...
		if(tour < TOURS_NB) break;
	}
	if(galette >= OrgaGalMax) return(CABLAGE_POS_UNDEF);
...
static TypeCablePosition OrgaCablageBrancheSlot(TypeCablePosition pos_hexa, void *adrs) {
	...
	if(pos_hexa == CABLAGE_POS_UNDEF) return(pos_hexa);
	if(CablageSortant[pos_hexa].type != -1) {
		bouton[0] = "Abandonner"; bouton[1] = "Remplacer";
		if(!OpiumChoix(2,bouton,"Emplacement %s deja occupe",CablageEncodePosition(pos_hexa))) return(CABLAGE_POS_UNDEF);
	}
	modl = (int)adrs;
...
static void OrgaCablageFischerAdefinir(TypeCablePosition pos_hexa) {
	...
	if(pos_hexa == CABLAGE_POS_UNDEF) return;
...
static void OrgaCablageFischerCopie(TypeCablePosition pos_nouv, TypeCablePosition pos_hexa) {
	...
	if((pos_nouv == CABLAGE_POS_UNDEF) || (pos_hexa == CABLAGE_POS_UNDEF)) return;
...
/* ========================================================================== */
static void OrgaCablageAjoute(Figure fig, WndUserRequest *e, void *adrs) {
	OrgaCablageFischerAdefinir(OrgaCablageBrancheSlot(OrgaCablageSlotVise(fig),adrs));
	OrgaMagasinRefait();
}
/* ========================================================================== */
static void OrgaCablageRetire(Figure fig, WndUserRequest *e, void *adrs) {
	int bolo; TypeCablePosition pos_hexa;

	pos_hexa = (TypeCablePosition)((int)adrs);
	if(pos_hexa == CABLAGE_POS_UNDEF) return;
...
/* ========================================================================== */
static void OrgaConnectSoude(Figure fig, WndUserRequest *e, void *adrs) {
...
/* ========================================================================== */
static void OrgaCablageDeplace(Figure fig, WndUserRequest *e, void *adrs, int x, int y) {
	...
	if(x < OrgaFinEtagere) OrgaCablageRetire(fig,e,adrs);
		...
		if(pos_nouv == CABLAGE_POS_UNDEF) return;
		pos_hexa = (TypeCablePosition)((int)adrs);
		if(pos_nouv != pos_hexa) {
			OrgaCablageBrancheSlot(pos_nouv,(void *)(UInt64)(CablageSortant[pos_hexa].type));
...
/* ========================================================================== */
static int OrgaDetecAjoute(Figure fig, WndUserRequest *e, void *adrs) {
	TypeDetModele *modele_det; int bolo;
	char nom_detec[DETEC_NOM],texte[80]; size_t lngr;
	char racine[MAXFILENAME],nom_complet[MAXFILENAME],*nom_partiel; int nb_erreurs;
	...
	snprintf(nom_complet,MAXFILENAME,"%s%s",racine,nom_detec);
	...
	bolo = BoloNb;
	nb_erreurs = DetecteurCree(&(Bolo[bolo]),nom_detec,modele_det->num,0,"local",nom_partiel);
	if(nb_erreurs) { OpiumError("Creation problematique (voir journal)"); return(0); }
	...
	if(!strncmp(nom_detec,modele_det->nom,lngr)) {
		int courant;
		if(nom_detec[lngr] == '_') lngr++;
...
/* ========================================================================== */
static void OrgaDetecDeplace(Figure fig, WndUserRequest *e, void *adrs, int x, int y) {
	char *nom_clique; int i,bolo; int xc,yc,xsup;
	char racine[MAXFILENAME],nom_complet[MAXFILENAME]; FILE *fichier;
	char lien[80];
	char replace; short galette,tour; int nb_erreurs;
...
/* ========================================================================== */
static void OrgaDetecRetire(Figure fig, WndUserRequest *e, void *adrs) {
	int i; char *nom_clique;
...
/* ========================================================================== */
static int OrgaAtelierClique(Figure fig, WndUserRequest *e) {
	OrgaMagMode = (char)((int)(fig->adrs));
	OrgaMagasinOnglet(OrgaCablageMontre);
	return(0);
}
/* ========================================================================== */
static int OrgaGaletteClique(Figure fig, WndUserRequest *e) {
	OrgaMagasinOnglet((int)(fig->adrs));
	return(0);
}
/* ========================================================================== */
static int OrgaMagasinClique(Figure cliquee, WndUserRequest *e) {
	Figure fig; int x,y; Cadre cdr; OrgaRef ref;
	TypeCablePosition pos_hexa;
	char poubellise;
		...
		if(poubellise) {
			...
			case ORGA_REF_CABLE:
				if(e->code == WND_MSELEFT) OrgaCablageDeplace(fig,e,ref->adrs,x,y); 
					...
					pos_hexa = (TypeCablePosition)((int)(ref->adrs));
...
//			} else strcpy(slot,"ext");
			...
			if(rang_bb != rang_prec) {
				...
				if(OrgaMagMode) {
					...
					ref = OrgaNouvelleRef(OrgaObjet,ORGA_REF_FISCHER,(void *)(UInt64)code,k,ORGA_MAXOBJ);
				...
				if(bb >= 0) {
					...
					if(OrgaMagMode) {
						...
						if(i < OrgaNumerNb) ref = OrgaNouvelleRef(OrgaObjet,ORGA_REF_NUMER,(void *)(UInt64)i,k,ORGA_MAXOBJ); else ref = 0;
...
#endif
				if(cap == 0) {
					ref = OrgaNouvelleRef(OrgaObjet,ORGA_REF_CABLE,(void *)((UInt64)pos_hexa),k,ORGA_MAXOBJ);
#ifdef AVEC_ICONE
		...
		if(CablageDeplie[pos_hexa]) {
			...
			for(cap=0; cap<modele_det->nbcapt; cap++) {
				...
				if(voie >= 0) fig = FigureCreate(FIG_ZONE,(void *)(&OrgaZoneCapt),(void *)(UInt64)voie,ReglageClique);
	...
	ref = OrgaNouvelleRef(OrgaObjet,ORGA_REF_POSITION,(void *)((UInt64)pos_hexa),k,ORGA_MAXOBJ);
#ifdef AVEC_ICONE
	...
	if(detectr) {
		...
		if(OrgaMagMode) {
			...
			if(i < OrgaDetecNb) ref = OrgaNouvelleRef(OrgaObjet,ORGA_REF_DETEC,(void *)(UInt64)i,k,ORGA_MAXOBJ); else ref = 0;
...
static void OrgaAjouteIconeBarre(TypeOrgaFig *orgafig, char *titre, ORGA_MODES mode, int *x, int y, int *ymin) {
	...
	fig = FigureCreate(OrgFigType[OrgaStyle],orgafig[OrgaStyle].modl,(void *)(UInt64)mode,OrgaAtelierClique);
...
int OrgaMagasinMontre() {
	...
	short galette,tour; short quadrant,distance;
...
//		BoardAddFigure(bOrgaCryostat,fig,x,y,0); x += (OrgaCadreVide.larg + 32);
	...
	if(OrgaMagMode == ORGA_MAG_REPART) {
		for(i=0; i<ModeleRepNb; i++) {
			ref = OrgaNouvelleRef(OrgaObjet,ORGA_REF_MODELE,(void *)(UInt64)i,&k,ORGA_MAXOBJ);
	...
	if(OrgaMagMode == ORGA_MAG_NUMER) {
		...
		for(i=0; i<OrgaNumerNb; i++) if(!OrgaNumerNum[i]) {
			ref = OrgaNouvelleRef(OrgaObjet,ORGA_REF_NUMER,(void *)(UInt64)i,&k,ORGA_MAXOBJ);
#ifdef AVEC_ICONE_NUMER
			fig = FigureCreate(OrgFigType[OrgaStyle],OrgaFigNumer[OrgaStyle].modl,(void *)ref,OrgaMagasinClique);
		...
		for(i=0; i<OrgaDetecNb; i++) if(!OrgaDetecNum[i]) {
			ref = OrgaNouvelleRef(OrgaObjet,ORGA_REF_DETEC,(void *)(UInt64)i,&k,ORGA_MAXOBJ);
#ifdef AVEC_ICONE
			fig = FigureCreate(OrgFigType[OrgaStyle],OrgaFigDetec[OrgaStyle].modl,(void *)ref,OrgaMagasinClique);
	...
	if(OrgaStyle == ORGA_ICONES) {
		if(OrgaGalettesNb > 1) for(i=0; i<OrgaGalettesNb; i++) {
			fig = FigureCreate(OrgFigType[OrgaStyle],OrgaFigGalette[OrgaStyle].modl,(void *)(UInt64)i,OrgaGaletteClique);
...
/*  Espaces specialises                                                       */
/* ========================================================================== */
static int OrgaCliqueSurMac(Figure fig, WndUserRequest *e) {
	if(e->code == WND_MSERIGHT) {
...
/* ========================================================================== */
static int OrgaEspaceNumerClique(Figure cliquee, WndUserRequest *e) {
	Figure fig; int x,y; Cadre cdr; OrgaRef ref;
...
static int OrgaEspaceNumerMontre() {
	...
	for(i=0; i<OrgaNumerNb; i++) if((bb = OrgaNumerNum[i]) > 0) {
		...
		ref = OrgaNouvelleRef(OrgaObjet,ORGA_REF_FIBRE,(void *)(UInt64)i,&k,ORGA_MAXOBJ);
...
/* ========================================================================== */
static int OrgaEspaceDetecClique(Figure cliquee, WndUserRequest *e) {
	Figure fig; int x,y; Cadre cdr; OrgaRef ref;
...
static int OrgaDetecteurTrace(TypeDetecteur *detectr, char croise, Cadre planche, TypeOrgaLiaisons *cablage, int x0, int y0) {
	...
	for(cap=0; cap<detectr->nbcapt; cap++) {
		...
		if(voie_normale) fig = FigureCreate(FIG_ZONE,(void *)(&OrgaZoneCapt),(void *)(UInt64)voie,ReglageClique);
		else fig = FigureCreate(FIG_ZONE,(void *)(&OrgaZoneCapt),0,0);
		FigureTexte(fig,nom_voie);
		BoardAddFigure(planche,fig,xcap,ycap,0);
		ycap += (OrgaZoneVoie.haut + ICONE_MARGE);
...
/* .......................................................................... */
static MenuItem iOrgaRepartLocaux[] = {
			...
			if(numeriseur && (icone == &OrgaZoneNumer)) {
				...
				if(OrgaRepartAfficheCryo) {
					...
					for(vbb=0; vbb<numeriseur->nbadc; vbb++) {
						...
						if(voie >= 0) {
							fig = FigureCreate(FIG_ZONE,(void *)(&OrgaZoneVoie),(void *)(UInt64)voie,ReglageClique);
...
/*  Organigrammes                                                             */
/* ========================================================================== */
static int OrgaCliqueSurMenu(Figure fig, WndUserRequest *e) {
	Menu menu;
