...
static int BasicParms[MAXPARMS];
...
static unsigned char BasicSmbCmde[2],BasicSmbC1,BasicSmbC2;
static unsigned short BasicSmbVal;
...
int BasicRecupere() {
	...
	do {
		...
		if(*r == '=') {
			...
			sscanf(ligne,"%x",&(BasicParms[BasicParmsNb++]));
		}
	} forever;
...
int BasicRepertoire() {
	...
	for(i=0; i<BasicBlocsNb; ) {
		sprintf(texte,"%s [%d]",BasicBloc[i].nom,BasicBloc[i].taille);
...
int BasicModifie() {
	...
	for(j=0; j<nb; j++) {
		...
		for(i=imin; i<imax; i++) {
			sprintf(BasicParmText[i],"%s[%d]",
				BasicBloc[BasicBlocNum].nom,i-BasicBloc[BasicBlocNum].debut+1);
			PanelHex(pBasicModifs,BasicParmText[i],&(BasicParms[i]));
		}
...
//	printf("Commandes @%08X, Donnees @%08X, Reset @%08X\n",BasicCmdes,BasicDonnees,BasicReset);
	...
	printf("%d detecteur%s x %d voie%s/detecteur, soit %d voie%s/bloc,\n.. accedee%s directement a l'adresse %08X.\n",
		Accord1s(Repart[BasicRep].nbentrees),Accord1s(Repart[BasicRep].voies_par_numer),Accord2s(Repart[BasicRep].nbdonnees),(hexa)BasicDonnees);
...
//++			TermRelease(tBasicBlocs);
	...
	printf("===== Lecture de %d mot%s @%08X =====\n",Accord1s(BasicNbLect),(hexa)BasicDonnees);
	nb = BasicNbLect;
	if(BasicReadOnly) {
		if(BasicWait) {
			nbzero = 0;
...
static int BasicIpMaj() {
	...
	*/
	char niveau; unsigned char buffer[4];
...
int BasicIpRaz() {
	...
	if(Repart[BasicRep].simule) Repart[BasicRep].lec.path = BasicRep;
	else {
		if((Repart[BasicRep].lec.path = SocketUDP(port)) < 0) {
			printf("            !!! Connexion pour lecture sur %s:%d impossible (%s)\n",Repart[BasicRep].parm,port,strerror(errno));
			return(0);
		}
		printf("            > Connexion prete pour la lecture (chemin #%d)\n",Repart[BasicRep].lec.path);
		if(fcntl(Repart[BasicRep].lec.path,F_SETFL,O_NONBLOCK) == -1) {
...
int BasicIpLit() {
//	socklen_t lngr;
	unsigned char buffer[BASIC_IPBLOC];
	unsigned int lngr;
	int nb_lus;

	lngr = sizeof(TypeSocket);
	nb_lus = (int)RepartIpLitTrame(&(Repart[BasicRep]),buffer,BASIC_IPBLOC,0,&(Repart[BasicRep].lec.skt),&lngr);
	if(BigEndianOrder) ByteSwappeIntArray((unsigned int *)buffer,nb_lus / 4);
...
int BasicCaliRaz() {
	...
	printf("            > Affectation du port %d pour la lecture\n",port);
	if(repart->simule) repart->lec.path = BasicRep;
	else {
		if((repart->lec.path = SocketUDP(port)) < 0) {
...
int BasicCaliLit() {
	//	socklen_t lngr;
	unsigned char buffer[BASIC_IPBLOC]; unsigned int lngr;
	ssize_t nb_lus;
	
	lngr = sizeof(TypeSocket);
	nb_lus = RepartIpLitTrame(&(Repart[BasicRep]),buffer,BASIC_IPBLOC,0,&(Repart[BasicRep].lec.skt),&lngr);
	if(nb_lus < 0) printf("          ! Pas de trame (%s)\n",strerror(errno));
	else if(!nb_lus) printf("          ! Trame vide\n");
	else {
	printf("          * Trame avec %ld octets\n",nb_lus);
		if(!BigEndianOrder) CaliSwappeTrame(&(Repart[BasicRep]),(CaliTrame)buffer,(int)nb_lus);
		CaliDump(&(Repart[BasicRep]),buffer,(int)nb_lus,BasicDumpHexa);
	}
	return(0);
}
/* ========================================================================== */
int BasicSmbRaz() {
	...
	SmbEcrit(repart,SMB_CONNECT,(unsigned short)port);
	printf("          > Ecrit: '%s'\n",RepartiteurValeurEnvoyee);
	return(0);
}
/* ========================================================================== */
int BasicSmbEcrit() {
	...
	printf("          * Ecriture de '%s' via <%d> [%d/5]\n",RepartiteurValeurEnvoyee,Repart[BasicRep].ecr.path,n);
	if(n < 0) printf("          ! en erreur: %s\n",strerror(errno));
	return(0);
}
/* ========================================================================== */
...
static int BasicChangeAdrs(Panel p, void *arg) {
	...
	printf("          * Lecture PCI a l'adresse %08X\n",(hexa)BasicDonnees);
	return(1);
}
/* ========================================================================== */
MenuItem iBasicDefinitions[] = {
...
MenuItem iBasicDestin[] = {
	{ "pci",     MNU_CONSTANTE 1 },
	{ "cc",      MNU_CONSTANTE 2 },
	{ "quitter", MNU_CONSTANTE 9 },
	MNU_END
};
MenuItem iBasicStyleMiG[] = {
...
//-	OpiumEnterFctn(mBasic->cdr,AccesRestreint,0);
	...
	PanelFormat(pBasicDirectICA,PanelHex(pBasicDirectICA,"Adresse",&BasicAdrsICA),"%02X");
	PanelFormat(pBasicDirectICA,PanelHex(pBasicDirectICA,"Valeur",&BasicValICA),"%08X");
	pBasicDirectCC = PanelCreate(3);
	PanelFormat(pBasicDirectCC,PanelHex(pBasicDirectCC,"Adresse",&BasicAdrsCC),"%02X");
	PanelFormat(pBasicDirectCC,PanelHex(pBasicDirectCC,"Sous-adresse",&BasicSsAdrsCC),"%02X");
	PanelFormat(pBasicDirectCC,PanelHex(pBasicDirectCC,"Valeur",&BasicValCC),"%04X");
	mBasicStyleMiG = MenuLocalise(iBasicStyleMiG);
	pBasicOctet = PanelCreate(1);
	PanelFormat(pBasicOctet,PanelHex(pBasicOctet,"Octet a ecrire",&BasicOctet),"%02X");
#ifdef AVEC_PCI
	...
	for(i=0; i<ICA_REG_NB; i++) {
		...
		PanelItemSelect(pBasicRegsICA,PanelFormat(pBasicRegsICA,j,"%08X"),0);
	}
