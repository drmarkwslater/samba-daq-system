...
// #define DEBUG_STATUS_BBV2
	...
	#define MAXSTATUSDECODE 64
		static unsigned short StatusCode[MAXSTATUSCODE],StatusDecode[MAXSTATUSDECODE];
		static short BBlu[MAXSTATUSCODE];
		static int IndexStatusCode,IndexStatusDecode;
	#define MAXTRAMES 4
		static unsigned short BOtrame0[MAXTRAMES][sizeof(TypeOperaTrame)/sizeof(unsigned short)];
	#ifdef SOFT_AB
		static unsigned short ABtrame0[MAXTRAMES][sizeof(TypeOperaTrame)/sizeof(unsigned short)];
	#endif
#endif

static char  LectSigne,LectParBloc;
static char  LectDepileSousIt,LectTrmtEnCours;
static char  LectRunNormal,LectMaintenance,ReglagesEtatPeutStopper;
...
static char  LectReglagesOuverts,LectBBaffiches,LectRepAffiches;
static int   LectRepNbAdemarrer;
...
static int64 LectSimulProchain;
...
static TypeFigZone LectMonitCadreEvt,LectMonitCadreCntl,LectMonitCadreAnalyse,LectMonitCadreCapacite; // LectMonitCadreSignal
static TypeFigZone LectMonitCadreBuffDump;
...
static int LectCompacteCliquePlanche(Cadre planche, WndUserRequest *e, void *arg);
...
int LectSpectreTampons() {
	int bolo,cap,vp,voie; int nbfreq,dim,n;
	...
	for(bolo=0; bolo<BoloNb; bolo++) if(CalcAutoBolo[bolo]) {
		for(cap=0; cap<Bolo[bolo].nbcapt; cap++) {
			...
			nbfreq = CalcSpectreAuto[vp].nbfreq = (CalcSpectreAuto[vp].dim / 2) + 1;
...
static char LectFixeMode(LECT_MODE mode, char init) {
	...
	if(LectureLog) {
		//#ifdef ADRS_PCI_DISPO
		//	printf("==================== Demarrage de %s (@%08X) ====================\n",LectSrceTexte,(hexa)PCIadrs[PCInum]);
		//#endif
		if(LectSurFichier) {
			// printf("============= Mise en route de la lecture du fichier %s: %s =============\n",SrceNoms[SrceType],SrceName);
			printf("============= Mise en route de la lecture du fichier %s: %s =============\n",(SrceType == SRC_REEL)?"reel":"simule",SrceName);
		} else {
			// printf("==================== Mise en route de %s ====================\n",LectSrceTexte);
			k = 86;
			printf("\n\n");
			printf(" ====================================================================================\n");
	...
	if(init) {
		...
		if(LectReglagesOuverts || LectBBaffiches || LectRepAffiches || ReglVi.actif) {
			LectMaintenance = 1;
			CalcAuVolDemande = 0; CalcAuVolActif = 0;
		}
		if(CalcAuVolDemande) CalcAuVolActif = 1;
...
static void LectSelecteVoies() {
	int bolo,cap,voie,v,vp,bb,fmt; Oscillo oscillo; // short num_hdr[ARCH_TYPEDATA];
	int i,j,l;
	...
	for(voie=0; voie<VoiesNb; voie++) {
        VoieTampon[voie].lue = 0; VoieTampon[voie].sauvee = NEANT;
	...
	if(LectModeSpectresAuto) {
		for(bolo=0; bolo<BoloNb; bolo++) if(CalcAutoBolo[bolo]) {
			for(cap=0; cap<Bolo[bolo].nbcapt; cap++) {
				...
				vp = ModeleVoie[VoieManip[voie].type].physique;
				if(CalcAutoPhys[vp] && (CalcSpectreAuto[vp].dim > 0)) {
					if((VoieTampon[voie].trig.trgr = (TypeTrigger *)malloc(sizeof(TypeTrigger)))) {
						Trigger.enservice = 0;
	...
	if(LectMaintenance || LectModeSpectresAuto) {
		for(i=0, f=MonitFen; i<MonitFenNb; i++,f++) if(f->affiche >= 2) {
	...
	if(LectModeSpectresBanc > 0) BancSptrSelecteVoies(log);
		...
		for(voie=0; voie<VoiesNb; voie++) {
			...
			if(bolo < 0) {
				...
				int vp;
				vp = ModeleVoie[VoieManip[voie].type].physique;
				VoieTampon[voie].lue = CalcAutoBolo[bolo] && (CalcSpectreAuto[vp].dim > 0);
...
/* Seulement hors mode=LECT_IDENT */
	int voie,bolo,cap,classe,modele,ph,i,j,k;
//?	char *svt,*cur; char texte[TRMT_NOM];
	char trmt,nb_templates;
	int compactage;
	short d2;
	char meme_horloge,tampon_traitees_ajuste,voies_regulees;
	int nbpts,memoire,manque;
		...
		for(classe = 0; classe < TRMT_NBCLASSES; classe++) {
			...
			switch(trmt) {
			  case TRMT_DEMODUL:
				...
				if(VoieManip[voie].def.trmt[classe].parm >= (d2 / 2)) VoieManip[voie].def.trmt[classe].parm = (d2 / 2) - 1;
				VoieTampon[voie].trmt[classe].nbdata = (d2 / 2) - VoieManip[voie].def.trmt[classe].parm;
				VoieTampon[voie].trmt[classe].filtre = 0;
			  ...
			  case TRMT_LISSAGE:
				if(VoieManip[voie].def.trmt[classe].parm <= 0) VoieManip[voie].def.trmt[classe].parm = 1;
				VoieTampon[voie].trmt[classe].compactage = 1;
				...
				VoieTampon[voie].trmt[classe].X.dim = VoieManip[voie].def.trmt[classe].parm + 1;
			  ...
			  case TRMT_MAXIMUM:
				if(VoieManip[voie].def.trmt[classe].parm <= 0) VoieManip[voie].def.trmt[classe].parm = 1;
				VoieTampon[voie].trmt[classe].compactage = VoieManip[voie].def.trmt[classe].parm;
				VoieTampon[voie].trmt[classe].nbdata = 0;
				VoieTampon[voie].trmt[classe].filtre = 0;
				snprintf(VoieManip[voie].def.trmt[classe].texte,TRMT_NOM,"sur %d pts",VoieManip[voie].def.trmt[classe].parm);
				break;
			  case TRMT_DECONV:
				...
				VoieTampon[voie].trmt[classe].nbdata = 0;
				VoieTampon[voie].trmt[classe].filtre = 0;
				snprintf(VoieManip[voie].def.trmt[classe].texte,TRMT_NOM,"attenuation %d",VoieManip[voie].def.trmt[classe].parm);
				break;
			#ifdef A_LA_GG
			  case TRMT_INTEDIF:
				strcpy(texte,VoieManip[voie].def.trmt[classe].texte);
				svt = texte; cur = ItemJusquA(',',&svt);
				sscanf(cur,"%d",&(VoieManip[voie].def.trmt[classe].parm));
				sscanf(svt,"%d",&(VoieManip[voie].def.trmt[classe].parm2));
				if(VoieManip[voie].def.trmt[classe].parm <= 0) VoieManip[voie].def.trmt[classe].parm = 7;
				if(VoieManip[voie].def.trmt[classe].parm2 <= 0) VoieManip[voie].def.trmt[classe].parm2 = 20;
				VoieTampon[voie].trmt[classe].compactage = 1;
				VoieTampon[voie].trmt[classe].nbdata = 0;
				VoieTampon[voie].trmt[classe].filtre = 0;
				// snprintf(VoieManip[voie].def.trmt[classe].texte,TRMT_NOM,"sur %d+%d pts",VoieManip[voie].def.trmt[classe].parm,VoieManip[voie].def.trmt[classe].parm2);
				VoieTampon[voie].trmt[classe].X.dim = VoieManip[voie].def.trmt[classe].parm + 1;
				if(VoieTampon[voie].trmt[classe].X.max != VoieTampon[voie].trmt[classe].X.dim) {
					if(VoieTampon[voie].trmt[classe].X.val) {
						free(VoieTampon[voie].trmt[classe].X.val);
				...
				VoieTampon[voie].trmt[classe].X.nb = VoieTampon[voie].trmt[classe].X.prem = 0;
				VoieTampon[voie].trmt[classe].Y.dim = VoieManip[voie].def.trmt[classe].parm2 + 1;
			  ...
			  case TRMT_CSNSM:
				if((VoieManip[voie].def.trmt[classe].parm < 0) || (VoieManip[voie].def.trmt[classe].parm >= TrmtCsnsmNb)) VoieManip[voie].def.trmt[classe].parm = 0;
				i = VoieManip[voie].def.trmt[classe].parm;
				VoieTampon[voie].trmt[classe].compactage = 1;
				VoieTampon[voie].trmt[classe].nbdata = 0;
				VoieTampon[voie].trmt[classe].filtre = 0;
			  ...
			  case TRMT_FILTRAGE:
				...
				} else k = VoieManip[voie].def.trmt[classe].parm;
				if((k >= 0) && (k < FiltreNb)) {
					filtrage = &(FiltreGeneral[k]);
					...
					if(VoieManip[voie].def.trmt[classe].texte[0] == '\0') strncpy(VoieManip[voie].def.trmt[classe].texte,FiltreGeneral[k].nom,TRMT_NOM);
				} else filtre = 0;
 ...
 */
	...
	/* resultat du traitement pre-trigger */
        ...
        if(EvtASauver < 0) {
            if(VoieTampon[voie].brutes->type == TAMPON_INT16)
                printf("(%s) %s, tampon Sbrut cree (%08x)\n",__func__,VoieManip[voie].nom,(hexa)VoieTampon[voie].brutes->t.sval);
            else printf("(%s) %s, tampon Rbrut cree (%08x)\n",__func__,VoieManip[voie].nom,(hexa)VoieTampon[voie].brutes->t.rval);
            if(VoieTampon[voie].traitees->type == TAMPON_INT16)
                printf("(%s) %s, tampon Strig cree (%08x)\n",__func__,VoieManip[voie].nom,(hexa)VoieTampon[voie].traitees->t.sval);
            else printf("(%s) %s, tampon Rtrig cree (%08x)\n",__func__,VoieManip[voie].nom,(hexa)VoieTampon[voie].traitees->t.rval);
        }
	...
	for(voie=0; voie<VoiesNb; voie++) if(VoieTampon[voie].lue) {
		if(VoieManip[voie].def.trmt[TRMT_PRETRG].template == TRMT_TEMPL_DB) {
			...
			if(VoieManip[voie].simu >= 0) { TemplatesCalcule(voie,0); VoieTampon[voie].delai_template = 0; }
	...
	if(LectCntl.LectMode == LECT_DONNEES) {
		...
		for(i=0, f=MonitFen; i<MonitFenNb; i++,f++) {
			...
			MonitFenMemo(f);
			affiche = (LectMaintenance || LectModeSpectresAuto)? (f->affiche >= 2): f->affiche;
			if(affiche) {
			...
			} else { MonitFenClear(f); MonitFenFree(f); }
		}
#ifdef NTUPLES_ONLINE
		if(CalcNtp && (SettingsNtupleMax != NtDim)) {
			if(NtDim) { PlotNtupleSupprime(); NtDim = 0; }
			NtUsed = SettingsNtupleMax;  /* Juste pour la creation des adresses */
			if(PlotNtupleCree(NtUsed,0.0)) NtDim = SettingsNtupleMax;
		}
#endif
		if(MonitAffgSeuils && gSeuils && TrmtRegulActive) {
			int x,y,k;
			GraphErase(gSeuils);
		...
		if(MonitAffgTaux && gTauxDetaille) {
			int x,y,k;
...
#endif
#ifdef AJOUTE_COSINUS
		for(j=0; j<VoieTampon[voie].brutes->max; j++) VoieTampon[voie].brutes->t.sval[j] = LectTrmtInitValBrute;
		for(j=0; j<VoieTampon[voie].traitees->max; j++) VoieTampon[voie].traitees->t.rval[j] = LectTrmtInitValTraitee;
#else
		for(j=0; j<VoieTampon[voie].brutes->max; j++) VoieTampon[voie].brutes->t.sval[j] = 0;
...
static void LectRazPartiel() {
	...
	LectNonNul = 0;
#endif
	mc_init(TestEnergie,TestResolution);
	LectSimulProchain = 0;
#ifdef BRANCHE_TESTS
...
//	else if(VerifConsoCpu) InstrumILimits(cCpuTauxOccupe,0,400);
	...
	DernierPointTraite = TrmtLastEvt = 0; /* refait dans TrmtRAZ() */
	...
	LectStampPerdus = LectStampJetes = LectValZappees = 0;
		...
		if(rep >= RepartNb) { OpiumError("Pas de repartiteur capable de donner une synchro D3"); break; }
			...
			unless(RepartiteurRepereD3(repart,&LectEnCours.valeur,LectureLog)) {
				...
				if(LectAcqCollective && (LectSession < 2)) {
					...
					RepartD3Attendues = 1;
				} else if(LectRepNbAdemarrer > 1) SambaAttends(SettingsD3wait); /* ms */
...
/* Vide la pile (PCI ou IP) jusqu'a l'apparition du debut de la synchro (D2 ou D3) */
	...
	byte msg_ip[UDP_SYNC_LNGR]; unsigned int lngr; int lu;
	...
	ze_repart = &(Repart[0]); 
	LectRepNbAdemarrer = 0; for(rep=0; rep<RepartNb; rep++) if(Repart[rep].actif) LectRepNbAdemarrer++;
	unless(LectRepNbAdemarrer) printf("        ! Bizarre... aucun repartiteur n'a ete active!!\n");
	...
	if(LectSurFichier) {
		...
		/* rc = Lect1Val();
		Repart[RepartFile].fifo16[Repart[RepartFile].bottom] = (LectEnCours.valeur & 0xFFFF);
...
//		}
		...
		if(((RepartD3Attendues && (LectRepNbAdemarrer > 1)) || LectAcqCollective || LectDureeReset) && !LectStampMini) {
		...
		if(LectModeStatus) {
			...
			if(LectureLog) printf("%s/ Recherche d'une synchro %s%s\n",DateHeure(),RepartD3Attendues?"D3":"D2",(LectRepNbAdemarrer > 1)?" commune":"");
			synchro1.tv_sec = 0;
//#define START_D3_SERIE
		...
		for(rep=0; rep<RepartNb; rep++) if(Repart[rep].actif) {
			...
			repart->simu.prochaine = DateMicroSecs();
		}
		if(LectModeStatus) printf("          . Reference du timestamp: %s\n",(LectRefStamp >= 0)? Repart[LectRefStamp].nom: "aucune");
		if(nb_actifs) {
			if(nb_actifs == LectRepNbAdemarrer) printf("          . %d repartiteur%s en service\n",Accord1s(nb_actifs));
			else {
				printf("          ! Seulement %d repartiteur%s utilise%s sur %d prevu%s\n",Accord2s(nb_actifs),Accord1s(LectRepNbAdemarrer));
				LectRepNbAdemarrer = nb_actifs;
			}
		} else {
	...
	for(rep=0; rep<RepartNb; rep++) if(Repart[rep].actif && Repart[rep].simule) Repart[rep].simu.prochaine = LectT0Run;
...
//						   DateHeure(),EvtASauver,(int)LectDebutEvts,(int)(LectStampsLus-UsageTampons),(int)UsageTampons,(int)LectStampsLus);
			...
			if(FichierStatusBB[0]) {
				...
				if(f) {
					...
					for(bb=0; bb<NumeriseurNb; bb++) if(Numeriseur[bb].enreg_stamp) {
						...
						fwrite(Numeriseur[bb].enreg_val,sizeof(unsigned short),ModeleBN[Numeriseur[bb].def.type].enreg_dim,f);
					}
					fclose(f);
					LectStatusDispo = 0;
				} else {
	...
	/* for(voie=0; voie<VoiesNb; voie++) if(prochain[voie] != VoieTampon[voie].prochain) {
		printf("(%s) %16s @%08X -> %08X [%.1f]\n",__func__,VoieManip[voie].nom,
			(hexa)(prochain[voie]),(hexa)(VoieTampon[voie].prochain),
...
//2		nb = SynchroD2lues / LectIntervUser;
//2		LectNextUser = (nb + 1) * LectIntervUser;
		LectNextUser = SynchroD2lues + LectIntervUser;
...
static void LectAfficheEvt() {
	...
	MonitEvtAmpl = EvtPartage->amplitude;
...
static void LectFenRefresh(TypeMonitFenetre *f) {
	Graph g; Cadre cdr; TypeMonitTrace *t; char doit_terminer;
	...
	if(!f->affiche || !(g = f->g)) return;
	if(LectMaintenance || LectModeSpectresAuto) { if(f->affiche < 2) return; }
	cdr = g->cdr;
	...
	if(f->type == MONIT_SIGNAL) {
		...
		for(j=0, t=f->trace; j<f->nb; j++,t++) {
			...
			if(trmt == TRMT_AU_VOL) {
				...
				//	VoieTampon[voie].brutes->nb,VoieTampon[voie].brutes->max,VoieTampon[voie].trmt[TRMT_AU_VOL].plus_ancien,VoieTampon[voie].circulaire?"complet":"partiel",position,VoieTampon[voie].brutes->prem);
			#ifdef A_RE_EXAMINER
...
/* ========================================================================== */
void LectDisplay() {
	int i,j,k,n,bolo,voie,vt,bb,v;
	char trmt;
	TypeMonitFenetre *f; TypeMonitTrace *t; Graph g; // Cadre cdr;
	TypeNumeriseur *numeriseur; Oscillo oscillo;
	float temps,expo;
	...
	char status_montre,affiche_evt,doit_terminer,taux_change;
	...
	LectDansDisplay = 1;
	temps = 0.0; cpu_utilise = 0; temps_utilise = 0; taux_change = 0;// GCC
	// printf("(%s) Appel apres %lld synchroD2 lues\n",__func__,SynchroD2lues);
/*
	...
	for(voie=0; voie<VoiesNb; voie++) if(VoieInfo[voie].planche && OpiumDisplayed(VoieInfo[voie].planche)) {
		...
		if((oscillo = VoieInfo[voie].oscillo)) {
			...
			if(!VoieManip[voie].modulee || oscillo->demodule) {
				...
				if(lngr) parm = VoieManip[voie].def.trmt[TRMT_AU_VOL].parm;
		...
		for(num=0; num<OscilloNb; num++) {
			...
			for(v=0; v<oscillo->nbvoies; v++) {
				...
				if(!VoieManip[voie].modulee || oscillo->demodule) {
					...
					// if(nbdata > 0.0) VoieInfo[voie].bruit = (float)valeur/ nbdata; else VoieInfo[voie].bruit = 0.0;
					if((nbsup > 0.0) && (nbinf > 0.0)) {
						VoieInfo[voie].moyenne = 0.5 * (((float)valsup / nbsup) + ((float)valinf / nbinf));
						VoieInfo[voie].bruit = (((float)valsup / nbsup) - ((float)valinf / nbinf));
					} else VoieInfo[voie].moyenne = VoieInfo[voie].bruit = 0.0;
...
//-			for(v=0; v<ReglVi.nb_voies; v++) printf("|_______|_______|_______"); printf("|\n"); // temporaire
	...
	if(Trigger.enservice) {
		...
		if(LectDateRun.tv_sec > LectProchainTaux) {
			...
			if(Acquis[AcquisLocale].etat.evt_trouves == LectEvtVus) {
				if(DernierPointTraite != TrmtLastEvt) {
					LectCntl.EvtTaux = 1.0 / ((float)(DernierPointTraite - TrmtLastEvt) * EnSecondes); /* secondes car Echantillonnage en kHz */
				} else if(TrmtDelai > 0.0) LectCntl.EvtTaux = 1.0 / TrmtDelai; /* Hz */
				else LectCntl.EvtTaux = 0.0;
			}
			LectEvtVus = Acquis[AcquisLocale].etat.evt_trouves; LectDernierEvt = TrmtLastEvt;
			...
			if((MonitAffpTaux && pTauxDetaille) || (MonitAffgTaux && gTauxDetaille)) {
				if(MonitAffgTaux && gTauxDetaille) { LectStockeTauxVoies(); taux_change = 1; }
			}
			if(LectCntl.EvtTaux > LectTauxSeuil) {
				// OpiumFail("Taux d'evenements EXCESSIF !! (%g / %g Hz)",LectCntl.EvtTaux,LectTauxSeuil);
				printf("%s/ Taux d'evenements en exces, a %g Hz (contre %g Hz permis)\n",DateHeure(),LectCntl.EvtTaux,LectTauxSeuil);
				LectDeclencheErreur(_ORIGINE_,70,LECT_EVTS);
			}
		}
		/* if(OpiumDisplayed(bLecture)) {
			double mag,nbchiffres; float min,max;
			if(LectCntl.EvtTaux > 0.0) {
				mag = log10((double)LectCntl.EvtTaux);
				modf(mag,&nbchiffres);
		...
		affiche_evt = (LectAffEvt && MonitAffEvt && !LectReglagesOuverts); // && (LectCntl.MonitEvtNum != MonitEvtAff)
#ifdef PARMS_RESEAU
...
#endif
				MonitEvtAffiche(/* EvtAffichable */0,0,0);
 ...
 */
	if(SambaInfos > 0) {
		...
		SambaInfos->taux_actuel = LectCntl.EvtTaux;
...
/*		Si poussoir 'suspendre/activer trigger' dans planche bLecture
		if(Trigger.actif != TriggerActif) {
			...
			if(TriggerActif) MenuItemAllume(mLectTrigger,1,"Suspendre",GRF_RGB_GREY);
			else MenuItemAllume(mLectTrigger,1," Activer ",GRF_RGB_YELLOW);
...
monite:
	...
	for(i=0, f=MonitFen; i<MonitFenNb; i++,f++) if(f->affiche && (g = f->g)) {
		if(LectMaintenance || LectModeSpectresAuto) { if(f->affiche < 2) continue; }
...
*/
	if(CalcAuVolActif && Acquis[AcquisLocale].etat.active) {
		int i,j,action;
		...
		tampon = &(VoieTampon[CalcVoie].trmt[CalcDonnees].donnees);
		if(!tampon->max) goto suite1;
        lus = 0; premier = 0;
		if(CalcDonnees == TRMT_AU_VOL) {
			// tampon = &(VoieTampon[CalcVoie].brutes);
			lus = VoieTampon[CalcVoie].lus;
			premier = VoieTampon[CalcVoie].trmt[TRMT_AU_VOL].plus_ancien;
			if((lus < (int64)CalcDim) && (premier <= 0)) goto suite1;
		} else if(CalcDonnees == TRMT_PRETRG) {
			// tampon = &(VoieTampon[CalcVoie].traitees);
			lus = tampon->prem + tampon->nb;
			premier = tampon->prem;
		}
		pt0 = lus - (int64)CalcDim;
		/* if(pt0 < premier) {
			pt0 = premier;
			if((pt0 + (int64)CalcDim) > lus) CalcDim = (int)(lus - pt0);
		} */
		...
		action = 2;
		if((CalcModeAuVol == CALC_SPTV_MANU) || (CalcModeAuVol == CALC_SPTV_SURV)) {
	/*		{
				printf("Nouvelle FFT @%08X -> %08X:",CalcFftFreq,CalcAuVolSpectreNouveau);
				for(i=0; i<CalcDim; i++) {
					if(!(i % 10)) printf("\n%4d|",i);
					printf(" %10.0f",CalcFftFreq[i]);
				}
				printf("\n");
			}*/
			CalculePuissance(CalcFftFreq,CalcAuVolSpectreNouveau,CalcDim,CalcAuVolNorme,VoieManip[CalcVoie].ADUennV);
			if(CalcModeAuVol == CALC_SPTV_MANU) {
				if(CalcAuVolNbSpectres < 0.5) {
					GraphDataReplace(CalcAuVolGraphe,1,GRF_FLOAT|GRF_YAXIS,CalcAuVolSpectreNouveau,CalcAuVolNbFreq);
					GraphDataRGB(CalcAuVolGraphe,1,GRF_RGB_YELLOW);
					GraphDataName(CalcAuVolGraphe,1,"A ajouter");
					GraphDataDisconnect(CalcAuVolGraphe,2);
				} else {
					if(CalcAuVolNbSpectres < 1.5) {
						GraphDataReplace(CalcAuVolGraphe,1,GRF_FLOAT|GRF_YAXIS,CalcAuVolSpectreMoyenne,CalcAuVolNbFreq);
						GraphDataRGB(CalcAuVolGraphe,1,GRF_RGB_GREEN);
						GraphReset(CalcAuVolGraphe);
					}
					GraphDataReplace(CalcAuVolGraphe,2,GRF_FLOAT|GRF_YAXIS,CalcAuVolSpectreNouveau,CalcAuVolNbFreq);
				}
				OpiumDisplay(CalcAuVolGraphe->cdr);
		...
		//		OpiumPrioritaire = 0;
			} else {  /* (CalcModeAuVol == CALC_SPTV_SURV) */
				if(CalcAuVolNbSpectres < 1.5)
					GraphDataReplace(CalcAuVolGraphe,2,GRF_FLOAT|GRF_YAXIS,CalcAuVolSpectreNouveau,CalcAuVolNbFreq);
			}
		}
		if(action == 2) {
			if(CalcModeAuVol == CALC_SPTV_GLIS) {
				CalcAuVolNbSpectres = 1.0;
				CalcAuVolCarreSomme[0] = (CalcAuVolCarreSomme[0] * (1.0 - CalcGlissantEpsilon))
					 + (CalcGlissantEpsilon * CalcFftFreq[0] * CalcFftFreq[0] * CalcAuVolNorme);
				for(i=1; i<(CalcDim + 1) / 2; i++) {
					CalcAuVolCarreSomme[i] = (CalcAuVolCarreSomme[i] * (1.0 - CalcGlissantEpsilon))
						+ (CalcGlissantEpsilon * ((CalcFftFreq[i] * CalcFftFreq[i]) + (CalcFftFreq[CalcDim - i] * CalcFftFreq[CalcDim - i])) * CalcAuVolNorme);
				}
				if(!(CalcDim % 2)) CalcAuVolCarreSomme[CalcDim / 2] = (CalcAuVolCarreSomme[CalcDim / 2] * (1.0 - CalcGlissantEpsilon))
					+ (CalcGlissantEpsilon * CalcFftFreq[CalcDim / 2] * CalcFftFreq[CalcDim / 2] * CalcAuVolNorme);
				sprintf(nom,"Moyenne/%d spectres",CalcSpectresMax);
			} else /* if(CalcModeAuVol == CALC_SPTV_AUTO, _MANU et _SURV) */ {
				CalcAuVolNbSpectres += 1.0;
				CalcAuVolCarreSomme[0] = CalcAuVolCarreSomme[0] + (CalcFftFreq[0] * CalcFftFreq[0] * CalcAuVolNorme);
				for(i=1; i<(CalcDim + 1) / 2; i++) {
					CalcAuVolCarreSomme[i] = CalcAuVolCarreSomme[i]
						+ (((CalcFftFreq[i] * CalcFftFreq[i]) + (CalcFftFreq[CalcDim - i] * CalcFftFreq[CalcDim - i])) * CalcAuVolNorme);
				}
				if(!(CalcDim % 2)) CalcAuVolCarreSomme[CalcDim / 2] = CalcAuVolCarreSomme[CalcDim / 2]
					+ (CalcFftFreq[CalcDim / 2] * CalcFftFreq[CalcDim / 2] * CalcAuVolNorme);
				sprintf(nom,"Moyenne/%g spectres",CalcAuVolNbSpectres);
			}
/*			{
				printf("FFT moyenne @%08X -> %08X sur %g:",CalcAuVolCarreMoyenne,CalcAuVolCarreSomme,CalcAuVolNbSpectres);
				for(i=0; i<CalcDim; i++) {
					if(!(i % 10)) printf("\n%4d|",i);
					printf(" %10.0f",CalcAuVolCarreMoyenne[i]);
				}
				printf("\n");
			}*/
			unite = VoieManip[CalcVoie].nV_reels; if(unite < 0.0) unite = -unite; // classiquement ADUennV au lieu de nV_reels
			for(i=0; i<CalcAuVolNbFreq; i++) {
				CalcAuVolSpectreMoyenne[i] = unite * sqrtf(CalcAuVolCarreSomme[i] / CalcAuVolNbSpectres);
			}
			GraphDataName(CalcAuVolGraphe,1,nom);
		} // else if(action == 3) CalcAuVolActif = 0;
		if(CalcModeAuVol != CALC_SPTV_SURV) GraphDataDisconnect(CalcAuVolGraphe,2);
		OpiumDisplay(CalcAuVolGraphe->cdr);
	}
suite1:
#ifdef VISU_FONCTION_TRANSFERT
...
*/
	if(PicsNb) {
		...
		for(k=0; k<PicsNb; k++) {
			...
			fond = 0.0;
			if(FitGaussienne(PicX,PicY,PicV,j,fond,&amp,&sigma,&x0) > 0) {
...
fin_pics:
	...
	if(MonitH2D) {
		if(MonitH2DY[0] == MonitH2DY[1]) {
			...
			GraphDataIntGamma(g2D,2,min,max,0,DIM_LUT - 1);
		}
	...
	if(taux_change /* && MonitAffgTaux && gTauxDetaille */) {
...
static int LectSauveSpectre() {
	...
	total = 0;
	for(i=0, f=MonitFen; i<MonitFenNb; i++,f++) if(f->affiche && (f->type == MONIT_FFT) && (g = f->g)) {
		for(j=0, t=f->trace; j<f->nb; j++,t++) {
			voie = t->voie;
			trmt = (int)t->var;
			if(sauve[voie][trmt]) {
	...
	for(classe=0; classe<TRMT_NBCLASSES; classe++) {
		if(voie >= 0) {
			...
			switch(trmt) {
			  ...
			  case TRMT_DECONV:   printf("%-14sx%2d x%-3d|",TrmtTypeListe[trmt],gain,parm); break;
			#ifdef A_LA_GG
			  case TRMT_INTEDIF:  printf("%-14sx%d /%d,%-2d|",TrmtTypeListe[trmt],gain,parm,VoieManip[voie].def.trmt[classe].parm2); break;
			#endif /* A_LA_GG */
			#ifdef A_LA_MARINIERE
...
static void LectJournalLaSuite(char log_tout) {
	...
	if(log_tout) {
		...
		printf("  Lecture                  : "); if(SettingsMultitasks) printf("toutes les %2d ms, ",SettingsReadoutPeriod); printf("%s\n",LectModeTexte[SettingsMultitasks]);
...
static void LectJournalPMD() {
	...
	if(LectDecodeStatus) {
		...
		for(k=0; k<MAXTRAMES; k++) {
			...
			for(i=0; i<sizeof(TypeOperaTrame)/sizeof(unsigned short); i++) {
				if(!((i - 18) % 96)) printf("\n[%d]",(i - 18)/96);
				if(!(i % 6)) printf("\n%4d:",i);
...
static void LectJournalLaFin(TypeADU rc, char avec_physique, char avec_perfos) {
	...
	if(LectCntl.LectMode == LECT_COMP) {
		...
		if(LectStampPerdus) printf("  ! tous tampons: %lld valeurs non recues  (%.3f s, TEMPS MORT: %.1f%%)\n",
			LectStampPerdus,(float)LectStampPerdus * EnSecondes,LectStampsLus? 100.0 * (double)LectStampPerdus / (double)LectStampsLus: 100.0);
		if(LectStampJetes) printf("  ! tous tampons: %lld valeurs non traitees (%.3f s, TEMPS MORT: %.1f%%)\n",
			LectStampJetes,(float)LectStampJetes * EnSecondes,LectStampsLus? 100.0 * (double)LectStampJetes / (double)LectStampsLus: 100.0);
		else if(LectValZappees) printf("  ! tous tampons: %lld valeurs non sauvees (%.3f s, TEMPS MORT: %.1f%%)\n",
...
//?		if(!ok) printf("\n");
		...
		if(avec_perfos) {
			...
			else if(LectRepNbAdemarrer > 1) LectJournalCPU(tache,TempsTotalLect,"soit total ",LectUsageInterfacesToutes,temps_reel);
...
/* ========================================================================== */
INLINE char SambaTraiteDonnee(int classe, char trmt, int voie, TypeDonnee donnee, char d3, int64 index, TypeDonnee *adrs, float *reelle) {
	...
	switch(trmt) {
		...
		case TRMT_DEMODUL:
			...
			if(d3) {
				if(VoieTampon[voie].trmt[classe].compte > 1) {
					if(reelle) *reelle = VoieTampon[voie].trmt[classe].reelle * (float)VoieManip[voie].def.trmt[classe].gain / (float)nbdata;
					else *adrs = (TypeDonnee)((VoieTampon[voie].trmt[classe].valeur * VoieManip[voie].def.trmt[classe].gain) / nbdata);
					dispo = 1;
					LectRecaleD3++;
				}
				VoieTampon[voie].trmt[classe].phase = 0;
		...
		case TRMT_LISSAGE:
			if(reelle) {
				...
				if(VoieTampon[voie].trmt[classe].compte < VoieManip[voie].def.trmt[classe].parm) {
					VoieTampon[voie].trmt[classe].reelle += *reelle;
					VoieTampon[voie].trmt[classe].compte += 1;
				} else {
					int k1;
					k1 = Modulo(index - VoieManip[voie].def.trmt[classe].parm,VoieTampon[voie].trmt[classe].X.max);
					VoieTampon[voie].trmt[classe].reelle += (*reelle - VoieTampon[voie].trmt[classe].X.val[k1]);
				}
				*reelle = VoieTampon[voie].trmt[classe].reelle * (float)VoieManip[voie].def.trmt[classe].gain / (float)VoieTampon[voie].trmt[classe].compte;
			} else {
				if(VoieTampon[voie].trmt[classe].compte < VoieManip[voie].def.trmt[classe].parm) {
					...
					int k1;
					k1 = Modulo(index - VoieManip[voie].def.trmt[classe].parm,VoieTampon[voie].brutes->max);				     
		...
		case TRMT_MOYENNE:
			...
			VoieTampon[voie].trmt[classe].compte += 1;
			if(VoieTampon[voie].trmt[classe].compte < VoieManip[voie].def.trmt[classe].parm) dispo = 0;
			else {
				if(reelle) *reelle = VoieTampon[voie].trmt[classe].reelle * (float)VoieManip[voie].def.trmt[classe].gain / (float)VoieTampon[voie].trmt[classe].compte;
				else *adrs = (TypeDonnee)((VoieTampon[voie].trmt[classe].valeur * VoieManip[voie].def.trmt[classe].gain) / VoieTampon[voie].trmt[classe].compte);
				VoieTampon[voie].trmt[classe].reelle = 0.0;
				VoieTampon[voie].trmt[classe].valeur = 0;
				VoieTampon[voie].trmt[classe].compte = 0;
				dispo = 1;
		...
		case TRMT_MAXIMUM:
			...
			if(VoieTampon[voie].trmt[classe].compte < VoieManip[voie].def.trmt[classe].parm) dispo = 0;
				...
				else *adrs = (TypeDonnee)((VoieTampon[voie].trmt[classe].valeur * VoieManip[voie].def.trmt[classe].gain));
				VoieTampon[voie].trmt[classe].reelle = 0.0;
				VoieTampon[voie].trmt[classe].valeur = 0;
				VoieTampon[voie].trmt[classe].compte = 0;
				dispo = 1;
		...
		case TRMT_INTEDIF:
			...
			valeur = (reelle)? VoieTampon[voie].trmt[classe].reelle: (float)VoieTampon[voie].trmt[classe].valeur;
			...
			/* derivation */
			if(VoieTampon[voie].trmt[classe].Y.nb < VoieManip[voie].def.trmt[classe].parm2) { if(reelle) *reelle = 0.0; else *adrs = 0; }
			else {
				ky = kx - VoieManip[voie].def.trmt[classe].parm2; if(ky < 0) ky += VoieTampon[voie].trmt[classe].Y.max;
				if(reelle) *reelle = (VoieTampon[voie].trmt[classe].Y.val[kx] - VoieTampon[voie].trmt[classe].Y.val[ky]) * (float)VoieManip[voie].def.trmt[classe].gain / (float)VoieTampon[voie].trmt[classe].compte);
				else *adrs = (TypeDonnee)(((VoieTampon[voie].trmt[classe].Y.val[kx] - VoieTampon[voie].trmt[classe].Y.val[ky]) * VoieManip[voie].def.trmt[classe].gain) / VoieTampon[voie].trmt[classe].compte);
		...
		{ int ix,iAx,iy,i1y,iCy,iBCy,i2BCy,iz,iDz; int64 d;
		  ...
		  VoieTampon[voie].trmt[classe].X.val[ix] = (reelle)? *reelle: (float)donnee;
		  if(VoieTampon[voie].trmt[classe].Y.nb < VoieTampon[voie].trmt[classe].Y.max) {
		  ...
		  if(LectStampsLus >= d) {
			  ...
			  else *adrs = (TypeDonnee)(Z - VoieTampon[voie].trmt[classe].Z.val[iDz]);
		  } else if(reelle) *reelle = Z;
		  else *adrs = (TypeDonnee)Z;
		}
		dispo = 1;
		break;
		#endif /* A_LA_MARINIERE */
			...
			if(!(filtre = VoieTampon[voie].trmt[classe].filtre)) { if(!reelle) *adrs = donnee; }
				...
				if(VoieManip[voie].def.trmt[TRMT_PRETRG].template && VoieTampon[voie].unite.val) {	// ajout etage de filtrage par evt unite;
					...
					if(nbdata >= dim) {												// taille suivi superieure a taille template !
						if(dim > 0) filtree = template[dim-1] * entree;
						else filtree = 0.0;
...
#endif
				if(reelle) *reelle = (float)filtree * (float)VoieManip[voie].def.trmt[classe].gain;
				else
				#ifdef DONNEES_NON_SIGNEES
					*adrs = (TypeDonnee)(((filtree + 8192.0) * (float)VoieManip[voie].def.trmt[classe].gain) + 0.5);
				#else
					*adrs = (TypeDonnee)((filtree * (float)VoieManip[voie].def.trmt[classe].gain) + 0.5); // *adrs = (TypeDonnee)filtree;
				#endif
...
INLINE char SambaDeclenche(int voie, float reelle) {
	...
	if(VoieTampon[voie].trig.cherche == TRGR_EXTREMA) {
		...
		if((suivi->pente * VoieTampon[voie].signal.climbs) < 0) {
			...
			vh = VoieHisto[voie];
			while(vh) {
				if(vh->Xvar == MONIT_BRUIT) HistoFillIntToInt(vh->hd,fabsf(amplitude),1);
				vh = vh->suivant;
			}
			/* Histogrammes bruit */
	...
	if(*a_depiler_global) { LectDerniereNonVide = t0; LectSerieVide = 0; }
		...
		if(((t0 - LectDerniereNonVide) < 3000000) /* && (LectSerieVide < SettingsDepileMax) */ ) {
			...
			for(rep=0; rep<RepartNb; rep++) if(Repart[rep].actif && (Repart[rep].top == Repart[rep].bottom)) {
				...
				on_arrete = (LectRepNbAdemarrer == 1);
				if(!on_arrete) {
					char buffer[80];
				...
				if(on_arrete) {
					...
					if(!actifs) {
						...
						if(VerifErreurStoppe) RepartiteurStoppeTransmissions(repart,1);
...
void LectDepileSat() {
	...
	unsigned int lngr; unsigned char trame[LECT_SAT_LNGR];
	TypeAcquisMsg *msg;
	...
	for(j=0; j<Partit[PartitLocale].nbsat; j++)  {
		...
		do {
			...
			if(nb == -1) { if(errno != EAGAIN) printf("%s/ ! %s hors service (%s)\n",DateHeure(),Acquis[sat].code,strerror(errno)); break; } 
				...
				if(BigEndianOrder) ByteSwappeShortArray((unsigned short *)trame,2);
				msg = (TypeAcquisMsg *)trame;
				// printf("%9s msg: %04X.%04X/%s/%s/%d\n"," ",msg->tag,msg->sat,msg->m.etat.nom_run,msg->m.etat.duree_lue,msg->m.etat.evt_trouves);
				...
				switch(msg->tag) {
					case SAT_ETAT:
						...
						if(BigEndianOrder) {
							ByteSwappeInt((unsigned int *)(&msg->m.etat.evt_trouves));
							ByteSwappeInt((unsigned int *)(&msg->m.etat.evt_ecrits));
						}
...
void LectDepileDonnees() {
	...
	if((RepartNb == 1) && (Repart[0].famille == FAMILLE_IPE)) {
        Repart[0].passages = 0;
		if(log) { nb_avant = Repart[0].donnees_recues; t1 = (VerifTempsPasse? DateMicroSecs(): 0); }
		do {
			pas_tous_secs = 0;
			pleins = 0;
			lues = IpeVidePile(&(Repart[0]),&pas_tous_secs,&pleins,log);
			if(LectErreur.code) goto retour;
			if(Repart[0].s.ip.en_defaut) episode_en_cours = 1;
			if(Repart[0].simule) OpiumUserAction();
		} while(pas_tous_secs && (++(Repart[0].passages) < SettingsDepileMax));
		if(log) {
			nb_lues = Repart[0].donnees_recues - nb_avant; t2 = (VerifTempsPasse? DateMicroSecs(): 0) - t1;
			printf("          %8lld[%3d] %lld/%3lld us\n",t1-LectT0Run,Repart[0].passages,nb_lues,t2);
		}
	} else {
	...
	// if(LectRepNbAdemarrer > 1) LectUsageInterfacesToutes += ((VerifTempsPasse? DateMicroSecs(): 0) - t0);
	...
	forever {
		...
		if(repart) {
			 ...
			 */
				...
				// if(log > 2) printf("          Bas pile: %04X %04X, D3 %s\n",repart->fifo16[repart->bottom],repart->fifo16[repart->bottom+1],RepartD3Attendues?"attendue":"deja trouvee");
				...
				if(repart->s.ip.manquant && (repart->bottom == repart->s.ip.saut)) {
					...
					if(voie >= 0) LectEnCours.valeur = VoieTampon[voie].derniere;
					LectEnCours.dispo = 1;
				} else if(repart->bottom < repart->top) {
					if(BigEndianOrder) LectEnCours.valeur = (int)repart->fifo16[repart->bottom++];
					else if(repart->bottom & 1) LectEnCours.valeur = (int)repart->fifo16[repart->bottom++ - 1];
					else LectEnCours.valeur = (int)repart->fifo16[++repart->bottom];
					LectEnCours.dispo = 1;
				...
				// if(log > 2) printf("          Utilise: %04X\n",LectEnCours.valeur);
			} else if(repart->famille == FAMILLE_ARCHIVE) {
				if(repart->bottom < FIFOdim) LectEnCours.valeur = repart->fifo16[repart->bottom++]; else break;
		...
		if(LecturePourRepart && repart && (repart == RepartConnecte)) {
			...
			if(repart->status_demande) {
				...
				if(RepartConnexionEnCours[LectVoieRep].tampon.t.sval) {
					...
					donnee = LectEnCours.valeur & repart->masque; if(donnee & repart->zero) donnee |= repart->extens;
					if(RepartConnexionEnCours[LectVoieRep].tampon.nb < RepartConnexionEnCours[LectVoieRep].tampon.max) {
						RepartConnexionEnCours[LectVoieRep].tampon.t.sval[(RepartConnexionEnCours[LectVoieRep].tampon.nb)++] = donnee;
					} else {
						RepartConnexionEnCours[LectVoieRep].tampon.t.sval[(RepartConnexionEnCours[LectVoieRep].tampon.prem)++] = donnee;
		...
		if(LectCntl.LectMode != LECT_IDENT) {
			if(voie >= 0) {
				if(VoieManip[voie].pseudo) {
					...
					composante = VoieManip[voie].pseudo;
					while(composante) {
						resul += composante->coef * (float)(VoieTampon[composante->srce].courante);
						composante = composante->svt;
					}
					LectEnCours.valeur = (TypeDonnee)(resul + 0.5); LectEnCours.dispo = 1;
					VoieTampon[voie].derniere = LectEnCours.valeur;
				} else if(LectEnCours.dispo) VoieTampon[voie].derniere = LectEnCours.valeur;
				else { LectEnCours.valeur = VoieTampon[voie].derniere; LectEnCours.dispo = 1; }
		...
		if((voie == SAMBA_ECHANT_STATUS) && (LectCntl.LectMode != LECT_IDENT)) {
			if(repart) {
			#ifdef DEBUG_STATUS_MIG
				donnee = LectEnCours.valeur & repart->masque;
				if(LectStatusNb < STATUSLIST_LNGR) { /*if(donnee) */ LectStatusList[LectStatusNb++] = LectEnCours.valeur; }
			#endif
           ...
           /* 2.a/ donnee pour identification */
            if(LectCntl.LectMode == LECT_IDENT) {
				if(repart) {
					...
					if(VoieIdent[transmise].lus >= VoieIdent[transmise].dim) {
						...
						*(VoieIdent[transmise].prochain)++ = donnee;
			...
			/* 2.b/ donnee provenant des detecteurs */
                ...
                /* Decodage de la donnee */
				if(repart && (repart->famille == FAMILLE_CALI)) {
					//+ int adu;
					//+ adu = DATAVALEUR(voie,LectEnCours.valeur);
					//+ donnee = (TypeDonnee)(adu - (int)VoieManip[voie].zero);
					if(repart->revision == 1) donnee = (TypeDonnee)(LectEnCours.valeur - (int)0x8000);
					else donnee = (TypeDonnee)((int)0x8000 - LectEnCours.valeur);
					//+ if(repart->depile < 32) printf("%4d/ recu: %04X(%d), extrait: %04X, retenu: %04X, valeur: %d\n",repart->depile, LectEnCours.valeur,LectEnCours.valeur,adu,donnee,donnee);
				// } else if((repart->famille == FAMILLE_OPERA) && (repart->r.opera.mode == code_acqui_EDW_BB2)) {
				// 		donnee = (LectEnCours.valeur & 0x8000)? LectEnCours.valeur & 0x7FFF: LectEnCours.valeur | 0x8000;
				} else {
					if(VoieManip[voie].ADCmask == 0xFFFF) donnee = (TypeDonnee)LectEnCours.valeur;
					else {
						donnee = DATAVALEUR(voie,LectEnCours.valeur);
			...
			// #define TEST_CPU
			#ifdef TEST_CPU
				*(VoieTampon[voie].prochain) = donnee; dispo = 1;
			#else
				trmt = VoieTampon[voie].trmt[TRMT_AU_VOL].utilise;
				dispo = SambaTraiteDonnee(TRMT_AU_VOL,trmt,voie,donnee,LectSynchroD3,VoieTampon[voie].lus,VoieTampon[voie].prochain,0);
			#endif
				/* Stockage de la meme */
				if(dispo) {
					TypeDonnee *prochain;
				...
				#ifdef BRANCHE_TESTS
					if(TestSimul) {
						TypeDonnee suppl; int delai; float mc_amp(int *delai);
						if(TestSimul == TEST_VOIE_UNIQUE) {
							if(voie == VoieTouchee) {
								suppl = AmplDeposee[voie] * Template[voie][TemplTemps++];
								*(VoieTampon[voie].prochain) += DATAVALEUR(voie,suppl);
								if(TemplTemps >= TemplDim[voie]) VoieTouchee = -1;
							}
						} else if(TestSimul == TEST_EVENEMENT) {
							if(LectSimulProchain < t0) {
								suppl = mc_amp(&delai) * AmplDeposee[voie] * Template[voie][TemplTemps++];
								*(VoieTampon[voie].prochain) += DATAVALEUR(voie,suppl);
								if(TemplTemps >= TemplDim[voie]) VoieTouchee = -1;
							}
						}
					...
					if(TestSimul) {
						float ajout;
						ajout = AmplitudeCosinus * (float)cos(2.0 * PI * (double)VoieTampon[voie].lus * (double)VoieEvent[voie].horloge / PeriodeCosinus);
						*(VoieTampon[voie].prochain) += (TypeDonnee)ajout;
					}
				#endif /* AJOUTE_COSINUS */
...
fin_vidage:
	if(Acquis[AcquisLocale].etat.active) {
		...
		if(PCIdemande) {
			...
			if(LectRepNbAdemarrer > 1) LectUsageInterfacesToutes += ((VerifTempsPasse? DateMicroSecs(): 0) - t2);
		}
#endif /* AVEC_PCI */
	}
...
retour:
	...
	if(LectErreur.code && PCIdemande) for(rep=0; rep<RepartNb; rep++) if((Repart[rep].famille == FAMILLE_CLUZEL) && Repart[rep].actif) {
		printf("%s/ Abandon du depilement code %04X\n",DateHeure(),(unsigned short)LectErreur.code);
	...
	for(vi=0; vi<VoieIdentMax; vi++) if(SambaEchantillon[vi].voie == 0) {
		...
		if(!numeriseur && (VoieIdent[vi].resultat == IDENT_ABSENT)) 
			...
			if(bolo < BoloNb) {
				...
				if(connecteur) {
					if(detectr->pos == CABLAGE_INDEFINI) printf(" nulle|");
					else printf(" %3s  |",CablageEncodePosition(detectr->pos));
				} else {
					if(detectr->pos == CABLAGE_INDEFINI) printf("      |");
...
//-	ArchiveDefini(LectureLog);
	...
	MenuItemAllume(mLectDemarrage,1,L_("En cours"),GRF_RGB_GREEN);
	MenuItemAllume(mLectArret,1,L_("Stopper "),GRF_RGB_YELLOW);
	if(OpiumAlEcran(gEvtPlanche)) GraphBlank(gEvtPlanche);
...
//		printf("* Prochaine sequence: #%d (%s)\n",SequenceCourante,LectAutreSequence?"a suivre":"unique");
	...
	do {
		if(LectSession < 2) {
			...
			if(Archive.enservice) {
				if(!LectModeSpectresAuto) {
					...
					if(Trigger.demande) {
						l = printf("| Sauvegarde des detecteurs %s, %s leurs associes",
							ArchModeTexte[ArchDetecMode],ArchAssocMode? "avec":"sans");
						SambaCompleteLigne(86,l);
					}
					if(ArchiveSurRepert) {
					...
					if(LoggerType == CAHIER_ELOG) {
						...
						if((f = fopen(LoggerAdrs,"a"))) {
							...
							if(VoiesNb == 1) {
								voie = 0;
								fprintf(f,"                       ");
								fprintf(f,"Tmin=%f, Tmax=%f, ADC=%g\n",VoieTampon[voie].trig.trgr->montee,VoieTampon[voie].trig.trgr->montmax,VoieTampon[voie].trig.trgr->minampl);
							}
							com1 = 1;
							for(i=0; i<MAXCOMMENTNB; i++) if(LectComment[i][0] != '\0') {
								LectComment[i][MAXCOMMENTLNGR-1] = '\0';
		...
		if(mCalcSpectreControle->cdr) {
			if(OpiumDisplayed((mCalcSpectreControle->cdr)->planche)) doit_terminer = OpiumRefreshBegin((mCalcSpectreControle->cdr)->planche);
			else doit_terminer = 0;
...
//		if(Acquis[AcquisLocale].etat.active || LectureSuspendue) LectStop();
		if(LectureSuspendue) LectureSuspendue= 0;
			...
			if(Trigger.demande && (Trigger.type != TRGR_CABLE)) {
				OpiumWarn("Mode de recherche d'evenements type '%s' abandonne, impose: 'cable'",
					TrgrDefListe[Trigger.type]);
				Trigger.type = TRGR_CABLE;
			}
			if(ArchModeStream != ARCH_BRUTES) {
				OpiumWarn("Sauvegarde type %s abandonne, impose: %s",
					ArchNomStream[ArchModeStream],ArchNomStream[ARCH_BRUTES]);
			...
			while(LectDemarrageDemande) {
				...
				if(!OpiumDisplayed(bLecture)) OpiumFork(bLecture); 
			#ifdef LECT_MODE_ABANDONNE
				if(LectCntl.LectMode != LECT_DONNEES) {
...
*/
	...
	for(voie=0; voie<VoiesNb; voie++) if(VoieTampon[voie].lue) {
		...
		printf("%16s (x%3d)%c  %9d %9lld %9d    %6lld   %08X\n",VoieManip[voie].nom,VoieManip[voie].def.trmt[TRMT_PRETRG].gain,
			...
			(hexa)VoieTampon[voie].traitees->t.rval);
	}
...
MenuItem itemsLectSpectre[] = { { "Non", MNU_RETOUR }, { "Oui", MNU_RETOUR }, MNU_END };
...
int LectEvtAffiche(Panel p, int i, void *arg) {
	return(MonitEvtAffiche(0,0,(int)arg));
}
/* ========================================================================== */
...
//	char nomvoie[VOIES_MAX][VOIE_NOM]; char *c,*r;
			...
			for(cap=0; cap<Bolo[bolo].nbcapt; cap++) {
				...
				if(cap == 0) {
					i = PanelKeyB(p,Bolo[bolo].nom,DETEC_STATUTS,&(Bolo[bolo].lu),5);
						PanelItemColors(p,i,SambaRougeVertJaune,3);
	...
	for(classe=0; classe<TRMT_NBCLASSES; classe++) {
		...
		PanelItemSouligne(p,PanelLngr(p,PanelBlank(p),3),1);
		i = PanelItemSouligne(p,PanelLngr(p,PanelBlank(p),3),1);
		PanelItemBarreDroite(p,i,1);
	}
	PanelItemSouligne(p,PanelItemSelect(p,PanelText(p,0,"Trigger",20),0),1);
	i = PanelItemSouligne(p,PanelLngr(p,PanelBlank(p),6),1);
		PanelItemBarreDroite(p,i,1);
	...
	if(BoloNb > 1) for(vm=0; vm<ModeleVoieNb; vm++) if(ModeleVoie[vm].utilisee) {
		...
		for(classe=0; classe<TRMT_NBCLASSES; classe++) {
			...
			PanelLngr(p,i,MAXFILTRENOM); PanelItemSouligne(p,i,fin_de_serie);
		#ifdef A_LA_GG
			if(VoieStandard[vm].def.trmt[classe].type == TRMT_INTEDIF)
				i = PanelText(p,0,VoieStandard[vm].def.trmt[classe].texte,TRMT_NOM);
			else 
		#endif /* A_LA_GG */
				i = PanelInt(p,0,&(VoieStandard[vm].def.trmt[classe].parm));
			PanelLngr(p,i,3); PanelItemSouligne(p,i,fin_de_serie);
		...
		PanelLngr(p,i,20); PanelItemSouligne(p,i,fin_de_serie);
		i = PanelKeyB(p,0,L_("fixe/regule"),&(VoieStandard[vm].def.trgr.regul_demandee),6);
		PanelLngr(p,i,6); PanelItemSouligne(p,i,fin_de_serie);
...
//			} else i = PanelText(p,0,ModeleVoie[Bolo[bolo].captr[cap].modele].nom,8);
			...
			for(classe=0; classe<TRMT_NBCLASSES; classe++) {
				...
				if(VoieManip[voie].def.trmt[classe].type == TRMT_INTEDIF)
					i = PanelText(p,0,VoieManip[voie].def.trmt[classe].texte,TRMT_NOM);
				else 
			#endif /* A_LA_GG */
					i = PanelInt(p,0,&(VoieManip[voie].def.trmt[classe].parm));
				PanelItemExit(p,i,LectConsigneTouche,&(Bolo[bolo]));
				PanelItemReturn(p,i,LectConsigneTouche,&(Bolo[bolo]));
				PanelLngr(p,i,3); PanelItemSouligne(p,i,fin_de_serie);
...
//		}
		...
		hval = NumeriseurChargeRessource(numeriseur,num,0);
	#else
		hval = NumeriseurChargeRessourceAuto(numeriseur,num,0,0);
	#endif
		...
		#else
			hval = NumeriseurChargeRessourceAuto(numeriseur,k,0,0);
		#endif
			printf("            . %s: %12s- = %-8s (transmis: %s)\n",
...
//		LectConsigneCharge(DetecReglagesLocaux[item-1],1);
		...
		NumeriseurChargeRessourceFin(0);
	#endif
	}
	LogOffFile();
	return(0);
		...
		pour_tout(cap,Bolo[bolo].nbcapt) if((voie = Bolo[bolo].captr[cap].voie) >= 0) {
			pour_tout(classe,TRMT_NBCLASSES) {
				if(VoieManip[voie].def.trmt[classe].std) VoieManip[voie].numtrmt[classe] = TRMT_FILTRAGE;
					...
					if((VoieManip[voie].def.trmt[classe].parm < 0) || (VoieManip[voie].def.trmt[classe].parm >= k))
					VoieManip[voie].def.trmt[classe].parm = 0;
					VoieManip[voie].numtrmt[classe] = TRMT_NBTYPES + VoieManip[voie].def.trmt[classe].parm;
	...
	pour_tout(vm,ModeleVoieNb) {
		pour_tout(classe,TRMT_NBCLASSES) {
			if(VoieStandard[vm].def.trmt[classe].type == TRMT_FILTRAGE) {
				if((VoieStandard[vm].def.trmt[classe].parm < 0) || (VoieStandard[vm].def.trmt[classe].parm >= k))
				VoieStandard[vm].def.trmt[classe].parm = 0;
				VoieStandard[vm].numtrmt[classe] = TRMT_NBTYPES + VoieStandard[vm].def.trmt[classe].parm;
	...
	for(vm=0; vm<ModeleVoieNb; vm++) {
		for(classe=0; classe<TRMT_NBCLASSES; classe++) {
			if(VoieStandard[vm].numtrmt[classe] >= TRMT_NBTYPES) {
				...
				VoieStandard[vm].def.trmt[classe].parm = VoieStandard[vm].numtrmt[classe] - TRMT_NBTYPES;
			...
			for(classe=0; classe<TRMT_NBCLASSES; classe++) {
				if(VoieManip[voie].numtrmt[classe] == TRMT_FILTRAGE) /* c'est qu'on a mis 'Standard' */ {
					...
					if(VoieManip[voie].numtrmt[classe] >= TRMT_NBTYPES) {
						...
						VoieManip[voie].def.trmt[classe].parm = VoieManip[voie].numtrmt[classe] - TRMT_NBTYPES;
...
MenuItem iLectRazMaxPile[] = { { "C", MNU_FONCTION LectRazMaxPile }, MNU_END };
...
static Graph LectMonitHistoConstruit(MONIT_VARIABLE var, TypeMonitFenetre *f, char *titre, int larg, int haut, char nouveau) {
	...
	if(nouveau) {
		...
		f->p.h.histo = 0;
	} else MonitFenFree(f); // extrapoler le nouvel histo avec les valeurs de l'ancien
...
static void LectPlancheGenerale() {
	...
	if(BolosAdemarrer) {
		i = PanelKeyB(p,L_("Demarre det."),L_("sans/active"),&SettingsStartactive,12); PanelLngr(p,i,12);
		PanelItemColors(p,i,SambaJauneVertOrangeBleu,2); PanelItemSouligne(p,i,0);
	}
	if(BolosAentretenir) {
		i = PanelKeyB(p,"Maintenance",L_("sans/active"),&SettingsDLUactive,12); PanelLngr(p,i,12);
		PanelItemColors(p,i,SambaJauneVertOrangeBleu,2); PanelItemSouligne(p,i,0);
	}
	if(BolosUtilises > 1) {
	...
	i = PanelKeyB(p,"Regulation",L_("sans/active"),&TrmtRegulDemandee,12); PanelLngr(p,i,12);
	PanelItemColors(p,i,SambaJauneVertOrangeBleu,2); PanelItemSouligne(p,i,0);
	i = PanelKeyB(p,L_("Archivage"),L_("sans/demande"),&Archive.demandee,12); PanelLngr(p,i,12);
	PanelItemColors(p,i,SambaJauneVertOrangeBleu,2); PanelItemSouligne(p,i,0);
	...
	//		PanelItemColors(p,i,SambaJauneVertOrangeBleu,2); PanelItemSouligne(p,i,0);
	//	i = PanelBool(p,"Sequences",&LectAutoriseSequences); PanelLngr(p,i,12);
	//		PanelItemColors(p,i,SambaJauneVertOrangeBleu,2); PanelItemSouligne(p,i,0);
	...
	i = PanelFormat(p,PanelLngr(p,PanelInt(p,"Sessions",&LectSession),3),"%3d");
	...
	if(SambaMaitre) {
		...
		for(j=0; j<Partit[PartitLocale].nbsat; j++)  {
			...
			i = PanelItemSouligne(p,PanelInt(p,0,&Acquis[sat].etat.evt_trouves),fin_de_serie); PanelLngr(p,i,10); PanelFormat(p,i,"%-10d");
			i = PanelItemSouligne(p,PanelInt(p,0,&Acquis[sat].etat.evt_ecrits),fin_de_serie); PanelLngr(p,i,10); PanelFormat(p,i,"%-10d");
		...
		i = PanelInt(p,0,&Acquis[AcquisLocale].etat.evt_trouves); PanelLngr(p,i,10); PanelFormat(p,i,"%-10d");
		i = PanelInt(p,0,&Acquis[AcquisLocale].etat.evt_ecrits); PanelLngr(p,i,10); PanelFormat(p,i,"%-10d");
	...
	if(nb > 0) {
		p = pRepartXmit = PanelCreate(nb);
		for(rep=0; rep<RepartNb; rep++) if(Repart[rep].local) {
			PanelItemColors(p,PanelKeyB(p,Repart[rep].code_rep,"arrete/en route",&(Repart[rep].en_route),9),SambaOrangeVert,2);
		}
		PanelMode(p,PNL_DIRECT|PNL_RDONLY); PanelSupport(p,WND_CREUX);
	...
	BoardAdd(bLecture,p->cdr,19*Dx,y+(3*Dy),0);
#endif
	
	/* partie "evenements" */
	x = x0; y = Dy;
	xg = 58 * Dx; yg = bas_zone_cmde - (12 * Dy); if(yg < (8 * Dy)) yg = 8 * Dy;
	g = gEvtPlanche = GraphCreate(xg,yg,6);
	...
	x = x0 + (0 * Dx); y += (4 * Dy);
 	BoardAddInstrum(bLecture,InstrumKeyB(INS_GLISSIERE,(void *)&Gliss2lignes,&MonitAffEvt,L_("fige/dernier")),x,y - (Dy / 2),0); x += (12 * Dx);
	...
	i = PanelFloat(p,L_("taux actuel (Hz)"),&LectCntl.EvtTaux); // PanelItemSelect(p,i,0);
	...
	BoardAddInstrum(bLecture,cIntervAff,x0 + (34 * Dx),y,0);
    x = x0 + xg + (4 * Dx);
	...
	PanelLngr(p,PanelInt(p,"max",&RepartIp.depilees),4);
	PanelMode(p,PNL_DIRECT|PNL_RDONLY); PanelSupport(p,WND_CREUX);
	BoardAdd(bLecture,p->cdr,x,y,0);
...
//	PanelItemSelect(p,PanelText (p,"detecteur",BoloTrigge,DETEC_NOM),0);
	...
	x = 2 * Dx; y += (4 * Dy);
	BoardAddInstrum(bLecture,InstrumKeyB(INS_GLISSIERE,(void *)&Gliss2lignes,&MonitAffEvt,L_("fige/dernier")),x,y - (Dy / 2),0);
	...
	i = PanelFloat(p,L_("taux actuel (Hz)"),&LectCntl.EvtTaux); 
	i = PanelFloat(p,L_("taux global (Hz)"),&LectCntl.TauxGlobal); 
	PanelMode(p,PNL_DIRECT|PNL_RDONLY); PanelSupport(p,WND_CREUX);
	BoardAdd(bLecture,p->cdr,x,y - Dy,0);
	BoardAreaClose(bLecture,&LectMonitCadreEvt,&x,&y);
	...
	k = PanelFloat(LectPanelSeuil,0,&LectSeuilReel); PanelLngr(LectPanelSeuil,k,9); // PanelFormat(LectPanelSeuil,k,"%9.2f");
...
//	BoardAreaOpen("Signal",WND_PLAQUETTE); // donne des rainures
	...
	k = PanelInt(p,0,&(LectOscilloSignal.ampl)); PanelLngr(p,k,7); // PanelFormat(p,k,"%5d");
...
//	BoardAreaClose(bLecture,&LectMonitCadreSignal,&x,&y);
	...
	BoardAreaClose(bLecture,&LectMonitCadreCntl,&x,&y);
	...
	PanelFloat(p,"Amplitude min",&LectMonitAmplMin); PanelLngr(p,1,6);
	PanelItemExit(p,1,LectMonitAmplChangeMin,0);
	PanelFloat(p,"max",&LectMonitAmplMax); PanelLngr(p,2,6);
	...
	p = PanelCreate(2);
	PanelColumns(p,2); PanelMode(p,PNL_DIRECT); PanelSupport(p,WND_CREUX);
	PanelFloat(p,"RMS min",&LectMonitRmsMin); PanelLngr(p,1,5);
	PanelItemExit(p,1,LectMonitRmsChangeMin,0);
	PanelFloat(p,"max",&LectMonitRmsMax); PanelLngr(p,2,5);
	...
	BoardAreaClose(bLecture,&LectMonitCadreAnalyse,&x,&y);
	x += (f->larg + (5 * Dx));
	...
	BoardAdd(bLecture,p->cdr,x,y,0);
 	...
 	BoardAddMenu(bLecture,MenuBouton("C", MNU_FONCTION LectRazMaxPile),x - Dx,y,0);
	BoardAreaMargesVert(Dy/2,0);
	BoardAreaClose(bLecture,&LectMonitCadreCapacite,&x,&y);
...
static void LectCompacteCom() {
	...
	BoardAreaClose(bLecture,&LectMonitCadreBuffDump,&x,&y);
 ...
 */
	...
	RepartIp.depilees = 0;
#ifdef AJOUTE_COSINUS
	AmplitudeCosinus = 5000.0; PeriodeCosinus = 10000.0;
	LectTrmtInitValBrute = LectTrmtInitValTraitee = 0;
#endif
	LectSyncMesure = 0;
//	LectNBolosParVoie = 0;
	LectBuffVal0 = 0; LectBuffValN = 0; LectBuffOrigType = LectBuffDestType = 0; LectBuffHexa = 0;
	LectCompacteMaxFifo = 1024;
	strcpy(LectBuffFichier,"Buffer SAMBA");
	LectCntl.LectTauxPile = 0;
	LectCntl.EvtTaux = 0.0; LectTauxMin = 0.1;
	LectCntl.LectMode = LECT_DONNEES;
	...
	RepertoireCreeRacine(FichierRunCaract);
	printf("= Lecture des caracteristiques          dans '%s'\n",FichierRunCaract);
	...
	RepertoireCreeRacine(FichierEtatElec);
	printf("= Lecture de l'etat electrique          dans '%s'\n",FichierEtatElec);
...
//	LectAllume = 0;
    ...
    LectLogComp = 0;
	LectErreurFichier = 0;
	for(i=0; i<MAXCOMMENTNB; i++) LectComment[i][0] = '\0';
 ...
 */
//	pRunParms = PanelCreate(2 + MAXCOMMENTNB);
//	PanelKeyB(pRunParms,L_("Run de"),RunEnvir,&LectCntl.SettingsRunEnvr,20);
//	PanelKeyB(pRunParms,L_("avec source"),RunCategCles,&LectCntl.RunCategNum,TYPERUN_NOM);
...
/*	PanelItemSelect(pLectEtat,PanelInt(pLectEtat,"Points lus sur FIFO",&LectStampsLus),0); */
	...
	PanelItemSelect(pLectEtat,PanelFormat(pLectEtat,PanelFloat(pLectEtat,"Taux (Hz)",&LectCntl.EvtTaux),"%.2f"),0);
