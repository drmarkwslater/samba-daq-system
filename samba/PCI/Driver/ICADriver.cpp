...
/* ========================================================================== */
IOReturn ICADriver::ConfigRead16(int adrs, unsigned short *val) {
    if(!isOpen()) return(kIOReturnNotOpen);
    *val = (unsigned short)carte->configRead16((UInt8)adrs);
    return(kIOReturnSuccess);
}
/* ========================================================================== */
...
/* ========================================================================== */
IOReturn ICADriver::ConfigRead8(int adrs, unsigned char *val) {
	if(!isOpen()) return(kIOReturnNotOpen);
	*val = (unsigned char)carte->configRead8((UInt8)adrs);
	return(kIOReturnSuccess);
}
/* ========================================================================== */
...
/* ========================================================================== */
static void Attente(unsigned int microsec) {
#ifdef TROP_BEAU
...
IOReturn ICADriver::CommandSend(int adrs, int action, int ss_adrs, int valeur) {
	unsigned int delai_mot=1,delai_msg=3000;  /* microsecondes */
...
//		PCIadrs = (Ulong *)mapping_pci->getPhysicalAddress();
		...
		IOLog("ICADriver::RegisterUse(0x%02X) PCIadrs=%08X, ResetAdrs=%08X\n",reg,
			(unsigned int)PCIadrs,(unsigned int)ResetAdrs);
		return(kIOReturnSuccess);
...
IOReturn ICADriver::XferInit(int avec_dma, Ulong taille, Ulong *fifo) {
	...
	IOLog("ICADriver::XferInit(%s DMA, FIFO @%08X + %08X)\n",
		avec_dma?"avec":"sans",(unsigned int)fifo,(unsigned int)taille);
#ifdef PPC
...
//			IOLog("ICADriver::XferInit,  Lecture accordee de %d mots\n",max_lues);
			if(of7 == 0) {
				...
				if(DMAsize == taille) {
					IOLog("ICADriver::XferInit,  Bloc DMA unique: ->%08X + %08X\n",
						  (unsigned int)DMAdest,(unsigned int)DMAsize);
					break;
				}
			}
			if(nb_blocs == 0) {
			...
			IOLog("ICADriver::XferInit,  Bloc DMA #%-2d: ->%08X + %08X, suivant @%08X\n",
				  nb_blocs,(unsigned int)DMAdest,(unsigned int)DMAsize,val);
...
//			(unsigned int)fifo,(unsigned int)FifoUserAdrs);
	}
...
//				PLXwrite(PLX_CNTRL,val);
    ...
    if(mapping_pci) {
		...
		IOLog("ICADriver::start,  Acces PCI @%02X (PCIadrs=%08X, ResetAdrs=%08X)\n",
			  acces_pci,(unsigned int)PCIadrs,(unsigned int)ResetAdrs);
