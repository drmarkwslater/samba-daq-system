...
INLINE void CablageAnalysePosition(TypeCablePosition pos_hexa, short *galette, short *tour, short *branche) {
	*branche = (pos_hexa >> 8) & 0x3;
	*galette = CABLAGE_GALETTE(pos_hexa);
	*tour = CABLAGE_TOUR(pos_hexa);
	//- printf("(%s) Position codee %03X soit branche %d, galette %d, tour %d\n",__func__,pos_hexa,*branche,*galette,*tour);
}
/* ========================================================================== */
//INLINE TypeCablePosition CablageSynthetisePosition(short galette, short tour, short branche) {
//	return(((branche & 0x3) << 8) | CABLAGE_POSITION(galette,tour));
//}
INLINE TypeCablePosition CablageSynthetisePosition(short *position) {
	return(((position[2] & 0x3) << 8) | CABLAGE_POSITION(position[0],position[1]));
}
/* ========================================================================== */
...
#define CODE_CABLAGE_A_LA_MAIN
	...
	if(!code[0] || !strcmp(code,"---") || !strcmp(code,"ext")) return(CABLAGE_INDEFINI);
	if((code[0] >= 'a') && (code[0] <= 'z')) galette = code[0] - 'a';
	else if((code[0] >= 'A') && (code[0] <= 'Z')) galette = code[0] - 'A';
	else { sscanf(code,"%d",&pos); return((TypeCablePosition)(pos - 1)); /* pos-1 parce que tour-- */ }
	if((galette < 0) || (galette > 15)) galette = 15;
	sscanf(code+1,"%hd",&tour); tour--;
	if((tour < 0) || (tour > 15)) tour = 15;
	l = strlen(code) - 1;
	...
	return(CablageSynthetisePosition(position));
#endif
}
/* ========================================================================== */
char *CablageEncodePosition(TypeCablePosition pos_hexa) {

	if(pos_hexa == CABLAGE_INDEFINI) strcpy(CablageCodePosition,"---");
	else {
		//* short galette,trou,branche; 
		//* CablageAnalysePosition(pos_hexa,&galette,&trou,&branche);
		//- printf("(%s) Position codee %03X soit branche %d, galette %d, tour %d\n",__func__,pos_hexa,branche,galette,trou);
		//* sprintf(CablageCodePosition,"%c%d%c",'a'+galette,trou+1,branche?'a'+branche-1:'\0');
		short position[CHASSIS_DIMMAX];
...
TypeCableConnecteur CablageDecodeConnecteur(char *texte) {
	size_t i,l,m;
	short numero;
	TypeCableConnecteur connecteur,serie;
	char c; char hb,gd;

	if(!strcmp(texte,CABLAGE_TXTNUL) || !strcmp(texte,"ext") || !texte[0]) return(0);
	else if(!strcmp(texte,CABLAGE_TXTDIR)) return(CABLAGE_CONN_DIR);
...
INLINE void CablageAnalyseConnecteur(TypeCableConnecteur connecteur, short *quadrant, short *distance) {
	if(connecteur > 0) {
		*quadrant = (connecteur - 1) / QUADRANT_OF7;
		*distance = (connecteur - 1) - (*quadrant * QUADRANT_OF7);
	} else *quadrant = *distance = 0; 
}
/* ========================================================================== */
char *CablageEncodeConnecteur(TypeCableConnecteur connecteur) {
	...
	if(connecteur == CABLAGE_CONN_DIR) strcpy(CablageCodeConnecteur,CABLAGE_TXTDIR);
		...
		short position[CHASSIS_DIMMAX];
		CablageAnalyseConnecteur(connecteur,&(position[0]),&(position[1]));
...
static char CablageCreeDico() {
	...
	if((d = CablageModeleNb) >= CABLAGE_DICO_MAX) {
		printf("  ! Trop de types de cablage (CABLAGE_DICO_MAX=%d). Type %s ignore\n",
			CablageModeleNb,SambaDicoTempo.nom);
		return(0);
	}
	cablage = CablageModele + d;
...
void CablageModlLit(char *fichier) {
	...
	if(f) {
		SambaLogDef(". Lecture des modeles de cablage","dans",auxiliaire);
...
void CablageEcrit(char *fichier) {
	...
	if((f = fopen(fichier,"w"))) {
		printf("\n= Creation du fichier de cablage '%s'\n",fichier);
		fprintf(f,"# Description du cablage\n");
		fprintf(f,"#\n");
		...
		fprintf(f,"# <gain> par defaut: %g, <capa_pF> par defaut: %g\n",CABLAGE_DEFAUT_GAIN,CABLAGE_DEFAUT_CAPA);
		fprintf(f,"#\n");
				...
				if(!ecrit_calib && CablageSortant[pos_hexa].nbcapt) {
					...
					for(capteur=0; capteur<CablageSortant[pos_hexa].nbcapt; capteur++) {
						if((CablageSortant[pos_hexa].captr[capteur].connecteur != connecteur)
						   || (CablageSortant[pos_hexa].captr[capteur].vbb != capteur)
						   || (CablageSortant[pos_hexa].captr[capteur].gain != CABLAGE_DEFAUT_GAIN)
						   || (CablageSortant[pos_hexa].captr[capteur].capa != CABLAGE_DEFAUT_CAPA)) break;
					}
			...
			do {
				...
				if(v2) {
					if(rang_bb < ModeleCable[modl].nb_numer) {
						...
						for(j=0; j<ModeleCable[modl].max_adc; j++) {
							if(ModeleCable[modl].capteur[j].numer_index == rang_bb) {
								...
								CablageSortant[pos_hexa].captr[j].gain = 1.0; CablageSortant[pos_hexa].captr[j].capa = 10.0;
					...
					if((*v != '\0') && (*v != '*')) {
						...
						// CablageInverse[connecteur][vbb] = (((unsigned short)pos_hexa & 0xFF) << 4) | (capteur & 0xF);
						CablageSortant[pos_hexa].captr[capteur].connecteur = connecteur;
						...
						if(*g != '\0') {
							...
							else CablageSortant[pos_hexa].captr[capteur].capa = 10.0;
						} else { CablageSortant[pos_hexa].captr[capteur].gain = 1.0; CablageSortant[pos_hexa].captr[capteur].capa = 10.0; }
						...
						if(*g != '\0') {
							...
							ItemJusquA('/',&v);
							sscanf(g,"%g",&gain);
							if(*v) sscanf(v,"%g",&capa); else capa = 10.0;
						} else { gain = 1.0; capa = 10.0; }
						capteur = CablageSortant[pos_hexa].nbcapt;
