...
//	PRINT_OPIUM_ADRS(pScriptEtat);
	...
	PanelLngr(pScriptCntl,i,6); PanelItemSelect(pScriptCntl,i,0);
...
//	PRINT_OPIUM_ADRS(pScriptCntl);
	...
	PanelLngr(pScriptWait,i,4); PanelItemSelect(pScriptWait,i,0);
...
static int ScriptStoppe(Panel p, void *arg) {
	...
	k = (int)arg; if(k) { ScriptKill = 1; ScriptDuree = 0; } else ScriptDuree -= ScriptWait;
	ScriptWait = 0; 
...
static void ScriptDAC(char *nom, TypeNumeriseur *numeriseur, TypeBNModele *modele_bn, int k, char *entete, char *marge) {
	...
	NumeriseurChargeRessourceAuto(numeriseur,k,entete,marge);
}
/* ========================================================================== */
...
static INLINE char ScriptChargeRessource(TypeHwInstr *instr, 
	...
	int ival,ilim; float rval,rlim; unsigned short hval;
	...
	if(instr->oper == '=') {
		if(instr->val.type_sym == HW_NOM) {
			...
			numeriseur->ress[k].hval = (unsigned short)(instr->val.v.val);
			NumeriseurRessHexaChange(numeriseur,k);
		}
		NumeriseurChargeRessourceAuto(numeriseur,k,suite,marge);
	} else {
		unsigned short mval; int ival; float rval; char *cval; char texte[80],prec[80]; // NUMER_RESS_VALTXT
		...
		if(instr->val.type_sym == HW_NOM) {
			...
			hval = (unsigned short)(instr->val.v.val);
...
static char ScriptAffecteReglage(TypeHwInstr *instr, TypeDetecteur *detectr, int num, 
	...
	unsigned short mval; char prec[80]; // NUMER_RESS_VALTXT
	...
	if((k < 0) || (k >= modele_bn->nbress)) {
		...
		if((instr->oper != '=') && (instr->oper != ':')) 
			...
			if((instr->val.type_sym == HW_NOM) && !strcmp(instr->val.v.nom,"standard")) {
				...
				else raz = ((unsigned short)(instr->val.v.val) > 0);
				if(ScriptModeExec) {
					if(valeurs[0]) {
...
char ScriptExecBatch(HwInstr prgm, HW_OBJ obj, void *hw, HwExecContext pc, short derniere, short *secs, char *prefixe) {
	...
	int i,j,l; int rc; int ival; // unsigned short hval;
	...
	/*
	 ...
	 AffecteReglage  [oriente detecteur]:  si standard, detectr->regl[num] => numeriseur.ress[k] + NumeriseurChargeRessourceAuto
				...
				if(!connu) {
					if(instr->val.type_sym == HW_NOM) {
						...
						for(val=instr->reg.v.i.val1; val<=instr->reg.v.i.valN; val++) {
							NumeriseurChargeAdrsAuto(numeriseur,val,(unsigned short)(instr->val.v.val),suite,ScriptSuite);
							if(imprime) suite = ScriptSuite;
						}
					}
					continue;
	...
	//					adrs = modele_bn->ress[k].ssadrs;
			  ...
			  case HW_MEDIA:
				...
				k = (int)write(media->path,instr->val.v.nom,l);
				if(imprime) {
...
static int ScriptRun(HwScript script, char *prefixe) {
	...
	ScriptKill = 0;
	NumeriseurChargeRessourceFin(prefixe);
	return(0);
}
/* ========================================================================== */
static int ScriptAjoute(HwScript script, char *nomcomplet, char *prefixe, char *bloc_ouvert, int *bcur) {
