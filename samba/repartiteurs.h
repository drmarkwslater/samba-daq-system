...
#define REPART_IP_CNTL (unsigned short)0x8000
...
#ifdef REPARTITEURS_C
 = { "Cluzel", "OPERA", "CALI", "IPE", "Media", "Archive", "SAMBA", "NI", "\0" }
#endif
;
...
typedef struct {
	...
	unsigned int masque;         /* bits de donnee utiles                      */
...
#endif
;
REPARTITEUR_VAR char NumerModeAcquis;
...
#endif
;
REPARTITEUR_VAR unsigned char OperaValMaj[]
#ifdef REPARTITEURS_C
#ifdef BB2_SEULE
...
#define CALI_RESET   (unsigned int)0x00000020

#define CALI_START   (unsigned int)0x00000001
#define CALI_STOP    (unsigned int)0x00000002

#define CALI_EXTERNE (unsigned int)0x00000000
#define CALI_INDEX   (unsigned int)0x00010000
#define CALI_COUNTER (unsigned int)0x00020000
...
#endif
;
REPARTITEUR_VAR unsigned int CaliModeCode[CALI_NBDATAMODES]
#ifdef REPARTITEURS_C
...
#define CALI_STATUS_READ_ERROR  (unsigned int)0x01
#define CALI_STATUS_WRITE_ERROR (unsigned int)0x02
#define CALI_STATUS_FULL        (unsigned int)0x04
#define CALI_STATUS_EMPTY       (unsigned int)0x08
#define CALI_STATUS_HALF_FULL   (unsigned int)0x10
#define CALI_STATUS_HALF_EMPTY  (unsigned int)0x20
#define CALI_STATUS_OVERFLOW    (unsigned int)0x40
#define CALI_STATUS_ENABLED     (unsigned int)0x80
...
REPARTITEUR_VAR unsigned int CaliTrameLue;
REPARTITEUR_VAR unsigned char CaliVersionLue;
REPARTITEUR_VAR unsigned char CaliSelectMsg[32];
...
#define IPE_TRAME_STS      (unsigned short)0xFFFF
#define IPE_TRAME_BB       (unsigned short)0xFFFE
#define IPE_TRAME_CRATE    (unsigned short)0xFFD0

typedef union {
	unsigned char fibre[IPE_FIBRES_PARMOT];
	unsigned int total;
} TypeIpeFibreReg;

typedef struct {
	union {
		struct { unsigned char modeFLT,modeBB,enable,speciaux; } partiel;
		unsigned int complet;
	} controle;
	unsigned int fibres_fermees;
	unsigned char branchee,active,enabled;
...
#endif
;
REPARTITEUR_VAR unsigned char IpeDelais[IPE_NBADCMODES]
#ifdef REPARTITEURS_C
 = { 0x00, 0x73, 0x75 }
#endif
;
...
typedef struct {
	unsigned int stamp_haut;   // en int64, la valeur est "int-swappee" (donc pas de 'int64 stamp;')
	unsigned int stamp_bas;
	unsigned int temps_seconde;
	unsigned int dim;          // nombre d'octets = sizeof(TypeIpeCrateSts)
	// SLT registers:
...
typedef struct {
	...
	union {
		struct {
			unsigned char   type,crate;
			unsigned char   flt,fbr;
		} b;
		unsigned int complete;
	} source;
	unsigned int    inutile;
	unsigned short  val[IPE_BBSTS_LNGR];
} TypeIpeBBsts;
#define IPE_BBSTS_HDR (sizeof(TypeIpeBBsts) - (IPE_BBSTS_LNGR * sizeof(unsigned short)))
...
typedef struct {
	union {
		...
		struct { int mode; short d0; unsigned char retard,d2d3; char clck,sync; int lngr_echant; } opera;
		struct { char clck; } cali;
		struct {
			unsigned short chassis; short fifo; unsigned int masque;
			short d0; char clck,ports_BB; unsigned char bourrage; short fltnb,decale;
			unsigned char modeBB[REPART_ENTREES_MAX];
			char fpga[80]; short of7; // of7 en nb d'entrees
			TypeIpeFlt *flt; unsigned int ctl_defaut[IPE_FLT_MAX]; unsigned char retard[REPART_ENTREES_MAX];
		} ipe;
		struct { char fmt; } f;
...
//	char sans_donnee;              /* n'envoie que des status, et pas de donnee          */
	...
	unsigned int masque;           /* bits de donnees utiles                             */
	unsigned short zero,extens;
...
//	short donnee[REPART_VOIES_MAX];   /* liste des voies lues                                   */
	...
	int   maxentrees;                 /* nombre effectif de numeriseurs transmissibles          */
	int   nbentrees;                  /* dimension de la liste ci-dessous (0 si inactif)        */
	TypeRepartEntree *entree;         /* liste des numeriseurs connectes                        */
	union {
		struct {
			int   taille_entete;      /* taille de l'entete                              */
			int   taille_struct;      /* taille de la structure d'accueil                */
			int   taille_trame;       /* taille d'une trame non terminale                */
			int   taille_derniere;    /* taille d'une trame terminale                    */
			unsigned short num_sts;   /* numero de trame si status                       */
			int   duree_trame;        /* nombre d'ech./voie dans une trame               */
...
REPARTITEUR_VAR int   RepartActif;
...
REPARTITEUR_VAR int   RepartNumLibre;
REPARTITEUR_VAR unsigned short RepartIpeNb;
...
#ifdef REPARTITEURS_C
	= { RepartCluzelDesc, RepartOperaDesc, RepartCaliDesc, RepartIpeDesc, RepartMediaDesc, RepartArchiveDesc, RepartSambaDesc, RepartNiDesc, 0 }
#endif
; 
...
char RepartiteurEcritNumeriseur(TypeRepartiteur *repart, short entree, unsigned char *cmde, int lngr);
...
void RepartiteurDemarreNumeriseurs(TypeRepartiteur *repart, NUMER_MODE mode, unsigned char adrs, char log);
...
char OperaNouvelle(TypeRepartiteur *repart, char *adrs, char categ, int mode, unsigned char retard, char horloge);
...
INLINE void CaliSwappeTrame(TypeRepartiteur *repart, CaliTrame trame, int nb);
...
int  CaliSelecteur(TypeRepartiteur *repart, int selection, float echantillonage, unsigned char *buffer, int *l);
...
// int  IpeSelecteur(TypeRepartiteur *repart, int selection, float echantillonage, unsigned char *buffer, int *l);
...
static unsigned int IpeTempsLu,IpePixBus,IpeErreursNb,IpeInternalInfo,IpeSWstatus,IpeTrameDim;
static unsigned int IpeStampMsb,IpeStampLsb;
static short IpeFifoNb,IpeAdcNb;
static struct {
	unsigned char delai12;
	unsigned char delai120;
