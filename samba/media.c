...
static void MediaImprime(Media media) {
	...
	printf("%-8s de type %-6s (%s <%d>) @%s%s\n",media->nom,MediaNomType[media->type],
		(media->mode == MEDIA_RD)? "Rd":((media->mode == MEDIA_WR)? "Wr":"RW"),media->path,
...
int MediaConnecte(Media media) {	
	if(media) switch(media->type) {
		case MEDIA_USB: media->path = UsbOpen(media->adrs); break;
		case MEDIA_TCP_WR: case MEDIA_UDP_WR:
			SocketInit(media->adrs,media->port,&(media->skt));
			if(media->type == MEDIA_TCP_WR) media->path = SocketConnect(&(media->skt));
			else media->path = SocketConnectUDP(&(media->skt));
			break;
		case MEDIA_TCP_RD: case MEDIA_UDP_RD:
			if(media->type == MEDIA_TCP_RD) media->path = SocketTCPServeur(media->port);
			else media->path = SocketUDP(media->port);
			if(media->path >= 0) fcntl(media->path,F_SETFL,O_NONBLOCK);
			break;
	} else return(-1);
	return(media->path);
}
...
/* ========================================================================== */
char MediaListeAjouteRef(TypeMediaListe *liste, Media media) {
	if(!liste) return(0);
	if(liste->nb >= liste->max) return(0);
...
/* ========================================================================== */
char MediaListeAjouteLus(TypeMediaListe *liste) {
	int m;
...
int MediaListeChemin(TypeMediaListe *liste, char *nom, char mode) {
	...
	if(liste) for(m=0; m<liste->nb; m++) 
		if(!strcmp(nom,liste->media[m]->nom) && (liste->media[m]->mode & mode)) return(liste->media[m]->path);
	return(-1);
}
/* ========================================================================== */
