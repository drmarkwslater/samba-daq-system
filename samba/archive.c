	...
	if(Archive.enservice) {
		...
		if(Trigger.enservice) {
			if(log) {
				...
				SambaCompleteLigne(86,printf("| Sauvegarde des detecteurs %s %s leurs associes;",ArchModeTexte[(int)ArchDetecMode],ArchAssocMode?"avec":"sans"));
			}
#ifdef PAW_H
			...
			if(ArchSauveNtp) {
				...
				if((f = fopen(FichierNtuples,"r"))) {
					...
					if(f) {
						...
						fprintf(f," Detecteur Voie Delai(s)");
			...
			} else if(log) SambaCompleteLigne(86,printf("| Pas de sauvegarde de Ntuple;"));
			if(TrmtRegulActive) {
		...
		if(LectSauveStatus) {
			...
			if(bb < NumeriseurNb) {
				...
				if((f = fopen(FichierStatusBB,"r"))) {
					...
					if(f) {
						int i,n;
						for(i=0; i<ModeleBNNb; i++) if(ModeleBN[i].enreg_dim) {
							fprintf(f,"%s [%d]:",ModeleBN[i].nom,ModeleBN[i].enreg_dim);
							/* for(k=0; k<ModeleBN[i].nbress; k++) if(ModeleBN[i].ress[k].enreg >= 0) fprintf(f," %s",ModeleBN[i].ress[k].nom); */
							for(n=0; n<ModeleBN[i].enreg_dim; n++) fprintf(f," %s",ModeleBN[i].ress[ModeleBN[i].numress[ModeleBN[i].enreg_sts[n]]].nom);
							fprintf(f,"\n");
						}
						fprintf(f,"Donnees :\n");
						if(log) {
...
FILE *ArchiveAccede(int fmt) {
	FILE *f; int bolo,voie,etage,nb,i,k;
...
#ifdef PAW_H
	/* Ntuple de PAW (voie qui a trigge) */
	if(ArchSauveNtp == NTP_PAW) {
		HbVar.Voie  = (float)triggee->num;
		HbVar.Type  = (float)VoieManip[voietrig].type;
		HbVar.Base  = triggee->base;
		HbVar.Ampl  = triggee->amplitude;
		HbVar.Mont  = triggee->montee;
		HbVar.Desc  = triggee->descente;
		HbVar.Bruit = triggee->bruit;
		...
		if(ArchSauveNtp && FichierNtuples[0] && !RegenEnCours) {
			...
			if(g) {
				for(n=0; n<evt_total; n++) {
					...
					fprintf(g," %d %d %g",Evt[n].bolo_hdr,Evt[n].voie_hdr,Evt[n].delai);
					for(voie=0; voie<VoiesNb; voie++) if(VoieTampon[voie].lue) {
						...
						else fprintf(g," %g %g %g %g %g",ajoutee->montee,ajoutee->amplitude,ajoutee->base,ajoutee->bruit,ajoutee->descente);
