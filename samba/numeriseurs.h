...
#define NUMER_ACCES_MAXMED 16
#define NUMER_ACCES_MAXFMT 40
#define NUMER_ACCES_MAXCOM 16
...
#define ACCES_VARIABLES "ident/adrs/nom/valeur"
...
typedef struct {
	...
	unsigned short ssadrs;      /* adresse interne du BN                                    */
	unsigned int masque;        /* masque indiquant le nombre de bits utilises              */
...
//	char  enreg;                /* si ressource a enregistrer                               */
	unsigned short defaut;      /* valeur par defaut si ressource initialisable (cf <init>) */
	...
	union {
		...
		struct { char cles[NUMER_RESS_KEYSTR]; unsigned short hval[NUMER_CLES_MAX]; int nb; } k; /* mots-cles */
	} lim;                    /* parametres de conversion                                */
#else
	struct { float min,max; } lim_r;    /* flottants */
	struct { int min,max; } lim_i;      /* entiers   */
	struct { char cles[NUMER_RESS_KEYSTR]; unsigned short hval[NUMER_CLES_MAX]; int nb; } lim_k; /* mots-cles */
#endif
	//? unsigned short (*usertohexa)(void *ptr); /* fonction speciale de conversion user->hexa */
	//? void (*hexatouser)(unsigned short hval, void *ptr); /* idem, hexa->user */
} TypeBNModlRessource;
...
NUMERISEUR_VAR ArgDesc ModeleBnAccesDesc[]
 ...
 = {
	...
	{ "variables",    DESC_KEY_SIZE(ModeleBNLu.acces.nb,NUMER_ACCES_MAXVAR) ACCES_VARIABLES, ModeleBNLu.acces.var,  0 },
...
typedef struct {
	...
	unsigned short hval;     /* valeur a envoyer dans la ressource                  */
	union {
...
//	char             fmt;         /* type de donnees transmises                          */
	unsigned short   ident;       /* identificateur du numeriseur                        */
	unsigned short   adc_id[NUMER_ADC_MAX]; /* identificateurs des ADC                   */
	...
	unsigned short status_val[NUMER_RESS_MAX]; /* table desdites valeurs                  */
	unsigned short enreg_val[NUMER_RESS_MAX];  /* table de ces valeurs a enregistrer      */
...
NUMERISEUR_VAR MenuItem iNumeriseurRessUpDown[]
#ifdef NUMERISEURS_C
 = {
...
unsigned short NumeriseurChargeRessourceAuto(TypeNumeriseur *numeriseur, int k, char *prefixe, char *marge);
