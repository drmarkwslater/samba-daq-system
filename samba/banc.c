...
static char BancSptrExec(int etape, char *explics) {
	int vbb,voie,vp; int nbfreq,dim,n;
	// char affiche_evts;
	...
	pour_tout(vbb,BancSptrResultat[etape].trace.nbadc) {
		...
		CalcSpectreAuto[vp].dim = BancSptrPoints;
...
/* ========================================================================== */
static int BancSptrLance(Figure fig, WndUserRequest *e) {
	BancSptrMesure((int)(fig->adrs)); return(0);
}
/* ========================================================================== */
...
static MenuItem iBancSauvegardes[] = {
...
static void BancAfficheOnglet(int numero) {
	...
	cas_ou(BancPhase) {
	  ...
	  vaut BANC_CONSOS:
		...
		PanelKeyB(p,"Alim","Inutilisable/Connectee",&BancAlimConnectee,13); PanelItemColors(p,1,SambaRougeVertJaune,2);
		PanelKeyB(p,"Gene","Inutilisable/Connecte",&BancGeneConnecte,13); PanelItemColors(p,2,SambaRougeVertJaune,2);
		PanelSupport(p,WND_CREUX);
		BoardAddPanel(bBanc,p,x + (11 * Dx),y,0);
		
		p = pBancAlimDemande = PanelCreate(2); PanelMode(p,PNL_DIRECT);
		PanelSupport(p,WND_CREUX);
		i = PanelFloat(p,"Tension nominale",&BancAlimTension); PanelLngr(p,PanelFormat(p,i,"%.3f"),7);
		i = PanelFloat(p,"Courant limite",&BancAlimCourant); PanelLngr(p,PanelFormat(p,i,"%.3f"),7);
		BoardAddPanel(bBanc,p,OPIUM_A_DROITE_DE 0);
		
		//			cdr = p->cdr;
		p = pBancAlimMesure = PanelCreate(2); PanelMode(p,PNL_DIRECT|PNL_RDONLY);
		i = PanelFloat(p,"Tension effective",&BancAlimVmesure); PanelLngr(p,PanelFormat(p,i,"%.3f"),7);
		i = PanelFloat(p,"Courant effectif ",&BancAlimImesure); PanelLngr(p,PanelFormat(p,i,"%.3f"),7);
		PanelSupport(p,WND_CREUX);
		BoardAddPanel(bBanc,p,OPIUM_A_DROITE_DE 0);
		...
		pour_tout(etape,BANC_CONSOS_NBETAPES) {
			...
			i = PanelText(p,"le",BancAlimMesure[etape].date,9); PanelLngr(p,i,8);
			i = PanelText(p,"a",BancAlimMesure[etape].heure,9); PanelLngr(p,i,8);
			i = PanelFloat(p,"Duree (s)",&BancAlimMesure[etape].duree); PanelLngr(p,PanelFormat(p,i,"%.3f"),7);
			i = PanelFloat(p,"Tension effective (V)",&BancAlimMesure[etape].tension); PanelLngr(p,PanelFormat(p,i,"%.3f"),7);
			i = PanelFloat(p,"Courant effectif  (A)",&BancAlimMesure[etape].courant); PanelLngr(p,PanelFormat(p,i,"%.3f"),7);
	  ...
	  vaut BANC_FET:
		...
		pour_tout(etape,BANC_SPECTRE_NBETAPES) {
			...
			PanelItemColors(p,i,BancJauneOrangeVert,3);
			i = PanelText(p,"le",BancSptrResultat[etape].date,9); PanelLngr(p,i,8);
			i = PanelText(p,"a",BancSptrResultat[etape].heure,9); PanelLngr(p,i,8);
