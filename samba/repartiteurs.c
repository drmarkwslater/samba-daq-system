...
static unsigned short OperaMasque;
static unsigned short  OperaCodeEntrees;
...
static int OperaEntreesLimite(TypeRepartiteur *repart, unsigned short *code_entrees);
static void OperaReset(TypeRepartiteur *repart, float echantillonage, char avec_opium);
static unsigned short OperaDecodeSelecteur(TypeRepartiteur *repart, unsigned short selecteur);
...
static unsigned short CaliLissage(unsigned short diviseur0, char revision);
...
INLINE static void IpeEcritRessourceBB(TypeRepartiteur *repart, short entree,unsigned short adrs, char ss_adrs, TypeADU valeur);
...
static MenuItem iRepartiteurClique[] = {
...
char RepartiteurModlLit(char *fichier) {
	...
	if((f = fopen(nom_complet,"r"))) {
		SambaLogDef(". Lecture des modeles de repartiteurs","dans",nom_complet);
...
void RepartiteurInit(int modl, TypeRepartiteur *repart, char interface) {
	...
	repart->extens = 0xFFFF - (unsigned short)repart->masque; repart->zero = (repart->masque + 1) / 2;
	switch(repart->famille) {
		case FAMILLE_OPERA: case FAMILLE_SAMBA: case FAMILLE_IPE: repart->ecr.type = MEDIA_UDP_WR; break;
		case FAMILLE_CALI: repart->ecr.type = MEDIA_TCP_WR; break;
...
//	repart->event = REP_DATA_STREAM;
	...
	repart->octets = 0; repart->fifo32 = 0; repart->fifo16 = 0;
	repart->depilable = 0;
	repart->entree = 0;
	repart->nbdonnees = 0;
	repart->nbident = 0;
...
//		repart->ecr.type = MEDIA_TCP_WR;
		...
		repart->r.ipe.chassis = 0; // RepartIpeNb++;
		repart->r.ipe.fifo = -1;
...
void RepartiteurVerifieParmsEtFIFO(TypeRepartiteur *repart, char *message) {
	...
	int voies_par_mot; unsigned short d1;
	...
	switch(repart->famille) {
		...
		case FAMILLE_OPERA: case FAMILLE_CALI: case FAMILLE_SAMBA: case FAMILLE_IPE:
			if(repart->famille == FAMILLE_OPERA) {
				...
				repart->r.ipe.chassis = (rep < repart->rep)? Repart[rep].r.ipe.chassis: k+1;
...
//			repart->ecr.path = -1; repart->lec.path = -1;
			...
			num_utilise =
			   ...
			   (((repart->famille == FAMILLE_SAMBA) && (repart->adrs.champ[3] > 20))? repart->adrs.champ[3] - 20:
				repart->adrs.champ[3]);
			if(RepartNumLibre <= num_utilise) RepartNumLibre = num_utilise + 1;
			...
			if(rep < repart->rep) {
				...
				memcpy(&(repart->ecr.skt),&(Repart[rep].ecr.skt),sizeof(TypeSocket));
				repart->valide = 0;
			} else if(SocketInitNum(repart->adrs.val,repart->ecr.port,&(repart->ecr.skt)) <= 0) {
...
//-	repart->nbentrees = repart->maxentrees;
	...
	repart->maxdonnees = ((int)(repart->fifo32) + repart->octets - (int)(repart->fifo16)) / sizeof(TypeDonnee);
...
static char RepartiteurBrancheIdent(TypeRepartiteur *repart) {
	...
	unsigned short ident; short vpn;
	unsigned short d1; int voies_par_mot;
	TypeNumeriseur *numeriseur;
...
/* ========================================================================== */
char RepartiteurAdopte(TypeRepartiteur *repart, char verifie) {
	char ok; int i,j,sat;
	...
	if(!repart) { printf("  ! Repartiteur indefini\n"); return(0); }
		...
		if((repart->interf == INTERF_IP) && verifie) {
			if(repart->valide) {
				if(SambaMaitre) {
					...
					if(j >= Partit[PartitLocale].nbsat) {
						printf("  ! %s n'est pas lu par la partition locale\n",repart->nom);
			...
			} else { printf("  ! Repartiteur %s[%d.%d.%d.%d:%d] HS\n",repart->nom,IP_CHAMPS(repart->adrs),repart->ecr.port); ok = 0; }
		}
	}
	return(ok);
}
...
int RepartiteursVerifie(char toutes, char verifie, char log) {
	int voie,bolo,cap,bb,rep,j,n,l; TypeRepartiteur *repart;
	...
	for(voie=0; voie<VoiesNb; voie++) if(VoieTampon[voie].lue && !VoieManip[voie].pseudo) {
		...
		if(!(repart->utile)) {
			if(log) { l = printf("|  + %s",repart->nom); SambaCompleteLigne(86,l); }
			if(!RepartiteurAdopte(repart,verifie)) n++;
		}
	}
	for(rep=0; rep<RepartNb; rep++) if(Repart[rep].status_demande) {
		repart = &(Repart[rep]);
		if(log) { l = printf("|  + %s (connexion demandee)",repart->nom); SambaCompleteLigne(86,l); }
		if(!RepartiteurAdopte(repart,verifie)) n++;
		break;
	}
...
void RepartiteurAppliquePerimetre() {
	...
	for(rep=0; rep<RepartNb; rep++) if((Repart[rep].categ == DONNEES_STD) || (Repart[rep].categ == DONNEES_BB)) {
		...
		for(l=0; l<Repart[rep].nbentrees; l++) {
			if((numeriseur = (TypeNumeriseur *)(Repart[rep].entree[l].adrs))) {
				...
				if(log) printf("    %d> Numeriseur %s: %s (%d ADC%s)\n",l+1,numeriseur->nom,numeriseur->local? "local":"etranger",Accord1s(numeriseur->nbadc));
				for(vbb=0; vbb<numeriseur->nbadc; vbb++) {
					...
					if((bolo = VoieManip[voie].det) >= 0) {
						...
						if(log) printf(", detecteur#%d ('%s') %s\n",bolo+1,Bolo[bolo].nom,Bolo[bolo].local? "local":"etranger");
	...
	if(CompteRendu.detectr.perimetre) {
		...
		for(bolo=0; bolo<BoloNb; bolo++) printf("    | %s: %s\n",Bolo[bolo].nom,Bolo[bolo].local? "local":"etranger");
...
void RepartiteursSelectionneVoiesLues(char toutes, char log) {
	int i,k,l,adc,rep,vl,voie,modl,nbregs,reg,pflt,flt;
	int lngr;
	short vpn; // ,saut
...
#define SEL_NUMER_PAR_OCTET
	byte selcanal[4*MAX_SEL]; unsigned int selection[MAX_SEL],autorise;
	byte selcanevt[4*MAX_SEL]; unsigned int selectevt[MAX_SEL];
	byte buffer[65536];
	unsigned short d1; int voies_par_mot;

	printf(". Selection de %s voies\n",toutes?"toutes les":"certaines");
	for(rep=0; rep<RepartNb; rep++) printf(". %s: %s\n",Repart[rep].nom,Repart[rep].utile?"a utiliser":"pas concerne");
	printf("\n\n");
	if(log) {
		LogBlocBegin();
		lngr =
	...
	autre = 0;
	SambaEchantillonReel = SambaEchantillonLngr = 0; RepartActif = 0;
	  ...
	  case INTERF_IP:
		...
		if((repart->famille == FAMILLE_OPERA) && (repart->nbdonnees < VOIES_PAR_4OCTETS)) {
			...
			k = printf("%s Trame       : %d echantillon%s [%g ms], %d stamp%s, %d octet%s;",prefixe,
				Accord1s(repart->s.ip.duree_trame),temps,Accord1s(nb_stamps),Accord1s(repart->s.ip.taille_trame));
				if(lngr) SambaLigneTable(' ',k,lngr); else printf("\n");
		} else {
			k = printf("%s Trame       : %d echantillon%s [%g ms], %d octet%s;",prefixe,
				Accord1s(repart->s.ip.duree_trame),temps,Accord1s(repart->s.ip.taille_trame));
				if(lngr) SambaLigneTable(' ',k,lngr); else printf("\n");
		}
...
int RepartChargeFpga(char *explics) {
	...
	for(rep=0; rep<RepartNb; rep++) if(Repart[rep].charge_fpga) {
		...
		if(repart->interf == INTERF_IP) {
			if(repart->famille == FAMILLE_IPE) {
				byte texte[80]; int l;
				l = sprintf((char *)texte,"%s_0x%08X_0x%08X",IPE_REG_WRITE,IPE_SLT_PIXBUS,IpePixBus); texte[l] = '\0';
				if(repart->ecr.path >= 0) { RepartIpEcrit(repart,texte,l); SambaMicrosec(ModeleRep[repart->type].delai_msg); }
			}
...
//			nbsleeps = 8; /* fois timesleep */
			if(repart->famille == FAMILLE_OPERA) {
				Structure_trame_status *trame; unsigned int lngr; int limite;
				...
				do {
					nb = RepartIpLitTrame(repart,trame,sizeof(Structure_trame_status),0,&(repart->lec.skt),&lngr);
					...
					if(nb == -1) { if(errno == EAGAIN) SambaMicrosec(timesleep); else break; }
						...
						if(BigEndianOrder) ByteSwappeInt((unsigned int *)trame);
						// printf("               > trame #%x/%x\n",OPERA_TRAME_NUMERO(trame),OPERA_TRAME_STS);
						if(OPERA_TRAME_NUMERO(trame) == OPERA_TRAME_STS) {
							vue[k] = 1;
							if(BigEndianOrder) ByteSwappeIntArray((unsigned int *)trame+1,(nb - 1) / sizeof(Ulong));
				...
				IpeTrame trame; unsigned int lngr; int limite;
				gettimeofday(&LectDateRun,0); limite = LectDateRun.tv_sec + (timesleep * nbsleeps / 1000000);
				trame = (IpeTrame)repart->fifo32;
				lngr = sizeof(TypeSocket);
				do {
					nb = RepartIpLitTrame(repart,trame,repart->s.ip.taille_struct,0,&(repart->lec.skt),&lngr);
					if(nb == -1) { if(errno != EAGAIN) {
						...
						if(BigEndianOrder) ByteSwappeIntArray((unsigned int *)trame,nb / sizeof(Ulong));
...
char RepartAjusteHorloge(float *frequence, float *relance) /* kHz, sec */ {
	int rep,modl; unsigned short diviseur0,d0,d1,lissage; float horloge,echantillonnage;
	int voies_par_mot;
	char clck,ok;
...
int RepartiteurRafraichi() {
	TypeRepartiteur *repart; TypeNumeriseur *numeriseur;
	int i,k,l,vl,vt,nb,max,prem,actives,vpn; unsigned int zoom; char fin_flt,trait_a_droite;
...
/* ========================================================================== */
INLINE static void CluzelEcritRessourceBB(TypeRepartiteur *repart, unsigned short adrs, char ss_adrs, TypeADU valeur) {
	if(!repart) { printf("! Repartiteur indefini\n"); return; }
#ifdef AVEC_PCI
...
/* ========================================================================== */
char CluzelDepile1Donnee(TypeRepartiteur *repart, int32 *donnee, int *a_decompter, char log) {
	int premier,num,dep;
	char lue;
...
char OperaNouvelle(TypeRepartiteur *repart, char *adrs, char categ,
	int mode, unsigned char retard, char horloge) {
	//- char message[256];
...
void OperaRecopieStatus(Structure_trame_status *trame, TypeRepartiteur *repart) {
	...
	if(trame->status_opera.micro_bbv2 == 1) {
		...
		if(stade <= 100) snprintf(RepartInfoFpgaBB,REPART_INFOFPGA_DIM,"%3d/100 prgm BB#%02d",
			...
			int port; unsigned char buffer[8];
...
/* ==========================================================================
static int OperaEntreesNb(unsigned short code_entrees) {
	int i,n;
   ...
   ========================================================================== */
static int OperaEntreesLimite(TypeRepartiteur *repart, unsigned short *code_entrees) {
	int i,n,max; unsigned short code_final;
...
/* ========================================================================== */
static unsigned short OperaEncodeSelecteur(TypeRepartiteur *repart, unsigned short code_entrees, int voies_par_mot) {
	unsigned short entrees,sel,mots;
	int i;
...
/* ========================================================================== */
static unsigned short OperaDecodeSelecteur(TypeRepartiteur *repart, unsigned short selecteur) {
	unsigned short sel; int i;
	unsigned short code_entrees;
...
static void OperaReset(TypeRepartiteur *repart, float echantillonage, char avec_opium) {
	...
	unsigned char buffer[16]; char explics[256];
...
/* ========================================================================== */
void OperaDimensions(int mode, unsigned short *d1, int *voies_par_mot, short *vpn) {
	int bbvoies;
	switch(mode) {
...
static void OperaCompteursTrame(TypeRepartiteur *repart) {
	...
	unsigned short d1; int voies_par_mot; short vpn;

	OperaDimensions(repart->r.opera.mode,&d1,&voies_par_mot,&vpn);
	repart->s.ip.taille_struct = sizeof(TypeOperaTrame);
	repart->s.ip.taille_entete = OPERA_TAILLE_ENTETE;
	...
	repart->s.ip.taille_trame = (repart->s.ip.duree_trame * nbdonnees * sizeof(TypeDonnee)) + OPERA_TAILLE_ENTETE;
	...
	repart->simu.intervalle = (int64)(1000.0 * (float)(repart->s.ip.duree_trame) / Echantillonnage);
}
/* ========================================================================== */
static void OperaConfigure(TypeRepartiteur *repart) {
	int l,nb_bn,voies_par_mot,type,vsnid; unsigned short d1;
...
/* ========================================================================== */
int OperaSelecteur(TypeRepartiteur *repart, int selection, float echantillonage, unsigned char *buffer, int *l, char *explics) {
	int modl,voies_par_mot; int n; unsigned short diviseur1;
	...
	repart->r.opera.d0 = (unsigned short)((ModeleRep[modl].horloge * 1000.0 / (echantillonage * (float)diviseur1)) + 0.5);
	buffer[0] = 'h';
	...
	buffer[5] = (unsigned char)OperaEncodeSelecteur(repart,selection,voies_par_mot);
...
/* ========================================================================== */
static int OperaCmdeFpga(TypeRepartiteur *repart, int fpga) {
	unsigned char buffer[4]; int port;
	...
	buffer[0] = 'Q'; buffer[1] = (unsigned char)FpgaModl[fpga].opera;
...
/* ========================================================================== */
static int OperaCmdeQ(TypeRepartiteur *repart) {
	unsigned char buffer[4],code; int port; int i;
...
static int OperaCmdeH(TypeRepartiteur *repart) {
	unsigned char buffer[8]; int i,l;
	int voies_par_mot,nb_bn; unsigned short d1;
...
INLINE void OperaSwappeTrame(void *buffer, int nb) {
	ByteSwappeIntArray((unsigned int *)buffer,nb / sizeof(unsigned int));
}
/* ========================================================================== */
void OperaDump(unsigned char *buffer, int nb, char hexa) {
...
/* ========================================================================== */
INLINE static void OperaDemarreNumeriseurs(TypeRepartiteur *repart, NUMER_MODE mode, unsigned char adrs, char *prefixe) {
	unsigned char buffer[8]; unsigned short cde_cluzel;
...
/* ========================================================================== */
INLINE static void OperaEcritRessourceBB(TypeRepartiteur *repart, unsigned short adrs, char ss_adrs, TypeADU valeur) {
	unsigned char buffer[8]; int n;
...
/* ==========================================================================
INLINE static void OperaEcritNumeriseur(TypeRepartiteur *repart, unsigned char *cmde, int lngr) {
	unsigned char buffer[8]; int i,j,n;
...
static INLINE char OperaVidePile(TypeRepartiteur *repart, char *pas_tous_secs, short *pleins, char log) {
	...
	unsigned char *pile,fin_trame[OPERA_TAILLE_ENTETE];
	OperaTrame trame;
	...
	pile = (unsigned char *)repart->fifo32; /* fifo16 est decalee de OPERA_TAILLE_ENTETE octets */
	...
	octets_lus = RepartIpLitTrame(repart,trame,sizeof(TypeOperaTrame),0,&(repart->lec.skt),&lngr);
	...
	} else if(((TypeSocketIP *)(&(repart->lec.skt)))->sin_addr.s_addr != repart->adrs.val) {
	...
	*pas_tous_secs = 1;
	...
	if(BigEndianOrder) ByteSwappeIntArray((unsigned int *)trame,octets_lus / sizeof(Ulong));
			...
			for(l=0; l<repart->nbentrees; l++) {
				if((numeriseur = (TypeNumeriseur *)repart->entree[l].adrs)) {
					...
					if(modele_bn->enreg_dim && !numeriseur->enreg_stamp) {
						unsigned int *status_lu; int nb;
						if(numeriseur->avec_status) {
							status_lu = (unsigned int *)(((Structure_trame_status *)trame)->status_bbv2_1);
		...
		if(LectLitStatus) {
			...
			for(l=0; l<repart->nbentrees; l++) {
				...
				if(numeriseur && numeriseur->status.a_copier) {
					unsigned int *status_lu; int i,nb;
					if(numeriseur->avec_status) {
						status_lu = (unsigned int *)(((Structure_trame_status *)trame)->status_bbv2_1);
						nb = _nb_mots_status_bbv2;
					} else status_lu = 0;
...
static void CaliCompteursTrame(TypeRepartiteur *repart) {
	int n;
	repart->s.ip.taille_struct = sizeof(TypeCaliTrame);
	repart->s.ip.taille_entete = CALI_TAILLE_ENTETE;
	repart->s.ip.num_sts = 0xFFFF;
	n = repart->nbdonnees; if(n <= 0) n = 1;
	repart->s.ip.duree_trame = CALI_DONNEES_NB / n; // ((CALI_DONNEES_NB - 1) / n) + 1;
	repart->s.ip.taille_trame = (repart->s.ip.duree_trame * n * sizeof(TypeDonnee)) + sizeof(TypeCaliEntete);
	...
	repart->s.ip.taille_derniere = (int)(repart->s.ip.stampD3 - (repart->s.ip.duree_trame * repart->s.ip.tramesD3));
	repart->simu.intervalle = (int64)(1000.0 * (float)(repart->s.ip.duree_trame) / Echantillonnage);
//	printf("(%s) Intervalle entre deux trames: %lld us\n",__func__,repart->simu.intervalle);
}
#define CALI_LISSAGE
/* ========================================================================== */
static unsigned short CaliLissage(unsigned short diviseur0, char revision) {
	unsigned short lissage;
#ifdef CALI_LISSAGE
	short i; float div,dist,min;
#endif
...
/* ========================================================================== */
int CaliSelectCanaux(TypeRepartiteur *repart, unsigned char selection, float echantillonage, unsigned char *buffer, int *t) {
	int modl; int l,n;
	unsigned short diviseur0,diviseur1,d0,lissage; unsigned int cntrl;

	if(!RepartIpEcriturePrete(repart,"  ")) { *t = 0; return(0); }
	modl = repart->type;
	diviseur1 = 1; /* curieusement... */
	diviseur0 = (unsigned short)((ModeleRep[modl].horloge * 1000.0 / (echantillonage * (float)diviseur1)) + 0.5);
	...
	cntrl = (unsigned int)(selection & 0xF);
	if(repart->r.cali.clck) cntrl = (cntrl | 0x10);
...
/* ========================================================================== */
int CaliSelecteur(TypeRepartiteur *repart, int selection, float echantillonage, unsigned char *buffer, int *t) {
	int modl; int l,n;
	unsigned short diviseur0,diviseur1,d0,lissage; unsigned int cntrl;
...
//		printf("Niveau 1: inversion!\n");
		...
		ByteSwappeInt((unsigned int *)(buffer + 8));
		ByteSwappeIntArray((unsigned int *)(buffer + 16),(nb - 16) / sizeof(unsigned int));
	} else {
		unsigned int id;
//		printf("Niveau 2: inversion!\n");
		id = *(unsigned int *)(buffer + 8); ByteSwappeInt(&id);
		*(unsigned int *)(buffer + 8) = *(unsigned int *)(buffer + 12);
		*(unsigned int *)(buffer + 12) = id;
		//? FourShortsSwappeLongArray((int64 *)(buffer + 16),(nb - 16) / sizeof(int64));
		// TwoShortsSwappeIntArray((unsigned int *)(buffer + 16),(nb - 16) / sizeof(unsigned int));
...
/* ========================================================================== */
void CaliSwappeTrame(TypeRepartiteur *repart, CaliTrame trame, int nb) {
	if(repart->revision == 1) {
		ByteSwappeLong(&(trame->hdr.stamp));
		ByteSwappeInt((unsigned int *)&(trame->hdr.id));
		ByteSwappeIntArray((unsigned int *)&(trame->valeur),(nb - CALI_TAILLE_ENTETE) / sizeof(unsigned int));
	} else {
		unsigned int id; int *adrs32;
		adrs32 = (int *)(trame->hdr.status);
		id = trame->hdr.id; ByteSwappeInt(&id);
		trame->hdr.id = *adrs32;
		*adrs32 = id;
		{	int i; int64 *adrs64; int n64; FourShortsSwappable tempo;
...
/* ========================================================================== */
static void CaliDeSwappeRev2(void *buffer, int nb) {
	ByteSwappeLong((int64 *)buffer);
	ByteSwappeInt((unsigned int *)(buffer + 8));
	ByteSwappeIntArray((unsigned int *)(buffer + 16),(nb - 16) / sizeof(unsigned int));
...
/* ========================================================================== */
void CaliDump(TypeRepartiteur *repart, unsigned char *buffer, int nb, char hexa) {
	int num; int64 stamp;
	...
	if(nb == -1) {
		...
		if(hexa) {
			if(!BigEndianOrder) {
				...
				else CaliDeSwappeRev2((void *)buffer,nb);
			}
			court = 0; // GCC
		} else {
			if(!BigEndianOrder) {
		...
		if(hexa) {
			...
			if(!BigEndianOrder) {
				FourShortsSwappeLong((int64 *)buffer);
				TwoShortsSwappeInt((unsigned int *)(buffer + 8));
				if(repart->revision == 1) TwoShortsSwappeIntArray((unsigned int *)(buffer + 16),(nb - 16) / sizeof(unsigned short));
			}
		}
	}
}
...
/* ========================================================================== */
INLINE static void CaliEcritRessourceBB(TypeRepartiteur *repart, unsigned short adrs, char ss_adrs, TypeADU valeur) {
	int l;
...
void CaliDumpTrameEvt(CaliTrame trame_cali, size_t taille_trame) {
	...
	{
		unsigned char *c; unsigned short *s; int k; size_t hdr_lngr;
		...
		c = (unsigned char *)trame_cali; k = 0;
		while(n--) {
		...
		if(trame_evt->type.etiquette == REPART_EVENT_DATA) {
			...
			s = (unsigned short *)((char *)trame_cali + hdr_lngr); k = 0;
			printf("  | Donnees   :\n  |  000: ");
			while(n--) {
				printf("%7d",*s++); k++;
...
static INLINE char CaliVidePile(TypeRepartiteur *repart, char *pas_tous_secs, short *pleins, char log) {
	unsigned int lngr;
	int octets_lus,donnees_lues,num_trame,haut_pile,sautees,stamp_absents,lues,nb,nbD3;
	unsigned char *pile,fin_trame[CALI_TAILLE_ENTETE];
	...
	pile = (unsigned char *)repart->fifo32; /* fifo16 est decalee de CALI_TAILLE_ENTETE octets */
	...
	octets_lus = RepartIpLitTrame(repart,trame,sizeof(TypeCaliTrame),0,&(repart->lec.skt),&lngr);
	...
	*pas_tous_secs = 1;
	...
	/* CaliDump(repart,(unsigned char *)trame,octets_lus,1); */
		...
		if(trame_evt->type.etiquette == REPART_EVENT_DEF) {
			...
			for(c=0; c<info->dim; c++) {
				...
				if((nb = EvtStocke[RepartEvtEmpile].nbvoies) < REPART_VOIES_MAX) {
					...
					EvtStocke[RepartEvtEmpile].voie[nb].adrs = (voie >= 0)? VoieTampon[voie].prochain - VoieTampon[voie].brutes->t.sval: 0;
			...
			TypeEventData *donnees; TypeDonnee *val,*prochain,donnee; int nb; short voie;
...
//				if(BigEndianOrder) val_lue = val[nb++]; else if(nb & 1) val_lue = val[nb++ - 1]; else val_lue = val[++nb];
				if((voie >= 0) && VoieTampon[voie].brutes && VoieTampon[voie].brutes->max) {
					...
					if(repart->famille == FAMILLE_CALI) {
						int adu;
						adu = DATAVALEUR(voie,val_lue);
						donnee = (TypeDonnee)(adu - (int)VoieManip[voie].zero);
					} else if((repart->famille == FAMILLE_OPERA) && (repart->r.opera.mode == code_acqui_EDW_BB2)) {
						donnee = (LectEnCours.valeur & 0x8000)? LectEnCours.valeur & 0x7FFF: LectEnCours.valeur | 0x8000;
					} else {
						if(VoieManip[voie].ADCmask == 0xFFFF) donnee = (TypeDonnee)val_lue;
						else {
							donnee = DATAVALEUR(voie,val_lue);
							if(VoieManip[voie].signe) {
							#ifdef DONNEES_NON_SIGNEES
								if(donnee < VoieManip[voie].zero) donnee += VoieManip[voie].zero;
								else donnee -= VoieManip[voie].zero;
							#else
							#ifdef SIGNE_ARITHMETIQUE
								if(donnee >= VoieManip[voie].zero) donnee -= (2 * VoieManip[voie].zero);
							#else  /* SIGNE_ARITHMETIQUE */
								if(donnee & VoieManip[voie].zero) donnee |= VoieManip[voie].extens;
					...
					VoieTampon[voie].derniere = donnee;
					*(VoieTampon[voie].prochain) = donnee;
					LectStampsLus++;
					prochain = VoieTampon[voie].prochain;
					if(((VoieTampon[voie].brutes->prem + 1) == VoieTampon[voie].brutes->max) || ((VoieTampon[voie].brutes->nb + 1) == VoieTampon[voie].brutes->max))
						prochain = VoieTampon[voie].brutes->t.sval;
					else prochain++;
					VoieTampon[voie].courante = *VoieTampon[voie].prochain;  /* pour la reutilisation de voies (pseudos) */
					VoieTampon[voie].lus++;
					if(VoieTampon[voie].brutes->nb < VoieTampon[voie].brutes->max) {
						...
						} else VoieTampon[voie].prochain += 1;
					} else {
					...
					if(prochain != VoieTampon[voie].prochain) {
		...
		if(repart->s.ip.numAVenir >= 0) {
			...
			if((timestamp != repart->s.ip.stampEnCours) || (num_trame != repart->s.ip.numAVenir)) {
				...
				if(VerifErreurLog) {
					...
					charge_utile = octets_lus - repart->s.ip.taille_entete;
...
#pragma mark ----------- SAMBA -----------
/* ========================================================================== */
INLINE int SmbEcrit(TypeRepartiteur *repart, unsigned char cmde, unsigned char c1, unsigned char c2, unsigned short valeur) {
	unsigned char buffer[8]; int n;
...
/* ========================================================================== */
INLINE static void SmbDemarreNumeriseurs(TypeRepartiteur *repart, NUMER_MODE mode, unsigned char adrs, char *prefixe) {
	unsigned short cde_cluzel;
...
static int ArchDemarre(TypeRepartiteur *repart, int l) {
	...
	if(repart->r.f.fmt == ARCH_FMT_EDW0) {
		...
		for(vt=0; vt<VoiesLues; vt++) {
			...
			voie = vt;
			VoieManip[voie].def.trmt[TRMT_AU_VOL].parm = parm;
...
static INLINE char ArchLitBloc(TypeRepartiteur *repart, char *pas_tous_secs, short *pleins, char log) {
	...
	for(l=0; l<repart->nbentrees; l++) {
		...
		lu = 0; //? lu = attendu;
		if(repart->p.f.bin) {
			if(repart->p.f.a[l].o.path >= 0) lu = read(repart->p.f.a[l].o.path,repart->fifo16 + repart->top,attendu); // LectArchPath
		} else {
			if(repart->p.f.a[l].o.file) lu = fread(repart->fifo16 + repart->top,sizeof(TypeDonnee),nb,repart->p.f.a[l].o.file) * sizeof(TypeDonnee); // LectArchFile
		}
		if(lu != attendu) {
			if(repart->p.f.bin) {
		...
		if(!BigEndianOrder) ByteSwappeWordArray((unsigned short *)(repart->fifo16 + repart->top),nb);
		repart->top += nb;
...
static INLINE char NiVidePile(TypeRepartiteur *repart, char *pas_tous_secs, short *pleins, char log) {
	...
	if(repart->top == 0) {
		...
		DAQmxBaseReadBinaryI16(LectNITache[repart->adrs.val-1],DAQmx_Val_Auto,2.0,DAQmx_Val_GroupByScanNumber,
							   (int16 *)(repart->fifo16 + repart->top),max,&nb,NULL);
		if(nb) {
			repart->acces_remplis++;
			repart->top += nb;
			repart->donnees_recues += nb;
...
/* ========================================================================== */
char NiDepile1Donnee(TypeRepartiteur *repart, int32 *donnee) {
	if((repart->bottom < repart->top) && (repart->bottom < (repart->octets / sizeof(Ulong)))) {
		*donnee = repart->fifo16[repart->bottom++];
		repart->depile += 1;
		if(repart->bottom >= repart->top) repart->top = repart->bottom = 0;
	} else {
		repart->top = repart->bottom = 0;
...
static void IpeCompteursTrame(TypeRepartiteur *repart) {
	...
	if(!repart) return;
	repart->s.ip.taille_struct = sizeof(TypeIpeTrame);
	repart->s.ip.taille_entete = IPE_TAILLE_ENTETE;
	...
	repart->s.ip.taille_trame = (donnees_trame * sizeof(TypeDonnee)) + IPE_TAILLE_ENTETE;
...
#ifdef IPE_MESSAGES_V2
/* ========================================================================== */
int IpeSelectCanauxV2(TypeRepartiteur *repart, int nbregs, unsigned char *selection, float echantillonage, unsigned char *buffer, int *t) {
	int i,j,n;
...
#ifdef IPE_MESSAGES_V3
/* ==========================================================================
static void IpeCmdeSlt(TypeRepartiteur *repart, unsigned int reg, unsigned int val) {
	unsigned char texte[80]; int k,l;
   ...
   ==========================================================================
 static char IpeEcritConfig(TypeRepartiteur *repart, int flt, char *reg, unsigned int val, char log) {
 ...
 return(k == l);
   ...
   ==========================================================================
static int IpeHorloges(TypeRepartiteur *repart, float echantillonage, unsigned char *buffer, int *l, char *explics) {
	int modl; int i,n; unsigned short d0,d1;
	...
	modl = repart->type;
	d1 = 60;
	d0 = (unsigned short)((ModeleRep[modl].horloge * 1000.0 / (echantillonage * (float)d1)) + 0.5);
	buffer[0] = 'h';
	buffer[1] = _FPGA_code_horloge;
...
/* ========================================================================== */
static char IpeEcritCommande(TypeRepartiteur *repart, unsigned char *texte, unsigned char *buffer, int *t, int *n) {
	int j,k,l; char ok;
...
/* ========================================================================== */
char IpeEcritFlt(TypeRepartiteur *repart, int flt, int reg, unsigned int val, char log) {
	char texte[80]; int adrs,k,l;

	if(!repart) return(0);
	adrs = reg; if(BigEndianOrder) ByteSwappeInt((unsigned int *)&adrs);
...
/* ========================================================================== */
int IpeSelectCanaux(TypeRepartiteur *repart, int nbregs, unsigned char *selection, float echantillonage, unsigned char *buffer, int *t) {
	int i,j,k,l,m,n,o,flt,fbr,fltabs; unsigned short d0,d1; unsigned char texte[80]; char en_finale,louppe;
	unsigned int pixbus,masque,complet,bit; TypeNumeriseur *numeriseur;
	...
	d1 = 60;
	d0 = (unsigned short)((ModeleRep[repart->type].horloge * 1000.0 / (echantillonage * (float)d1)) + 0.5);
...
/* projet IPE initial:
	ByteSwappeShortArray((unsigned short *)buffer,2);
	ByteSwappeLong((int64 *)&(((IpeTrame)buffer)->p.donnees.hdr.stamp));
//	ByteSwappeShortArray((unsigned short *)((char *)(((IpeTrame)buffer)->p.donnees.hdr)+sizeof(int64)),sizeof(TypeIpeEntete)-sizeof(int64));
	ByteSwappeShortArray((unsigned short *)((char *)(&(((IpeTrame)buffer)->p.donnees.hdr.nbdetec))),sizeof(TypeIpeEntete)-sizeof(int64));
	ByteSwappeIntArray((unsigned int *)((IpeTrame)buffer)->p.donnees.val,(nb - sizeof(TypeIpeEntete) - (2 * sizeof(short))) / sizeof(unsigned int));
*/
	ByteSwappeIntArray((unsigned int *)buffer,nb / sizeof(unsigned int));
}
/* ========================================================================== */
...
/* ========================================================================== */
void IpeDump(unsigned char *buffer, int nb, char hexa) {
	int i,j,k; long n; unsigned short num,nb_d3; short *court; char deja_vu;
	int64 stamp_slt;
	TypeIpeBBsts *sts_bb;
...
void IpeRecopieStatus(IpeTrame trame, TypeRepartiteur *repart, char avec_pixbus) {
	...
	if(avec_pixbus) IpePixBus = trame->p.sts_crate.pixbus_enable;
	...
	if(trame->p.sts_crate.micro_bbv2 == 1) repart->arrete = 1;
		...
		int port,flt; unsigned char buffer[8];
		port = PortLectRep + repart->rep;
		buffer[0] = 'S'; buffer[1] = 0;
		buffer[2] = (byte)(port & 0xFF); buffer[3] = (byte)((port >> 8) & 0xFF);
		RepartIpEcrit(repart,buffer,4); SambaMicrosec(ModeleRep[repart->type].delai_msg);
...
/* ========================================================================== */
int IpeEcritPixbus(TypeRepartiteur *repart, unsigned char *texte, int *l) {
	int k;
...
INLINE static int IpeChangeParmsFlt(Panel p, void *adrs) {
	TypeRepartiteur *repart; int flt,fibre,fltabs,l,k,m,o; unsigned char texte[80];
	int pixbus,bit; unsigned int complet;
	TypeIpeFibreReg delais[IPE_FIBRE_REGS],selecteur[IPE_FIBRE_REGS];
...
INLINE static void IpeChangeMode(TypeRepartiteur *repart, char mode, char *prefixe) {
	unsigned char buffer[80]; unsigned int complet; int l,flt;
...
/* ========================================================================== */
INLINE static void IpeDemarreNumeriseurs(TypeRepartiteur *repart, NUMER_MODE mode, unsigned char adrs, char *prefixe) {
	unsigned char buffer[80]; unsigned short cde_cluzel; int flt;
...
/* static */ INLINE char IpeVidePile(TypeRepartiteur *repart, char *pas_tous_secs, short *pleins, char log) {
	...
	unsigned int lngr; long n; unsigned short num_trame;
	int64 stamp_recu,timestamp,stamp_attendu; int64 t2,t3;
	// char episode_en_cours;
	unsigned char *pile,fin_trame[IPE_TAILLE_ENTETE];
	TypeDonnee val;
	IpeTrame trame;
	char debug;
...
// #define TIMESTAMP_NON_CONTROLE
	...
	pile = (unsigned char *)repart->fifo32; /* fifo16 est decalee de IPE_TAILLE_ENTETE octets */
	...
	octets_lus = RepartIpLitTrame(repart,trame,sizeof(TypeIpeTrame),0,&(repart->lec.skt),&lngr);
	if(repart->s.ip.stamp_redemande >= 0) t3 = VerifTempsPasse? (DateMicroSecs() - LectT0Run): 0;
	if(octets_lus == -1) {
		if(errno == EAGAIN) {
			if(log) printf("          %s: vide\n",repart->nom);
	...
	*pas_tous_secs = 1;
	...
	//a dernier_temps = (float)(LectDateRun.tv_sec % 60) + ((float)LectDateRun.tv_usec / 1000000.0);
	if(BigEndianOrder) {
		ByteSwappeIntArray((unsigned int *)trame,octets_lus / sizeof(Ulong));
		TwoShortsSwappeInt((unsigned int *)trame);
	}
		...
		if(num_trame == IPE_TRAME_STS) {
			...
			if(repart->s.ip.synchro_vue) {
				...
				if((nbD3 * repart->s.ip.inc_stamp) != stamp_recu) /* pas un multiple de inc_stamp=100000 */ {
					...
					if(VerifErreurLog) IpeDump((unsigned char *)trame,octets_lus,1);
		...
		if(LectLitStatus || (Archive.enservice && LectSauveStatus)) {
			while(sts_bb->dim) {
				...
				if((l >= 0) && (l < repart->nbentrees) && (numeriseur = (TypeNumeriseur *)repart->entree[l].adrs) && (numeriseur->avec_status)) {
					...
					if(copie || archive) {
						nb = (sts_bb->dim - IPE_BBSTS_HDR) / sizeof(unsigned short);
						if(archive) {
	...
	if(num_trame == 0) {
		...
		if(repart->s.ip.lu_entre_D3 && (repart->s.ip.lu_entre_D3 != repart->s.ip.dim_D3)) {
			...
			if(repart->s.ip.lu_entre_D3 > repart->s.ip.dim_D3) {
				...
				dechet = 0;
				val = (repart->top > 0)? *(TypeDonnee *)(repart->fifo16 + (repart->top - 1)): 0;
				while(repart->s.ip.lu_entre_D3 < repart->s.ip.dim_D3) {
					*(TypeDonnee *)(repart->fifo16 + repart->top++) = val;
...
//	controle_strict = 0;
		...
		if(trame_inattendue) {
			...
			if((sautees < 0) && (RepartIpCorrection != REPART_IP_ABANDON)) {
				if(RepartIpCorrection == REPART_IP_INSISTE) /* loupee une deuxieme fois! */ {
					...
					printf("), %d en trop\n",-sautees);
					IpeDump((unsigned char *)trame,octets_lus,1);
	...
	if(repart->s.ip.numAVenir > repart->s.ip.tramesD3) /* (repart->s.ip.numAVenir == 0) 0 <=> chgmt timestamp <=> synchro D3 <=> trop tard .. */ {
		...
		if(debug) printf("%s! %s{%lld}: trame %12lld/%3d, %d octets recus soit %d donnees recues entre D3, remise a zero\n",DateHeure(),
			   repart->nom,repart->acces_remplis,timestamp,num_trame,octets_lus,repart->s.ip.lu_entre_D3);
		if(repart->s.ip.lu_entre_D3 == repart->s.ip.dim_D3) repart->s.ip.lu_entre_D3 = 0;
	} else if(octets_lus != repart->s.ip.taille_trame) {
		printf("%s! %s{%lld}: trame %12lld/%3d, %d octets recus (au lieu de %d), soit %d donnees recues entre D3\n",DateHeure(),
			repart->nom,repart->acces_remplis,timestamp,num_trame,octets_lus,repart->s.ip.taille_trame,repart->s.ip.lu_entre_D3);
		RepartIpErreurs++;
...
INLINE char RepartIpEcriturePrete(TypeRepartiteur *repart, char *marge) {
	...
	if(!repart) { printf("%s! Repartiteur indefini\n",marge); return(0); }
		...
		if(repart->simule) {
			...
			if((repart->ecr.path = SocketConnectUDP(&(repart->ecr.skt))) < 0) { printf("%s impossible (%s)\n",qui,SocketErreur()); return(0); }
			...
			if((repart->ecr.path = SocketConnect(&(repart->ecr.skt))) < 0) { printf("%s impossible (%s)\n",qui,SocketErreur()); return(0); }
			printf("%s ouvert en <%d>\n",qui,repart->ecr.path); repart->valide = 1;
...
INLINE ssize_t RepartIpEcrit(TypeRepartiteur *repart, const void *buffer, size_t lngr) {
	...
	if(repart->simule) return((ssize_t)lngr);
		...
		ssize_t rc;
		if(repart->ecr.path >= 0) rc = write(repart->ecr.path,buffer,lngr); else rc = -1;
		return(rc);
	}
}
/* ========================================================================== */
...
/* ========================================================================== */
static void RepartIpEnvoie(TypeRepartiteur *repart, unsigned char *buffer, int l, char termine, char attente, char log) {
	int rc;
...
/* ========================================================================== */
static void RepartIpToOpera(TypeRepartiteur *repart, unsigned char *buffer, char log) {
	int rc;
...
INLINE void RepartIpTransmission(TypeRepartiteur *repart, char nature, int duree, char log) {
	...
	int l,i,adc,voie,num; char trgr; int trames; unsigned short val; unsigned char buffer[256];
	TypeNumeriseur *numeriseur;

	if(!repart) return;
	if(!RepartIpEcriturePrete(repart,"            ")) return;
	trames = (duree >= 0)? duree: 0;
	// if(trames) {
	if(repart->lec.path < 0) {
		if(log) printf("%s/   . Affectation du port %d pour la lecture\n",DateHeure(),repart->lec.port);
		if(repart->simule) {
			repart->lec.path = repart->rep;
			if(log) printf("%s/   ! Socket pour lecture sur %d.%d.%d.%d:%d simulee.\n",DateHeure(),IP_CHAMPS(repart->adrs),repart->lec.port);
		} else {
			if((repart->lec.path = SocketUDP(repart->lec.port)) >= 0) {
				if(fcntl(repart->lec.path,F_SETFL,O_NONBLOCK) == -1) {
		...
		if(repart->famille == FAMILLE_SAMBA) {
			SmbEcrit(repart,SMB_CONNECT,(unsigned short)repart->lec.port);
			if(log >= 0) printf("%s/   . Ecrit '%s'\n",DateHeure(),RepartiteurValeurEnvoyee);
		}
	}
	// }
	if(repart->lec.path >= 0) {
		if(log) printf("%s/   . Acces en lecture sur %d.%d.%d.%d:%d ouvert en <%d>\n",DateHeure(),IP_CHAMPS(repart->adrs),repart->lec.port,repart->lec.path);
		if(repart->famille == FAMILLE_OPERA) {
			buffer[0] = (nature == REPART_ENVOI_DETEC)? 'P':'S'; buffer[1] = (unsigned char)(trames & 0xFF);
			...
			if(trames) {
				if(repart->revision > 2) {
					...
					l = sprintf((char *)buffer,"k %ld %d %d %d",heure_depart.tv_sec,heure_depart.tv_usec,tz.tz_minuteswest,tz.tz_dsttime);
					...
					for(i=0; i<repart->nbentrees; i++) if((numeriseur = (TypeNumeriseur *)(repart->entree[i].adrs))) {
						for(adc=0; adc<numeriseur->nbadc; adc++) if((voie = numeriseur->voie[adc]) >= 0) {
							...
							if(!Trigger.enservice || !VoieTampon[voie].lue || (trgr == TRGR_INTERNE)) {
								l = sprintf((char *)buffer,"t%d off",num); RepartIpEnvoie(repart,buffer,l,1,0,log);
							} else {
								...
								lissage = (VoieManip[voie].def.trmt[TRMT_PRETRG].type == TRMT_LISSAGE)? VoieManip[voie].def.trmt[TRMT_PRETRG].parm: 0;
								derivee = (VoieTampon[voie].trig.trgr->type == TRGR_DERIVEE)? VoieTampon[voie].trig.mincount: 0.0;
								l = sprintf((char *)buffer,"t%d %s %g %d %d %d %d",num,sens,seuil,lissage,derivee,VoieEvent[voie].lngr,VoieEvent[voie].avant_evt);
								RepartIpEnvoie(repart,buffer,l,1,0,log);
							}
						}
					}
				}
				l = sprintf((char *)buffer,"p %d %x",repart->lec.port,trames); RepartIpEnvoie(repart,buffer,l,1,0,log);
				// l = sprintf((char *)buffer,"w %x %x/w %x %x",CALI_REG_RUN,CALI_START,CALI_REG_TAILLE,CALI_DONNEES_NB);
				l = sprintf((char *)buffer,"w %x %x",CALI_REG_RUN,CALI_START); RepartIpEnvoie(repart,buffer,l,1,0,log);
			}
		} else if(repart->famille == FAMILLE_SAMBA) {
			if(trames) {
				val = (unsigned short)(trames & 0xFFFF);
			...
			buffer[0] = /* (nature == REPART_ENVOI_DETEC)? 'P':'S' */ 'P'; buffer[1] = (unsigned char)(trames & 0xFF);
			buffer[2] = (byte)(repart->lec.port & 0xFF);
			buffer[3] = (byte)((repart->lec.port >> 8) & 0xFF);
			RepartIpToOpera(repart,buffer,log);
			if(!trames) {
	...
	if(trames) {
		if(repart->simule) {
			((TypeSocketIP *)(&(repart->lec.skt)))->sin_addr.s_addr = repart->adrs.val;
			if(repart->famille == FAMILLE_CALI) repart->simu.num_trame = 0;
			else repart->simu.num_trame = REPART_IP_CNTL;
			repart->simu.prochaine = 0;
			repart->simu.sample = 0;
			repart->simu.timestamp = repart->s.ip.inc_stamp;
			repart->simu.max = trames;
			repart->simu.started = 1;
		}
	} else {
		if(repart->famille == FAMILLE_CALI) {
			l = sprintf((char *)buffer,"w %x %x",CALI_REG_RUN,CALI_STOP);
			if(repart->revision >= 2) { strcat(buffer,"/q"); l += 2; }
			RepartIpEnvoie(repart,buffer,l,1,1,log);
		} else if(repart->famille == FAMILLE_SAMBA) {
			SmbEcrit(repart,SMB_ARRET);
			if(log >= 0) printf("%s/   . Ecrit '%s'\n",DateHeure(),RepartiteurValeurEnvoyee);
		}
		if(repart->simule) repart->simu.started = 0;
		gettimeofday(&LectDateRun,0);
		if(log >= 0) printf("%s/   . [%06d] Transmission arretee\n",DateHeure(),LectDateRun.tv_usec);
	}
	if(!duree && (repart->lec.path >= 0)) {
		#define TRAME_STD 1440
		unsigned int lngr; int octets_lus; unsigned char trame[TRAME_STD];
		usleep(100000); /* 100 ms */
		if(log) printf("%s/   . Vidage de la FIFO\n",DateHeure());
		do octets_lus = RepartIpLitTrame(repart,&trame,TRAME_STD,0,&(repart->lec.skt),&lngr); while(octets_lus >= 0);
		RepartIpLectureFerme(repart);
		if(log) printf("%s/   . Fermeture de la liaison\n",DateHeure());
	} else if((duree < 0) && log) printf("%s/   . FIFO laissee pleine\n",DateHeure());
	if((duree <= 0) && (repart->famille == FAMILLE_CALI)) {
		...
		SocketFermee(repart->ecr.path); repart->ecr.path = -1;
...
void RepartIpContinue(TypeRepartiteur *repart, int duree, char log) {
	/* demande l'envoi des donnees pendant <duree> synchros D3. Ne ferme pas la socket si duree < 0 */
	int l; unsigned short val; unsigned char buffer[80];

	if(!repart) return;
	if(!RepartIpEcriturePrete(repart,"            ")) return;

	if((repart->famille == FAMILLE_OPERA) || (repart->famille == FAMILLE_IPE)) {
		buffer[0] = 'P'; buffer[1] = (unsigned char)(duree & 0xFF);
		buffer[2] = (byte)(repart->lec.port & 0xFF);
		buffer[3] = (byte)((repart->lec.port >> 8) & 0xFF);
		RepartIpToOpera(repart,buffer,log);
	} else if(repart->famille == FAMILLE_CALI) {
		...
		val = (unsigned short)(duree & 0xFFFF);
		SmbEcrit(repart,SMB_MARCHE,val);
		if(log) printf("%s/   . Ecrit: '%s'\n",DateHeure(),RepartiteurValeurEnvoyee);
	}
	if(repart->simule) repart->simu.max = duree;
}
/* ==========================================================================
INLINE char RepartIpDepile1Donnee(TypeRepartiteur *repart, int32 *donnee) {
	...
	if(repart->s.ip.manquant && (repart->bottom == repart->s.ip.saut)) {
		...
		if(BigEndianOrder) *donnee = repart->fifo16[repart->bottom++];
		else {
			if(repart->bottom & 1) *donnee = repart->fifo16[repart->bottom++ - 1];
			else *donnee = repart->fifo16[++repart->bottom];
		}
		lue = 1;
...
INLINE void RepartIpEcritureFerme(TypeRepartiteur *repart) {
	if(!repart->simule) SocketFermee(repart->ecr.path);
	repart->ecr.path = -1;
}
/* ========================================================================== */
static int RepartGenere(TypeRepartiteur *repart, int nb_stamps, TypeDonnee *valeurs, int max) {
	int nb_donnees;
	int t,vt0,vt,voie,simu; char rep_vu;
	float ldb,bruit,freq,pic,reso; float fluct,val,evt,periode; int v;

//	if(repart->simu.num_trame <= 2) {
//		for(vt=0; vt<SambaEchantillonLngr; vt++) printf("(%s) repartiteur pour stamp[%d]: #%d (#%d demande)\n",__func__,vt,SambaEchantillon[vt].rep,repart->rep);
//		printf("(%s) demande %d stamps de %d valeurs, au maxi %d\n",__func__,nb_stamps,SambaEchantillonLngr,max);
//	}
	nb_donnees = 0;
	vt0 = 0;
	for(t=0; t<nb_stamps; t++) {
		...
		for(vt=vt0; vt<SambaEchantillonLngr; vt++) {
			if(SambaEchantillon[vt].rep == repart->rep) {
				if((voie = SambaEchantillon[vt].voie) >= 0) {
					if((simu = VoieManip[voie].simu) >= 0) {
						ldb = Simu[simu].ldb; bruit = Simu[simu].bruit * 12.0;
						periode = Simu[simu].periode; pic = Simu[simu].pic; reso = Simu[simu].reso / 6.4; // 6.4 magique
						if((VoieTampon[voie].unite.taille == 0) && (Simu[simu].panneau.message[0] == '\0')) {
							strcpy(Simu[simu].panneau.message,L_("Pas d'evenement genere (longueur template=0)"));
							PanelRefreshVars(Simu[simu].panneau.texte.msg);
						} else if((VoieTampon[voie].unite.taille > 0) && (Simu[simu].panneau.message[0] != '\0')) {
							Simu[simu].panneau.message[0] = '\0'; PanelRefreshVars(Simu[simu].panneau.texte.msg);
						}
					} else {
						ldb = 5.0; bruit = 50.0; freq = 1.0; pic = 1000.0; reso = 5.0;
						periode = 1.4 / EnSecondes;
					}
					HasardTirage(bruit,12,fluct);
					val = ldb + fluct - (bruit / 2.0);
					evt = 0;
					if((Simu[simu].mode_auto && (repart->simu.sample >= VoieTampon[voie].simu.futur_evt)) || (Simu[simu].mode_manu && (VoieTampon[voie].simu.etape < 0))) {
						if(Simu[simu].mode_manu) {
							printf("%s/ Impulsion sur %s demandee par l'utilisateur\n",DateHeure(),VoieManip[voie].nom);
						}
						HasardTirage(reso,12,fluct);
						// VoieTampon[voie].simu.max = pic * (1.0 + (0.06 * fluct)) * (1.0 - 0.03 * (reso - 5.0));
						VoieTampon[voie].simu.max = pic * (1.0 + ((fluct - (0.5 * reso)) * 0.8));
						// printf("%s/ Impulsion gene#%d sur %s: %g/%g ADU\n",DateHeure(),simu,VoieManip[voie].nom,VoieTampon[voie].simu.max,Simu[simu].pic);
						VoieTampon[voie].simu.etape = 0;
						do HasardTirage(1.0,12,fluct) while(fluct <= 0.0);
						VoieTampon[voie].simu.futur_evt = repart->simu.sample - (int64)(logf(fluct) * periode);
					}
					if(VoieManip[voie].modulee) {
						int diff,k; float evtJ,evtK,dev;
						diff = VoieTampon[voie].simu.compens - VoieTampon[voie].simu.modul;
						if(VoieTampon[voie].simu.etape >= 0) {
							if(VoieTampon[voie].unite.val) {
								k = VoieTampon[voie].simu.etape;
								if(k == 0) evtJ = 0.0;
								else evtJ = (VoieTampon[voie].simu.max * VoieTampon[voie].unite.val[k-1]);
								evtK = (VoieTampon[voie].simu.max * VoieTampon[voie].unite.val[k]);
								evt = evtJ + ((evtK - evtJ) * ((0.5 * VoieTampon[voie].simu.phase / VoieTampon[voie].simu.demiperiode) + 0.5));
								dev = (float)diff + (0.5 * evt);
								v = (int)val;
								if((VoieTampon[voie].simu.phase)++ < 0) v = (int)(val + dev); else {
									v = (int)(val - dev);
									if(VoieTampon[voie].simu.phase >= VoieTampon[voie].simu.demiperiode) {
										VoieTampon[voie].simu.phase = -VoieTampon[voie].simu.demiperiode;
										if(++VoieTampon[voie].simu.etape >= VoieTampon[voie].unite.taille) VoieTampon[voie].simu.etape = -1;
									}
								}
							}
						} else {
							if((VoieTampon[voie].simu.phase)++ < 0) v = (int)val + diff; else {
								v = (int)val - diff;
								if(VoieTampon[voie].simu.phase >= VoieTampon[voie].simu.demiperiode) {
									VoieTampon[voie].simu.phase = -VoieTampon[voie].simu.demiperiode;
								}
						...
						if(VoieTampon[voie].simu.etape >= 0) {
							if(VoieTampon[voie].unite.val) {
								evt = (VoieTampon[voie].simu.max * VoieTampon[voie].unite.val[VoieTampon[voie].simu.etape]);
							}
							if(++VoieTampon[voie].simu.etape >= VoieTampon[voie].unite.taille) VoieTampon[voie].simu.etape = -1;
						}
						v = (int)(val + evt);
					}
					if(repart->famille == FAMILLE_CALI) { v += 0x8000; if(repart->revision > 1) v = -v; }
				} else v = 0xCACA;
				if(BigEndianOrder) valeurs[nb_donnees++] = (TypeDonnee)v;
				else if(nb_donnees & 1) valeurs[nb_donnees++ - 1] = (TypeDonnee)v;
				else valeurs[++nb_donnees] = (TypeDonnee)v;
//				if(repart->simu.num_trame <= 2) printf("(%s) t=%3d/%d: v[%d][%d/%d] = %04X\n",__func__,t,nb_stamps,vt,nb_donnees,max,(TypeDonnee)v);
				if(nb_donnees >= max) break;
				rep_vu = 1;
			} else {
				if(rep_vu) break; else vt0 = vt + 1;
			}
		}
		for(simu=0; simu<SimuNb; simu++) if(Simu[simu].mode_manu) {
			Simu[simu].mode_manu = 0;
			if(OpiumDisplay(Simu[simu].panneau.bouton.manu)) OpiumRefreshAgain(Simu[simu].panneau.bouton.manu);
		}
		if(nb_donnees >= max) break;
	}
	repart->simu.sample += t;
	return(nb_donnees);
}
/* ========================================================================== */
INLINE ssize_t RepartIpLitTrame(TypeRepartiteur *repart, void *trame, size_t lngr,
	int k, struct sockaddr * __restrict skt, socklen_t * __restrict taille) {
	int i;
	int64 date_us;
	int max,nb_stamps,nb_donnees; ssize_t taille_sts,octets_lus;
	CaliTrame trame_cali; IpeTrame trame_ipe; OperaTrame trame_opera; TypeDonnee *val;

	if(repart->simule) {
        octets_lus = 0;
		errno = EAGAIN;
		if(!repart->simu.started) return(-1);
		date_us = DateMicroSecs();
		if(date_us < repart->simu.prochaine) return(-1);
		max = (repart->s.ip.taille_struct - repart->s.ip.taille_entete) / sizeof(TypeDonnee);
		if(repart->famille == FAMILLE_CALI) {
			if(repart->simu.num_trame > repart->simu.max) return(-1);
			trame_cali = (CaliTrame)trame;
			trame_cali->hdr.stamp = repart->simu.timestamp;
			trame_cali->hdr.id = repart->simu.num_trame << 8;
			for(i=0; i<4; i++) trame_cali->hdr.status[i] = 0xA8;
			nb_stamps = repart->s.ip.duree_trame;
			val = (TypeDonnee *)(trame_cali->valeur);
			nb_donnees = RepartGenere(repart,nb_stamps,val,CALI_DONNEES_NB);
			octets_lus = repart->s.ip.taille_entete + (nb_donnees * sizeof(TypeDonnee));
			if(!BigEndianOrder) {
				if(repart->revision > 1) ByteSwappeInt(&trame_cali->hdr.id);
				CaliSwappeTrame(repart,trame_cali,octets_lus);
			}
			repart->simu.timestamp += nb_stamps;
			repart->simu.num_trame++;
			repart->simu.prochaine += repart->simu.intervalle; // pas d'incrementation si trame de controle
		} else {
			if(repart->simu.num_trame >= REPART_IP_CNTL) {
				switch(repart->famille) {
				  case FAMILLE_OPERA:
					trame_opera = (OperaTrame)trame;
					trame_opera->identifiant = OPERA_TRAME_STS;
					((Structure_trame_status *)trame)->status_opera.temps_pd_fort = (int)(repart->simu.timestamp >> 30);
					((Structure_trame_status *)trame)->status_opera.temps_pd_faible = (int)(repart->simu.timestamp & 0x3FFFFFFF);
					taille_sts = sizeof(Structure_trame_status);
					break;
				  case FAMILLE_IPE:
					trame_ipe = (IpeTrame)trame;
					trame_ipe->type = IPE_TRAME_STS;
					trame_ipe->nb_d3 = repart->simu.timestamp / repart->s.ip.inc_stamp;
					trame_ipe->p.sts_crate.stamp_haut = (int)(repart->simu.timestamp >> 30);
					trame_ipe->p.sts_crate.stamp_bas = (int)(repart->simu.timestamp & 0x3FFFFFFF);
					trame_ipe->p.sts_crate.temps_seconde = trame_ipe->nb_d3;
					trame_ipe->p.sts_crate.dim = sizeof(TypeIpeCrateSts);
					trame_ipe->p.sts_crate.SLTTimeLow = (uint32_t)(repart->simu.timestamp & 0xFFFFFFFF);
					trame_ipe->p.sts_crate.SLTTimeHigh = (uint32_t)(repart->simu.timestamp >> 32);
					trame_ipe->p.sts_crate.d0 = (uint32_t)(ModeleRep[repart->type].diviseur0);
					trame_ipe->p.sts_crate.pixbus_enable = (uint32_t)IpePixBus;
					...
					trame_ipe->p.sts_crate.maxUDPSize = (uint32_t)(repart->s.ip.taille_struct);
					taille_sts = sizeof(TypeIpeCrateSts);
					break;
				}
				octets_lus = repart->s.ip.taille_entete + taille_sts;
				// if(repart->acces_remplis < 10) printf("%s/ %s[%lld], trame[%d] de status generee %12lld/%-3d\n",DateHeure(),
				// 	repart->nom,repart->acces_remplis,octets_lus,repart->simu.timestamp,IPE_TRAME_STS);
				repart->simu.num_trame = 0;
			} else {
				switch(repart->famille) {
				  case FAMILLE_OPERA:
					trame_opera = (OperaTrame)trame;
					trame_opera->identifiant = (((repart->simu.timestamp / repart->s.ip.inc_stamp) << 16) & 0x3FFF0000) & (repart->simu.num_trame & 0xFFFF);
					val = (TypeDonnee *)(trame_opera->data);
					break;
				  case FAMILLE_IPE:
					trame_ipe = (IpeTrame)trame;
					trame_ipe->type = repart->simu.num_trame;
					trame_ipe->nb_d3 = repart->simu.timestamp / repart->s.ip.inc_stamp;
					//- TwoShortsSwappeInt((unsigned int *)trame);
					val = (TypeDonnee *)(trame_ipe->p.donnees.val);
					break;
				}
				if(repart->simu.num_trame == repart->s.ip.tramesD3) nb_stamps = repart->s.ip.taille_derniere;
				else nb_stamps = repart->s.ip.duree_trame;
				nb_donnees = RepartGenere(repart,nb_stamps,val,max);
				octets_lus = repart->s.ip.taille_entete + (nb_donnees * sizeof(TypeDonnee));
				// if(repart->acces_remplis < 10) printf("%s/ %s[%lld], trame[%d] de donnees generee %12lld/%-3d\n",DateHeure(),
				// 	repart->nom,repart->acces_remplis,octets_lus,repart->simu.timestamp,repart->simu.num_trame);
				if(repart->simu.num_trame < repart->s.ip.tramesD3) {
					repart->simu.num_trame++;
				} else {
					repart->simu.timestamp += repart->s.ip.inc_stamp;
					repart->simu.num_trame = REPART_IP_CNTL;
				}
				repart->simu.prochaine += repart->simu.intervalle; // pas d'incrementation si trame de controle
			}
			// if(repart->acces_remplis < 10) IpeDump((unsigned char *)trame,octets_lus,1);
			switch(repart->famille) {
				case FAMILLE_OPERA: if(BigEndianOrder) OperaSwappeTrame(trame,octets_lus); break;
				case FAMILLE_IPE: if(BigEndianOrder) IpeSwappeTrame(trame,octets_lus); break;
			}
		}
		errno = 0;
		return(octets_lus);
	} else return(recvfrom(repart->lec.path,trame,lngr,k,skt,taille));
}
/* ========================================================================== */
INLINE void RepartIpLectureFerme(TypeRepartiteur *repart) {
	if(!repart->simule) SocketFermee(repart->lec.path);
	printf("%s/   . Acces en lecture depuis %d.%d.%d.%d:%d ferme\n",DateHeure(),IP_CHAMPS(repart->adrs),repart->lec.port);
	repart->lec.path = -1;
}
/* ========================================================================== */
INLINE char RepartiteurEcritRessourceBB(TypeRepartiteur *repart, short entree, unsigned short adrs, char ss_adrs, TypeADU valeur) {
	if(!repart) { sprintf(RepartiteurValeurEnvoyee,"! Repartiteur indetermine"); return(0); }
	  ...
	  case INTERF_IP:
		...
		switch(repart->famille) {
		  ...
		  case FAMILLE_SAMBA: SmbEcrit(repart,SMB_NUMER,(unsigned char)adrs,(unsigned char)ss_adrs,valeur);  break;
...
INLINE char RepartiteurEcritNumeriseur(TypeRepartiteur *repart, short entree, byte *cmde, int lngr) {
	...
	if(!repart) { sprintf(RepartiteurValeurEnvoyee,"! Repartiteur indetermine"); return(0); }
		...
		case INTERF_IP:
			...
			break;
		case INTERF_USB:
			UsbWrite(repart->ecr.path,(char *)cmde,RepartiteurValeurEnvoyee);
			break;
		case INTERF_NAT:  break;
		case INTERF_FILE: break;
	}
...
/* ========================================================================== */
char RepartiteurRepereD2(TypeRepartiteur *repart, int32 *val1, char log) {
	...
	*val1 = 0;
	...
	if(repart->famille == FAMILLE_CLUZEL) {
		...
		do {
			repart->acces_demandes++;
			SambaTesteDAQmx("Lecture de donnees",DAQmxBaseReadBinaryI16(LectNITache[repart->adrs.val-1],DAQmx_Val_Auto,3.0,DAQmx_Val_GroupByScanNumber,(int16 *)(repart->fifo16 + repart->top),FIFOdim - repart->top,&nb,NULL),log);
			if(nb) {
				repart->acces_remplis++;
				repart->top += nb;
				repart->donnees_recues += nb;
				mot_lu = repart->fifo16[repart->bottom];
				trouvee = 1; break;
		...
		int limite;
		unsigned int lngr; int octets_lus,donnees_lues; OperaTrame trame;
		...
		do {
			...
			if(octets_lus == -1) {
				...
				if(repart->bottom < repart->top) {
					...
					if(BigEndianOrder) mot_lu = repart->fifo16[repart->bottom] & 0xFFFF;
					else if(repart->bottom & 1) mot_lu = repart->fifo16[repart->bottom - 1] & 0xFFFF;
					else mot_lu = repart->fifo16[repart->bottom + 1] & 0xFFFF;
					trouvee = 1; break;
				}
			}
			gettimeofday(&LectDateRun,0);
		...
		int limite;
		unsigned int lngr; int octets_lus,donnees_lues; IpeTrame *trame;
		// unsigned short num_trame;
		int num_bloc,blocs_a_sauter;
		timeout = 5.0;
		if(log) printf("%s/   . Attente d'une synchro D2, au plus %g secondes\n",DateHeure(),timeout);
		gettimeofday(&LectDateRun,0); limite = LectDateRun.tv_sec + (int)timeout;
		...
		lngr = sizeof(TypeSocket);
		do {
			octets_lus = RepartIpLitTrame(repart,trame,sizeof(IpeTrame),0,&(repart->lec.skt),&lngr);
			if(octets_lus == -1) {
				if(errno == EAGAIN) SambaMicrosec(1000);
				else { printf("%s/     ! Repartiteur illisible (%s)\n",DateHeure(),strerror(errno)); return(0); }
			} else if(octets_lus > 0) {
		...
		int limite;
		unsigned int lngr; int octets_lus,donnees_lues; CaliTrame trame;
		timeout = 5.0;
		if(log) printf("%s/   . Attente d'une synchro D2, au plus %g secondes\n",DateHeure(),timeout);
		gettimeofday(&LectDateRun,0); limite = LectDateRun.tv_sec + (int)timeout;
		...
		do {
			...
			if(octets_lus == -1) {
				...
				if(!BigEndianOrder) {
					ByteSwappeLong((int64 *)trame);
					ByteSwappeIntArray((unsigned int *)((char *)trame + 8),(octets_lus - 8) / sizeof(Ulong));
				}
...
/* ========================================================================== */
char RepartiteurRepereD3(TypeRepartiteur *repart, int32 *val1, char log) {
	...
	if(repart->famille == FAMILLE_CLUZEL) {
		...
		do {
			...
			SambaTesteDAQmx("Lecture de donnees NI",DAQmxBaseReadBinaryI16(LectNITache[num],DAQmx_Val_Auto,2.0,DAQmx_Val_GroupByScanNumber,
				(int16 *)(repart->fifo16 + repart->top),FIFOdim - repart->top,&nb,NULL),log);
			if(nb) {
				repart->D3trouvee = 1;
				repart->acces_remplis++;
				repart->top += nb;
		...
		unsigned int lngr; int octets_lus,donnees_lues; OperaTrame trame;
		unsigned short num_trame,precedent;
		int nb;
		...
		if(trouvee) {
			...
			repart->s.ip.synchro_vue = 1;
			LectDerniereD3 = 0;
			if(BigEndianOrder) mot_lu = repart->fifo16[0] & 0xFFFF; /* en principe, repart->bottom */
			else mot_lu = repart->fifo16[1] & 0xFFFF; /* en principe, repart->bottom+1 */
			if(log) {
		...
		unsigned int lngr; int octets_lus,donnees_lues; TypeIpeTrame *trame;
		unsigned short num_trame,precedent;
		int nb; int essais;
...
char RepartiteurRepereD3commune(char log) {
	...
	int rep;
	unsigned int lngr; int octets_lus,donnees_lues;
	// unsigned short num_trame,num_status,precedent;
	int num,num_trame,num_status,precedent,nb_trames;
	int timeout;   /* delai maxi en secondes avant abandon */
	int timesleep; /* attente en microsecs si pile vide (pas de donnees) */
	int limite;
	...
	do {
		...
		for(rep=0; rep<RepartNb; rep++) if(Repart[rep].actif && !Repart[rep].D3trouvee) {
			...
			if(repart->famille == FAMILLE_CLUZEL) {
				...
				repart->acces_demandes++;
				num = repart->adrs.val - 1;
				SambaTesteDAQmx("Lecture de donnees NI",DAQmxBaseReadBinaryI16(LectNITache[num],DAQmx_Val_Auto,2.0,DAQmx_Val_GroupByScanNumber,(int16 *)(repart->fifo16 + repart->top),FIFOdim - repart->top,&nb,NULL),log);
				if(nb) {
					repart->acces_remplis++;
					repart->top += nb;
					repart->donnees_recues += nb;
				...
				octets_lus = RepartIpLitTrame(repart,trame,repart->s.ip.taille_struct,0,&(repart->lec.skt),&lngr);
				if(octets_lus == -1) {
					if(errno != EAGAIN) {
						repart->actif = 0;
					...
					nb_trames++;
					donnees_lues = (octets_lus - repart->s.ip.taille_entete) / sizeof(TypeDonnee); /* 2 octets par voie */
...
char RepartiteursAttendStamp(char log) {
	...
	int nb,num0; unsigned int lngr; int limite;
	char d3_trouvee;
	void *trame;
	...
	do {
		for(rep=0; rep<RepartNb; rep++) if(Repart[rep].actif && !termine[rep]) {
			...
			do {
				if(repart->interf == INTERF_IP) {
					lngr = sizeof(TypeSocket);
					nb = RepartIpLitTrame(repart,trame,RepartTrameDef[repart->famille].lngr,0,&(repart->lec.skt),&lngr);
					// printf("> recu via <%d>: %d octet%s\n",repart->lec.path,Accord1s(nb));
					if(nb == -1) { if(errno != EAGAIN) printf("%9s   ! %s %s hors service (%s)\n"," ",ModeleRep[repart->type].nom,repart->nom,strerror(errno)); LectRepartAttendus--; break; }
						...
						if(BigEndianOrder) {
							ByteSwappeInt((unsigned int *)trame);
							if(repart->famille == FAMILLE_IPE) TwoShortsSwappeInt((unsigned int *)trame);
						...
						if(d3_trouvee) {
							if(repart->s.ip.stampDernier >= LectStampMini) {
								...
								if(repart->categ != DONNEES_STAMP) {
									if(BigEndianOrder) ByteSwappeIntArray((unsigned int *)trame+1,(nb - sizeof(int)) / sizeof(Ulong));
									else if(repart->famille == FAMILLE_CALI)
										ByteSwappeIntArray((unsigned int *)((char *)trame + sizeof(int64)),(nb - sizeof(int64)) / sizeof(Ulong));
									switch(repart->famille) {
...
int RepartiteurChercheIP() {
	...
	for(i=RepartChercheIPdep; i<=RepartChercheIParr; i++) {
		...
		if(RepartIpEcriturePrete(&repart_tempo,"          | ")) {
			...
			k = m = 0;
			do {
				nb = RepartIpLitTrame(&repart_tempo,trame,sizeof(TypeOperaTrame),0,&(repart_tempo.lec.skt),&lngr);
				k++;
				// printf("            recvfrom(%d) rend %d: %s\n",repart_tempo.lec.path,nb,(nb == -1)?strerror(errno):"OK");
				if(nb == -1) { if(errno == EAGAIN) SambaMicrosec(timesleep); else break; }
				else if(((TypeSocketIP *)(&(repart_tempo.lec.skt)))->sin_addr.s_addr != ip) continue;
				else if(nb > 0) {
					m++;
					// if(BigEndianOrder) ByteSwappeInt((unsigned int *)trame);
					if(BigEndianOrder) ByteSwappeIntArray((unsigned int *)trame,nb / sizeof(Ulong));
					if(OPERA_TRAME_NUMERO(trame) == OPERA_TRAME_STS) {
						// if(BigEndianOrder) ByteSwappeIntArray((unsigned int *)trame+1,nb / sizeof(Ulong));
	...
	do {
		...
		for(rep=0; rep<RepartNb; rep++) if(Repart[rep].local && Repart[rep].valide && (RepartEtat[rep] == REPART_ACCES)) {
			...
			if(repart->famille == FAMILLE_CLUZEL) {
				...
				OperaTrame trame; unsigned int lngr; int limite;
				gettimeofday(&LectDateRun,0); limite = LectDateRun.tv_sec + (timesleep * nbsleeps / 1000000);
				trame = (OperaTrame)repart->fifo32;
				lngr = sizeof(TypeSocket);
				do {
					nb = RepartIpLitTrame(repart,trame,sizeof(TypeOperaTrame),0,&(repart->lec.skt),&lngr);
					if(nb == -1) { if(errno == EAGAIN) SambaMicrosec(timesleep); else break; }
					else if(nb > 0) {
				...
				lngr = sizeof(TypeSocket);
				do {
					nb = RepartIpLitTrame(repart,trame,sizeof(TypeIpeTrame),0,&(repart->lec.skt),&lngr);
					if(nb == -1) { if(errno == EAGAIN) SambaMicrosec(timesleep); else break; }
					else if(nb > 0) { RepartEtat[rep] = REPART_DISPO; break; }
					gettimeofday(&LectDateRun,0);
				} while(LectDateRun.tv_sec < limite);
			} else if(repart->famille == FAMILLE_CALI) {
				CaliTrame trame; unsigned int lngr; int limite;
				gettimeofday(&LectDateRun,0); limite = LectDateRun.tv_sec + (timesleep * nbsleeps / 1000000);
				trame = (CaliTrame)repart->fifo32;
				lngr = sizeof(TypeSocket);
				do {
					nb = RepartIpLitTrame(repart,trame,sizeof(TypeCaliTrame),0,&(repart->lec.skt),&lngr);
...
static MenuItem iOperaPlusMoins[] = {
...
static MenuItem iOperaConnections[] = {
...
int RepartiteursAffiche(TypeRepartiteur *repart) {
	...
	if(a_construire) {
		...
		if(repart->famille == FAMILLE_IPE) {
			...
			PanelHex(p,"PixBus",(int *)&IpePixBus);
			PanelSupport(p,WND_CREUX);
		...
		if(repart->famille == FAMILLE_IPE) {
			...
			if(style_chassis) {
				...
				larg = 3 * 5 * Dx;
			#else
				for(fibre=0; fibre<IPE_FLT_SIZE; fibre++) PanelItemSouligne(p,PanelItemSelect(p,PanelLngr(p,PanelFormat(p,PanelChar(p,0,&(SambaUserNum[fibre])),"%2d"),3),0),1);
				for(fibre=0; fibre<IPE_FLT_SIZE; fibre++) PanelItemSouligne(p,PanelItemSelect(p,PanelLngr(p,PanelFormat(p,PanelChar(p,0,&(SambaUserNum[fibre])),"%2d"),3),0),1);
				for(fibre=0; fibre<IPE_FLT_SIZE; fibre++) PanelItemSouligne(p,PanelItemSelect(p,PanelLngr(p,PanelFormat(p,PanelChar(p,0,&(SambaUserNum[fibre])),"%2d"),3),0),1);
			...
			for(flt=0; flt<repart->r.ipe.fltnb; flt++) {
				...
				if(style_chassis) {
					...
					PanelItemSouligne(p,PanelItemSelect(p,PanelLngr(p,PanelFormat(p,PanelChar(p,texte,&(SambaUserNum[fltabs])),"%2d"),3),0),1);
			...
			k = PanelKeyB(p,texte," -/ +",(char *)&(repart->r.ipe.flt[flt].active),3);
				...
				PanelLngr(p,PanelFormat(p,PanelByte(p,texte,&(repart->r.ipe.flt[flt].controle.partiel.speciaux)),"%02X"),3);
				if(style_chassis & !flt) texte = "InEnable"; else texte = 0;
				PanelLngr(p,PanelFormat(p,PanelByte(p,texte,&(repart->r.ipe.flt[flt].controle.partiel.enable)),"%02X"),3);
				if(style_chassis & !flt) texte = "BB1 clock"; else texte = 0;
				PanelLngr(p,PanelFormat(p,PanelByte(p,texte,&(repart->r.ipe.flt[flt].controle.partiel.modeBB)),"%02X"),3);
				if(style_chassis & !flt) texte = "FLT mode"; else texte = 0;
			k = PanelLngr(p,PanelFormat(p,PanelByte(p,texte,&(repart->r.ipe.flt[flt].controle.partiel.modeFLT)),"%02X"),3);
				if(style_chassis) PanelItemSouligne(p,k,1);
			#ifdef PAR_FIBRE
				for(fibre=0; fibre<IPE_FLT_SIZE; fibre++) {
					PanelLngr(p,PanelFormat(p,PanelByte(p,0,&(IpeStatus[flt][fibre].delai12)),"%02X"),3);
					PanelLngr(p,PanelFormat(p,PanelByte(p,0,&(IpeStatus[flt][fibre].delai120)),"%02X"),3);
					PanelLngr(p,PanelFormat(p,PanelByte(p,0,&(IpeStatus[flt][fibre].selection)),"%02X"),3);
				}
			#else
				h = 0;
				for(fibre=0; fibre<IPE_FLT_SIZE; fibre++) {
		...
		if(repart->famille == FAMILLE_CLUZEL) {
			...
			PanelItemSelect(pRepartStatus,PanelFormat(pRepartStatus,PanelByte(pRepartStatus,"Status v1",&(CaliEnteteLue.status[0])),"%02X"),0);
			PanelItemSelect(pRepartStatus,PanelFormat(pRepartStatus,PanelByte(pRepartStatus,"Status v2",&(CaliEnteteLue.status[1])),"%02X"),0);
			PanelItemSelect(pRepartStatus,PanelFormat(pRepartStatus,PanelByte(pRepartStatus,"Status v3",&(CaliEnteteLue.status[2])),"%02X"),0);
			PanelItemSelect(pRepartStatus,PanelFormat(pRepartStatus,PanelByte(pRepartStatus,"Status v4",&(CaliEnteteLue.status[3])),"%02X"),0);
			cols += 9 + 5;
		} else if(repart->famille == FAMILLE_IPE) {
			...
			PanelItemSelect(pRepartStatus,PanelHex  (pRepartStatus,"PixBus",(int *)&IpePixBus),0);
...
/* ========================================================================== */
int RepartiteursClique(Figure fig, WndUserRequest *e) {
	TypeRepartiteur *repart;
