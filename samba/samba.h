...
typedef struct {
	...
	struct {
		...
		char message[GEGENE_MSG_LNGR];
	} panneau;
} TypeSimu;

#define SAMBA_NTPVOIE_NB 9
...
SAMBA_VAR float *NtBase[VOIES_TYPES];
SAMBA_VAR float *NtAmpl[VOIES_TYPES];
SAMBA_VAR float *NtBrute[VOIES_TYPES];
SAMBA_VAR float *NtMax[VOIES_TYPES];
SAMBA_VAR float *NtBruit[VOIES_TYPES];
SAMBA_VAR float *NtMont[VOIES_TYPES];
SAMBA_VAR float *NtDesc[VOIES_TYPES];
SAMBA_VAR float *NtDecal[VOIES_TYPES];
SAMBA_VAR float *NtTemps[VOIES_TYPES];
SAMBA_VAR float *NtEnerg[VOIES_TYPES];
SAMBA_VAR float *NtSeuil[VOIES_TYPES];
...
typedef struct {
	...
	// char maj;               /* demande de mise a jour des pointeurs apres traitement au vol */
...
//	TypeDonnee *tampon;     /* adresse du tampon alloue                                     */
	TypeDonnee derniere;    /* derniere valeur entree dans le tampon                        */
	TypeDonnee courante;    /* derniere valeur decodee                                      */
	int64 lus;              /* nb total de points memorises                                 */
//	int64 first;            /* plus ancien point memorise                                   */
	int64 sauve;            /* dernier point sauve par ArchiveBloc                          */
	int64 jetees;           /* nb ecrasees avant trmt pre-trigger                           */
//	int64 point0;           /* numero de point pour le 1er point du tampon                  */
//	TypeTamponDonnees lissage;   /* tampon de lissage pour le calcul de la ligne de base    */
	...
	struct {                /* traitements (au vol ou pre-trigger)                          */
		...
		int  valeur;           /* valeur temporaire calculee, idem                          */
		int  reelle;           /* valeur temporaire calculee, idem                          */
...
//			TypeRegulInterv parm;
			...
			TypeDonnee limp,limm;       /* limite pour depassement positif,negatif           */
...
//	short voie[REPART_VOIES_MAX]; /* liste des voies (index dans VoieManip)               */
//	short lngr[REPART_VOIES_MAX]; /* longueur de l'evenement (index dans VoieManip)       */
//	short lus[REPART_VOIES_MAX];  /* nb. donnees deja lues (index dans VoieManip)         */
//	int   adrs[REPART_VOIES_MAX]; /* liste des adresses du premier point dans VoieTampon  */
} TypeEvtDetecte;
...
SAMBA_VAR char InstalleSamba,CreationFichiers,EdbActive;
SAMBA_VAR char EdbServeur[HOTE_NOM];
SAMBA_VAR char NomOrdiLocal[HOTE_NOM];
SAMBA_VAR short SambaDefMax,SambaDefInitial;
#undef FILESEP
...
SAMBA_VAR char  SambaExploreHW,SambaIdentSauvable,SettingsSauveChoix,SettingsTramesMaxi;
...
SAMBA_VAR struct {
	...
	struct { char repart,detectr; } cablage;
...
SAMBA_VAR ArgDesc SambaCrDesc[]
 ...
 = {
	...
	{ "detecteurs.perimetre",   DESC_BOOL &CompteRendu.detectr.perimetre, "Liste des detecteurs avec mention local/etranger" },
...
SAMBA_VAR char  SambaDocFormat,SambaDocSupport;
SAMBA_VAR char  SambaDocCablage,SambaDocModeles,SambaDocAutom,SambaDocScripts;
...
#define SIMU_MAX VOIES_TYPES
SAMBA_VAR TypeSimu Simu[SIMU_MAX],SimuLue;
SAMBA_VAR char *SimuListe[SIMU_MAX+1];
SAMBA_VAR int SimuNum,SimuNb;
SAMBA_VAR char SimuModifiees;
...
SAMBA_VAR char FichierPrefModeles[MAXFILENAME],FichierPrefMedia[MAXFILENAME],FichierPrefSimu[MAXFILENAME];
...
SAMBA_VAR char FichierEtatElec[MAXFILENAME],FichierRunEnvir[MAXFILENAME],FichierRunCaract[MAXFILENAME],FichierCaldrRef[MAXFILENAME];
SAMBA_VAR char FichierPrefLecture[MAXFILENAME],FichierPrefMonit[MAXFILENAME];
...
SAMBA_VAR char FichierPrefCalcul[MAXFILENAME],FichierPrefFinesses[MAXFILENAME],FichierPrefExports[MAXFILENAME],FichierPrefBanc[MAXFILENAME];
SAMBA_VAR char FichierPrefBasic[MAXFILENAME];
...
SAMBA_VAR char  SettingsPauseEvt,CalcNtp,SettingsStockeFiltrees; // SettingsAttendsSeuil
...
// static char *SettingsTypes[SETTINGS_NBTYPES+1] = { "trmt", "trgr", "rgul", "\0" };
...
SAMBA_VAR struct {
	...
	TypeFigZone cadre_gal[DETEC_CAPT_MAX],cadre_std[DETEC_CAPT_MAX],cadre_plus[DETEC_CAPT_MAX];
	int (*creation)(Panel appelant, void *v2);
	int (*standardise)(Panel panel, void *v2);
	int (*standard2)(Panel panel, void *v2);
} SettingsVar[SETTINGS_NBTYPES];


SAMBA_VAR char  RunCategNum,SettingsRunFamille,SettingsVoiesNom,SettingsImprBolosVsn,SettingsInlineProcess;
...
SAMBA_VAR int   LectEvtVus;
SAMBA_VAR int64 LectDernierEvt;
#ifdef REGUL_UNIQUE
...
SAMBA_VAR int64 LectNextTrmt,LectNextExport[EXPORT_MAXPACKS],SynchroD2lues,LectNextDisp; // utilise dans traimt.c pour diagnostics
SAMBA_VAR char  LectTrmtSousIt;

SAMBA_VAR HistoData hdBruitAmpl,hdEvtAmpl,hdBruitMontee,hdEvtMontee;
SAMBA_VAR H2D h2D;
SAMBA_VAR Graph gEvtSolo,gEvtPlanche,gBruitAmpl,gBruitMontee,g2D;
...
//+ endif
SAMBA_VAR struct {
	int32 valeur;                 /* valeur deja recuperee                              */
	int32 precedente;             /* precedente <valeur>                                */
...
SAMBA_VAR float TestEnergie,TestResolution;
...
SAMBA_VAR char *TrmtPhysiqueListe[PHYS_TYPES+1];
...
#ifdef SAMBA_C
= { "externe", "aleatoire", "script", "front", "porte", "niveau", "\0" }
#endif
;
...
SAMBA_VAR int    CalcPlanCree;
SAMBA_VAR rfftw_plan CalcPlan;
SAMBA_VAR fftw_real *CalcFftTemps,*CalcFftFreq;
...
SAMBA_VAR char   CalcModeAuVol,CalcModeAuto;
SAMBA_VAR int    CalcSpectresMax;

SAMBA_VAR char   CalcAuVolDemande,CalcAuVolActif;
SAMBA_VAR int    CalcVoie;
SAMBA_VAR int    CalcAuVolNbFreq;
SAMBA_VAR float  CalcAuVolNorme;
SAMBA_VAR float  CalcAuVolNbSpectres,CalcGlissantEpsilon;
SAMBA_VAR float *CalcAuVolCarreSomme;
SAMBA_VAR float *CalcAuVolCarreMoyenne;
SAMBA_VAR float *CalcAuVolSpectreNouveau;
SAMBA_VAR float *CalcAuVolSpectreMoyenne;
SAMBA_VAR float  CalcAuVolAb6[2];
SAMBA_VAR Graph  CalcAuVolGraphe;
...
SAMBA_VAR Menu mSwDump,mGestAutom,mDiag;
...
#endif
;
SAMBA_VAR OpiumColor SambaRougeVertJaune[]
#ifdef SAMBA_C
...
SAMBA_VAR Cadre bTests,bTableauGeneral,bOptions,bFinesses,bLecture,bCntrl;
...
SAMBA_VAR Panel pLectMode,pLectTraitements,pLectConfigChoix,pDetecConfigDup,pDetecConfigMov;
...
#endif
SAMBA_VAR Instrum cPeriodes,cNtuples;
...
int SambaSauveArgs(),SambaSauveSettings();
int SambaLitSimu();
char SambaSauveSimu();
...
#ifdef MSGS_RESEAU
int LectReglage(),LectTrigger(),LectRazHistos(),LectDemarre(),LectStop();
SAMBA_VAR TypeMailFunction SambaCommandes[]
#ifdef SAMBA_C
= {
	{ "Reglages",   LectReglage,   0 },
	{ "Suspendre",  LectTrigger,   0 },
